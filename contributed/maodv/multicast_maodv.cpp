    ...
    MaodvNeighborTable *neighborTable)
{
    ...
    newNode->lastHeared = node->getNodeTime();
}
    ...
    MaodvRoutingTable *routeTable)
{
    ...
    {
        if (routeTable->rtEntry->lifetime <= node->getNodeTime())
        {
    ...
    else
    {
        ...
        if (current->next != NULL && current->next->destination.address ==
            destAddr && current->next->lifetime <= node->getNodeTime())
        {
            toFree = current->next;
            current->next = toFree->next;
    ...
    MaodvRoutingTable *routeTable)
{
        ...
        current = current->next)
    {
        ...
        if (current->destination.address == destAddress)
        {
            ...
            current->lifetime   = node->getNodeTime() + MAODV_DELETE_PERIOD;
    ...
    MaodvNextHops *nextHops)
{
    ...
    if ((current == NULL) || (current->nextHopAddr != nextHopAddr))
    {
        ...
        if (isEnabled)
        {
            newNode->lifetime = node->getNodeTime() + MAODV_HELLO_LIFE;
            ...
            {
                newNode->lifetime = node->getNodeTime()
        ...
        {
            newNode->lifetime = node->getNodeTime() + MAODV_MTREE_BUILD;
    ...
    int incomingInterface)
{
    ...
    if (validNextHops > 0)
    {
        ...
        maodv->lastBroadcastSent = node->getNodeTime();
    ...
    NodeAddress previousHopAddress)
{
    ...
    rtEntryToDest->lifetime = node->getNodeTime() + MAODV_ACTIVE_ROUTE_TIMEOUT;
...
void MaodvDeclareAsLeader(Node *node, NodeAddress mcastAddr)
{
    ...
    if (mrtToDest == NULL)
    {
        mrtToDest = MaodvCreateMrouteEntry(
            ...
            0,
            TRUE,
            node->getNodeTime() + MAODV_ACTIVE_ROUTE_TIMEOUT,
            &maodv->mrouteTable);
    }
    ...
    MaodvRouteEntry *rtToDest)
{
    ...
    if ((unsigned int) ((node->getNodeTime() - rtToDest->lifetime) /
        MILLI_SECOND) == 0)
    {
        return;
    }
    ...
    rrepPkt->lifetime = (unsigned int)
        ((rtToDest->lifetime - node->getNodeTime()) / MILLI_SECOND);
    ...
    BOOL isUnicast)
{
        ...
        &maodv->seenTable))
    {
                   ...
                   (int)RreqPacketGetHopCount(rreqPkt->RreqPkt) + 1)))
        {
            ...
            if (rtToSrc->lifetime == node->getNodeTime() + revRtLifetime)
            {
        ...
        if (rreqPkt->source.address != srcAddr)
        {
            ...
            ptrToPrevHop = MaodvReplaceInsertRouteTable(
                ...
                1,
                srcAddr,
                node->getNodeTime() + MAODV_MY_ROUTE_TIMEOUT,
                TRUE,
                interfaceIndex,
                &maodv->routeTable);

            // Update lifetime ofthe previous hop address
            if (ptrToPrevHop->lifetime == node->getNodeTime() +
                    ...
                    if (isUnicast && downstreamNode != NULL)
                    {
                        if (!downstreamNode->isEnabled)
                        {
                            downstreamNode->lifetime =
                                node->getNodeTime() + MAODV_MTREE_BUILD;
    ...
    unsigned int interfaceIndex)
{
    ...
    if (MactPacketGetJ(mactPkt->MactPkt) == 1)
    {
        ...
        if (isValid)
        {
            ...
            nextHop->lifetime = node->getNodeTime() + MAODV_HELLO_LIFE;
        ...
        else
        {
            ...
            upstreamNode->lifetime = node->getNodeTime() +
                MAODV_MDATA_FORWARD_TIMEOUT;
    ...
    int interfaceIndex)
{
    ...
    else  if (mrtToDest == NULL || !isValidRt)
    {
        ...
        mrtToDest->isMember = TRUE;
        mrtToDest->lifetime =
            MAX(node->getNodeTime() + MAODV_ACTIVE_ROUTE_TIMEOUT,
                mrtToDest->lifetime);
    ...
    unsigned int interfaceIndex)
{
    ...
    if (rrepPkt->destination.seqNum >= prevRtPtr->destination.seqNum)
    {
        ...
        else
        {
            ...
            prevRtPtr->lifetime =
                MAX(prevRtPtr->lifetime, node->getNodeTime() +
    ...
    int interfaceIndex)
{
                ...
                && (prevRtPtr->hopCount > hopCount)))
    {
        ...
        if (newRtPtr->lifetime == node->getNodeTime() + lifetime)
        {
            MaodvSetTimer(
                node,
                NULL,
    ...
    unsigned int interfaceIndex)
{
        ...
        &maodv->seenTable))
    {
                ...
                (int)(MglextPacketGetHopCount(rreqPkt->MglextPkt) + 1))))
        {
            ...
            if (rtToSrc->lifetime == node->getNodeTime() +
                MAODV_ACTIVE_ROUTE_TIMEOUT)
            {
                MaodvSetTimer(
                    node,
    ...
    MaodvMrouteEntry *mrtToDest)
{
    ...
    if (mrtToDest->nextHops.size == 1)
    {
        ...
        else if (mrtToDest->lifetime < node->getNodeTime())
        {
            MaodvDeleteMroute(node, mcastAddr, mrouteTable);
            mrtToDest = NULL;
        }
    ...
    unsigned int interfaceIndex)
{
        ...
        &maodv->seenTable))
    {
        ...
        if (rtToLeader == NULL || !isValidEntry )
        {
            ...
            if (rtToLeader->lifetime ==  node->getNodeTime() +
                MAODV_MY_ROUTE_TIMEOUT)
            {
                MaodvSetTimer(
                    node,
    ...
    else
    {
        ...
        if (mrtToDest == NULL)
        {
            ...
            mrtToDest = MaodvCreateMrouteEntry(
                ...
                node->getNodeTime() + MAODV_ACTIVE_ROUTE_TIMEOUT, // life time
                &maodv->mrouteTable);
        }
        else
        {
            ...
            if (upstreamNode != NULL)
            {
                ...
                upstreamNode->lifetime =
                    node->getNodeTime() + MAODV_MDATA_FORWARD_TIMEOUT;
    ...
    case MSG_ROUTING_MaodvCheckNextHopTimeout:
        {
            ...
            else
            {
                ...
                if (nextHopNode == NULL)
                {
                    ...
                    && (nextHopNode->lifetime <= node->getNodeTime()))
                {
                    // nexthop failed
                    BOOL lastEntry = FALSE;
    ...
    case MSG_ROUTING_MaodvTreeUtilizationTimer:
        {
            ...
            else
            {
                ...
                {
                    if (lastHeared + MAODV_HELLO_LIFE <= node->getNodeTime())
                    {
                        if (DEBUG_FAILURE)
                        {
    ...
    case MSG_ROUTING_MaodvRetransmitTimer:
        {
            ...
            else
            {
                ...
                {
                    if (lastHeared + MAODV_RETRANSMIT_TIME <= node->getNodeTime())
                    {
                        if (DEBUG_FAILURE)
                        {
                            char addr[25];
    ...
    case MSG_ROUTING_MaodvCheckMroute:
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    else if (nextHopNode->lifetime <= node->getNodeTime())
                    {
                        if (DEBUG_FAILURE)
                        {
                            char addr[25];
    ...
    case MSG_NETWORK_CheckRouteTimeout:
        {
            ...
            if (isValidRt && (rtPtr->lifetime <= node->getNodeTime()))
            {
                // Disable the route and then delete it after delete period
    ...
    case MSG_NETWORK_SendHello:
        {
            ...
            if (maodv->lastBroadcastSent
                <= (node->getNodeTime() - MAODV_HELLO_INTERVAL)
