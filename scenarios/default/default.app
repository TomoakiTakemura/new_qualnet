...
# 4. HTTPD
# 
# HTTPD simulates single-TCP connection web servers. 
# 
...
# where
# 
#     <address> is the address/node-id of a node which will be serving
#               Web pages.
# 
# EXAMPLE:
# 
...
#     HTTPD 5
# 
#     There are HTTP servers on nodes 2, 5
# ------------------------------------------------------------------------
# 5. HTTP
# 
...
# 
# ---------------------------------------------------------------
# 9. TRAFFIC-GEN and TRAFFIC-TRACE
#
...
#     REQUEST-DATA-RATE <data-rate> : is the rate at which
#   packets are transmitted.
#
#     REQUEST-TOS [TOS <tos-value> | DSCP <dscp-value> | PRECEDENCE
#                 <precedence-value>] : <tos-value> is the value of
#   the TOS bits of the IP header.<dscp-value> is the value of the DSCP
#   bits of the IP header.<precedence-value> is the contents of the
#   Precedence bits of IP header. When no REQUEST-TOS is specified,
#   default value of APP_DEFAULT_TOS ( Ox00) is used.
#
#     FRAGMENT-SIZE <bytes> : is the max UDP application fragment size.
#   Packets will be broken into multiple fragments when packet size
...
#   be unique.
#
#     CHAIN-ID <chain-id> : chain-id of the session. It is used to trigger
#   conditions based applications.
#
...
#   the TOS bits of the IP header.<dscp-value> is the value of the DSCP
#   bits of the IP header.<precedence-value> is the contents of the
#   Precedence bits of IP header. When no REPLY-TOS is specified,
#   default value of APP_DEFAULT_TOS ( Ox00) is used. It is optional.
#
#     CONNECTION-RETRY <retry-parameters> : In case of TCP connection,
#   it defines the count of retry in case connection is not established.
#
#     APPLICATION-NAME <application-name> : Name of the super application
#   session. This name is printed in the statistics file.
#
#     CONDITIONS <conditions_expr> : It defines the triggering conditions
#   for the session.
#   Conditions parameter specification has the following format:
...
#
#
#     ENCODING-SCHEME <traffic-type> <codec> : <traffic-type> can be VIDEO
#   or VOICE. Depending on traffic type, various codec are specified.
#   If <traffic-type> is VOICE, then AVERAGE-TALK-TIME and
#   MEAN-SPURT-GAP-RATIO should be specified.
#
#     AVERAGE-TALK-TIME <avg-talk-time> : Average talk time of the
#   voice session.
#
#     MEAN-SPURT-GAP-RATIO <gap-ratio> : Gap ratio between talk spurts.
#   AVERAGE-TALK-TIME and MEAN-SPURT-GAP-RATIO are used to calculate the
#   talk time. The session initiator will transmit the data/voice until
#   talk time expires.
#
#     REPEAT <time_distribution_type> <integer> : It defines the delay
#   between successive session and the number of times session will repeat.
#
#
#
# Where
...
#                                         EXP integer
#
#             <conditions_expr> ::= <conditions_expr> AND <conditions_expr> |
#                                   <conditions_expr> OR <conditions_expr> |
#                                   <conditions_expr> XOR <conditions_expr> |
#                                   ( <conditions_expr> ) | NOT <conditions_expr> |
#                                   <condition>
#
#
# EXAMPLE:
...
#    DET 10S REQUEST-NUM DET 5 REQUEST-SIZE DET 1460  REQUEST-INTERVAL DET 1S
...
#
# a) One of the following combination is required while applying SUPER-APP session
#    (i)  DELIVERY-TYPE, REQUEST-NUM, REQUEST-SIZE, and REQUEST-INTERVAL
#    (ii) ENCODING-SCHEME
#
# b) Response is only supported with DELIVERY-TYPE UNRELIABLE.
#
# c) Default value for REPLY-PROCESS is NO. Replies are not processed unless
#    this is explicitly set to YES.
#
# d) Both client and server print out all possible statistics even if they are
...
#
# e) The REPLY-INTERDEPARTURE-DELAY and REQUEST-INTERVAL is applicable between
#    packets and not fragments. If fragmentation occurs then all fragments are
#    sent back to back.
#
# f) 0 second duration indicates "till end of simulation".
#
# --------------------------------------------------------------------------
# 11. VBR
#
