...
# --------------------------------------------------------------------------
# 9. VOIP
#VOIP  <initiator_node_id> <receiver_node_id> <avg_talking_time>
#      <start_time> <end_time> [<status>] [<packetization_interval>]
#
#<initiator_node_id> : The terminal node that initiates the call
#
#<receiver_node_id> : The terminal node to which the call is destined
#
#<avg_talking_time> : The average talking time when a terminal starts talking.
#
#<start_time> : When initiating terminal requests to the receiver for the
#               voice conversation.
#
#<end_time> : When conversation is over.
#
#[<status>] : The status may be ACCEPT or REJECT. When this is set as ACCEPT
#             that means receiver will receive the call. The setting of REJECT
#             indicates the receiver will not receive the call and the call
#             request will be rejected. That means there will be no conversation.
#             This is optional and when not set, the call will be received by default.
#
# [<packetization_interval >] This is the interval at which Voip application sends
#             packets to RTP. The default value is 20MS if not set. Otherwise user
#             can specify a value in between 20MS to 200MS.
# Encoding Scheme G.711, Bit Rate = 64000/Sec, PacketizationInterval=20MS
# Encoding Scheme G.729, Bit Rate = 8000/Sec, PacketizationInterval=20MS
# Encoding Scheme G.723.1ar6.3, Bit Rate = 6300/Sec, PacketizationInterval=30MS
# Encoding Scheme G.723.1ar5.3, Bit Rate = 5300/Sec, PacketizationInterval=30MS
# Encoding Scheme G.726ar32, Bit Rate = 32000/Sec, PacketizationInterval=20MS
# Encoding Scheme G.726ar24, Bit Rate = 24000/Sec, PacketizationInterval=20MS
# Encoding Scheme G.728ar16, Bit Rate = 16000/Sec, PacketizationInterval=30MS


VOIP 1 20 10S 5M 6M CALL-STATUS ACCEPT ENCODING G.711 PACKETIZATION-INTERVAL 20MS
VOIP 1 20 10S 7M 8M CALL-STATUS ACCEPT ENCODING G.723.1ar6.3 PACKETIZATION-INTERVAL 30MS
VOIP 1 20 10S 9M 10M CALL-STATUS ACCEPT ENCODING G.711 PACKETIZATION-INTERVAL 20MS
VOIP 1 20 10S 10M 11M CALL-STATUS ACCEPT ENCODING G.723.1ar6.3 PACKETIZATION-INTERVAL 30MS
