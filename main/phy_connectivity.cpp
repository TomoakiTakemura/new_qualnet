...
void PHY_CONN_SendLinkNodeMsg(PartitionData * partitionData)
{
    ...
    PARALLEL_SynchronizePartitions(partitionData, BARRIER_TYPE_PhyLinkConnectivity);
    {
        ...
        for (int i = 0;
             i < partitionData->getNumPartitions();
             i++)
        {
            ...
            // copy over
            ERROR_Assert(i < (int)LinkNodeList.size(),
                        "Error in PHY_CONN_SendLinkNodeMsg." );
...
void PHY_CONN_Send802_3NodeMsg(PartitionData * partitionData)
{
...

    {
             ...
             i++)
        {
            ...
            // copy over
            ERROR_Assert(i < (int)EthernetNodeList.size(),
                        "Error in PHY_CONN_Send802_3NodeMsg." );
            PHY_CONN_NodePositionData::PER_PARTITION_SATCOM_NODELIST *
...
void PHY_CONN_SendSatComNodeMsg(PartitionData * partitionData)
{
...

    {
        ...
        i++)
        {
            ...
            // copy over
            ERROR_Assert(i < (int)SatcomNodeList.size(),
                        "Error in PHY_CONN_SendSatComNodeMsg." );
            PHY_CONN_NodePositionData::PER_PARTITION_SATCOM_NODELIST *
                                   ...
                                   PartitionData* partition)
{
        ...
        ++txrx_iter)
    {
        ...
        phyParam.m_ChannelIndex.set("Link");
        ...
        ++tx_iter)
    {
        ...
        // find the member nodes
            ...
            while (rx_node_iter != rx_iter->second->end())
            {
                ...
                phyParam.m_ChannelIndex.set("Ethernet");
        ...
        ++tx_iter)
    {
        ...
        if (type == (int)MAC_SATCOM_GROUND)
        {
            ...
            while (tx_node_iter != tx_iter->second->end())
            {
                ...
                phyParam.m_ReceiverPhyIndex = rx_node_iter->interfaceIndex;
                phyParam.m_ChannelIndex.set("SatCom");
        ...
        else if (type ==  (int)MAC_PROTOCOL_SWITCHED_ETHERNET )
        {
            ...
            while (tx_node_iter != tx_iter->second->end())
            {
                ...
                while (rx_node_iter != tx_iter->second->end())
                {
                    ...
                    phyParam.m_PhyIndex = tx_node_iter->interfaceIndex;
                    phyParam.m_ReceiverPhyIndex = rx_node_iter->interfaceIndex;
                    phyParam.m_ChannelIndex.set("Switched Ethernet");
                    STATSDB_HandlePhyConnTableUpdate(node, phyParam);
#endif
                    HandlePhyConnSenderListInsertion(partition,
                        tx_node_iter->nodeId, tx_node_iter->interfaceIndex,
...
// Get the best gain at given direction for switched beam antenna
//
//
// \param node  Pointer to node.
// \param phyindex  phy index.
// \param DOA  Given direction.
//
// \return The best gain at given direction
...
// Get the worst gain at given direction for switched beam antenna
//
//
// \param node  Pointer to node.
// \param phyindex  phy index.
// \param DOA  Given direction.
//
// \return The worst gain at given direction
...
// Get the steering angle at given direction for steerable antenna
//
//
// \param node  Pointer to node.
// \param phyindex  phy index.
// \param DOA  Given direction.
// \param steeringAngle  To store the steering angle.
//
...
// set the steering angle for steerable antenna
//
//
// \param node  Pointer to node.
// \param phyindex  phy index.
...
// Get the best gain at given direction for steerable antenna
//
//
// \param node  Pointer to node.
// \param phyindex  phy index.
...
// Get the worst gain at given direction for steerable antenna
//
//
// \param node  Pointer to node.
// \param phyindex  phy index.
// \param DOA  Given direction.
//
// \return The worst gain at given direction for steering antenna
...
// Get the best gain at given direction for patterned antenna
//
//
// \param node  Pointer to node.
// \param phyindex  phy index.
// \param DOA  Given direction.
//
// \return The best gain at given direction for patterned antenna
...
// Get the worst gain at given direction for patterned antenna
//
//
// \param node  Pointer to node.
// \param phyindex  phy index.
...
// check if the nodes pair are reachable for given signal power
//
//
// \param txNode  Pointer to tx node.
// \param rxNode  Pointer to rx node.
// \param txModel  Tx phy model
// \param rxInterfaceIndex  interface index for the rx node.
// \param channelIndex  index of the channel.
// \param rxpower_mW  the signal power which is used to check if the nodes pair are reachable
                ...
                double *rxpower_mW)
{
    ...
    switch (phyModel) {
        ...
        case PHY802_11b:
        {
            ...
            if (*rxpower_mW >=
               phy802_11->rxSensitivity_mW[phy802_11->pDot11Base->
                                           rxDataRateType])
            {
                isReachable = TRUE;
            }
...
// Compute receiver power from tx side
// Updates rxInfo
//
// \param partition  The current partition
// \param txNode  Pointer to the Tx node.
// \param rxNode  Pointer to the Rx node.
// \param txInfo  Info for tx node
// \param rxInfo  Info for rx node
// \param channelIndex  Channel index.
// \param rxPower_mw  Receiving power (output)
// \param rxPower_mwWorst  Receiving power in worst case (output)
//
static
...
// Estimate the connectivity for the nodes pair
//
//
// \param partition  The current partition
// \param txNode  Pointer to the Tx node.
...
// \param rxPhyIndex  Info for rx node
// \param rxPower_mw  Receiver power
// \param rxPower_mwWorst  Receiver power in worst case
// \param channelIndex  Channel index.
// \param reachable  Reachable
// \param ReachableWorst  Reachable for the worst case
//
// \return The reachable result for the best case
static
            ...
            tx_node_iter++)
        {
                ...
                rx_node_iter++)
            {
                ...
                PROP_CalculatePathloss(
                 ...
                 // If shadowing applies, the mean shadowing has to be added
                 // to the pathloss for CONSTANT shadowing model.
                 if (PROP_ShadowingApplies(node, channelIndex))
                 {
                     if (node->propChannel[channelIndex].profile->
                         shadowingModel == CONSTANT)
                     {
                         pathloss_dB +=
                             node->propChannel[channelIndex].profile->
                             shadowingMean_dB;
                     }
                 }
                ...
                rxConn++)
            {
                    ...
                    adj++)
                {
                    ...
                    if (adj->second.reachable)
                    {
                        ...
                        phyParam.SetChannelIndex(adj->first.channelIndex);
                        BOOL s_listening = FALSE;
                        BOOL r_listening = FALSE;
