...
void PARTITION_PrintUsage( const char* commandName ) {
    ...
    printf(
        ...
        "  -sops-port port           Port used to communicate with visualizer programs,\n"
        "                            e.g., Scenario Player, using the Sops/Vops interface.\n"
#ifdef PARALLEL //Parallel
        ...
        "  -pipeline <x>             This option enables pipelined syncing where <x>\n"
        "                            if the pipeline factor and improves runtime performance\n"
        "                            when running in distributed mode with MPI. The higher the <x>\n"
        "                            the greater the possible improvements but also increases\n"
        "                            the number of sync messages by <x> \n"
        "  -mpiLookahead             Set the lookahead used when running in distributed mode with\n"
        "                            MPI. Useful to force a larger lookahead than what is set\n"
        "                            by default, will improve distributed performance buy may\n"
        "                            lead to out of order events.\n"
        "  -optThroughput            Perform throughput optimizations for distributed mode with MPI\n"
        "  -multiMachine             Turn on multi machine mode for distributed mode with MPI\n"
        "                            which allows pcap and other external interfaces to run on\n"
        "                            all partitions. This mode requires all the machines to have\n"
        "                            clock synced within <1ms and there will be a 5s delay at startup\n"
        "                            for which the partitions on the different machines use to sync\n"
        "                            their startup times. This mode works best when there is only one\n"
        "                            partition on each machine.\n"
#endif //endUse_MPI
#endif //endParallel
 ...
 *    sopsvopsInterface: BOOL: If the Sops/Vops interface should be enabled
 *    sopsPort: int: Used to set Sops Port in the kernel
...
BOOL PARTITION_ParseArgv(int argc, char **argv,
    ...
    while (thisArg < argc) {
        ...
        if (!strcmp(argv[thisArg], "-seed")) {
...
#ifdef USE_MPI
        else if (!strcmp(argv[thisArg], "-pipeline")) {
            g_usePipeline = true;
            thisArg++;
            ...
            {
                ERROR_ReportError(
                      "Not enough arguments to -pipeline\n"
                      "Correct usage:\t -pipeline <pipeline-factor>\n");
            }

            g_pipelineFactor = atoi(argv[thisArg]);
            thisArg++;
        }
        else if (!strcmp(argv[thisArg], "-mpiLookahead")) {
            thisArg++;
            ...
            {
                ERROR_ReportError(
                      "Not enough arguments to -mpiLookahead\n"
                      "Correct usage:\t -mpiLookahead <lookahead-factor>\n");
            }

            g_mpiLookahead = TIME_ConvertToClock(argv[thisArg]);
            thisArg++;
        }
        else if (!strcmp(argv[thisArg], "-optThroughput")) {
            g_optimizeThroughput = true;
            thisArg++;
        }
        else if (!strcmp(argv[thisArg], "-multiMachine")) {
            g_multiMachine = true;
            thisArg++;
        }
#endif //endUse_MPI
        ...
        // This is for legacy compatibility
            ...
            sopsvopsInterface = true;
            if (argc < thisArg + 2) {
                ERROR_ReportError(
                    "-with_ocula_gui is deprecated. Use -sops-port.\n"
                    "Not enough arguments to -sops-port.\n"
                    "Correct usage:\t -sops-port PORT_NUMBER.\n");
            }
            sopsPort = atoi(argv[thisArg+1]);
            thisArg += 2;
        }
        else if (!strcmp(argv[thisArg], "-sops-port")) {
            sopsvopsInterface = true;
            if (argc < thisArg + 2) {
                ERROR_ReportError(
                    "Not enough arguments to -sops-port.\n"
        ...
        else if (!strcmp(argv[thisArg], "-sops-protocol")) {
            if (argc < thisArg + 2) {
                ERROR_ReportError(
                    "Not enough arguments to -sops-protocol.\n"
                    "Correct usage:\t -sops-protocol TCP|UDP\n");
            }
            if (!strcasecmp(argv[thisArg+1], "TCP")) {
                sopsProtocol = SOPS_PROTOCOL_TCP;
            }
            else if (!strcasecmp(argv[thisArg+1], "UDP")) {
                sopsProtocol = SOPS_PROTOCOL_UDP;
            }
            else {
                ERROR_ReportErrorArgs(
                    "Invalid -sops-protocol: %s\n"
                    "Specify TCP or UDP.\n",
                    argv[thisArg+1]);
            }
        ...
        // Values for these arguments are being parsed in gui.cpp.
        // Handling here just to avoid unknown option type error.
                          ...
                          char* experimentPrefix)
{
    ...
    if (SopsVopsInterfaceEnabled())
    {
        ...
        SetSopsProperty("/global/processorCount", value);
    }
#endif
...
PartitionData::PartitionData(int thePartitionId, int numPartitions, int numOPHosts = 0)
{
...
void PartitionData::removeOpHost()
{
    m_numOHAvailable++;
}
    ...
    int             numOPHosts)
{
    ...
    return new PartitionData(partitionId, numPartitions, numOPHosts);
}
    ...
    clocktype       startSimClock)
{
    ...
    if (!partitionData->statFd)
    {
        char currentWorkingDirectory[FILENAME_MAX];
        char* cwdReturn;
        memset(currentWorkingDirectory, 0, FILENAME_MAX);
#ifdef _WIN32
        cwdReturn = _getcwd(currentWorkingDirectory, FILENAME_MAX-1);
#else
        cwdReturn = getcwd(currentWorkingDirectory, FILENAME_MAX-1);
#endif
        if (cwdReturn == 0)
        {
            strcpy(currentWorkingDirectory, "*unknown*");
        }
        ERROR_ReportErrorArgs("\nYou must have write permission in the %s folder in order to run this simulation.\n\n"
            "If you installed %s as the superuser and you are running the %s program from "
            "a non-privileged account, you will have to modify file permissions to allow non-privileged users to "
            "create new files in the same folder as the config file that you are attempting to run.",
            currentWorkingDirectory, Product::GetProductName(), Product::GetProductName());
    }
    ...
    if (SopsVopsInterfaceEnabled())
    {
    // When using MPI every partition needs this initialization
#ifndef USE_MPI
        if (partitionData->partitionId == 0)
        {
#endif
    ...
    SCHED_Initialize(partitionData, nodeInput);
...
void InitializeNodes_Sops(PartitionData* partitionData)
{
        ...
        node = node->nextNodeData)
    {
        ...
        sprintf(key, "/node/%d/numberPhys", node->nodeId);
        sprintf(value, "%d", node->numberPhys);
        SetSopsProperty(key, value);

        for (int i = 0; i < node->numberPhys; i++)
        {
            sprintf(key, "/node/%d/phy/%d/protocol", node->nodeId, i);
            SetSopsProperty(
                key,
            ...
            sprintf(key, "/node/%d/phy/%d/propagationRange", node->nodeId, i);
            sprintf(value, "%f", maxRange);
            SetSopsProperty(key, value);
        }
        ...
        for (int i = 0; i < node->numberInterfaces; i++)
        {
            ...
            if ((networkType == NETWORK_DUAL) || (networkType == NETWORK_IPV4))
            {
                sprintf(key, "/node/%d/interface/%d/address/IPV4", node->nodeId, i);
                IO_ConvertIpAddressToString(
                    interfaceAddress,
                    value);
                SetSopsProperty(key, value);
            }
            if ((networkType == NETWORK_DUAL) || (networkType == NETWORK_IPV6))
            {
                Address intfAddress;
                Ipv6GetLinkLocalAddress(node, i, &intfAddress.interfaceAddr.ipv6);
                ...
                MAPPING_GetIpv6InterfaceInfoForInterface(node,
                                                 node->nodeId,
                                                 i,
                                                 &globalAddr1,
                                                 &subnetAddr1,
                ...
                sprintf(key, "/node/%d/interface/%d/address/IPV6", node->nodeId, i);
                IO_ConvertIpAddressToString(
                    &globalAddr1,
                    value);
                SetSopsProperty(key, value);
        ...
        IO_ReadString(node->nodeId, ANY_ADDRESS, nodeInput, "GUI-NODE-3D-ICON", &wasFound, value);
        if (wasFound)
        {
            sprintf(key, "/node/%d/gui/model", node->nodeId);
            SetSopsProperty(key, value);
        }
        if (!wasFound)
        {
            // Try reading GUI-NODE-2D-ICON
            IO_ReadString(node->nodeId, ANY_ADDRESS, nodeInput, "GUI-NODE-2D-ICON", &wasFound, value);
            if (wasFound)
            {
                sprintf(key, "/node/%d/gui/model", node->nodeId);
                SetSopsProperty(key, value);
        ...
        if (!wasFound)
        {
            ...
            IO_ReadString(node->nodeId, ANY_ADDRESS, nodeInput, "NODE-ICON", &wasFound, value);
            if (wasFound)
            {
                sprintf(key, "/node/%d/gui/model", node->nodeId);
                SetSopsProperty(key, value);
        ...
        // Set scale
        wasFound = FALSE;
        IO_ReadString(node->nodeId, ANY_ADDRESS, nodeInput, "GUI-NODE-SCALE", &wasFound, value);
        if (wasFound)
        {
            sprintf(key, "/node/%d/gui/scale", node->nodeId);
            SetSopsProperty(key, value);
        }

        // Set ground mobility
        sprintf(key, "/node/%d/groundNode", node->nodeId);
    ...
    if (partitionData->partitionId == 0)
    {
        SetSopsProperty("/locked", "0");
    }
}
#endif // SOPSVOPS_INTERFACE
...
void Initialize_Sops(PartitionData* partitionData)
{
    ...
    SetSopsProperty("/partition/0/numUrban", str);
    for (i = 1; i < urbanStrings.size(); i++)
    {
        sprintf(str, "/partition/0/urban/%d/name", i - 1);
        SetSopsProperty(str, urbanStrings[i]);
    }
...
// openssl one time calls(just done for 1st partition)
    ...
    partitionData->dbController = new Proc::DB::StatsDBController();
    ...
    while (nextNode != NULL)
    {
        ...
        else
        {
            ...
            {
                ERROR_Assert(sizeof(nextNode->hostname) >= sizeof(name), "hostname size error");
                strcpy(nextNode->hostname, name);
            }
        }
...
#endif
    partitionData->dbController->SetPartitionData(partitionData);
    partitionData->dbController->PublishFinishedTablesInterestsToDb();
...
void SopsVops_ProcessEvent(PartitionData* partitionData, Message* msg)
{
    char key[MAX_STRING_LENGTH];
    char value[MAX_STRING_LENGTH];
...
void PARTITION_Finalize(PartitionData* partitionData)
{
    ...
    if (SopsVopsInterfaceEnabled())
    {
        SopsVops_ProcessEvent(partitionData, NULL);
    }
#endif
    ...
    STATSDB_Finalize(partitionData);
#endif
         ...
         it++)
    {
        ...
        {
            nextNode->resourceManager->finalize();
        }
#endif
...
#ifdef ADDON_DB
    //STATSDB_CloseDriver(partitionData); TW
#endif
...
#endif
    else if (msg->eventType == MSG_SOPSVOPS_Global)
    {
        SopsVops_ProcessEvent(partitionData, msg);
        return;
    }
...
struct Vid
{
    int nodeId;
    int interfaceIndex;
    Vid(Node *node, int ix) {nodeId = node->nodeId; interfaceIndex = ix;}
};

// This could be refactored to a map or something better than O(n),
// but it is filled during initialization and used for certain HITL commands,
// so it is probably fast enough.
static std::vector<Vid> virtualInterfaces;

void PARTITION_DeclareVirtualInterface(Node *node, int interfaceIndex)
{
    if (!PARTITION_IsVirtualInterface(node, interfaceIndex))
    {
        virtualInterfaces.push_back(Vid(node, interfaceIndex));
    }
}

bool PARTITION_IsVirtualInterface(Node *node, int interfaceIndex)
{
    std::vector<Vid>::iterator it;
    for (it = virtualInterfaces.begin(); it != virtualInterfaces.end(); it++)
    {
        if (node->nodeId == it->nodeId &&
