...
void APP_TraceInitialize(Node *node, const NodeInput *nodeInput)
{
    ...
    if (retVal == TRUE)
    {
       for (int index = 0; index < appInput.numLines; index++)
       {
		   ...
		   //takemura
		   else if (!strcmp(apStr, "MQTT"))
			{
				AppMQTTInitTrace(node, nodeInput);
			}
#ifdef SENSOR_NETWORKS_LIB
                ...
                interfaceIndex++)
            {
                    ...
                    || networkType == NETWORK_IPV6)
                {
                    ...
                    IO_ReadBool(
                            ...
                            &addr,
                            nodeInput,
                            "EMULATED-FTP-ENABLED",
                            &wasFound,
                            &eFtpEnabled);

                    if (wasFound == FALSE || eFtpEnabled == TRUE)
                    {
                        AppEFtpServerInit(
                                node,
                                addr,
                ...
                interfaceIndex++)
            {
                    ...
                    || networkType == NETWORK_IPV6)
                {
                    ...
                    IO_ReadBool(
                            ...
                            &addr,
                            nodeInput,
                            "EMULATED-HTTP-ENABLED",
                            &wasFound,
                            &eHttpEnabled);

                     if (wasFound == FALSE || eHttpEnabled == TRUE)
                     {
                        AppEHttpServerInit(
                            node,
                            addr,
                ...
                interfaceIndex++)
            {
                    ...
                    || networkType == NETWORK_IPV6)
                {
                    ...
                    IO_ReadBool(
                            ...
                            &addr,
                            nodeInput,
                            "EMULATED-TELNET-ENABLED",
                            &wasFound,
                            &eTelnetEnabled);
                    if (wasFound == FALSE || eTelnetEnabled == TRUE)
                    {
        ...
        for (i = 0; i < appInput.numLines; i++)
        {
            ...
            else if (strcmp(appStr, "GSM") == 0)
            {
                    ...
                    callEndTime <= 0 || callEndTime >= simEndTime)
                {
                    ...
                    ERROR_ReportErrorArgs("Illegal Call Time:\n'%s'"
...
//takemura MQTT
			else if (strcmp(appStr, "MQTT") == 0)
            {
                ...
                if (numValues != 8)
                {
                    char errorString[MAX_STRING_LENGTH];
                    sprintf(errorString,
                            "Wrong MQTT configuration format!\n"
                            "MQTT <src> <dest> <items to Publish> <interval to Publish> <Size of Publish> <QoS Level> <start time> <end time>\n");
                    ERROR_ReportError(errorString);
                }
                ...
                if (node != NULL)
                {
                    ...
                    printf("Starting MQTT client with:\n");
                    printf("  src nodeId:    %u\n", sourceNodeId);
                    printf("  dst nodeId:    %u\n", destNodeId);
                    IO_ConvertIpAddressToString(&destAddr, addrStr);
                    printf("  dst address:   %s\n", addrStr);
                    printf("  items to Publish: %d\n", itemsToPub);
					ctoa(interval, clockStr);
					printf("  interval to Publish: %d\n ", clockStr);
					printf("  size of Publish Data: %s\n", itemSize);
                    ...
                    {
                        ERROR_ReportErrorArgs(
                            "MQTT: At node %d, Source and Destination IP"
                            " version mismatch inside %s\n",
                        node->nodeId,
                        appInput.inputStrings[i]);
                    }
...
//InsertPatch LAYER_FUNCTION
	...
	//takemura
		case APP_MQTT_CLIENT:
		{
			AppMQTTClient(node, msg);
			break;
		}
		case APP_MQTT_SERVER:
		{
			AppMQTTServer(node, msg);
			break;
		}
        default:
        {
    ...
    for (appList = node->appData.appPtr; appList != NULL; appList = nextApp)
    {
        switch (appList->appType)
        {
           ...
           // DHCP
			...
			case APP_MQTT_CLIENT:
			{
				AppMQTTClientFinalize(node, appList);
				break;
			}
			case APP_MQTT_SERVER:
			{
				AppMQTTServerFinalize(node, appList);
				break;
			}
			case APP_MQTT_SERVER_ALL:
			{
				AppMQTTServerAllFinalize(node, appList);
				break;
			}
