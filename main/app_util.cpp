    ...
    if (appParam != NULL)
    {
        ...
        if (strcmp(appParam->m_ApplicationType, "SUPER-APPLICATION") == 0)
        {
            ...
            if (sessionIdInfo == NULL)
            {
                ...
                *sessionIdInfo = appParam->getSessionId();
            }
        }
    }
#endif
    ...
    UInt8 ttl)
{
    ...
    if (appParam != NULL)
    {
        if (appParam->m_PktCreationTime.isNULL())
        {
    ...
    )
{
    ...
    if (appParam != NULL)
    {
        if (!appParam->m_SessionId.isNULL())
        {
            int* sessionIdInfo = (int*)
                MESSAGE_ReturnInfo(msg, INFO_TYPE_StatsDbAppSessionId);
            if (sessionIdInfo == NULL)
...
{
    char lastWrittenChar = 0x00;
    char* newLocation = str;
    char newChar = 0;
    while ((newChar = *str++) != 0x00)
    {
       if (newChar != ASCII_SPACE || newChar != lastWrittenChar)
       {
           *newLocation++ = lastWrittenChar = newChar;
       }
    }
    *newLocation = 0x00;
}
                      ...
                      bool* success)
{
    ...
    currentNode = partition->firstNode;

    while (currentNode)
    {
        string currentNodeName (currentNode->hostname);

        // Convert name of the current code name to Upper case.
        // This is done as <host-name> that was passed had been
        // changed to upper case by the caller function.

        for (size_t i = 0; i < currentNodeName.length(); i++)
        {
            currentNodeName.at(i) = toupper(currentNodeName.at(i));
        }

        if (currentNodeName.compare(hostName) == 0)
        {
            *success = TRUE;
            return currentNode;
        }
        ...
        {
            currentNode = currentNode->nextNodeData;
        }
    }
...
Node* ResolveNodeId(PartitionData* partition,
                    NodeAddress nodeId)
{
    Node* currentNode = partition->firstNode;
...
#ifdef PARALLEL
    NodePointerCollection* allNodes = partition->allNodes;

    for (size_t i = 0; i < allNodes->size(); i++)
    {
        if (nodeId == allNodes->at(i)->nodeId)
        {
                  ...
                  GuiHITLResponse responseType)
{
    if (!hid.empty())
    {
        GUI_CreateHITLResponse(hid, responseType);
    }
}
