...
// this is referenced in some template functions in a header.
...
void STAT_StatisticsList::Aggregate(PartitionData* partition)
{
    ...
    if (partition->getNumPartitions() > 1)
    {
        ...
        for (i = 0; i < partition->getNumPartitions(); i++)
        {
            if (i != partition->partitionId)
            {
                for (STAT_DataList* agg = gIncomingData[i]; agg != NULL; agg = agg->next)
                {
                    ERROR_Assert(
        ...
        STAT_DataList* agg = gIncomingData[partition->partitionId];

        while (agg != NULL)
        {
            STAT_DataList* temp = agg;
            agg = agg->next;
            MEM_free(temp->data);
            MEM_free(temp);
        }
        gIncomingData[partition->partitionId] = NULL;
...
void STAT_StatisticsList::SummarizeQueue(PartitionData* partition)
{
    ...
    for (unsigned int i = 0;
         i < partition->stats->global.inactiveQueues.size();
         i++)
    {
