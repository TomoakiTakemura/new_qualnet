...
static void ProcessMobilityEvent(Node* node, Message* msg)
{
    ...
    // Ignore this update if this msg is not originated from latest command received
    if (!mobilityEvent->isTruePosition
        && mobilityData->latestExternalMobilityCmdNum != mobilityEvent->mobilityCmdNum)
    {
        return;
    }
    ...
    // Copying the original mobilityCmdNum to new mobility event info
    newMobilityEvent->mobilityCmdNum = mobilityEvent->mobilityCmdNum;
...
// This function can be used as the interface's "time"
...
// This function can be used as the interface's "simulation
...
// This function is used when an external interface is running
...
//
// \param ifaceVoid  Pointer to the interface
//
// \return The next internal event
...
#ifdef TUTORIAL_INTERFACE
    if (EXTERNAL_ConfigStringIsYes(nodeInput, "INTERFACE-TUTORIAL"))
    {
        // Register InterfaceTutorial interface
        iface = EXTERNAL_RegisterExternalInterface(
                list,
...
#ifdef MULTI_GUI_INTERFACE
    if ((EXTERNAL_ConfigStringPresent(nodeInput, "MULTI-GUI-INTERFACE")
        || GUI_isMultigui())
        && list->partition->partitionId == 0)
    {
        iface = EXTERNAL_RegisterExternalInterface(
            list,
            "PROXY",
        ...
        EXTERNAL_RegisterFunction(
            iface,
            EXTERNAL_INITIALIZE,
            (EXTERNAL_Function) MultiGuiIntf_Initialize);

        // Register Receive function
        EXTERNAL_RegisterFunction(
            iface,
            EXTERNAL_RECEIVE,
            (EXTERNAL_Function) MultiGuiIntf_Receive);

       // Register Send function
        EXTERNAL_RegisterFunction(
            iface,
            EXTERNAL_FORWARD,
            (EXTERNAL_Function) MultiGuiIntf_Send);

        // Register Finalize function
        EXTERNAL_RegisterFunction(
            iface,
            EXTERNAL_FINALIZE,
            (EXTERNAL_Function) MultiGuiIntf_Finalize);
    }
#endif // MULTI_GUI_INTERFACE
            ...
            switch (msg->eventType)
            {
                ...
                case MSG_EXTERNAL_SendPacket:
                {
                                                   ...
                                                   == TRACE_IP_EXATA_NAT_NO)
                    {
                        ...
                        int interfaceIndex;
                        interfaceIndex =
                            MAPPING_GetInterfaceIndexFromInterfaceAddress(srcNode,
                                                                          srcAddr);
...
#ifdef MULTI_GUI_INTERFACE
        case EXTERNAL_MULTI_GUI:
        {
            MultiGuiIntf_ProcessEvent(node, msg);
            break;
        }
#endif // MULTI_GUI_INTERFACE
        default:
    ...
    PartitionData * partitionData)
{
    ...
    // however other external interfaces won't indicate
...
#ifdef ADDON_DB
STATSDB_Table_External_Events* EXTERNAL_Interface::get_STATSDB_Table() {
    if (db_exataEvents == NULL) db_exataEvents = new STATSDB_Table_External_Events(partition->statsDb);
    return db_exataEvents;
}
#endif
