...
// Prints interface hardware Address
//
...
// Return first four bytes of mac address
//
// \param byte[]  Mac address string
//
// \return unsigned int
...
// Sets first four bytes of mac address
//
// \param byte[]  Mac address string
// \param nodeId  unsigned int, node id of node
//
...
// Sets Interface index in mac address
//
// \param byte[]  Mac address string
// \param interfaceIndex  interfaceIndex of a node
//
...
// Retrieve the MacHWAddress of 6 bytes  from IP address.
//
// \param node  Pointer to Node structure
// \param macAddr  Pointer to hardware address structure
// \param ipv4Address  Ipv4 address from which the
...
// Convert Mac 802 addtess to Variable Hardware address
//
// \param node  Pointer to Node structure
// \param macHWAddr  Pointer to hardware address structure
// \param mac802Addr  Pointer to mac 802 address structure
...
// \param nodeInput  Ponter to the user specific file
// \param macAddr  Pointer to the hardware structure
//
static
    ...
    Address ipAddr)
{
    ...
    if (!macAddrConfigured)
    {
        if (strcmp(macProtocolName, "MAC802.3")==0 ||
           ...
           strcmp(macProtocolName, "MACDOT11p") == 0 ||
...
//
// \param node  Pointer to Node structure
// \param interfaceIndex  interface whose mac type requires
...
//
// \param node  Pointer to Node structure
// \param interfaceIndex  interface whose hardware length required
...
//
// \param node  Pointer to Node structure
// \param interfaceIndex  interface whose  hardware address retrieved
                ...
                Address* address,const NodeInput *nodeInput)
{
    ...
    if (node->macData == NULL) {
        node->macData = (MacData**)MEM_malloc(sizeof(MacData*) * MAX_NUM_INTERFACES);
        ERROR_Assert(node->macData != 0, "No memory for macData");
        memset(node->macData, 0, sizeof(MacData*) * MAX_NUM_INTERFACES);
    }

    node->macData[interfaceIndex] =
        (MacData *)MEM_malloc(sizeof(MacData));
    ERROR_Assert(node->macData[interfaceIndex] != 0, "No memory for macData element");
    ...
    short       subnetIndex)
{
    ...
    if ((strcmp(phyModelName, "PHY802.11a") == 0
        || strcmp(phyModelName, "PHY802.11b") == 0)
        && (strcmp(macProtocolName, "MACDOT11e") != 0
            && strcmp(macProtocolName, "MACDOT11") != 0
            && strcmp(macProtocolName, "ALOHA") != 0
            && strcmp(macProtocolName, "TDMA") != 0
            ...
            && strcmp(macProtocolName, "MAC-WORMHOLE") != 0))
    {
        char errorMessage[MAX_STRING_LENGTH];
        sprintf(errorMessage,
                "%s does not work with %s",
                macProtocolName, phyModelName);
        ERROR_ReportError(errorMessage);
    }

    if ((strcmp(phyModelName, "PHY802.11n") == 0 ||
         strcmp(phyModelName, "PHY802.11ac") == 0)&&
         strcmp(macProtocolName, "MACDOT11e") != 0)
    {
        char errorMessage[MAX_STRING_LENGTH];
        sprintf(errorMessage,
                "%s does not work with %s. MAC-PROTOCOL "
                "should be MACDOT11e",
                macProtocolName, phyModelName);
        ERROR_ReportError(errorMessage);
    }
    ...
    if (strncmp(macProtocolName, "FCSC-", 5) == 0) {
        ...
        if (strncmp(phyModelName, "FCSC-", 5) == 0) {
        ...
        else if (strcmp(phyModelName, "PHY802.11pCCH") == 0) {
            PHY_CreateAPhyForMac(
                    node,
                    nodeInput,
                    interfaceIndex,
            ...
            ERROR_Assert(interfaceIndex == 0,
                "The 802.11p Control Channel must be set at interface #0");
        }
        else if (strcmp(phyModelName, "PHY802.11pSCH") == 0) {
            PHY_CreateAPhyForMac(
                    node,
                    nodeInput,
                    interfaceIndex,
            ...
            ERROR_Assert(interfaceIndex != 0,
               "The 802.11p Service Channel can not be set at interface #0");
        }
        ...
        else if (strcmp(phyModelName, "PHY802.11ac") == 0) {
            PHY_CreateAPhyForMac(
                    node,
                    nodeInput,
                    interfaceIndex,
    ...
    STATSDB_HandlePhyDescTableInsert(node, interfaceIndex,
        ...
        else if (strcmp(macProtocolName, "MACDOT11p") == 0)
        {
            node->macData[interfaceIndex]->macProtocol =
                MAC_PROTOCOL_DOT11;
            ERROR_Assert(
                node->phyData[interfaceIndex]->phyModel == PHY802_11pCCH ||
                node->phyData[interfaceIndex]->phyModel == PHY802_11pSCH,
            "MAC802.11p requires PHY802.11pCCH or PHY802_11pSCH PHY model");

            MacDot11Init(node, interfaceIndex, nodeInput, phyModel,
                (SubnetMemberData *) subnetList, nodesInSubnet, subnetListIndex,
                subnetAddress, numHostBits, TRUE);
        }
...
#ifdef WIRELESS_LIB
    int phyNum = node->macData[interfaceIndex]->phyNumber;
    PHY_TuneRadio(node, phyNum);
#endif
}
               ...
               const NodeInput *nodeInput)
{
   ...
   //debug creation of subnet data table
    ...
    SetSopsProperty("/scenario/subnetCount", value);

    sprintf(value, "%d", numLinks);
    SetSopsProperty("/scenario/linkCount", value);
#endif
}
                                ...
                                int      interfaceIndex)
{
    ...
    // In case of STATIC_FAULT and RANDOM_FAULT,
    // enabling this interface only when all faults are removed.
    if (faultInfo->faultType == STATIC_FAULT
            || faultInfo->faultType == RANDOM_FAULT)
    {
    ...
    {
        // In case of EXTERNAL_FAULT (or enable node via HITL command),
        // forcibly enabling the interface and resetting the fault count.
        node->macData[interfaceIndex]->faultCount = 0;
    }
...
// Check for Multicast address or not.
//
...
MAC_ProcessEvent(Node *node, Message *msg)
{
    ...
    else if (MESSAGE_GetEvent(msg) == MSG_MAC_EndFault)
    {
        ...
        // DHCP
#ifdef CYBER_LIB
        // enabling OS Resource Model
        if (node->resourceManager)
        {
            OSResourceManager* resourceManager = node->resourceManager;
            if (resourceManager->isInShutDownMode())
            {
                resourceManager->setShutDownMode(FALSE);
            }
        }
#endif
    ...
    BOOL processMacHeader)
{
    ...
    std::string eventStr;
    static const std::string STATSDB_MacSendToLower("MacSendToLower");
    static const std::string STATSDB_MacReceiveFromLower("MacReceiveFromLower");
    static const std::string STATSDB_MacDrop("MacDrop");
    static const std::string STATSDB_MacSendToUpper("MacSendToUpper");
    static const std::string STATSDB_MacReceiveFromUpper("MacReceiveFromUpper");

    switch (eventType)
    {
    case MAC_SendToPhy:
        eventStr = STATSDB_MacSendToLower;
        break;
    case MAC_ReceiveFromPhy:
        eventStr = STATSDB_MacReceiveFromLower;
        break;
    case MAC_Drop:
        eventStr = STATSDB_MacDrop;
        break;
    case MAC_SendToUpper:
        eventStr = STATSDB_MacSendToUpper;
        break;
    case MAC_ReceiveFromUpper:
        eventStr = STATSDB_MacReceiveFromUpper;
        break;
    ...
    BOOL isAnyFrame)
{
    ...
    else if (eventType == MAC_ReceiveFromPhy)
    {
        ...
        ++iter->second.second  ;
        UInt64 packetRcvd = iter->second.second ;
