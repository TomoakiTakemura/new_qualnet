...
// This function comes up with a hash value for the given key.
// It is usable for an interface's hash table.
//
// \param key  Pointer to the key (not necessarily ASCII)
...
// This function will compare 2 keys
//
// \param key1  Pointer to the first key (not necessarily ASCII)
// \param keySize1  Size of the first key
// \param key2  Pointer to the second key (not necessarily ASCII)
// \param keySize2  Size of the second key
//
...
// This function will schedule a mobility event.
//
// \param iface  The external interface
// \param node  The node that will receive the mobility event
// \param mobilityEventTime  The absolute simulation time (not delay)
//    at which the mobility event should execute
// \param coordinates  The coordinates to move the node to
// \param orientation  The orientation to point the node in
// \param speed  The speed at which the node is moving in m/s
// \param velocity  The velocity vector of the node, in units of the
//    coordinates parameter, per second
// \param velocityOnly  Indicates whether this a velocity-only event (no
//    new information on the position or orientation)
// \param delaySpeedCalculation  Indicates whether the speed calculation
    ...
    BOOL delaySpeedCalculation)
{
    ...
    // increasing latestExternalMobilityCmdNum by 1
    node->mobilityData->latestExternalMobilityCmdNum++;
...
#ifdef ADDON_DB
    STATSDB_Table_External_Events* t = iface->get_STATSDB_Table();
    if (t->Use())
    {
        t->SetNodeID(nodeId);
        t->SetSource(srcAddr);
        t->SetDestination(destAddr);
        t->SetTOS(tos);
        t->SetTTL(ttl);
...
#ifdef ADDON_DB
static void emulateApplicationSendEvent(
    EXTERNAL_Interface* iface,
    NodeAddress nodeId,
    Message* msg,
    NodeAddress destAddr,
    ...
    )
{
    ...
    appParam.m_NodeId = nodeId;
    appParam.m_SessionInitiator = nodeId;
    appParam.m_ReceiverId = 0;
    appParam.SetAppType("EXTERNAL");
    appParam.SetFragNum(0);
    appParam.SetAppName("EXTERNAL");
    appParam.SetReceiverAddr(destAddr);
    appParam.SetPriority(tos);
    appParam.SetSessionId(0);
    appParam.SetMsgSize(0);
    appParam.m_TotalMsgSize = 0;
    appParam.m_fragEnabled = FALSE;

    Node* firstNode = iface->partition->firstNode;
    if ((protocol != IPPROTO_TCP) && (protocol != IPPROTO_UDP))
    {
        appParam.SetMsgSeqNum(msg->sequenceNumber);
        StatsDBAddMessageAppMsgId(firstNode, msg, &appParam);
        return;
    }

    HandleStatsDBAppSendEventsInsertion(firstNode, msg, 0, &appParam);
    STAT_Timing* timing = STAT_GetTiming(firstNode, msg);
    timing->sentApplicationDown = iface->partition->getGlobalTime();
    timing->receivedTransportUp = timing->sentApplicationDown;
    timing->sentTransportDown = timing->sentApplicationDown;

    int* sessionIdInfo = (int*)MESSAGE_ReturnInfo(msg, INFO_TYPE_StatsDbAppSessionId);
    if (sessionIdInfo == NULL)
    {
        sessionIdInfo = (int*)MESSAGE_AddInfo(firstNode, msg, sizeof(*sessionIdInfo), INFO_TYPE_StatsDbAppSessionId);
        *sessionIdInfo = 0;
    }
}
#endif //ADDON_DB
    ...
    NodeAddress nextHopAddr)
{
    ...
    {
        NodeAddress *headers;
        MESSAGE_AddHeader(iface->partition->firstNode,
                          sendMessage,
                          sizeof(NodeAddress) * 3,
                          TRACE_IP);
...
#ifdef ADDON_DB
    emulateApplicationSendEvent(iface, nodeId, sendMessage, destAddr, tos, protocol);

    // Update inofr for EXata_Events StatsDB table
    // reset messageId, it may have changed

    if (iface->partition->statsDb->doEXataEvents())
    {
        StatsDBMappingParam* mapParamInfo = (StatsDBMappingParam*)MESSAGE_ReturnInfo(sendMessage, INFO_TYPE_StatsDbMapping);
        if (mapParamInfo != NULL)
        {
            if (fragmentOffset || moreFragments)
            {
                char fragmentName[DEFAULT_MSG_ID_SIZE];
                sprintf(fragmentName, "NF%d", fragmentOffset);
                strcat(mapParamInfo->msgId, fragmentName);
            }
            STATSDB_Table_External_Events* table = iface->get_STATSDB_Table();
            table->SetMessageID(mapParamInfo->msgId);
        }
    }
#endif //ADDON_DB
...
void EXTERNAL_SendNetworkLayerPacket(Node* node, Message* msg)
{
    ...
    else
    {
        ...
        NetworkDataIp* ip = (NetworkDataIp *) node->networkData.networkVar;
        assert(ip);
        IPv6Data* ipv6 = (IPv6Data*) ip->ipv6;
...
// Assign a num physical layer transmission powere on the node
// where node might be on a remote partition.
//
// \param node  Pointer to node (local or proxy) that should be changed
// \param phyIndex  Which physical
// \param newTxPower  New transmission power - see PHY_SetTransmitPower ()
//
void
