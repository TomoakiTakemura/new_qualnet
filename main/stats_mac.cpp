...
{    
    ...
    if (dataSize > 0)
    {
        ...
        GetDataBytesSent(type).
            AddDataPoint(node->getNodeTime(), (double)dataSize);
        ...
        if (controlSize > 0)
        {
            GetControlBytesSent(type).
                AddDataPoint(node->getNodeTime(), (double)controlSize);
            stat->GetControlBytesSent(type).
                AddDataPoint(node->getNodeTime(), (double)controlSize);        
        }
    }
    else
    ...
    int interfaceIndex)
{
    ...
    if (dataSize > 0)
    {
        ...
        GetDataBytesReceived(type).
            AddDataPoint(node->getNodeTime(), (double)dataSize);
        stat->GetDataFramesReceived(type).AddDataPoint(node->getNodeTime(), 1);
        stat->GetDataBytesReceived(type).
            AddDataPoint(node->getNodeTime(), (double)dataSize);

        // TODO: This data should be considered overhead
        if (controlSize > 0)
        {
            GetControlBytesReceived(type).
                AddDataPoint(node->getNodeTime(), (double)controlSize);
            stat->GetControlBytesReceived(type).
                AddDataPoint(node->getNodeTime(), (double)controlSize);
        }
    }
    else
    ...
    GetCarriedLoad().AddDataPoint(node->getNodeTime(),
                                  (double)(controlSize + dataSize));
}
    ...
    UInt64 bytes)
{
    ...
    stat->GetBytesDroppedSender().
        AddDataPoint(node->getNodeTime(), (double)bytes);
}
