...
void Message::operator = (const Message &msg)
{
    ...
    {
        //assert(!m_mimoData);
        m_mimoData = new MIMO_Data();
        *m_mimoData = *(msg.m_mimoData);
    }
}
                       ...
                       std::string& buffer)
{
    ...
    int temp_i;
    if (msg->m_mimoData != NULL)
    {
        buffer.append((char*) &msg->m_mimoData->m_elementCount, sizeof(int));
        buffer.append((char*) &msg->m_mimoData->m_elementSpace, sizeof(double));
        buffer.append((char*) &msg->m_mimoData->m_LOS_weight, sizeof(double));
#if defined(DEBUG)
        printf("In serialize\n");
        printf("\n m_elementCount %d", msg->m_mimoData->m_elementCount);
        printf("\n m_elementSpace %lf", msg->m_mimoData->m_elementSpace);
        printf("\n m_LOS_weight %lf", msg->m_mimoData->m_LOS_weight);
#endif
        temp_i = msg->m_mimoData->m_correlation.rows();
        buffer.append((char*) &temp_i, sizeof(int));
#if defined(DEBUG)
        printf("\n Correlation matrix (real,imag)");
        printf("\n m_rows %d", temp_i);
#endif
        temp_i = msg->m_mimoData->m_correlation.cols();
        buffer.append((char*) &temp_i, sizeof(int));
#if defined(DEBUG)
        printf("\n m_cols %d\n", temp_i);
#endif
        double temp_num_real = 0.0;
        double temp_num_imag = 0.0;
        for (int row = 0; row < msg->m_mimoData->m_correlation.rows(); row++) {
            for (int col = 0; col < msg->m_mimoData->m_correlation.cols(); col++) {

                temp_num_real = std::real(msg->m_mimoData->m_correlation(row,col));
                buffer.append((char*) &temp_num_real, sizeof(double));
                temp_num_imag = std::imag(msg->m_mimoData->m_correlation(row,col));
                buffer.append((char*) &temp_num_imag, sizeof(double));
#if defined(DEBUG)
                printf("(%lf %lf)   ", temp_num_real,temp_num_imag);
#endif
            }
#if defined(DEBUG)
            printf("\n");
        ...
        temp_i = msg->m_mimoData->m_LOS.rows();
        buffer.append((char*) &temp_i, sizeof(int));
#if defined(DEBUG)
        printf("\n LOS Matrix (real,imag)");
        printf("\n m_rows %d", temp_i);
#endif
        temp_i = msg->m_mimoData->m_LOS.cols();
        buffer.append((char*) &temp_i, sizeof(int));
#if defined(DEBUG)
        printf("\n m_cols %d\n", temp_i);
#endif
        for (int row = 0; row < msg->m_mimoData->m_LOS.rows(); row++) {
            for (int col = 0; col < msg->m_mimoData->m_LOS.cols(); col++) {

                temp_num_real = std::real(msg->m_mimoData->m_LOS(row,col));
                buffer.append((char*) &temp_num_real, sizeof(double));
                temp_num_imag = std::imag(msg->m_mimoData->m_LOS(row,col));
                buffer.append((char*) &temp_num_imag, sizeof(double));
#if defined(DEBUG)
                printf("(%lf %lf)   ", temp_num_real,temp_num_imag);
#endif
    ...
    {
        // element count must be added so that the other end can
        // correctly interpret if mimoData has been received or not.
        int elementCount = 0;
        buffer.append((char*) &elementCount, sizeof(int));
    }
}

// FUNCTION   :: MESSAGE_ShortSerialize
// LAYER      :: ANY
// PURPOSE    :: Serialize a single message into a buffer so that the orignal
//               message can be recovered from the buffer
// PARAMETERS ::
                       ...
                       std::string& buffer)
{
    ...
    shortMsg.layerType = msg->layerType;
    shortMsg.protocolType = msg->protocolType;
    shortMsg.instanceId = msg->instanceId;
    shortMsg.m_radioId = msg->m_radioId;
    shortMsg.eventType = msg->eventType;
    shortMsg.naturalOrder = msg->naturalOrder;
    shortMsg.error = msg->error;
    shortMsg.mtWasMT = msg->mtWasMT;
    shortMsg.allowLoose = msg->allowLoose;
    shortMsg.nodeId = msg->nodeId;
    shortMsg.eventTime = msg->eventTime;
    shortMsg.eot = msg->eot;
    shortMsg.sourcePartitionId = msg->sourcePartitionId;
    shortMsg.packetSize = msg->packetSize;
    shortMsg.payloadSize = msg->payloadSize;
    shortMsg.virtualPayloadSize = msg->virtualPayloadSize;
    shortMsg.relayNodeAddr = msg->relayNodeAddr;
    shortMsg.subChannelIndex = msg->subChannelIndex;
    shortMsg.isPacked = msg->isPacked;
    shortMsg.actualPktSize = msg->actualPktSize;
    shortMsg.isEmulationPacket = msg->isEmulationPacket;
    shortMsg.offset = (msg->packet - msg->payload);
    shortMsg.originatingNodeId = msg->originatingNodeId;
    shortMsg.sequenceNumber = msg->sequenceNumber;
    shortMsg.originatingProtocol = msg->originatingProtocol;
    // short message
    buffer.append("s");

    buffer.append((const char*)&shortMsg, sizeof(ShortMessage));
    ...
    buffer.append((char*)&msg->numberOfHeaders, sizeof(msg->numberOfHeaders));

    for (unsigned int i = 0; i < msg->numberOfHeaders; i++)
    {
        buffer.append((char*)&msg->headerProtocols[i], sizeof(msg->headerProtocols[i]));
        buffer.append((char*)&msg->headerSizes[i], sizeof(msg->headerSizes[i]));
    }
}

// FUNCTION   :: MESSAGE_ShortUnserialize
// LAYER      :: MAC
...
// **/
Message* MESSAGE_ShortUnserialize(PartitionData* partitionData,
                             const char* buffer,
                             int& bufIndex,
                             bool mt)
{
    ...
    tmpMsg = (ShortMessage*) (buffer + bufIndex);
    bufIndex += sizeof(ShortMessage);
    ...
    memcpy(&newMsg->numberOfHeaders, &buffer[bufIndex], sizeof(newMsg->numberOfHeaders));
    bufIndex += sizeof(newMsg->numberOfHeaders);
    for (int i = 0; i < newMsg->numberOfHeaders; i ++)
    {
        memcpy(&newMsg->headerProtocols[i], &buffer[bufIndex], sizeof(newMsg->headerProtocols[i]));
        bufIndex += sizeof(newMsg->headerProtocols[i]);
        memcpy(&newMsg->headerSizes[i], &buffer[bufIndex], sizeof(newMsg->headerSizes[i]));
        bufIndex += sizeof(newMsg->headerSizes[i]);
    }
                             ...
                             bool mt)
{
    ...
    // Perform ShortUnserialize if a short message
    if (buffer[bufIndex] == 's') 
    {
        bufIndex++;
        return MESSAGE_ShortUnserialize(partitionData,buffer,bufIndex,mt);
    }
    ...
    if (band[0] != 0.0)
    {
      spectralBand* sb = spectralBand_Square::makeBand(band[0], band[1], "", partitionData);
      MESSAGE_SetSpectralBand(0, newMsg, sb);
    }
    int elementCount = 0;
    double elementSpace = 0.0;
    double LOSweight = 0.0;
    int row = 0, temp_row = 0;
    int col = 0, temp_col = 0;
    double r_part = 0.0, i_part = 0.0;

    memcpy(&elementCount, &buffer[bufIndex], sizeof(elementCount));
    bufIndex += sizeof(elementCount);
#if defined(DEBUG)
    printf("In unserialize\n");
    printf("elementCount %d\n",elementCount);
#endif
    if (elementCount)
    {
        newMsg->m_mimoData = new MIMO_Data();
        newMsg->m_mimoData->m_elementCount = elementCount;

        memcpy(&elementSpace, &buffer[bufIndex], sizeof(elementSpace));
        newMsg->m_mimoData->m_elementSpace = elementSpace;
        bufIndex += sizeof(elementSpace);
#if defined(DEBUG)
        printf("elementSpace %lf\n",elementSpace);
#endif
        memcpy(&LOSweight, &buffer[bufIndex], sizeof(LOSweight));
        newMsg->m_mimoData->m_LOS_weight = LOSweight;
        bufIndex += sizeof(LOSweight);
#if defined(DEBUG)
        printf("LOSweight %lf\n",LOSweight);
#endif
        memcpy(&row, &buffer[bufIndex], sizeof(row));
        bufIndex += sizeof(row);
#if defined(DEBUG)
        printf("m_correlation matrix (real,imag)\n");
        printf("row %d\n",row);
#endif
        memcpy(&col, &buffer[bufIndex], sizeof(col));
        bufIndex += sizeof(col);
#if defined(DEBUG)
        printf("col %d\n",col);
#endif
        newMsg->m_mimoData->m_correlation.resize(row, col);

        for (temp_row = 0; temp_row < row; temp_row++) {
            for (temp_col = 0; temp_col < col; temp_col++) {

                memcpy(&r_part, &buffer[bufIndex], sizeof(double));
                bufIndex += sizeof(double);
                memcpy(&i_part, &buffer[bufIndex], sizeof(double));
                bufIndex += sizeof(double);
#if defined(DEBUG)
                printf("(%lf %lf)  ",r_part, i_part);
#endif
                newMsg->m_mimoData->m_correlation(temp_row,temp_col) = std::complex<double>(r_part,i_part);
            }
#if defined(DEBUG)
            printf("\n");
#endif
        ...
        printf("col %d\n",col);
#endif
        newMsg->m_mimoData->m_LOS.resize(row, col);
        ...
        for (temp_row = 0; temp_row < row; temp_row++) {
            for (temp_col = 0; temp_col < col; temp_col++) {
                ...
                printf("(%lf %lf)   ",r_part, i_part);
#endif
                newMsg->m_mimoData->m_LOS(temp_row,temp_col) = std::complex<double>(r_part,i_part);
            }
#if defined(DEBUG)
            printf("\n");
#endif
...
{
#if defined(DEBUG)
    std::cout << "MESSAGE_RemoteSendSafely"
      << " src: " << node->nodeId << " dst: " << destNodeId
      << " requestedDelay: " << delay << std::endl;
#endif
    ...
    const clocktype& safeTime = node->partitionData->safeTime;

    if (safeTime != CLOCKTYPE_MAX)
    {
...
#if defined(DEBUG)
    std::cout << "using calculated_delay: " << delay << std::endl;
#endif
...
#else
void MESSAGE_DebugSend(PartitionData*, Node*, Message*)
{
    return;
}
#endif
