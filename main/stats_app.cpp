...
void STAT_AppStatistics::SetStatNames()
{
    ...
    {
        name = STAT_AddrToString(i) + "SessionStart";
        description = STAT_AddrToString(i) + " Session Start";
        units = "seconds";
        m_AddrStats[i].m_SessionStart.SetInfo(name, description, units);

        name = STAT_AddrToString(i) + "SessionFinish";
        description = STAT_AddrToString(i) + " Session Finish";
        units = "seconds";
        m_AddrStats[i].m_SessionFinish.SetInfo(name, description, units);

        name = STAT_AddrToString(i) + "MessagesSent";
        description = "Total " + STAT_AddrToString(i) + " Messages Sent";
        units = "messages";
        m_AddrStats[i].m_MessagesSent.SetInfo(name, description, units);

        name = STAT_AddrToString(i) + "FirstMessageSent";
        description = "First " + STAT_AddrToString(i) + " Message Sent";
        units = "seconds";
        m_AddrStats[i].m_FirstMessageSent.SetInfo(name, description, units);

        name = STAT_AddrToString(i) + "LastMessageSent";
        description = "Last " + STAT_AddrToString(i) + " Message Sent";
        units = "seconds";
        m_AddrStats[i].m_LastMessageSent.SetInfo(name, description, units);

        name = STAT_AddrToString(i) + "MessagesReceived";
        description = "Total " + STAT_AddrToString(i) + " Messages Received";
        units = "messages";
        m_AddrStats[i].m_MessagesReceived.SetInfo(name, description, units);

        name = STAT_AddrToString(i) + "MessagesDropped";
        description = "Total " + STAT_AddrToString(i)
            + " Messages Dropped Due To Missing Fragment";
        units = "messages";
        m_AddrStats[i].m_MessagesDropped.SetInfo(name, description, units);

        name = STAT_AddrToString(i) + "FirstMessageReceived";
        description = "First " + STAT_AddrToString(i) + " Message Received";
        units = "seconds";
        m_AddrStats[i].m_FirstMessageReceived.SetInfo(
            name, description, units);

        name = STAT_AddrToString(i) + "LastMessageReceived";
        description = "Last " + STAT_AddrToString(i) + " Message Received";
        units = "seconds";
        m_AddrStats[i].m_LastMessageReceived.SetInfo(
            name, description, units);

        name = STAT_AddrToString(i) + "FirstFragmentSent";
        description = "First " + STAT_AddrToString(i) + " Fragment Sent";
        units = "seconds";
        m_AddrStats[i].m_FirstFragmentSent.SetInfo(
            name, description, units);

        name = STAT_AddrToString(i) + "LastFragmentSent";
        description = "Last " + STAT_AddrToString(i) + " Fragment Sent";
        units = "seconds";
        m_AddrStats[i].m_LastFragmentSent.SetInfo(name, description, units);

        name = STAT_AddrToString(i) + "FirstFragmentReceived";
        description = "First " + STAT_AddrToString(i) + " Fragment Received";
        units = "seconds";
        m_AddrStats[i].m_FirstFragmentReceived.SetInfo(
            name, description, units);

        name = STAT_AddrToString(i) + "LastFragmentReceived";
        description = "Last " + STAT_AddrToString(i) + " Fragment Received";
        units = "seconds";
        m_AddrStats[i].m_LastFragmentReceived.SetInfo(
            name, description, units);

        name = STAT_AddrToString(i) + "FragmentsSent";
        description = "Total " + STAT_AddrToString(i) + " Fragments Sent";
        units = "fragments";
        m_AddrStats[i].m_FragmentsSent.SetInfo(name, description, units);

        name = STAT_AddrToString(i) + "FragmentsReceived",
        description = "Total " + STAT_AddrToString(i) + " Fragments Received";
        units = "fragments";
        m_AddrStats[i].m_FragmentsReceived.SetInfo(
            name, description, units);

        name = STAT_AddrToString(i) + "FragmentsReceivedOutOfOrder",
        description = "Total " + STAT_AddrToString(i)
        ...
        name = STAT_AddrToString(i) + "DuplicateFragmentsReceived",
        description = "Total " + STAT_AddrToString(i)
        ...
        name = STAT_AddrToString(i) + "DataSent";
        description = "Total " + STAT_AddrToString(i) + " Data Sent";
        units = "bytes";
        m_AddrStats[i].m_DataSent.SetInfo(name, description, units);

        name = STAT_AddrToString(i) + "DataReceived";
        description = "Total " + STAT_AddrToString(i) + " Data Received";
        units = "bytes";
        m_AddrStats[i].m_DataReceived.SetInfo(name, description, units);

        name = STAT_AddrToString(i) + "OverheadSent";
        description = "Total " + STAT_AddrToString(i) + " Overhead Sent";
        units = "bytes";
        m_AddrStats[i].m_OverheadSent.SetInfo(name, description, units);

        name = STAT_AddrToString(i) + "OverheadReceived";
        description = "Total " + STAT_AddrToString(i) + " Overhead Received";
        units = "bytes";
        m_AddrStats[i].m_OverheadReceived.SetInfo(name, description, units);

        name = STAT_AddrToString(i) + "AverageDelay";
        description = "Average " + STAT_AddrToString(i) + " End-to-End Delay";
        units = "seconds";
        m_AddrStats[i].m_AverageDelay.SetInfo(name, description, units);

        name = STAT_AddrToString(i) + "OfferedLoad";
        description = STAT_AddrToString(i) + " Offered Load";
        units = "bits/second";
        m_AddrStats[i].m_OfferedLoad.SetInfo(name, description, units);

        name = STAT_AddrToString(i) + "Throughput";
        description = STAT_AddrToString(i) + " Received Throughput";
        units = "bits/second";
        m_AddrStats[i].m_Throughput.SetInfo(name, description, units);

        name = STAT_AddrToString(i) + "Jitter";
        description = "Smoothed " + STAT_AddrToString(i) + " Jitter";
        units = "seconds";
        m_AddrStats[i].m_Jitter.SetInfo(name, description, units);

        name = STAT_AddrToString(i) + "AverageJitter";
        description = "Average " + STAT_AddrToString(i) + " Jitter";
        units = "seconds";
        m_AddrStats[i].m_AverageJitter.SetInfo(name, description, units);

        name = STAT_AddrToString(i) + "TotalJitter";
        description = "Total " + STAT_AddrToString(i) + " Jitter";
        units = "seconds";
        m_AddrStats[i].m_TotalJitter.SetInfo(name, description, units);

        name = STAT_AddrToString(i) + "AverageHopCount";
        description = "Average Hop Count Of Received "
    ...
    STAT_DestAddressType type)
{
    ...
    PartitionData* partition = node->partitionData->firstNode->partitionData;
#ifdef ADDON_DB
    StatsDb* db = partition->statsDb;
#endif
    Int64 intervalIndex = 0;
    STAT_GlobalAppStatistics* pAppAggregate = &(partition->stats->global.appAggregate);
    STAT_GlobalAppAddressStatistics* m_addrGlobalStats;
    m_addrGlobalStats = pAppAggregate->GetAddrStats(type);
    AggregatedMCR_Contents_Stats* mcrGlobalStats = m_addrGlobalStats->GetMcrStats();
    ...
    if (db != NULL)
    {
        ...
        if (now > 0)
        {
            if (now % interval)
            {
                intervalIndex = now/interval;
            }
            else
            {
                intervalIndex = now/interval - 1;
            }
        }
        if (intervalIndex < db->statsAggregateTable->mcrSize)
        {
            mcrGlobalStats[intervalIndex]
                    .m_IntervalBasedMessagesSent.AddDataPoint(now, 1);
        }
    }
#endif
    ...
    if (type == STAT_Multicast)
    {
        ...
        if (count > 0)
        {
            ...
            if (db != NULL)
            {
                if (intervalIndex < db->statsAggregateTable->mcrSize)
                {
                    pAppAggregate->GetIntervalBasedAggregatedEffectiveMessagesSent(
                        STAT_Multicast,
                        intervalIndex).AddDataPoint(now,
                                                     count);
                }
            }
    ...
    STAT_DestAddressType type)
{
    ...
    {
        Int64 intervalIndex = 0;
        clocktype sentTime = timing->sentApplicationDown;
        if (sentTime > 0)
        {
            if (sentTime % db->statsAggregateTable->aggregateInterval)
            {
                intervalIndex = sentTime/db->statsAggregateTable->aggregateInterval;
            }
            else
            {
                intervalIndex = sentTime/db->statsAggregateTable->aggregateInterval - 1;
            }
        }
        ...
        {
            mcrGlobalStats[intervalIndex].
                m_IntervalBasedMessagesReceived.AddDataPoint(now, 1);
        }
    }
#endif
    ...
    STAT_DestAddressType type)
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        if (db != NULL)
        {
            ...
            if (sentTime > 0)
            {
                if (sentTime % interval)
                {
                    intervalIndex = sentTime/interval;
                }
                else
                {
                    intervalIndex = sentTime/interval - 1;
                }
            }
            ...
            {
                mcrGlobalStats[intervalIndex]
                        .m_IntervalBasedAverageDelay.AddDataPoint(
                            now, (Float64) delay /SECOND);
            }
        }
#endif
        ...
        if (SopsVopsInterfaceEnabled())
        {
            ...
            sprintf(key, "/node/%d/appLayer/app/%d/stats/fragmentsReceived",
            node->nodeId, m_UniqueId);
            sprintf(value, "%lf", GetFragmentsReceived(type).GetValue(now));
            SetSopsProperty(
                key,
                value);
        }
    ...
    STAT_StatisticsList* stats)
{
    ...
    for (int i = 0; i < STAT_NUM_ADDRESS_TYPES; i++)
    {
        ...
        if (db != NULL)
        {
            m_AddrStats[i].mcrStats = new AggregatedMCR_Contents_Stats[(int)db->statsAggregateTable->mcrSize];
        }
#endif
       ...
       if (partition->partitionId == 0)
       {
        ...
        if (db != NULL)
        {
            if (partition->partitionId == 0)
            {
                name = std::string("TotalIntervalBased") + STAT_AddrToString(i) + std::string("ApplicationMessagesSent");
                path = "/stats/application/IntervalBased" + STAT_AddrToString(i) + "MessagesSent";
                description = "Total interval based " + STAT_AddrToString(i) + " messages sent from the application layer";
                units = "messages";

                for (Int64 intervalIndex = 0; intervalIndex < db->statsAggregateTable->mcrSize; intervalIndex++)
                {
                    GetIntervalBasedMessagesSent(i, intervalIndex).SetInfo(
                    name,
                    description,
                    units);
                }
            ...
            for (Int64 intervalIndex = 0; intervalIndex < db->statsAggregateTable->mcrSize; intervalIndex++)
            {
                stats->RegisterAggregatedStatistic(&GetIntervalBasedMessagesSent(i, intervalIndex));
            }

            if (partition->partitionId == 0)
            {
                name = std::string("TotalIntervalBased") + STAT_AddrToString(i) + std::string("ApplicationMessagesReceived");
                path = "/stats/application/IntervalBased" + STAT_AddrToString(i) + "MessagesReceived";
                description = "Total interval based " + STAT_AddrToString(i) + " messages received at the application layer";
                units = "messages";
                for (Int64 intervalIndex = 0; intervalIndex < db->statsAggregateTable->mcrSize; intervalIndex++)
                {
                    GetIntervalBasedMessagesReceived(i, intervalIndex).SetInfo(
                        name,
                        description,
                        units);
                }
            ...
            for (Int64 intervalIndex = 0; intervalIndex < db->statsAggregateTable->mcrSize; intervalIndex++)
            {
                stats->RegisterAggregatedStatistic(&GetIntervalBasedMessagesReceived(i ,intervalIndex));
            }

            if (partition->partitionId == 0)
            {
                name = "TotalEffectiveIntervalBased" + STAT_AddrToString(i) + "MessagesSent";
                path = "/stats/application/totalEffectiveIntervalBased" + STAT_AddrToString(i) + "MessagesSent";
                description = "Interval based " + STAT_AddrToString(i)
                              + " total effective messages sent at the application layer";
                units = "messages";
                for (Int64 intervalIndex = 0; intervalIndex < db->statsAggregateTable->mcrSize; intervalIndex++)
                {
                    GetIntervalBasedAggregatedEffectiveMessagesSent(i, intervalIndex).SetInfo(
                        name,
                        description,
                        units);
                }
            ...
            for (Int64 intervalIndex = 0; intervalIndex < db->statsAggregateTable->mcrSize; intervalIndex++)
            {
                stats->RegisterAggregatedStatistic(&GetIntervalBasedAggregatedEffectiveMessagesSent(i, intervalIndex));
            }

            if (partition->partitionId == 0)
            {
                name = std::string("IntervalBasedAverage") + STAT_AddrToString(i) + std::string("ApplicationDelay");
                path = "/stats/application/IntervalBased" + STAT_AddrToString(i) + "AverageDelay";
                description = "Interval Based Average " + STAT_AddrToString(i) + " delay at the application layer";
                units = "s";
                for (Int64 intervalIndex = 0; intervalIndex < db->statsAggregateTable->mcrSize; intervalIndex++)
                {
                    GetIntervalBasedAverageDelay(i, intervalIndex).SetInfo(
                        name,
                        description,
                        units);
                }
            ...
            for (Int64 intervalIndex = 0; intervalIndex < db->statsAggregateTable->mcrSize; intervalIndex++)
            {
                stats->RegisterAggregatedStatistic(&GetIntervalBasedAverageDelay(i, intervalIndex));
            }
        }
#endif
    }
    ...
    STAT_AppStatistics* stats)
{
    ...
    Node* sender = NULL;
    if (PARTITION_ReturnNodePointer(node->partitionData, &sender, senderId))
    {
    ...
    const STAT_AppSummaryStatistics& stats)
{
    ...
    {
        strcpy(name, stats.name);
        strcpy(type, stats.type);
    }
