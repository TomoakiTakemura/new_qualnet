    ...
    int destPort)
{
    ...
    if (dataSize > 0)
    {
        ...
        addr->m_DataBytesSent.
            AddDataPoint(timing->sentTransportDown, (double)dataSize);
        stat->GetDataSegmentsSent(type).AddDataPoint(timing->sentTransportDown, 1);
        stat->GetDataBytesSent(type).
            AddDataPoint(timing->sentTransportDown, (double)dataSize);
    }
    if (overheadSize > 0)
    {
        addr->m_OverheadBytesSent.
            AddDataPoint(timing->sentTransportDown, (double)overheadSize);
        stat->GetOverheadBytesSent(type).
            AddDataPoint(timing->sentTransportDown, (double)overheadSize);
    }
    else
    {
        addr->m_ControlSegmentsSent.AddDataPoint(timing->sentTransportDown, 1);
        addr->m_ControlBytesSent.AddDataPoint(
                                     timing->sentTransportDown,
                                     (double)controlSize);
        stat->GetControlSegmentsSent(type).AddDataPoint(timing->sentTransportDown, 1);
        stat->GetControlBytesSent(type).AddDataPoint(
                                     timing->sentTransportDown,
                                     (double)controlSize);
    }
    addr->m_OfferedLoad.AddDataPoint(
        node->getNodeTime(),
        (double)(dataSize + overheadSize + controlSize));
    ...
    int destPort)
{
    ...
    if (overheadSize > 0)
    {
        addr->m_OverheadBytesReceived.AddDataPoint(
                                          node->getNodeTime(),
                                          (double)overheadSize);
        stat->GetOverheadBytesReceived(type).AddDataPoint(
                                          node->getNodeTime(),
                                          (double)overheadSize);
    }
    if (dataSize > 0)
    {
        ...
        addr->m_DataBytesReceived.AddDataPoint(
                                          node->getNodeTime(),
                                          (double)dataSize);
        stat->GetDataSegmentsReceived(type).AddDataPoint(node->getNodeTime(), 1);
        stat->GetDataBytesReceived(type).AddDataPoint(
                                          node->getNodeTime(),
                                          (double)dataSize);
    }
    else
    {
        ...
        addr->m_ControlBytesReceived.AddDataPoint(
                                          node->getNodeTime(),
                                          (double)controlSize);
        stat->GetControlSegmentsReceived(type).AddDataPoint(node->getNodeTime(), 1);
        stat->GetControlBytesReceived(type).AddDataPoint(
                                          node->getNodeTime(),
                                          (double)controlSize);
    }
