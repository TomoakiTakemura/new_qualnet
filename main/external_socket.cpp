...
// This function increases the size of a VarArray.  This should
...
// \param MYPORT  Pointer to the VarArray to send
...
void* EXTERNAL_SocketSenderThread(void *voidSocket)
{
    ...
    while (s->isOpen && !s->error)
    {
        if (s->sendSize > 0)
        {
            ...
            size = MIN((int)s->sendSize, THREADED_BUFFER_SIZE - s->sendHead);
    ...
    std::string* data)
{
    ...
    else
    {
        ...
        while (!foundNewline)
        {
            ...
            recv(s->socketFd, buffer, (int)lengthProcessed, 0);
        }
    }
