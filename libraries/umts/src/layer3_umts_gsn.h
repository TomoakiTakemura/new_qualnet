...
/// MM sublayer main states in the network side
typedef enum
{
    UMTS_MM_NW_IDLE = 1,
...
/// GMM sublayer main states in the network side
typedef enum
{
...
/// GMM sublayer substates in the state REGISTERED (NW side)
typedef enum
{
...
/// Call control substates in the network side
typedef enum
{
    UMTS_CC_NW_NULL = 0,
...
/// Call control states in the GMSC
typedef enum
{
    UMTS_CC_GMSC_NULL,
...
/// Session management states in the network side
typedef enum
{
...
/// Status of a PDP context at the GGSN
typedef enum
{
...
/// Structure of the network MM data for an UE
typedef struct
{
...
/// Structure of the UE specific GMM data in the network side
typedef struct
{
...
/// Structure of the NW specific CC data
typedef struct
{
    UmtsCcNwState state;
...
/// Structure of the NW specific SM data
typedef struct
{
...
/// Structure of the NW call management
typedef struct
{
    UmtsCcNwData* ccData;
    UmtsSmNwData* smData;
...
/// Structure of the application information at SGSN
...
/// Structure of the voice call info at GMSC
...
/// UE data structure per UE at SGSN
...
/// One entry which stores information of a UE in the VLR
...
/// UE Location entry MAP, cached at GGSN(GMSC)
typedef std::map<NodeId, UmtsUeLocCacheEntry*> UmtsUeLocEntryMap;

/// Structure of the GSN specific layer 3 data UMTS GSN node
