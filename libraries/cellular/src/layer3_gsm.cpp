...
/*
    ...
    between 1 and GSM_MAX_TRAFFIC_CONNECTION_ID.
*/
static
    ...
    int* trafficConnectionId)
{
    ...
    for (i = 0; i < GSM_MAX_TRAFFIC_CONNECTION_ID ; i++ ){
    ...
    int         sccpConnectionId)
{
    ...
    if (GsmLayer3MscAssignTrafficConnId(nwGsmData->mscInfo,
                &newTrafficConnectionId) == FALSE)
    {
        char errorString[MAX_STRING_LENGTH];
        sprintf(errorString,
            "GSM_MSC ERROR: Could not allocate traffic connection Id\n"
            "Attempting to establish more than the maximum"
            " number of calls (%d) allowed simultaneously in a scenario",
        ...
        case GSM_RR_MESSAGE_TYPE_CHANNEL_RELEASE:
            {
                ...
                // Cancel Timer308 if MS is in GSM_MS_CC_STATE_RELEASE_REQUEST
                // state and channel release message is received.
                // This happens in the cases where CHANNEL COMPLETE message
                // is not received before channel release.
                if (msInfo->ccState == GSM_MS_CC_STATE_RELEASE_REQUEST)
                {
                    if (msInfo->timerT308Msg != NULL)
                    {
                        if (DEBUG_GSM_LAYER3_TIMERS)
