...
//    LogLevelType defaultLogLevel = LOG_LEVEL_DEBUG2;
    ...
    if (!value){
        ...
        strncpy(layerMask, value, sizeof(layerMask) - 1);
    }
    ...
    }else
    {
        ...
        for (; i < NUM_VALIDATION_PATTENRS; ++ i)
        {
            ...
            {
                validationPatternBCI = validationPatternProfile[i].connectionInfo;
                validationPatternBCILen = validationPatternProfile[i].numConnInfo;
                break;
            }
        }
        ...
        {
            loadTopologyFile(value, &validationPatternBCI, &validationPatternBCILen);
        }
...
{
    if (theLteLog.validationPatternBCI)
    {
        *len = theLteLog.validationPatternBCILen;
        return theLteLog.validationPatternBCI;
    }else
    {
        *len = 0;
        return NULL;
...
{
    return (theLteLog.validationPatternBCI != NULL);
}

bool LteLog::loadTopologyFile(const char* fn, const BaseConnectionInfo** dest, int* len )
{
    FILE* logFp = stdout;
    fprintf(logFp, "# Start loading topology file %s\n", fn);
    std::vector<BaseConnectionInfo> v;
    FILE* fp = fopen(fn,"r");
    if (fp == NULL)
    {
    ...
    while (fgets(buf, sizeof(buf), fp))
    {
        ...
        if (f.size() >= 8)
        {
            // f[0] is drop number, f[1] is distance between UE and eNB
            // Information for drop #1 is adopted
            if (atoi(f[0].c_str()) == 1)
            {
                BaseConnectionInfo bci;
                bci.nodeIdEnb = atoi(f[2].c_str());
                strcpy(bci.addressEnb, f[3].c_str());
                bci.nodeIdUe = atoi(f[4].c_str());
                strcpy(bci.addressUe, f[5].c_str());
                bci.nodeIdCn = atoi(f[6].c_str());
                strcpy(bci.addressCn, f[7].c_str());
                fprintf(logFp,"# Add connection info : %d,%s,%d,%s,%d,%s\n",
                        bci.nodeIdEnb,
                        bci.addressEnb,
                        bci.nodeIdUe,
                        bci.addressUe,
    ...
    // This is not free-ed
    BaseConnectionInfo* mem = new BaseConnectionInfo[v.size()];
