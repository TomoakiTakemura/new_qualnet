...
// Initialize UE default scheduler
//
    ...
    std::vector < LteUlSchedulingResultInfo > &schedulingResult)
{
    ...
    result->di.ndi = dciFormat0.ndi;

    if (result->mcsIndex < PHY_LTE_REGULAR_MCS_INDEX_LEN)
    {
    ...
    {
        result->dequeueInfo.dequeueSizeByte = 0;
    }
