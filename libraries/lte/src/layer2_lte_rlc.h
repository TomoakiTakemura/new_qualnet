...
/// RLC PDU header struct (nomal fixed part)
...
/// RLC PDU header struct (extentional part)
...
/// RLC PDU header struct (lengthIndicator-info part)
...
/// RLC PDU header struct (extentional fixed part)
...
/// RLC status PDU header struct (nacked-info part)
...
/// RLC Sdu LteRlcAmReTxBuffer
struct LteRlcAmReTxBufferElement
{
    Message*    message;
    int         offset;
    UInt8       retxCount;
...
/// RLC-PDUsegment received data-part infomation
...
struct LteRlcAmEntity
{
    ...
    // Latest sending PDU
    LteRlcAmReTxBufferElement latestPdu;
...
/// RLC sublayer structure
typedef struct{
    LteRlcStatus status;
    LteRlcStats stats;
} LteRlcData;

/// add info when store RLC SDU in transmission buffer
