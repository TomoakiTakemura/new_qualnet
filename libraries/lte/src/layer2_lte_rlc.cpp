...
#define RADIO_LINK_FAILURE_ON_RLC_RETXMAX (1)

#ifdef LTE_LIB_LOG

#define LTE_LIB_RLC_CHECK_EVENT (1)
...
// \param rnti  RNTI
// \param bearerId  Radio Bearer ID
// \param rlcAmEntity  The RLC AM entity
// \param receivedRlcAckSn  SN of received RLC ACK
//
static
...
// delete the data-structure in RLC header
//
...
// copy the data-structure in RLC header
//
...
// get D/Cbit data from the info-feild of message.
//
//    + msg       : Message* : message
//
...
// get RFbit data from the info-feild of message.
//
//    + msg       : Message* : message
//
...
// get Poll bit data from the info-feild of message.
//
//    + msg       : Message* : message
//
...
// get FI data from the info-feild of message.
//
//    + msg       : Message* : message
//
...
// get LI-list data from the info-feild of message.
//
//    + msg       : Message* : message
//
...
// get Last segment flag from the info-feild of message.
//
//    + msg       : Message* : message
//
...
// get segment offset from the info-feild of message.
//
//    + msg       : Message* : message
//
...
// get RLC header size from the info-feild of message.
//
//    + msg       : Message* : message
//
...
// get CPT from the info-feild of message.
//
//    + msg       : Message* : message
//
...
// get ACK-SN from the info-feild of message.
//
//    + msg       : Message* : message
//
...
// get NACK list from the info-feild of message.
//
//    + msg       : Message* : message
...
// Dump a single message into a buffer so that the orignal
...
// Get a message size from the original list of messages
//
//    + node      : Node*    : Pointer to node.
//    + msg       : Message* : Pointer to the supper msg containing list of msgs
//
...
// Get next sequence number (AM)
// 
//  + seqNum : UInt32: Base sequence number
// \return UInt32 Next sequence number
// **/
UInt32 LteRlcAmNextSN(UInt32 seqNum)
{
    return (seqNum + 1) % LTE_RLC_AM_SN_UPPER_BOUND;
}


// increment a sequence number (AM)
//
...
// Return the distance between sn1 and sn2,
...
//
// \return Sequence Number distance
static
...
// Return whether a sequence number is within the window
//
...
// Return whether a sequence number is within the range
...
// Return whether a sequence number A is less than B,
// within the window.
...
// Get RLC sublayer pointer from node and interface Id
//
//    + node : Node* : node pointer
//    + ifac : int   : interface id
//
// \return RLC sublayer data
static
...
// Insert fragmented PDU into transmission buffer
//
//    + node       : Node* : Node pointer
//    + ifac       : int   : interface id
...
BOOL findSnRetxBuffer(Node* node,
                      int iface,
                      std::list < LteRlcAmReTxBufferElement > &retxBuffer,
                      int sn,
                      std::list < LteRlcAmReTxBufferElement > ::iterator& it)
{
...
// Delete ACKed PDU from retransmission buffer
//
//    + node       : Node*          : Node pointer
...
void DeleteSnRetxBuffer(Node* node,
                        ...
                        std::list < LteRlcAmReTxBufferElement > &retxBuffer)
{
    std::list < LteRlcAmReTxBufferElement > ::iterator it = retxBuffer.begin();
...
// Set PDU status=ACKed in the retransmission buffer
//
//    + node       : Node*          : Node pointer
...
// Set PDU status=NACKed in the retransmission buffer
//
//    + node       : Node*          : Node pointer
//    + iface      : int            : Interface index
//    + amEntity   : LteRlcAmEntity : The RLC AM entity
...
// collect NACKed info in the retransmission buffer
//
//    + node       : Node*          : Node pointer
//    + iface      : int            : Interface index
//    + retxBuffer : std::list<LteRlcAmReTxBuffer>& : retransmission buffer
...
//
// \return Pointer to the allocatd timer message
static
...
// set own pointer to Xm(Tm/Um/Am)'s entityVar
...
// store RLC-SDU to TxBuffer in the RLC-entity
//
//    + node  : Node*    : Node Pointer
//    + iface : int      : Inteface index
                             ...
                             std::list < Message* > &txMsg)
{
    ...
    makedPduSegment = getRetxBuffer(node,
                                    ...
                                    txMsg,
                                    retxPduByte);
    numRetxPdu = (int)(txMsg.size() - numStatusPdu);
    ...
    else
    {
        ...
        makedPdu = getTxBuffer(node,
                               ...
                               txMsg,
                               txPduByte);
        numTxPdu = (int)(txMsg.size() - numStatusPdu - numRetxPdu);
...
// discard PDU by re-transmit upper limit.
//
//    + node        : Node* : Node Pointer
//    + iface       : int   : Inteface index
//    + it          : std::list<LteRlcAmReTxBuffer>::iterator&
...
//
void LteRlcAmEntity::discardPdu(
    Node* node,
    int iface,
    std::list < LteRlcAmReTxBufferElement > ::iterator& it,
    std::list < LteRlcAmReTxBufferElement > &buf,
...
#if RADIO_LINK_FAILURE_ON_RLC_RETXMAX
        Layer3LteNotifyRadioLinkFailure(node, iface, entityVar->oppositeRnti);
#else
        // Notify other layers of RLC reset
        MacLteNotifyRlcReset(node, iface, entityVar->oppositeRnti);

        int phyIndex = LteGetPhyIndexFromMacInterfaceIndex(node, iface);
        PhyLteNotifyRlcReset(node, phyIndex, entityVar->oppositeRnti);
#endif
    }
}

// get PDU from re-transmit buffer.
//
//    + node        : Node* : Node Pointer
//    + iface       : int   : Inteface index
            ...
            if (nackDataSize == MESSAGE_ReturnPacketSize(it->message))
            {
                ...
                if (restSize > size)
                {
                    ...
                    if (it->retxCount > rlcConfig->maxRetxThreshold)
                    {
                        ...
                        discardPdu(node, iface, it, buf, pdu, retxPduByte);
                        return FALSE;
                    }
...
// get status PDU from statusPDU buffer.
//
//    + node        : Node* : Node Pointer
//    + iface       : int   : Inteface index
//    + restSize    : int&  : request data size
...
// get PDU from transmittion buffer.
//
//    + node        : Node* : Node Pointer
//    + iface       : int   : Inteface index
//    + restSize    : int&  : request data size
                 ...
                 int& txPduByte)
{
    ...
    if (pduMsg != NULL)
    {
        ...
        LteRlcAmReTxBufferElement retx;
...
// concatenation message, and add LI-info.
//
//    + node      : Node*         : Node Pointer
//    + iface     : int           : Inteface index
//    + rlcHeader : LteRlcHeader& : RLC header data
...
// division message. (into two parts)
//
//    + node         : Node*             : Node Pointer
//    + iface        : int               : Inteface index
...
// Insert the PDU segments extracted
...
// Insert the SDU to buffer for send PDCP.
//
//    +node:       Node pointer
//    +iface:      The interface index
...
// Insert the SDU segments extracted from
...
// Insert the SDU segments extracted
...
// Handle a pollRetrunsmitTimer event
//
...
// Handle a statusProhibitTimer event
//
//    +node:  Node*:    The node pointer
//    +iface: int:    The interface index
//    +amEntity: LteRlcAmEntity*: AM entity
...
// Handle negative and positive acknowledgement
//
//    +node:      The node pointer
//    +iface:     The interface index
//    +amEntity:  The AM entity
...
{
    // If received ACK_SN is not VT(A) <= ACK_SN <= VT(MS), discard status PDU.
    // Inversion of arrival of status PDU happens due to HARQ retransmission.
    // Note that can be equal to VT(MS).
    if (!LteRlcSeqNumInRange(
            rxStatusPDU.fixed.ackSn,
            amEntity->ackSn,
            LteRlcAmNextSN(amEntity->sndWnd)))
    {
#ifdef LTE_LIB_LOG
        lte::LteLog::DebugFormat(
            node,
            iface,
            LTE_STRING_LAYER_TYPE_RLC,
            "DiscardStatusPduDueToInversion,"
            LTE_STRING_FORMAT_RNTI","
            "ACK_SN,%d,VT(A),%d,VT(S),%d",
            amEntity->entityVar->oppositeRnti.nodeId,
            amEntity->entityVar->oppositeRnti.interfaceIndex,
            rxStatusPDU.fixed.ackSn,
            amEntity->ackSn,
    ...
    std::list < LteRlcAmReTxBufferElement > ::iterator itRetxBuff;
    ...
    while (itRetxBuff != amEntity->retxBuffer.end())
    {
                                    ...
                                    rxStatusPDU.fixed.ackSn)))
        {
            ...
            lte::LteLog::DebugFormat(
                    node, iface, LTE_STRING_LAYER_TYPE_RLC,
                    "RlcRetxBufStatusAcked,%d,%d,statusPduSn,%d,retxBufPduSn,%d,VT(A),%d,from,%d,to,%d",
                    amEntity->entityVar->oppositeRnti.nodeId,
                    amEntity->entityVar->oppositeRnti.interfaceIndex,
                    rxStatusPDU.fixed.ackSn,
                    chkSn,
                    amEntity->ackSn,
                    (int)itRetxBuff->pduStatus,
                    (int)LTE_RLC_AM_DATA_PDU_STATUS_ACK);
#endif
...
#ifdef LTE_LIB_LOG
#if LTE_LIB_RLC_CHECK_EVENT
    std::list < LteRlcAmReTxBufferElement > ::iterator retxItr =
...
// Invoked when AM entity receives a STATUS PDU
//
//    +node:      The node pointer
//    +iface:     The interface index
//    +amEntity:  AM entity
...
// Get address-pointer of RLC entity
//
//    +node:      Node pointer
//    +iface:     The interface index
//    +rnti:      RNTI
...
// When receiving a new PDU, the AM entity
...
// Separate segments belonging to various SDU so that
...
#ifdef LTE_LIB_VALIDATION_LOG
        LteLayer2ValidationData* valData =
            LteLayer2GetValidataionData(node, iface, rlcEntity->oppositeRnti);
        if (valData != NULL)
        {
            lte::LteLog::InfoFormat(
                ...
                iface,
                LTE_STRING_LAYER_TYPE_RLC,
                "OutOfWindowOrAlreadyReceived,%d,%d",
                rlcEntity->oppositeRnti.nodeId, rcvdSeqNum);
        }
#endif
#endif
...
// Invoked when MAC submit a PDU to the entity
//
//    +node:          Node pointer
//    +iface:         the interface index
...
// Invoked when uppler layer deliver a SDU to the
// AM entity.
//
//    +node:      Node pointer
//    +iface:     the interface index
...
// Prepare RLC PDUs sent to MAC through AM SAP,
...
// Initialize configurable parameters of LTE RLC Layer.
//
...
//
// \param node  Pointer to node
// \param iface  interface index of LTE
// \param rlc  Pointer to RLC data
//
...
// send all reassembled RLC SDUs to PDCP
// 
//
// \param node  Pointer to node.
// \param interfaceIndex  Interface index
// \param rnti  RNTI
// \param bearerId  Radio Bearer ID
// \param targetRnti  RNTI of target eNB
//
        ...
        const UInt16& pdcpSn)
{
    ...
    switch (rlcEntity->entityType) {
        case LTE_RLC_ENTITY_AM: {
            ...
            ERROR_Assert(
                    rlcAmEntity->txBufSize >= MESSAGE_ReturnPacketSize(itr->message),
...
// Add RLC-PDU (AM entity) header information
// to the message. (as Info)
//
// \param node  Pointer to node
// \param iface  interface index
// \param rlcHeader  RLC-PDU header structure.(source)
// \param msg  target message pointer.
//
void LteRlcSetMessageRlcHeaderInfo(Node* node,
                                   int iface,
                                   LteRlcHeader& rlcHeader,
...
#ifdef LTE_LIB_LOG
            lte::LteLog::DebugFormat(node, iface, LTE_STRING_LAYER_TYPE_RLC,
                    "LteRlcSetMessageRlcHeaderExtensionInfo,LIsize=,%d",
                    rlcHeader.extension->lengthIndicator.size());
#endif
        }
...
// Add RLC-statusPDU to the message. (as Info)
//
// \param node  Pointer to node
// \param iface  interface index
// \param msg  target message pointer.
// \param status  RLC-satusPDU structure. (source)
//
...
// Add RLC-statusPDU header information to the message.
// (as Info) RLC-statusPDU fixed part.
//
// \param node  Pointer to node
// \param iface  interface index
// \param rlcHeader  RLC-statusPDU fixed part structure.(source)
// \param msg  target message pointer.
//
void LteRlcSetMessageStatusPduFormatFixedInfo(
    Node* node,
...
// \param node  Pointer to node
// \param iface  interface index
// \param nackSoMap  multimap< UInt16, LteRlcStatusPduSoPair >&
...
// Get RLC-PDU (AM entity) header information
// from the message info-field.
// RLC-PDU fixed part.
//
// \param node  Pointer to node
// \param iface  interface index
// \param rlcHeader  RLC-PDU header structure.
// \param msg  target message pointer.(source)
//
void LteRlcGetMessageRlcHeaderFixedInfo(Node* node,
                                        int iface,
                                        LteRlcHeader& rlcHeader,
...
// Get RLC-PDU(AM entity) header + data size.
//
// \param node  Pointer to node
// \param iface  interface index
// \param rlcHeader  RLC-PDU header structure.
...
// Get RLC-PDU(AM entity) header size.
//
// \param node  Pointer to node
// \param iface  interface index
// \param rlcHeader  RLC-PDU header structure.
...
// Get RLC-PDU(AM entity) format header size.
//
// \param node  Pointer to node
// \param iface  interface index
// \param rlcHeader  RLC-PDU header structure.
//
// \return PDU format header size
int LteRlcGetAmPduFormatSize(Node* node,
                             int iface,
                             LteRlcHeader& rlcHeader)
{
...
// Get RLC-PDU(AM entity) header size of no segmented.
//
// \param node  Pointer to node
// \param iface  interface index
// \param rlcHeader  RLC-PDU header structure.
...
// Get RLC-PDU(AM entity) header size of segmented.
//
// \param node  Pointer to node
// \param iface  interface index
// \param rlcHeader  RLC-PDU header structure.
                             ...
                             LteRlcHeader& rlcHeader)
{
    if (rlcHeader.extension != NULL)
    {
        return LteRlcGetAmPduLiAndESize(
                    ...
                    iface,
                    rlcHeader,
                    (int)rlcHeader.extension->lengthIndicator.size());
    }
    else
    {
        return 0;
...
// Get RLC-PDU(AM entity) header size of re-segmented.
//
// \param node  Pointer to node
// \param iface  interface index
// \param rlcHeader  RLC-PDU header structure.
...
// Get RLC-PDU(AM entity) header's fixed part size of
// re-segmented.
//
// \param node  Pointer to node
// \param iface  interface index
...
// Get RLC-PDU(AM entity) header's extention part size
// re-segmented.
//
// \param node  Pointer to node
// \param iface  interface index
...
// Get RLC-PDU(AM entity) data size.
//
// \param node  Pointer to node
// \param iface  interface index
// \param rlcHeader  RLC-PDU header structure.
// \param msg  RLC-PDU messages
//
// \return data size
int LteRlcGetAmPduDataSize(Node* node,
                           int iface,
...
// \param iface  interface index
// \param rlcHeader  RLC-PDU header structure.
// \param msgList  RLC-PDU messages list
//
// \return data size
int LteRlcGetAmPduListDataSize(Node* node,
                               int iface,
...
// Get RLC-PDU(AM entity) expected size.
//
// \param node  Pointer to node
// \param iface  interface index
// \param rlcHeader  RLC-PDU header structure.
// \param reSegment  flag of re-segmented
// \param msgList  RLC-PDU messages list
// \param txBuffData  transmittion buffer
//
// \return header + data size
int LteRlcGetExpectedAmPduSize(Node* node,
                               int iface,
...
// Get RLC-PDU(AM entity) header expected size.
//
// \param node  Pointer to node
// \param iface  interface index
// \param rlcHeader  RLC-PDU header structure.
                                     ...
                                     BOOL reSegment)
{
    ...
    if (reSegment == FALSE)
    {
        ...
        if (rlcHeader.extension != NULL)
        {
            extSize += (int)rlcHeader.extension->lengthIndicator.size();
            extSize++;
...
// Get RLC-PDU(AM entity) data expected size.
//
// \param node  Pointer to node
// \param iface  interface index
// \param rlcHeader  RLC-PDU header structure.
// \param msgList  RLC-PDU messages list
// \param addMsg  RLC-PDU messages
//
// \return data size
int LteRlcGetExpectedAmPduDataSize(Node* node,
                                   int iface,
...
// allocate LI field for RLC-PDU(AM entity) header.
//
// \param node  Pointer to node
// \param iface  interface index
// \param rlcHeader  RLC-PDU header structure.
// \param size  LI (size of massage)
//
void LteRlcAllocAmPduLi(Node* node,
                        int iface,
                        LteRlcHeader& rlcHeader,
...
// add LI to RLC-PDU(AM entity) header.
//
// \param node  Pointer to node
// \param iface  interface index
// \param rlcHeader  RLC-PDU header structure.
...
//
// \return PDU size
int LteRlcAmReTxBufferElement::getPduSize(Node* node, int iface)
{
    return LteRlcGetAmPduSize(node, iface, rlcHeader, message);
}

// get RLC-PDU NACKed size
//
//    + node  : Node* : pointer to the network node
//    + iface : int   : interdex of interface
//
// \return NACKed data size
int LteRlcAmReTxBufferElement::getNackedSize(Node* node, int iface)
{
    UInt32 retSize = 0;
                 ...
                 const NodeInput* nodeInput)
{
    ...
    layer2DataLte->lteRlcVar->status = LTE_RLC_POWER_OFF;
    layer2DataLte->lteRlcVar->stats.numAmStatusPdusReceivedFromMacSublayer = 0;
    layer2DataLte->lteRlcVar->stats.numAmStatusPdusReceivedFromMacSublayerButDiscardedByReset = 0;
    layer2DataLte->lteRlcVar->stats.numAmStatusPdusSendToMacSublayer = 0;
    layer2DataLte->lteRlcVar->stats.numDataPdusDiscardedByRetransmissionThreshold = 0;
    layer2DataLte->lteRlcVar->stats.numDataPdusReceivedFromMacSublayer = 0;
    layer2DataLte->lteRlcVar->stats.numDataPdusReceivedFromMacSublayerButDiscardedByReset = 0;
    layer2DataLte->lteRlcVar->stats.numDataPdusSentToMacSublayer = 0;
    layer2DataLte->lteRlcVar->stats.numSdusDiscardedByOverflow = 0;
    layer2DataLte->lteRlcVar->stats.numSdusReceivedFromUpperLayer = 0;
    layer2DataLte->lteRlcVar->stats.numSdusSentToUpperLayer = 0;

    // init configureble parameters
    LteRlcInitConfigurableParameters(node, iface, nodeInput);

    // init dyanamic statistics
    LteRlcInitDynamicStats(node, iface, layer2DataLte->lteRlcVar);
}
...
// retuern sendable data size(byte) in queue
// (sum of all buffer)
// 
//
//    + node     : Node*          : Pointer to node.
...
// retuern sendable data size(byte) in transmittion buffer
// 
//
//    + node     : Node*          : Pointer to node.
//    + iface    : unsigned int   : Interdex of interface
...
#if RADIO_LINK_FAILURE_ON_RLC_RETXMAX
    Layer3LteNotifyRadioLinkFailure(node, iface, oppositeRnti);
#else
    // Notify other layers of RLC reset
    MacLteNotifyRlcReset(node, iface, oppositeRnti);

    int phyIndex = LteGetPhyIndexFromMacInterfaceIndex(node, iface);
    PhyLteNotifyRlcReset(node, phyIndex, oppositeRnti);
#endif
}
...
void LteRlcAmEntity::resetAll(BOOL withoutResetProcess)
{
    ...
    // reset retx buffer
    std::list < LteRlcAmReTxBufferElement > ::iterator
        it = retxBuffer.begin();
    while (it != retxBuffer.end())
    {
        LteRlcDelAmPduHdrObj((*it).rlcHeader);
    ...
    std::for_each(
        retxBuffer.begin(),
        retxBuffer.end(),
        LteRlcFreeMessageData < LteRlcAmReTxBufferElement > (entityVar->node));
    retxBuffer.clear();
    retxBufSize = 0;
    retxBufNackSize = 0;
...
// available number of PDU from retx buffer which fit into size
// 
//
//    + node           : Node*   : Node Pointer
//    + interfaceIndex : int     : Inteface index
...
{
    int ret = 0;
    std::list < LteRlcAmReTxBufferElement > ::iterator itr = retxBuffer.begin();
    int tempSize = 0;
