...
{
    ERROR_Assert(index >= 0 && index < (int)m_numDemFiles, "Invalid index");
    ...
    double* lowest)
{
    ...
    *lowest  = m_records[swIndex]->minElevation;
    ...
    return;
        ...
        *highest = MAX(*highest, (double)m_records[neIndex]->maxElevation);
        *lowest  = MIN(*lowest, (double)m_records[swIndex]->minElevation);
    }
}
