        ...
        const NodeInput *nodeInput)
{
  ...
  node->battery = (Battery*)MEM_malloc(sizeof(Battery));
  memset(node->battery, 0, sizeof(Battery));
...
BATTERY_RunTimeStat(Node *node)
{
    if (node->guiOption)
    {
        if (node->battery == NULL
            || node->battery->model == NO_MODEL)
        {
            // Currently runtime statistics are not available if no battery
            // model is configured at a node.
            return;
        }

        //Get remaining battery charge
        double rCharge = BatteryGetRemainingCharge(node);
        if (rCharge > 0.0)
        {
            rCharge /= 3600.0;
        }
