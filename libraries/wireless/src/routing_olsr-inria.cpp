...
// Searches an address in the two hop neighbor table
//
// \param node  Pointer to Node structure
// \param dest  address to be searched
//
...
// Delete a two hop neighbor entry results the deletion of
// its 1 hop neighbors list. We don't try to remove the one
// hop neighbor even if it has no more 2 hop neighbors.
//
// \param two_hop_neighbor  Pointer to two hop neighbor entry
//
void OlsrDelete2HopNeighborTable(
    neighbor_2_entry* two_hop_neighbor);

// Searches neighbor table for this address
//
// \param node  Pointer to Node structure
// \param dst  Address to look for
//
// \return if found, pointer to entry
// else NULL
neighbor_entry* OlsrLookupNeighborTable(
    Node* node,
    Address dst);

// Inserts neighbor entry in the neighbor table
//
// \param node  Pointer to Node structure
// \param neighbor  entry to be inserted
//
void OlsrInsertNeighborTable(
    Node* node,
    neighbor_entry* neighbor);

// Deletes current address from the 2 hop neighbor list of this
// neighbor entry
//
// \param neighbor  Pointer to neighbor entry
// \param address  Address to be deleted from two hop neighbor list
//
void OlsrDeleteNeighbor2Pointer(
    neighbor_entry* neighbor,
    Address address);

// Deletes current neighbor entry from the neighbor table
//
// \param neighbor  Pointer to entry to be deleted
//
...
// \param originator  originator of message
// \param seqno  seq no of message
// \param interfaceIndex  interface that packet arrived on
// \param from_addr  Neighbor Interface address that transmitted msg
//
// \return 1 if msg is sent to be fwded, 0 if not
...
// Function checks if address is not one of the
// current node's interface address
//
// \param node  Pointer to Node structure
// \param addr  Address to be checked
//
// \return Returns interface index if match, else -1
Int32 RoutingOlsrCheckMyIfaceAddress(
    Node* node,
    Address addr);

// Search an address in the MPR  selector table
//
// \param node  Pointer to Node structure
// \param dst  Address to be searched
//
// \return Pointer to entry if found, else NULL
...
// Deletes entry from MPR  selector table
//
// \param mprs  Pointer to mpr entry
//
...
// Converts a Float32 to mantissa/exponent
// format as per RFC 3626
//
// \param interval  value to be converted
//
// \return 8 bit mantissa/exponent product
...
// Converts a mantissa/exponent
// format  to Float32 as per RFC 3626
//
// \param me  8 bit m/e value to be converted
//
...
// Returns current message sequence counter
...
// Returns main_address given an alias
//
// \param node  Pointer to Node structure
// \param alias  alias address
//
// \return main address of the alias
...
// Returns a link list of aliases for a given main_address
//
// \param node  Pointer to Node structure
// \param main_addr  main_address
//
// \return link list of aliases of the given main_address
...
// Insert an alias entry into the MID table
//
// \param node  Pointer to Node structure
// \param main_addr  Main address of an olsr-node
// \param alias  alias entry
...
// Insert alias address into MID table
//
// \param node  Pointer to Node structure
// \param main_address  main address
// \param alias  alias address
// \param vtime  Validity time
//
static
...
// Delete a MID table entry
//
// \param entry  Pointer to MID table entry
//
static
...
// Check MID table entry for time out
//
// \param node  Pointer to Node structure
//
static
...
// Update timer of a MID table entry
//
// \param node  Pointer to Node structure
// \param main_addr  Main address of an olsr-node
// \param vtime  Validity time
...
// Print MID table of an olsr node
//
// \param node  Pointer to Node structure
//
static
...
// Looks up HNA table for a given net address
//
// \param node  Pointer to Node structure
// \param nets  pointer to the network list to look in
...
// Looks up HNA table for a given a gateway entry
//
// \param node  Pointer to Node structure
...
//
// \param hna_gw  Pointer to the gateway to the non-olsr network
// \param net  The network to add
// \param mask  The netmask to add
//
// \return pointer to the new entry
...
// updates network entry in HNA table
//
// \param node  Pointer to Node structure
// \param gw  The gateway address
...
// times out all the old entries in the hna table
//
// \param node  Pointer to Node structure
//
static
...
// print all the entries in the hna table
//
// \param node  Pointer to Node structure
//
static
...
// Looks up status of entry based on the timer entries
//
// \param node  Pointer to Node structure
// \param entry  Pointer to link set entry
//
...
// Looks up link set entry based on local and
// neighbor interface address
//
// \param node  Pointer to Node structure
// \param remote  neighbor address
// \param local  local interface address
//
// \return pointer to the link entry
// if found, else NULL
...
// Looks up neighbor address in link set and
// returns the link status
//
// \param node  Pointer to Node structure
// \param address  IP Address of neighbor
//
// \return status of link
static
...
// Updates neighbor status in the neighbor table entry depending
// on the link status passed
//
// \param node  Pointer to Node structure
// \param entry  Pointer to neighbor entry
// \param link  link status
//
// \return status of neighbor set
static
...
// Searches for an entry in link set
// If not found, adds a new link entry
// to the link set
//
// \param node  Pointer to Node structure
// \param local  local interface IP Address
// \param remote  Neighbor interface IP Address
// \param remote_main  Neighbor Main IP Address
// \param vtime  validity time of entry
// \param htime  hello interval of the neighbor node
//
// \return Pointer to the found/new added entry
...
// Lookup status of the link corresponding to the neighbor
// address
//
// \param node  Pointer to Node structure
// \param int_addr  interface address to look up
//
// \return link status
...
// Function returns a link where neighbor interface
...
//
// \param node  Pointer to Node structure
// \param message  Pointer to the hello message
// \param interfaceIndex  interface on which hello was received
//
// \return Link status in the hello packet
static
...
// Updates a link entry, is main entrypoint in link-sensing
...
// \param remote  Neighbor interface IP Address
// \param message  Pointer to the hello message
// \param interfaceIndex  interface on which message is received
//
// \return Pointer to the updated/new added entry
...
// Searches for a neighbor entry in link set
// If  found, replaces that entry
// with a new entry
//
// \param node  Pointer to Node structure
// \param old_neighbor  old neighbor entry
// \param new_neighbor  new neighbor entry
//
// \return Number of entries replaced
static
...
// Removes expired link entries in link set
...
// Deletes an entry from duplicate table
//
// \param dup_entry  Pointer to duplicate entry
//
...
// Inserts an entry in duplicate table
//
// \param node  Pointer to Node structure
// \param originator  address of node to be inserted
// \param packet_seq_number  sequence number to be inserted
//
// \return Pointer to entry inserted
...
// Search in duplicate table  if  originator and seq no are
// processed
//
// \param node  Pointer to Node structure
// \param originator  originator of the message
// \param packet_seq_number  sequence number of message
//
// \return Pointer to found entry else NULL
...
// Search in duplicate table  if originator and seq no exist
// in fwd list
//
// \param node  Pointer to Node structure
// \param originator  originator node
// \param packet_seq_number  sequence no
//
// \return 0 if already fwded, 1 if not
...
// Release all the entries from duplicate table
//
// \param node  Pointer to Node structure
//
...
// This function will be called when olsr->dup_hold_time
...
// Searches for an entry in duplicate table
// if not found, adds a new entry
// Then updates the status of the found/newly added entry
//
// \param node  Pointer to Node structure
// \param originator  originator of the msg
// \param packet_seq_number  sequence no of mesg
// \param interfaceAddress  Address of the interface
//
// \return 1 to indicate successful updation
...
// Searches for an entry in table and sets the retransmitted
// flag for the message entry
//
// \param node  Pointer to Node structure
// \param originator  originator of message
// \param packet_seq_number  sequence number of the message
//
// \return 0 if not found, 1 if found and flag set
// 
...
// Print contents of Duplicate table
//
// \param node  Pointer to Node structure
//
static
...
// Searches this neighbor address in the current neighbor entry
// neighbor entry
//
// \param neighbor  Pointer to neighbor entry
// \param neighbor_main_address  Address to be searched
//
// \return corresponding two hop neighbor list
// entry
...
// This procedure deletes the pointer to this addr from
// the list contained in the two hop neighbor entry
//
// \param two_hop_entry  Pointer to two hop neighbor list
// \param address  Address to be removed from 2 hop list
//
...
// Releases neighbor table for this node
//
// \param node  Pointer to Node structure
//
static
...
// This function will be called when NEIGHBOR_HOLD_TIMER
// expires. Removes two hop neighbors for the  entry for
// which timer expires
//
// \param node  Pointer to Node structure
// \param neighbor  Pointer to expired entry
//
...
// Delete a two hop neighbor entry results the deletion of its 1
...
// Inserts a two hop neighbor entry in the neighbor table
//
// \param two_hop_neighbor  Pointer to two hop neighbor entry
//
...
// Releases the two hop neighbor table for this node
//
// \param node  Pointer to Node structure
//
...
// Prints the two hop neighbor table for this node
//
// \param node  Pointer to Node structure
//
...
// Function to clear processed status of 2 hop neighbors
//
// \param node  Pointer to Node structure
//
...
// Finds the two hop neighbor with 1 link for this node
// with specified willingness
//
// \param node  Pointer to Node structure
// \param willingness  willingess value to be searched
//
// \return List of 2 hop neighbors satisfying
// criteria
...
// This function calculates the number of second hop neighbors for
// each neighbor
//
// \param node  Pointer to Node structure
// \param two_hop_count  Pointer to the counter to be filled
//
...
// This function links 2 hop neighbor to neighbor entry
//
// \param node  Pointer to Node structure
// \param neighbor  Pointer to neighbor entry
// \param two_hop_neighbor  Pointer to 2 hop neighbor entry
//
...
// Clear all  neighbors' MPR status
//
// \param node  Pointer to Node structure
//
...
// Inserts entry in MPR  selector table
//
// \param node  Pointer to Node structure
// \param mprs  Pointer to entry
//
...
// Updates MPR selector table entry vtime
// Adds a new entry if no entry exists
// and sets the vtime
//
// \param node  Pointer to Node structure
// \param addr  Address of node to be updated
// \param vtime  validity time of entry in seconds
//
...
// Releases MPR selector table for this node
//
// \param node  Pointer to Node structure
//
...
// Check whether there is an entry in the MPR selctor table
//
// \param node  Pointer to Node structure
//
// \return TRUE if non-empty, else FALSE
...
// Prints MPR selector table for this node
//
// \param node  Pointer to Node structure
//
...
// This function calculates maximum covered neighbor entry
// for a specified willingness
//
// \param node  Pointer to Node structure
// \param willingness  willingness of the node
//
// \return pointer to neighbor entry if found, else
// NULL
...
// This function  processes the chosen MPR and updates the
// counters used in calculations
//
// \param node  Pointer to Node structure
// \param one_hop_neighbor  Pointer to neighbor entry
// \param two_hop_covered_count  Pointer to counter
...
// Adds all willing nodes which are symmetric to MPR candidate set
//
// \param node  Pointer to Node structure
//
// \return number of nodes considered
...
// Calculate MPR neighbors for current node
//
// \param node  Pointer to Node structure
//
...
// Deletes Hello message working structure
//
// \param message  Pointer to Hello Message
//
...
// Prints Hello message working structure
//
// \param message  Pointer to Hello Message
//
...
// Prints Tc message working structure
//
// \param message  Pointer to TC Message
//
...
// Deletes TC message working structure
//
// \param message  Pointer to TC Message
//
...
// Send olsr packet to the UDP layer on all interfaces
//
// \param node  Pointer to Node structure
// \param msg  OLSR packet to be sent
// \param packetsize  Size of packet
// \param jitter  jitter to be added to packet
//
...
// Send olsr packet to the UDP layer to specified interface
//
// \param node  Pointer to Node structure
// \param msg  OLSR packet to be sent
// \param packetsize  Size of packet
// \param jitter  jitter to be added to packet
// \param interfaceIndex  interface to send on
//
...
// Build MID msg and Send it
//
// \param node  Pointer to Node structure
// \param message  Pointer to a MID msg
...
//
// \param node  Pointer to Node structure
// \param m_forward  Pointer to a generic olsr msg
// \param interfaceIndex  Outgoing Interface Index
//
static
void OlsrMidForward(
...
// Parse an olsr message and collects all information in MID
// msg
//
// \param node  Pointer to Node
// \param mid_msg  Pointer to MID msg
// \param m  Pointer to a generic olsr msg
//
static
...
// Destroy a MID msg
//
// \param message  Pointer to MID msg
//
static
...
//
// \param node  Pointer to Node structure
// \param message  Pointer to a HNA msg
//
...
// Builds and sends a HNA message
//
...
// Deletes address from destination list of current
// topology entries
//
// \param last_entry  Pointer to Topology entry list
// \param dst  Address to be deleted
//
...
// Deletes destination list from topology table
//
// \param dest_entry  Pointer to destination list
//
...
// Deletes address from destination list
//
// \param dest_entry  Pointer to Topology
//    destination list
// \param dst  Address to be deleted
//
...
// Deletes last entry from topology table
//
// \param last_entry  Pointer to last entry
//
...
// Inserts the originator message from TC message in last entry
//
// \param node  Pointer to Node structure
// \param last_entry  Pointer to topology last entry
// \param message  Pointer to tc message
//
...
// Inserts the MPR address from the TC message in dest entry
//
// \param node  Pointer to Node structure
// \param dest_entry  Pointer to Topology
//    destination  entry
// \param mpr  pointer to mpr address to be added
//
...
// Searches the address in the last entry
//
// \param node  Pointer to Node structure
// \param last  address to be searched
//
...
// Searches the address in the dest entry
//
// \param node  Pointer to Node structure
// \param dest  Address to be searched
//
...
// Searches the address in the dest entry of this last entry
//
// \param last_entry  Pointer to Topology
//    last  entry
// \param address  Address to be searched
//
// \return Pointer to destination list if found,
// else NULL
...
// This function updates time out for the
// existing last entry and
// creates new entries for the new destinations
//
// \param node  Pointer to Node structure
// \param last_entry  entry to be updated
// \param message  Pointer to tc msg
// \param addr_ip  address to be updated
//
 ...
 ***************************************************************************/
// Searches in the routing table for this address
//
// \param node  Pointer to Node structure
// \param dst  Address to be looked up
//
// \return Pointer to entry if found, else NULL
...
// Deletes the entry from routing table
//
// \param destination  Pointer to entry
//
...
// Inserts the address in the routing table
//
// \param node  Pointer to Node structure
// \param dst  address to be inserted
//
...
// Inserts the address in the routing table from topology
//
// \param node  Pointer to Node structure
// \param r_last  Pointer to routing entry
// \param dst  destination address
//
// \return pointer to the entry if added, else NULL
...
// Check if  a two hop neighbor is reachable through a i
// 1 hop neighbor with willingness != WILL NEVER
//
// \param neigh_2_list  Pointer to two hop neighbor
//    entry
//
...
// Adds two hop neighbors and returns the entries in the list
//
// \param node  Pointer to Node structure
//
...
// Fills routing table with 1 hop neighbors
//
// \param node  Pointer to Node structure
//
// \return 1
...
// Fills routing table from HNA table
//
// \param node  Pointer to Node structure
//
...
// Make mirror of current routing table
//
// \param node  Pointer to Node structure
//
...
//
// \param table  Pointer to Table to be printed
//
...
// This function processes for changes
//
// \param node  Pointer to Node structure
//
...
// This function removes all tuples for all tables when timer
// is expired
//
// \param node  Pointer to Node structure
//
...
// This function releases all entries for all tables
//
// \param node  Pointer to Node structure
//
...
// This function initialzes all the tables
//
// \param olsr  Pointer to olsr data structure
//
...
// This function process neighbors in hello msg
//
// \param node  Pointer to Node structure
// \param neighbor  Pointer to neighbor entry
// \param message  hello message pointer
//
...
// Check weather the olsr packet size be greater than
// the allowed size.
//
// \param node  Pointer to Node structure
// \param msg  Pointer to Olsr Packet
// \param h_ipv6_addr  Pointer to ipv6 address
//
static
...
// \param node  Pointer to Node structure
// \param msg  Pointer to Olsr Packet
// \param h_ipv4_addr  Pointer to ipv4 address
//
static
void OlsrCheckPacketSizeGreaterThanFragUnit(Node* node,
                                            olsrpacket* msg,
...
// Generate HELLO packet with the contents of the parameter
// "message". Can generate an empty HELLO packet if the
// neighbor table is empty.
//
// \param node  Pointer to Node structure
// \param message  hello message to be inserted in packet
// \param interfaceIndex  Interface on which packet is to be transmitted
//
...
// Generate TC packet with the contents of the parameter
// "message".
//
// \param node  Pointer to Node structure
// \param message  pointer to message which is to be inserted
//    in packet
// \param interfaceIndex  interface to send the packet on
//
...
// Generate and send TC packet with the contents of the
// original message "m_forward" from a received OLSR packet.
//
// \param node  Pointer to Node structure
// \param m_forward  TC message to be fwded
//
...
// Generate and send HNA packet with the contents of the
// original message "m_forward" from a received OLSR packet.
//
// \param node  Pointer to Node structure
// \param m_forward  HNA message to be fwded
//
...
// Forwarding the unknown message type with the contents of the
// original message "m_forward" from a received OLSR packet.
//
// \param node  Pointer to Node structure
// \param m_forward  unknown message to be fwded
//
static
...
// Build HELLO Message according to hello_message struct from
// olsrmsg struct.
//
// \param node*  Pointer to Node
// \param hmsg  Pointer to hl_message structure
// \param m  Pointer to the olsrmsg
//
...
// Build TC Message according to tc_message struct from
// olsrmsg struct.
//
// \param node  Pointer to Node structure
// \param tmsg  Pointer to the TC message
// \param m  OLSR message pointer
// \param from_addr  Neighbor interface address that transmitted TC
//
...
//
// \param node  Pointer to Node structure
// \param message  Pointer to TC message data structure
//
// \return 0 if message to be sent, -1 if not
...
// Function looks up hello message if the current node
// status is MPR or not
//
// \param node  Pointer to Node structure
// \param message  Pointer to hello message
// \param incomingInterface  interface on which hello message arrived
//
// \return 1  if node is MPR, 0 if not
...
// \param node  Pointer to Node structure
// \param message  Pointer to hello message
// \param incomingInterface  interface at which packet arrived
// \param from_addr  neighbor interface address that sent packet
//
// \return 1 if error
...
// Process received hna msg
//
// \param node  Pointer to Node structure
// \param message  Pointer to HNA msg
// \param source_addr  source address of the msg
// \param incomingInterface  Incoming Interface Index
// \param m  Pointer to a generic olsr msg
//
// \return Returns zero only if the hna msg is processed
static
...
// Parse an olsr message and collects all information
// in HNA msg
//
// \param node  Pointer to Node
// \param hna_msg  Pointer to HNA msg
// \param m  Pointer to a generic olsr msg
//
static
...
// \param m  Pointer to a generic olsr msg
//
// \return Returns zero only if the mid msg is processed
static
...
// \param node  Pointer to Node structure
// \param message  Pointer to TC message
// \param source_addr  Address of sender
...
//
// \param node  Pointer to Node structure
// \param message  Pointer to hello msg structure
// \param interfaceIndex  outgoing interface for hello pkt
//
// \return 0
...
//
// \param node  Pointer to Node structure
// \param interfaceIndex  interface for which msg is to be
//    generated
//
...
// Handles OLSR control message received from transport layer
//
// \param node  Pointer to Node structure
        ...
        case HELLO_PACKET:
            {
                ...
                // hello message received, convert it to hl_meqsage structure
                OlsrHelloChgeStruct(node, &hellopacket, m);
...
// Olsr initialization  for tracing
//
// \param node  Pointer to node
...
// Initialization function for OLSR.
//
// \param node  Pointer to Node structure
// \param nodeInput  Pointer to input configuration
...
//
// \param node  Pointer to Node structure
// \param interfaceId  interface index
//
...
// The main layer structure routine, called from application.cpp
//
// \param node  Pointer to Node structure
