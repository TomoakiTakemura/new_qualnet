...

{
    ...
    float sleep_load = thisPhy->powerConsmpTable->sleep_current_load;

    // Get number of configured antenna elements
    int numConfigAntennas = PHY_GetNumConfigAntennas(node, phyIndex);

    // Get number of active antenna elements
    int numActiveAntennas = PHY_GetNumActiveAntennas(node, phyIndex);

    // (numConfigAntennas - numActiveAntennas) reperesent the number of
    // antennas which are currently inactive. Inactive antennas are assumed
    // to be in sleep mode and consume power as per sleep load configured.

    if (!node->battery->dead)
    {
        // Decrement the battery charge for currently active antennas based
        // on current mode.
        BatteryDecCharge(node, duration, load * numActiveAntennas);

        // Decrement the battery charge for inactive antennas
        BatteryDecCharge(node, duration, sleep_load
            * (numConfigAntennas - numActiveAntennas));
    }
    else
    {
        now = node->battery->deadTime ;
        actDuration
            = (double)(now - thisPhy->curLoad->startTime) / (double)SECOND;
        if (actDuration < 0 )
        {
            return;
        }
    ...
    switch (prevStatus)
    {
        ...
        case PHY_IDLE:
        {
            thisPhy->curLoad->powStats.totalIdlePower
                += (load * actDuration * numActiveAntennas)
                    + (sleep_load * actDuration 
                    * (numConfigAntennas - numActiveAntennas));
        ...
        case PHY_RECEIVING:
        {
            ...
            if (ENERGY_DEBUG )
            {
                printf("Node %d:Total Receive charge consumed:%e\n",
                        node->nodeId,
                        (thisPhy->curLoad->powStats.totalRxPower/3600.0));
            }
            break;
        }
        ...
        case PHY_TRX_OFF:
        {
            thisPhy->curLoad->powStats.totalSleepPower
                += (load * actDuration * numActiveAntennas )
                    + (sleep_load * actDuration
                    * (numConfigAntennas - numActiveAntennas));
            thisPhy->curLoad->powStats.totalSleepDuration += 
            (clocktype )( now - thisPhy->curLoad->startTime );
            if (ENERGY_DEBUG )
            {
                printf("Node %d: Total Sleep duration:%e\n",
                    node->nodeId,
                (double)
                (thisPhy->curLoad->powStats.totalSleepDuration/SECOND));
            }
            break;
        }
    } //switch(prevStatus)
            ...
            const NodeInput *nodeInput)
{
    ...
    if (!strcmp(str, "MICA-MOTES")){
        ...
        txPower_mW = txPower_mW/numConfigAntennas;
