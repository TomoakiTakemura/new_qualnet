...
// \param mntMsg  Pointer to Message
//
...
// To delete a particular destination from the precursors table.
//
...
// for deleting an address from the precursor's list of
// all the routes
//
...
// Remove an entry from the seen table, deletion will be always
...
// Check whether interface is valid for AODV for IPv4 or IPv6.
//
//    +node:  Node* : Pointer to node.
...
// Function to advertise hello message if a node wants to.
//
//    +node:  Node* : Pointer to node.
//    +aodv:  AodvData* : Pointer to AODV Data.
//
         ...
         AodvRouteEntry* destRouteEntry)
{
    ...
    if (aodv->processAck)
    {
        // start RREP-ACK timer
        AodvRouteEntry* rtToNextHop = AodvCheckRouteExist(
                                         rtToSource->nextHop,
                                         &aodv->routeTable,
                                         &isRtToSrcExist);
        ...
        if (rtToNextHop != NULL)
        {
            AodvSetTimer(node,
                         ...
                         rtToNextHop->destination,
                         (clocktype) 2 * AODV_NODE_TRAVERSAL_TIME);

            rtToNextHop->numRrepAcksPending++;
        }
    }
...
// Create IPv6 Broadcast Address (ff02::1).
//
//    +node:  Node* : Pointer to Node.
...
// Sending route error message to other nodes if route is not
...
// Processing procedure when the node fails to deliver a data
...
// LAYER        : NETWORK
// PURPOSE      : Returns true if D flag is to be set.
// PARAMETERS   :
// +node        :Node*              : Pointer to node
// +nodeInput   :const NodeInput*   : Pointer to cached file
// +interfaceAddress:Address        : Address to be compared
// RETURN:
// +TRUE       :BOOL               : If D flag to be set
// +FALSE      :BOOL               : If D flag not to be set.
BOOL
AodvCheckDFlag(Node* node,
...
// To initialize the user configurable parameters or initialize
    ...
    NetworkRoutingProtocolType aodvProtocolType)
{
    ...
    aodv->Dflag = AodvCheckDFlag(node, nodeInput, interfaceAddress);
    ...
    if (aodvProtocolType == ROUTING_PROTOCOL_AODV)
    {
        ...
        IO_ReadTime(
            ...
            &interfaceAddress,
            nodeInput,
            "AODV-ACTIVE-ROUTE-TIMEOUT-IPv4",
            &wasFound,
        ...
        IO_ReadInt(
            ...
            &interfaceAddress,
            nodeInput,
            "AODV-ROUTE-DELETION-CONSTANT-IPv4",
            &wasFound,
        ...
        IO_ReadString(
            ...
            &interfaceAddress,
            nodeInput,
            "AODV-OPEN-BI-DIRECTIONAL-CONNECTION-IPv4",
            &wasFound,
            buf);
        ...
        else
{
    ...
    else if (aodvProtocolType == ROUTING_PROTOCOL_AODV6)
    {
        IO_ReadInt(
            nodeId,
            &interfaceAddress,
            nodeInput,
            "AODV-NET-DIAMETER-IPv6",
            &wasFound,
            &aodv->netDiameter);
    ...
    else
    {
        ...
        IO_ReadTime(
            ...
            &interfaceAddress,
            nodeInput,
            "AODV-ACTIVE-ROUTE-TIMEOUT-IPv6",
            &wasFound,
            &aodv->activeRouteTimeout);
    ...
    else
    {
        ...
        IO_ReadInt(
            ...
            &interfaceAddress,
            nodeInput,
            "AODV-ROUTE-DELETION-CONSTANT-IPv6",
            &wasFound,
            &aodv->rtDeletionConstant);
    ...
    IO_ReadString(
        ...
        &interfaceAddress,
        nodeInput,
            "AODV-OPEN-BI-DIRECTIONAL-CONNECTION-IPv6",
            &wasFound,
            buf);
    ...
    int interfaceIndex)
{
    ...
    if (MESSAGE_ReturnPacketSize(msg) == AODV_RREP_ACK_PACKET_SIZE)
    {
        UInt8* packet = (UInt8*)MESSAGE_ReturnPacket(msg);
        packetType = *packet;
    }
    else
    {
        UInt32* packet = (UInt32*)MESSAGE_ReturnPacket(msg);
        packetType = *packet >> 24;
    }
    ...
    switch (packetType)
    {
        ...
        case AODV_RREP_ACK:
        {
            ...
            rtToSrc = AodvCheckRouteExist(srcAddr,
                                          &aodv->routeTable,
                                          &isRtToSrcExist);
            ...
            else if (rtToSrc->numRrepAcksPending > 0)
            {
                rtToSrc->numRrepAcksPending--;
            }
    ...
    Message* msg)
{
    ...
    switch (MESSAGE_GetEvent(msg))
    {
        ...
        {
            Address* destAddr = (Address*) MESSAGE_ReturnInfo(msg);
            BOOL isRtToDstExist = FALSE;

            AodvRouteEntry* rtToDst = AodvCheckRouteExist(
                                             *destAddr,
                                             &aodv->routeTable,
                                             &isRtToDstExist);
            ...
            else if (rtToDst->numRrepAcksPending > 0)
            {
                AodvInsertBlacklistTable(
                    node,
                    aodv,
