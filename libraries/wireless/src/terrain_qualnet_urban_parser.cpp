...
//#ifdef ESRI_SHP_INTERFACE
            ...
            // This function creates the shape file(s) when called with
            // masterProcess true. It must still be called when not the
            // master process to setup the fileName array.
...
//#endif // ESRI_SHP_INTERFACE
    ...
    {
        if (parse->terrainFeatures->m_setFeaturesToGround)
        {
            SetSopsProperty("/partition/0/featuresToGround", "1");
        }
        else
        {
            SetSopsProperty("/partition/0/featuresToGround", "0");
        }
    }
#endif
    ...
    for (i = 0; i < numFiles; i++) {
...
#ifdef USE_MPI
        // When using MPI, the master process creates terain files
        // and the other processes read them. The other processes are
        // likely to arrive here before the master finishes creating
        // the file(s). The non-master processes are free to loop
        // forever because if there was a file error, the master
        // will exit and mpirun will kill all the parallel processes.
        do
        {
#endif
        ...
        for (;;) {
            ...
            if (XML_Parse(parser, buff, len, done) == XML_STATUS_ERROR) {
                sprintf(errorBuf, "Parse error at line %u while parsing "
    ...
    for (i = 0;i < numFiles; i++) {
        ...
        for (;;) {
            ...
            if (XML_Parse(parser, buff, len, done) == XML_STATUS_ERROR) {
                sprintf(errorBuf, "Parse error at line %u in %s:\n%s\n",
                        unsigned(XML_GetCurrentLineNumber(parser)),
                        fileName[i],
                        XML_ErrorString(XML_GetErrorCode(parser)));
                ERROR_ReportError(errorBuf);
