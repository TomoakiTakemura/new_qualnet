...
//--------------------DOT11e-End-Updates---------------------------------//
    ...
    if (DEBUG_UAPSD)
    {
        ...
        {
            printf("NodeId %d Time %" TYPES_64BITFMT "d AP NLPS\n",
...
/*** UAPSD *********************************START*******************/
        ...
        || getAc(dot11, 3).isAcHasPacket()))
    {
        ...
        if (DEBUG_UAPSD)
        {
            printf("NodeId %d Time %" TYPES_64BITFMT "d NLPS Waking up\n",
                node->nodeId, node->getNodeTime());
        }
        MacDot11StationStartListening(node, dot11);
        // cancel the previous awake timer
...
/*** UAPSD *********************************START*******************/
    if (DEBUG_UAPSD)
    {
        printf("NodeId %d Time %" TYPES_64BITFMT "d Beacon received. B=%d U=%d T=%d\n",
            node->nodeId, node->getNodeTime(),
    ...
    if (MacDot11IsBssStation(dot11)&& MacDot11IsStationJoined(dot11)
        ...
        && !dot11->ScanStarted)
    {
        if (dot11->uapsdInfo.spInProgress
            && !dot11->apHasUnicastDataToSend)
        {
            // AP has no more unicast data for this station.
            // If there's an ongoing SP, end this SP.
...
//---------------------------Power-Save-Mode-Updates---------------------//
                        ...
                        MacDot11StationCancelTimer(node, dot11); 
                        RfChainMode mode = k_All_Rf_Chain;
                        mc(dot11)->dc()->switchRfChains(dot11,
                                                        node,
                                                        mode);
                        MacDot11StationTransmitCTSFrame(node,
                                                        dot11,
                                                        msg);
...
#ifdef PRINT_DEBUG
            printf("%" TYPES_64BITFMT "d Node %d Received CTS\n",
                    node->getNodeTime(),
                    node->nodeId);
#endif
            if (dot11->useDvcs) {
...
//---------------------------Power-Save-Mode-End-Updates-----------------//
        ...
        case DOT11_ACK: {
            ...
            if (dot11->state != DOT11_S_WFACK &&
                ...
                dot11->state != DOT11_S_WFTRIGGER_ACK)
            {
                if (DEBUG)
                {
                    printf("MacDot11ProcessMyFrame: "
...
/*** UAPSD *********************************START*******************/
        ...
        {
            // For future use
            ERROR_Assert(false, "Invalid packet received");
            MESSAGE_Free(node, msg);
            break;
        }
/*** UAPSD *********************************END*******************/
...
//-------------------------Interop-------------------------------------//
           unsigned char neighbourMode = k_Dot11b;
           PHY_GetRxDataRateType(node,
               dot11->myMacData->phyNumber, neighbourMode);
           MacDot11InitNeighborInfo(
                    node,
                    dot11,
                    hdr2->sourceAddr,
                    (STA_Capability_Mode)neighbourMode);
//-----------------------  Interop------------------------------------//
       }
    ...
    // Start listening the transmission channel
        case MSG_MAC_DOT11_PSStartListenTxChannel: {
            ...
            if (timerSequenceNumber == dot11->awakeTimerSequenceNo){
                ...
                if (DEBUG_UAPSD)
                {
                    printf("NodeId %d Time %" TYPES_64BITFMT "d Waking up\n",
                        node->nodeId, node->getNodeTime());
                }
...
//---------------------------Power-Save-Mode-End-Updates-----------------//
        ...
        {
            Mac802_11p_SyncInterval(node, dot11, msg);
            break;
        }
...
//---------------------------Power-Save-Mode-Updates---------------------//
        ...
        if (dot11->isPSModeEnabledInAdHocMode == TRUE){
            ...
            queue = new Queue(node, "FIFO", dot11->broadcastQueueSize); // "FIFO"
        ...
        if (wasFound){
            if (strcmp(retString, "YES") == 0){
                ...
                ERROR_ReportError("MacDot11BssDynamicInit: "
                    "Invalid value for MAC-DOT11-STA-PS-MODE-ENABLE "
                    "in configuration file.\n"
                    "Expecting YES or NO.\n");
            }
        }
...
/*** UAPSD *********************************START*******************/
        if (dot11->isUapsdEnable)
        {
            ...
            dot11->myMacData->interfaceIndex,
            nodeInput,
            "MAC-DOT11-MAX-SP-LENGTH",
            &wasFound,
            &anIntInput);

            if (wasFound)
            {
                dot11->uapsdInfo.maxServicePeriodLength = anIntInput;
                ERROR_Assert(anIntInput >= 0 && anIntInput <= 3,
                    "MacDot11Init: "
                    "Value of parameter MAC-DOT11-MAX-SP-LENGTH"
                    "should be between 0 to 3\n");
            }
            else
            {
                dot11->uapsdInfo.maxServicePeriodLength = 1;
            }
        }

        if (dot11->isPSModeEnabled || dot11->isUapsdEnable)
        {
            IO_ReadInt(
                    node,
                    node->nodeId,
    ...
    unsigned int prefixLength)
{
    ...
    dot11->myPhyType = (STA_Capability_Mode)phyModel;
    ...
    {
        Mac802_11p_Init(node, interfaceIndex, nodeInput, phyModel, dot11);
    }
...
//--------------------HCCA-Updates Start---------------------------------//
        ...
        if (dot11->isVHTEnable)
        {
            ...
            dot11->macController =
                new MacController(node, interfaceIndex, k_Mode_Dot11ac,dot11);
        }
        else if (dot11->isHTEnable)
        {
            dot11->macController = 
                new MacController(node, interfaceIndex, k_Mode_Dot11n, dot11);
        }
        else if (dot11->isQosEnabled)
        {
            dot11->macController = 
                new MacController(node, interfaceIndex, k_Mode_Dot11e, dot11);
        }
...
//--------------------HCCA-Updates End---------------------------------//
    ...
    else
    {
        ...
        ERROR_Assert(phyModel != PHY802_11pCCH && phyModel != PHY802_11pSCH,
                "MacDot11Init: QoS must be enabled with 802.11.p.\n");
        ...
        ERROR_Assert(!PHY_IsPhyVHTEnabled(node,
                                         ...
                                         "at MAC layer");
        dot11->macController = 
            new MacController(node, interfaceIndex, k_Mode_Legacy, dot11);
        mc(dot11)->ac()->InitAc(node, dot11, phyModel);
    }
    ...
    if (dot11->isHTEnable)
    {
        ...
        if (wasFound)
        {
            if (anIntInput == 0)
            {
                dot11->smpsMode = k_Static;            // static mode
            }
            else if (anIntInput == 1)
            {
                dot11->smpsMode = k_Dynamic;            // dynamic mode
            }
            else if (anIntInput == 2)
            {
                char buf[1024];

                sprintf(buf, "anIntInput = 2 is an invalid value for "
                       "node = %d and interface index = %d",
                        node->nodeId, dot11->myMacData->interfaceIndex);
                ERROR_ReportError(buf);
            }
            else if (anIntInput == 3)
            {
                dot11->smpsMode = k_Disabled;            // SMPS disabled
            }
            else
            {
                ERROR_Assert(anIntInput >= 3, "Value of MAC-DOT11-SMPS-MODE"
...
/*** UAPSD *********************************START*******************/
    if (dot11->isHTEnable)
    {
        BOOL aBoolInput = FALSE;
        IO_ReadBool(
            node,
            node->nodeId,
            dot11->myMacData->interfaceIndex,
            nodeInput,
            "MAC-DOT11-UAPSD-ENABLE",
            &wasFound,
            &aBoolInput);

        if (wasFound)
        {
            dot11->isUapsdEnable = aBoolInput;
            ERROR_Assert(dot11->isUapsdEnable == 0 || dot11->isUapsdEnable == 1,
                "MacDot11Init: "
                "Value of parameter MAC-DOT11-UAPSD-ENABLE should be a boolean\n");
            if (dot11->isUapsdEnable)
            {
                if (dot11->enableImmediateBAAgreement)
                {
                    char buf[MAX_STRING_LENGTH];
                    sprintf(buf, "Data Burst feature is currently"
                        " incompatible with U-APSD. Disabling Data Burst"
                        " feature at Node %d Interfaceindex %d.",
                        node->nodeId, dot11->myMacData->interfaceIndex);
                    ERROR_ReportWarning(buf);
                ...
                if (dot11->smpsMode == k_Static
                    || dot11->smpsMode == k_Dynamic)
                {
                    char buf[MAX_STRING_LENGTH];
                    sprintf(buf, "Spatial Multiplexing Power Save (SMPS)"
                        " mode is currently incompatible with U-APSD."
                        " Disabling SMPS mode at Node %d Interfaceindex %d.",
                        node->nodeId, dot11->myMacData->interfaceIndex);
                    ERROR_ReportWarning(buf);
                    dot11->smpsMode = k_Disabled;
                }
                ...
                {
                    char buf[MAX_STRING_LENGTH];
                    sprintf(buf, "VHT TxOP Power Save mode is currently"
                        " incompatible with U-APSD. Disabling VHT TxOP"
                        " power save mode at Node %d Interfaceindex %d.",
                        node->nodeId, dot11->myMacData->interfaceIndex);
                    ERROR_ReportWarning(buf);
                ...
                {
                    char buf[MAX_STRING_LENGTH];
                    sprintf(buf, "AMPDU feature is currently"
                        " incompatible with U-APSD. Disabling AMPDU"
                        " feature at Node %d Interfaceindex %d.",
                        node->nodeId, dot11->myMacData->interfaceIndex);
                    ERROR_ReportWarning(buf);
        ...
        {
            dot11->isUapsdEnable = FALSE;
        }
    }
    memset(&dot11->uapsdInfo, 0, sizeof(dot11->uapsdInfo));
/*** UAPSD *********************************END*******************/
    IO_ReadString(
        node,
        node->nodeId,
    ...
    {
        ERROR_ReportWarningArgs("At Node %d interface %d Directional "
            "Antenna is configured which is currently not supported. "
            "Executing scenario with MAC-DOT11-DIRECTIONAL-ANTENNA-MODE "
            "set to NO",
            node->nodeId,
            interfaceIndex);
        dot11->useDvcs = false;
...
//--------------------HCCA-Updates End-----------------------------------//
    ...
    if (phyModel == PHY802_11pCCH || phyModel == PHY802_11pSCH)
    {
        ERROR_Assert(dot11->associationType == DOT11_ASSOCIATION_NONE,
                        "Association type should be none");
        ERROR_Assert(!dot11->isHCCAEnable,
                        "HCCA can not be configured with 802.11p");
        ERROR_Assert(!MacDot11IsAp(dot11),
                        "AP can not be used with 802.11p");
    }
...
//---------------------------Power-Save-Mode-End-Updates-----------------//
/*** UAPSD *********************************START*******************/
        mc(dot11)->uapsdPrintStats(node, dot11, interfaceIndex);
/*** UAPSD *********************************END*******************/
        if (MacDot11IsQoSEnabled(node, dot11))
        {
//--------------------HCCA-Updates Start---------------------------------//
...
//--------------------HCCA-Updates End-----------------------------------//
            ...
            if (node->phyData[dot11->myMacData->phyNumber]->phyModel ==
                                                            PHY802_11pCCH)
            {
                IO_PrintStat(node, "MAC", DOT11p_CCH_MAC_STATS_LABEL, ANY_DEST,
                             interfaceIndex, buf);
            }
            else if (node->phyData[dot11->myMacData->phyNumber]->phyModel ==
                                                              PHY802_11pSCH)
            {
                IO_PrintStat(node, "MAC", DOT11p_SCH_MAC_STATS_LABEL, ANY_DEST,
                     interfaceIndex, buf);
            }
            else
            {
...
DOT11_NeighborHtInfo* MacDot11GetNeighHtInfo(
...
/*** UAPSD *********************************START*******************/
void MacDot11UpdNeighborUApsdInfo(
        Node* node, 
        MacDataDot11* dot11,
        const Mac802Address& neighborAddr, 
        const DOT11e_EDCAParameterSet& edcaParameterSet)
{
    ...
    DOT11_ApStationListItem* stationItem = NULL;
                stationItem =
                    MacDot11ApStationListGetItemWithGivenAddress(
                                                ...
                                                dot11,
                                                neighborAddr);
    ERROR_Assert(stationItem,"stationItem should not be NULL");
    Dot11_UApsdInfo* neighUApsdInfo = &stationItem->data->uapsdInfo;
    neighUApsdInfo->isAp = FALSE;
    neighUApsdInfo->address = neighborAddr;

    if (edcaParameterSet.qosInfo.EDCFParameterSetUpdateCount == 15)
    {
        neighUApsdInfo->uApsdEnabled = TRUE;
    }

    if (edcaParameterSet.qosInfo.QueueRequest == 0 &&
        edcaParameterSet.qosInfo.TXOPRequest == 0)
    {
        neighUApsdInfo->useTxop = TRUE;
    }
    else if (edcaParameterSet.qosInfo.QueueRequest == 1 &&
                edcaParameterSet.qosInfo.TXOPRequest == 0)
    {
        neighUApsdInfo->noOfPacketsToBeSentInServicePeriod = 2;
    }
    else if (edcaParameterSet.qosInfo.QueueRequest == 0 &&
                edcaParameterSet.qosInfo.TXOPRequest == 1)
    {
        neighUApsdInfo->noOfPacketsToBeSentInServicePeriod = 4;
    }
    else
    {
        neighUApsdInfo->noOfPacketsToBeSentInServicePeriod = 6;
    }
}
/*** UAPSD *********************************END*******************/
        ...
        const DOT11_HT_CapabilityElement& htCapabilityElement)
{
    DOT11_NeighborHtInfo* neighHtInfo = 
        MacDot11GetNeighHtInfo(dot11->htInfoList, neighborAddr);
    if (!neighHtInfo) {
...
void MacDot11InitNeighborHtInfo(
        Node* node, 
        MacDataDot11* dot11,
        const Mac802Address& neighborAddr,
        STA_Capability_Mode mode)
{
    assert(dot11->isHTEnable);
    DOT11_NeighborHtInfo* neighHtInfo = 
        MacDot11GetNeighHtInfo(dot11->htInfoList, neighborAddr);
    if (!neighHtInfo)
    {
        ...
        // Set the maximum mcs assuming a single spatial stream
        // This information would get updated after receving
        // management packets.

        if (!dot11->isVHTEnable)
        {
            // 11n device communicating to a 11ac device.
            if (mode == k_Dot11ac)
            {
                mode = k_Dot11n;
            }
        ...
        if (mode == k_Dot11ac)
        {
            maxMcs = DOT11ac_MAX_MCS_SINGLE_SS;
        }
        else if (mode == k_Dot11n)
        {
            maxMcs = DOT11n_MAX_MCS_SINGLE_SS;
        }
        else if (mode == k_Dot11a)
        {
            maxMcs = DOT11a_MAX_MCS_SINGLE_SS;
        }
        else
        {
            assert(mode == k_Dot11b);
            maxMcs = DOT11b_MAX_MCS_SINGLE_SS;
        }
        neighHtInfo->neighbourMode = mode;
        neighHtInfo->maxMcsIdx = maxMcs;
    }
}

int MacDot11GetMinMcs(
    MacDataDot11* dot11, STA_Capability_Mode nMode, int maxMcs)
{
    int minMcs = 0;
    if (dot11->isVHTEnable)
    {
        if (nMode == k_Dot11n)
        {
            minMcs = maxMcs - DOT11n_MAX_MCS_SINGLE_SS;
        }
        else if (nMode == k_Dot11ac)
        {
            minMcs = maxMcs - DOT11ac_MAX_MCS_SINGLE_SS;
        }
        else if (nMode == k_Dot11a)
        {
            assert(maxMcs == DOT11a_MAX_MCS_SINGLE_SS);
        }
        else
        {
            assert(maxMcs == DOT11b_MAX_MCS_SINGLE_SS);
        }
    }
    else
    {
...
/************** Interoperability feature *********START*******************/
    DOT11_NeighborHtInfo* htInfo
        = MacDot11GetNeighHtInfo(dot11->htInfoList, rateInfo->ipAddress);
    if (htInfo)
    {
        STA_Capability_Mode nMode = htInfo->neighbourMode;
        int maxMcs = htInfo->maxMcsIdx;
        int minMcs = MacDot11GetMinMcs(dot11, nMode, maxMcs);
...
static
void MacDot11SetChnlBwdthForBroadcastFrame(
    Node* node, 
    MacDataDot11* dot11,
    ...
    DOT11_NeighborHtInfo* htInfo,
    BOOL tempCalc = FALSE,
    STA_Capability_Mode nm = k_Dot11b)
{
/************** Interoperability feature *********START*******************/
    switch (nm)
    {
                            ...
                            == DOT11_QOS_DATA)
                {
                    ...
                    if (htInfo)
                    {
                        ...
                        chBwdth = std::min(Phy802_11GetOperationChBwdth(
                                node->phyData[dot11->
                                myMacData->phyNumber]), dstChBwdth);
                        assert(chBwdth == CHBWDTH_80MHZ 
                            || chBwdth == CHBWDTH_160MHZ);
                        chBwdth = Phy802_11CCA
    ...
    case k_Dot11n:
        if (dot11->isVHTEnable)
        {
            if (frameType == DOT11_RTS)
            {
                if (htInfo && !htInfo->isFortyMhzIntolerant)
                {
        ...
        else
        {
            if (frameType == DOT11_RTS)
            {
                if ((ChBandwidth)Phy802_11nGetOperationChBwdth(
                    ...
                    && htInfo && !htInfo->isFortyMhzIntolerant)
                {
                    txVector->chBwdth = CHBWDTH_40MHZ;
                }
            }
...
void MacDot11nSetMCSIndexForBroadcastFrame(
                          ...
                          STA_Capability_Mode nieghbourMode = k_Dot11n)
{
...
static
void MacDot11SetChnlBwdthForDataMgmtFrame(
    Node* node, 
    MacDataDot11* dot11,
    MAC_PHY_TxRxVector* txVector,
    const DOT11_NeighborHtInfo* destHtInfo,
...
static
void MacDot11SetMCSIndexForDataMgmtFrame(
    MAC_PHY_TxRxVector* txVector,
    const DOT11_NeighborHtInfo* destHtInfo)
{
    if (destHtInfo && destHtInfo->curMcsIdx >= 0 ) {
...
static
void MacDot11SetChnlBwdthForCtrlRespFrame(
    Node* node,
    MacDataDot11* dot11,
    DOT11_MacFrameType frameType,
    MAC_PHY_TxRxVector* txVector,
...
static
void MacDot11SetMCSIndexForCtrlRespFrame(
    MAC_PHY_TxRxVector* txVector,
    const MAC_PHY_TxRxVector* rxFrameVector)
{
...
/************** Interoperability feature *********START*******************/
    ...
    txVector->gi = GI_LONG;
    BOOL isShortGICapable
        = Phy802_11ShortGiCapable(
            node->phyData[dot11->myMacData->phyNumber]);
    ...
    case k_Dot11ac:
        assert(dot11->isVHTEnable);
        if (isShortGICapable
            && destHtInfo && destHtInfo->shortGiCapable
            && txVector->mcs > DOT11ac_MAX_MCS_SINGLE_SS)
        {
            useShortGI = TRUE;
        }
        break;
    case k_Dot11n:
        if (isShortGICapable
            && destHtInfo && destHtInfo->shortGiCapable
            && txVector->mcs > DOT11n_MAX_MCS_SINGLE_SS)
        {
            useShortGI = TRUE;
        }
        break;
...
static
void MacDot11SetPHYType(
    MAC_PHY_TxRxVector* txVector,
    const DOT11_NeighborHtInfo* destHtInfo)
{
/************** Interoperability feature *********START*******************/
    txVector->phyType = k_Dot11b;
    if (destHtInfo)
    {
        txVector->phyType = destHtInfo->neighbourMode;
    }
/************** Interoperability feature *********END*******************/
}

static void MacDot11SetOperModeDataFrames(
    MacDataDot11* dot11,
    MAC_PHY_TxRxVector* txVector,
    BOOL isBroadcast,
    STA_Capability_Mode nm = k_Dot11b)
{
    if (isBroadcast)
    {
        // Use NON-HT mode for broadcast packets.
        txVector->format = MODE_NON_HT;
    }
    else
    {
        if (nm == k_Dot11ac)
        ...
        else if (nm == k_Dot11n)
        {
            if (dot11->isVHTEnable)
            {
                //11ac-11n communication
                txVector->format = MODE_HT_MF;
            }
            else
            {
                //11n-11n or 11n-11ac communication
                //Mode will be either HT_GF or HT_MF
                // Setting HT_MF for now.
                txVector->format = MODE_HT_MF;
            }
        }
        else
    ...
    BOOL tempCalc)
{
    ...
    STA_Capability_Mode nm = k_Dot11b;
    htInfo = MacDot11GetNeighHtInfo(dot11->htInfoList, destAddr);
    if (htInfo)
    {
        nm = htInfo->neighbourMode;
    }
    txVector.format = MODE_NON_HT;
    if (frameType == DOT11_DATA 
        || frameType == DOT11_QOS_DATA)
    {
        MacDot11SetMCSIndexForDataMgmtFrame(
            &txVector, htInfo);
        isBroadcast =
            MAC_IsBroadcastMac802Address((Mac802Address*)&destAddr);
        if (isBroadcast)
        {
            assert(txVector.mcs == 0);
        }
        MacDot11SetOperModeDataFrames(dot11, &txVector, isBroadcast, nm);
    ...
    {
        assert(rxFrameVector);
        txVector.format = rxFrameVector->format;
        MacDot11SetMCSIndexForCtrlRespFrame(
            &txVector, rxFrameVector);
        MacDot11SetChnlBwdthForCtrlRespFrame(
            node,
            dot11,
            frameType,
            &txVector,
        ...
        || frameType == DOT11N_DELAYED_BLOCK_ACK)
    {
        txVector.format = MODE_NON_HT;
        txVector.mcs = 0;
        MacDot11SetChnlBwdthForBroadcastFrame(node, dot11,
                    &txVector, frameType, htInfo, tempCalc, nm);
    }

    MacDot11SetPHYType(&txVector, htInfo);
    MacDot11SetGiType(node, dot11, frameType, &txVector,
            htInfo, rxFrameVector, nm);
    MacDot11SetModulation(node, dot11, frameType,
            &txVector, rxFrameVector, tempCalc, nm);

    if (nm == k_Dot11b)
        ERROR_Assert(txVector.mcs <= 3, "");
    else if (nm == k_Dot11a)
        ERROR_Assert(txVector.mcs <= 7, "");
    else if (nm == k_Dot11n)
        ERROR_Assert(txVector.mcs <= 31, "");
    else 
        ERROR_Assert(txVector.mcs <= 79, "");
    return txVector;
}
...
/// \brief Implementation of Get Antenna Elements for 802.11n
///
/// This function will get the number of antenna elements according
/// to the maximum Mcs parameter
///
/// \param maxMcs Maximum Mcs Index
int MacDot11nGetAtnaElmts(int maxMcs)
{
    int numAtnaElmts;

    switch (maxMcs)
    {
        case 7 : numAtnaElmts = 1;
                 break;

        case 15 : numAtnaElmts = 2;
                  break;

        case 23 : numAtnaElmts = 3;
                  break;

        case 31 : numAtnaElmts = 4;
                  break;

        default : ERROR_Assert(FALSE, "Invalid MCS Index");
                  break;
    }
...
/// \brief Implementation of Get Antenna Elements for 802.11ac
///
/// This function will get the number of antenna elements according
/// to the maximum Mcs parameter
///
/// \param maxMcs Maximum Mcs Index
int MacDot11acGetAtnaElmts(int maxMcs)
{
    int numAtnaElmts;

    switch (maxMcs)
    {
        case 9 : numAtnaElmts = 1;
                 break;

        case 19 : numAtnaElmts = 2;
                  break;

        case 29 : numAtnaElmts = 3;
                  break;

        case 39 : numAtnaElmts = 4;
                  break;

        case 49 : numAtnaElmts = 5;
                  break;

        case 59 : numAtnaElmts = 6;
                  break;

        case 69 : numAtnaElmts = 7;
                  break;

        case 79 : numAtnaElmts = 8;
                  break;
...
/// \brief Implementation of Get Max Mcs Index of neighbour
///
/// This function will get the Max Mcs index that should be
/// used for the neighbour node
///
/// \param node Node Pointer
/// \param dot11 Dot11 Pointer
/// \param nm neighbour capability mode
/// \param maxMcs Maximum Mcs Index
int MacDot11GetNeighborMaxMcs(Node* node,
                                  MacDataDot11* dot11,
                                  STA_Capability_Mode nm,
                                  int maxMcs)
{
    if (nm == k_Dot11a)
    {
        return DOT11a_MAX_MCS_SINGLE_SS;
    }
    else if (nm == k_Dot11b)
    {
        return DOT11b_MAX_MCS_SINGLE_SS;
    }

    int neighborAtnaElmts = 0;
    if (nm == k_Dot11n)
    {
        neighborAtnaElmts = MacDot11nGetAtnaElmts(maxMcs);
    }
    else if (nm == k_Dot11ac)
    {
        neighborAtnaElmts = MacDot11acGetAtnaElmts(maxMcs);
    }
    else
    {
        assert(false);
    ...
    int minAtna = std::min(neighborAtnaElmts, Phy802_11GetNumAtnaElems(
                    node->phyData[dot11->myMacData->phyNumber]));
    int maxMcsIndex = 0;

    if (nm == k_Dot11n)
    {
        maxMcsIndex = (minAtna * 8) - 1;
    }
    else if (nm == k_Dot11ac)
    {
        maxMcsIndex = (minAtna * 10) - 1;
    }
    return maxMcsIndex;
} //end of MacDot11_GetNeighborMaxMcsIdx

/// \brief Implementation of Initialization of Neighbour Info
///
/// This function will Initialize the Neighbour Info structure
/// for the given destination address
///
/// \param node Node Pointer
/// \param dot11 Dot11 Pointer
/// \param neighborAddr neighbour address
/// \param neighbourMode neighbour capability mode
void MacDot11InitNeighborInfo(
        Node* node, 
        MacDataDot11* dot11,
        const Mac802Address& neighborAddr,
        STA_Capability_Mode neighbourMode)
{
    DOT11_DataRateEntry* neighbourEntry =
        MacDot11StationGetDataRateEntry(node, dot11, neighborAddr);
    neighbourEntry->neighbourMode = k_Dot11b;
    neighbourEntry->maxDataRate = dot11->highestDataRateType;
    
    STA_Capability_Mode staMode;

    if (node->phyData[dot11->myMacData->phyNumber]->phyModel == PHY802_11b) {
        staMode = k_Dot11b;
    }
    else {
        staMode = k_Dot11a;
    ...
    if (staMode == k_Dot11a && neighbourMode == k_Dot11b)
    {
        if (dot11->highestDataRateType > 3)
        {
            neighbourEntry->maxDataRate = 3;
        }
    }

    if ((neighbourMode == k_Dot11a)
        && (staMode == k_Dot11a))
    {
        neighbourEntry->neighbourMode = k_Dot11a;
        neighbourEntry->maxDataRate = dot11->highestDataRateType;
    }
}
