...
/*** UAPSD *********************************START*******************/
void Dot11_ParseEdcaParameterSet(
    Node* node,
    DOT11_Ie* element,
    DOT11e_EDCAParameterSet *edcaParameterSet);

bool Dot11_GetEdcaParameterSet(
    Node* node,
    MacDataDot11* dot11,
    Message* msg,
    DOT11e_EDCAParameterSet *edcaParameterSet,
...
/*** UAPSD *********************************START*******************/
        if (((dot11->dot11TxFrameInfo->frameType >= DOT11_ASSOC_REQ
            ...
            || dot11->dot11TxFrameInfo->frameType == DOT11_QOS_NULL)) &&
...
/// Update the measurement history and recalculate the
/// mean value
///
    ...
    int TID)
{
    ...
    if (TID <8)
    {
         ...
         head = &(getAc(dot11, acIndex).m_seqNoHead);
         entry = *head;
         prev = NULL;
    }
    else
    ...
    int seqNo)
{
    ...
    if (seqNo > entry->fromSeqNo)
    {
        entry->fromSeqNo = seqNo;
        return true;
    }
    else
    {
...
/// Set the FromDS/ToDS flag in data frame header
/// depending on whether the AP/PC or BSS station is
/// the sender.
/// In case of relay, there could be a check for
/// destination which is outside BSS. However, in the
...
/// \param dot11  Structure of dot11.
/// \param phyModel  NPHY model used.
///
inline //inline
...
/// Set frame type, frame flag and address fields of the
/// data frame being sent.
///
/// \param const dot11  Pointer to Dot11 structure
/// \param const fHdr  Frame header
...
//--------------------HCCA-Updates Start---------------------------------//
    ...
    {
        fHdr->qoSControl.TID = (UInt16) (getAc((MacDataDot11*)dot11, acIndex).m_priority);
        fHdr->qoSControl.EOSP = 1;
        fHdr->qoSControl.TXOP = (UInt8)ceil(((Float64)
            getAc((MacDataDot11*)dot11, acIndex).m_TXOPLimit / MILLI_SECOND));
    }
//--------------------HCCA-Updates End-----------------------------------//
    ...
    if (DEBUG_QA)
    {
        ...
        getAc((MacDataDot11*)dot11, acIndex).m_priority,
        getAc((MacDataDot11*)dot11, acIndex).m_TXOPLimit,
        acIndex);
    }
...
//---------------------DOT11e--Updates------------------------------------//
    if (MacDot11IsQoSEnabled(node, dot11))
    {
        ...
        {
            AccessCategory& ac = getAc(dot11, dot11->currentACIndex);
            ac.m_CW = ac.m_cwMin;
        } else
        {
                             ...
                             clocktype backoff)
{
    ...
    for (int acCounter = 0; acCounter < mc(dot11)->ac()->getNumAc(); acCounter++)
    {
    ...
    MacDataDot11* dot11)
{
    ...
    if (MacDot11IsQoSEnabled(node, dot11))
    {
        if (dot11->currentACIndex >= DOT11e_AC_BK)
        {
            getAc(dot11, dot11->currentACIndex).m_CW =
                MIN((((getAc(dot11, dot11->currentACIndex).m_CW + 1) * 2) - 1),
                    getAc(dot11, dot11->currentACIndex).m_cwMax);
        }else {
        ...
        getAc(dot11, dot11->currentACIndex).m_CW  = dot11->CW;
    }
    ...
    int currentACIndex = DOT11e_INVALID_AC)
{
    ...
    if (MacDot11IsQoSEnabled(node, dot11)) 
    {
        if (currentACIndex != DOT11e_INVALID_AC)
        {
            ...
            if (ac.getBackoffInterval() ==0)
            {
                ac.setBackoffInterval((RANDOM_nrand(dot11->seed) %
                                        (ac.getCW() + 1)) *
                                        ac.getSlotTime());
            }
        }
        ...
        if (currentACIndex != DOT11e_INVALID_AC)
        {
            ...
            if (ac.getBackoffInterval() ==0)
            {
                    ac.setBackoffInterval((RANDOM_nrand(dot11->seed) %
                                        (ac.getCW() + 1)) *
                                        ac.getSlotTime());
                    dot11->BO = ac.getBackoffInterval();
            }
        }
        else
        {
...
void MacDot11StationPauseBackoff(Node* node, MacDataDot11* dot11)
{
    ...
    if (MacDot11IsQoSEnabled(node, dot11)) {
        if (dot11->currentACIndex >= DOT11e_AC_BK){
            getAc(dot11, dot11->currentACIndex).m_BO = dot11->BO;
            if (dot11->isHTEnable)
            {
    ...
    MacDataDot11* dot11)
{
    ...
    else
    {
        ...
        if (((!DirectionalNav_ThereAreNoNavs(
    ...
    {
        mc(dot11)->ac()->dqFromMac(dot11);
        mc(dot11)->startContention(dot11);
    }
    ...
    switch (dot11->state) {
        ...
        case DOT11_S_WFJOIN:
        {
            ...
            if (newNAV > dot11->NAV)
            {
                ...
                {
                    if (mc(dot11)->ac()->CheckStateIfDifsOrBo(dot11) == k_State_BO)
                    {
                        dot11->BO = 0;
                    }
                }
...
inline void MacDot11ResetTxRxVector(MAC_PHY_TxRxVector &txRxVector)
{
          ...
          DOT11_NUM_ACKS_FOR_RATE_INCREASE) )
    {
        int newDataRate
            = MIN(dot11->dataRateInfo->dataRateType + 1,
                  dot11->dataRateInfo->maxDataRate);
    ...
    Mac802Address desNodeAddress)
{
    ...
    queue = new Queue(node, "FIFO", dot11->unicastQueueSize); // "FIFO"
    ...
    MacDataDot11* dot11)
{
    ...
    if (attachNodeList == NULL){
        ...
        attachNodeList = MacDot11AttachNodeAtStation(
            node,
            dot11,
            getAc(dot11,dot11->currentACIndex).m_currentNextHopAddress);
        dot11->attachNodeList = attachNodeList;
        dot11->roundRobinItr = attachNodeList;
    }
    else{
    ...
    if (!MacDot11BufferPacket(
        ...
        getAc(dot11,dot11->currentACIndex).m_frameToSend,
        node->getNodeTime())){
...
//  ASSUMPTION:  None
//--------------------------------------------------------------------------
inline BOOL MacDot11IsDestinationQosEnable(Node* node,
                                     MacDataDot11* dot11,
...
//--------------------HCCA-Updates Start---------------------------------//
    ...
    if (getAc(dot11, acIndex).m_frameToSend!= NULL) {
        ...
        if ((!isACHasMsgForCurrentAccessCategory )) {
            ...
            dequeued =MAC_OutputQueueDequeuePacketForAPriority(
                                ...
                                queuePriority,
                                        &(tempMsg),
                                        &(tempNextHopAddress),
                                &(getAc(dot11, acIndex).m_networkType));
            ...
            memset(ac.m_frameInfo, 0, sizeof(DOT11_FrameInfo));
            ac.m_frameInfo->msg = tempMsg;
            ac.m_frameToSend = tempMsg;
            ac.m_frameInfo->RA = tempNextHopAddress;
            ac.m_frameInfo->TA = dot11->selfAddr;
            ac.m_frameInfo->SA = dot11->selfAddr;
            ac.m_frameInfo->DA = dot11->ipDestAddr;
            ac.m_frameInfo->insertTime = node->getNodeTime();

            if (SendQosFrame)
            {
                ac.m_frameInfo->frameType = DOT11_QOS_DATA;
                ac.m_priority = priority;
                newMsg = ac.m_frameInfo->msg;
                ...
                MacDot11eStationSetFieldsInDataFrameHdr(dot11,
                                                    ...
                                                    ac.m_frameInfo->RA,
                                                    ac.m_frameInfo->frameType,
                                                    acIndex);
             }
             else
             {
                //for DCF data processing in AC[BE]
                ac.m_frameInfo->frameType = DOT11_DATA;
                newMsg = ac.m_frameInfo->msg;
                ...
                if (DEBUG_QA)
                {
                    unsigned char* frame_type =
                        (unsigned char*) MESSAGE_ReturnPacket(ac.m_frameInfo->msg);
                    printf(" Node %u ac[ %d] used priority %d for frame %x\n",
                        node->nodeId,
                        acIndex,
                        ac.m_priority,*frame_type);
                }
    ...
    MacDataDot11* dot11)
{
    ...
    // Dequeue packets from network layer and drop extra packets to
    // get the event in case of new packets arrive at network layer
    dequeued = mc(dot11)->ac()->dqFromNetwork(dot11);

//---------------------------Power-Save-Mode-Updates---------------------//
    // Get reference of Access category
    ...
    if (MacDot11IsAPSupportPSMode(dot11)&& dequeued)
    {
        ...
        while (dequeued)
        {
            if (Ac.m_currentNextHopAddress == ANY_MAC802)
            {
                ...
                {
                    dot11->ipNextHopAddr = Ac.m_currentNextHopAddress;

                    if (MacDot11IsBssStation(dot11))
                    {
                        if (!MAC_IsBroadcastMac802Address(&Ac.m_currentNextHopAddress) &&
                        ...
                        Ac.m_currentNextHopAddress = dot11->bssAddr;
                    }

                    Ac.m_frameInfo =
                        (DOT11_FrameInfo*) MEM_malloc(sizeof(DOT11_FrameInfo));
                    memset(Ac.m_frameInfo, 0, sizeof(DOT11_FrameInfo));
                    Ac.m_frameInfo->msg = Ac.m_frameToSend;
                    Ac.m_frameInfo->RA = Ac.m_currentNextHopAddress;
                    Ac.m_frameInfo->TA = dot11->selfAddr;
                    Ac.m_frameInfo->SA = dot11->selfAddr;
                    Ac.m_frameInfo->DA = Ac.m_frameInfo->RA;
                    Ac.m_frameInfo->insertTime = node->getNodeTime();
                    Ac.m_frameInfo->frameType = DOT11_DATA;
...
{
    BOOL isMsgRetrieved = FALSE;
    DOT11_AttachedNodeList* attachNodeList = dot11->roundRobinItr;
    ...
    /* sending the packets form the existing NodeLists in a
       round-robin fashion, using the roundRobinItr pointer, which ensures
       that once a packet is dequeued from the current NodeList, the
       subsequent packet is dequeued from the next NodeList after each
       iteration.
    */

    do
    {
        if (attachNodeList->atimFramesACKReceived
            && (attachNodeList->noOfBufferedUnicastPacket > 0))
        {
            ...
            if (isMsgRetrieved)
            {
               ...
               if (attachNodeList->next != NULL)
               {
                   // move the pointer to next attachNodeList
                   dot11->roundRobinItr = attachNodeList->next;
               }
               else
               {
                   // reset the pointer to the begining
                   dot11->roundRobinItr = dot11->attachNodeList;
               }
               break;
            }
        ...
        if (attachNodeList == NULL)
        {
            attachNodeList = dot11->attachNodeList;
        }
    } while (attachNodeList != dot11->roundRobinItr);
        ...
        if (ret){
                ...
                && (dot11->txFrameInfo == NULL))
            {
                ...
                if (ac.m_frameInfo!= NULL)
                {
                    ...
                    ac.m_frameInfo = NULL;
                    dot11->dot11TxFrameInfo = NULL;
                    mc(dot11)->ac()->setMoreFrameField(dot11->currentACIndex);
                }
            }
        }
...
//--------------------DOT11e-End-Updates---------------------------------//
    ...
    if (dot11->dot11TxFrameInfo && dot11->dot11TxFrameInfo->isAMPDU)
    {
        destAddr = dot11->dot11TxFrameInfo->DA;
        headerSize = dot11->dot11TxFrameInfo->ampduOverhead;
    }
    else
    {
    ...
    char dstAdd[24] = {'\0'};
    MacDot11MacAddressToStr(dstAdd, &destAddr);

    mc(dot11)->updateStats("MacSent",
                            msgSize,
                            headerSize,
                            dstAdd);
...
/*********HT START*************************************************/
        ...
        else
        {
            ...
            setDataRateType, dot11->myPhyType);
                 dataRateType = 
...
/************** Interoperability feature *********START*******************/
        ...
        neighbourMode = dot11->dataRateInfo->neighbourMode;
...
/*********HT END****************************************************/
    ...
    if (!Mac802_11p_CanStartTransmitting(node, dot11, dataRateType))
    {
        MacDot11StationSetState(node, dot11, DOT11_S_IDLE);
        return;
    }
...
/*********HT END****************************************************/
    ...
    if (DEBUG_UAPSD)
    {
        if (dot11->isUapsdEnable)
        {
            ...
            MacDot11MacAddressToStr(dstAddress,
                ...
                printf("NodeId %d Time %" TYPES_64BITFMT "d Sending RTS to %s. \n",
    ...
    if (dot11->isHTEnable)
    {
        ...
        clocktype dur = PHY_GetTransmissionDuration(
                            node, dot11->myMacData->phyNumber,
                            txVector);
        printf("%" TYPES_64BITFMT "d Node %d Sending RTS type %d"
    ...
    else
    {
        clocktype dur = PHY_GetTransmissionDuration(
                            ...
                            dataRateType,
                            DOT11_LONG_CTRL_FRAME_SIZE);
        printf("%" TYPES_64BITFMT "d Node %d Sending RTS"
                " Datarate %d Duration %lld\n",
                 node->getNodeTime(),
                 node->nodeId,
                 dataRateType,
                 dur);
...
/*********HT START*************************************************/
    ...
    {
        dataRateType= PHY_GetRxDataRateType(node, dot11->myMacData->phyNumber, neighbourMode);
        if (!Mac802_11p_CanStartTransmitting(node, dot11,
            MIN(dataRateType, dot11->highestDataRateTypeForBC), DOT11_CTS,
...
/*********HT START*************************************************/
    ...
    if (dot11->isHTEnable)
    {
        ...
        printf("%" TYPES_64BITFMT "d Node %d Sending CTS"
                    " with mcs %d length %" TYPES_SIZEOFMFT 
                    "d gi %s ChBandwidth %d Format %d Mode %d Modulation %s Duration %lld\n",
                    node->getNodeTime(),
                    node->nodeId,
    ...
    clocktype dur = PHY_GetTransmissionDuration(
        ...
        printf("%" TYPES_64BITFMT "d Node %d Sending CTS"
                " Datarate %d Length %d Duration %lld\n",
                 node->getNodeTime(),
                 node->nodeId,
                 DOT11_SHORT_CTRL_FRAME_SIZE,
                 MIN(dataRateType, dot11->highestDataRateTypeForBC),
...
/*********HT START*************************************************/
    ...
    if (dot11->isHTEnable)
    {
        ...
        clocktype dur = PHY_GetTransmissionDuration(
                            node, dot11->myMacData->phyNumber,
                            tempTxVector);
        printf("%" TYPES_64BITFMT "d Node %d Sending ACK"
                    " with mcs %d length %" TYPES_SIZEOFMFT 
                    "d gi %s ChBandwidth %d Format %d Mode %d Modulation %s Duration %lld\n",
                    node->getNodeTime(),
                    node->nodeId,
    ...
    MacDataDot11* dot11)
{
    Mac802Address destAddr;
    unsigned char neighbourMode = k_Dot11b;
    if (node->phyData[dot11->myMacData->phyNumber]->phyModel == PHY802_11a)
    {
        PHY_GetRxDataRateType(node, dot11->myMacData->phyNumber, neighbourMode);
    }
/*********HT START*************************************************/
    int dataRateType;
    MAC_PHY_TxRxVector txVector;
...
/*********HT START*************************************************/
    ...
    {
    PHY_SetTxDataRateType(node, dot11->myMacData->phyNumber,
                          dot11->dataRateInfo->dataRateType, neighbourMode);
    dataRateType = PHY_GetTxDataRateType(node, dot11->myMacData->phyNumber);
    if (!Mac802_11p_CanStartTransmitting(node, dot11,
                    dataRateType, DOT11_DATA))
    {
        MacDot11StationSetState(node, dot11, DOT11_S_IDLE);
        return;
    }
    ...
    clocktype dur = PHY_GetTransmissionDuration(
        ...
        printf("%" TYPES_64BITFMT "d Node %d Sending Data"
                " Datarate %d Length %d Duration %lld\n",
                 node->getNodeTime(),
                 node->nodeId,
                 dataRateType,
...
/*********HT END****************************************************/
            ...
            ERROR_ReportError("MacDot11StationTransmitDataFrame: "
             ...
             // For future use
             /*else if (hdr->frameType == DOT11_QOS_NULL)
             {
                 MacDot11StationSetState(node, dot11, DOT11_X_TRIGGER);
             }*/
              else
