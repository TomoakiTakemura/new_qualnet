                                        ...
                                        OutputBuffer* outputBuff)
{
    ...
    {
        outputBuffer = getAc(dot11, acIndex).m_outputBuffer;
    }
    ...
    while (keyItr != outputBuffer->OutputBufferMap.end())
    {
        if (!(keyItr->second.frameQueue.empty()))
        {
            ...
            {
                getAc(dot11, acIndex).setIsAcHasPacket(FALSE);
            }
            else
            {
                getAc(dot11, acIndex).setIsAcHasPacket(TRUE);
                break;
            }
        }
        else
...
static
UInt8 MacDot11acGetMaximumMcs(Node* node,
                             MacDataDot11* dot11,
                             int maxMcsIdx)
{

    UInt8 maxMcs = 0;
    // Find maximum mcs that can be supported b/w two station
    maxMcs = std::min(maxMcsIdx,(Phy802_11GetNumAtnaElems(
                           ...
                           BOOL isKeyAlreadyPresent)
{
    ...
    OutputBuffer *outputBuffer = getAc(dot11, acIndex).m_outputBuffer;
                               ...
                               int acIndex)
{
    ...
    for (currentAcCount = 0;
         currentAcCount < mc(dot11)->ac()->getNumAc();
         currentAcCount++)
    {
        if (currentAcCount != acIndex
            && getAc(dot11, currentAcCount).m_isACHasPacket == FALSE
            && getAc(dot11, currentAcCount).m_frameInfo == NULL)
        {
            NoPacket++;
        }
    }
                    ...
                    int AmsduPresent)
{
    ...
    fHdr->qoSControl.TXOP
        = (unsigned char)(getAc((MacDataDot11*)dot11, acIndex).m_TXOPLimit / MILLI_SECOND);
}
            ...
            BOOL isKeyAlreadyPresent)
{
    ...
    else
    {
        ...
        tmpMapValue.blockAckVrbls = NULL;
        OutputBuffer::MacDot11nInitializeBap(node,
                                             dot11,
                                             &tmpMapValue,
                                             ipDestAddr);
        ...
        if (OutputBufferMap.size() == 1)
        {
            OutputBuffer::InitializeRoundRobinIterator();
        }
    }
}
...
BOOL MacDot11nIsACsHasMessage(MacDataDot11* dot11)
{
    ...
    for (acCounter = 0;
         ...
         acCounter++)
    {
        if (getAc(dot11, acCounter).m_isACHasPacket
            || getAc(dot11, acCounter).m_frameInfo != NULL)
        {
            return TRUE;
        }
    }
                                 ...
                                 MapValue>::iterator keyItr)
{
    ...
    Mac802Address tempNextHopAddress = keyItr->first.first;
    BOOL isBc = MAC_IsBroadcastMac802Address(
                    &tempNextHopAddress);
    DOT11_NeighborHtInfo* nm
        = MacDot11GetNeighHtInfo(dot11->htInfoList,
                                 keyItr->first.first);
    BOOL dprocess = FALSE;
    if (!isBc)
    {
        if ((nm && nm->neighbourMode != k_Dot11a && nm->neighbourMode != k_Dot11b)
         && (acIndex > 1 && duration >= ac.m_TXOPLimit))
        {
            // Letting TxOp increase temporarily for sending QOS packets
            // destined to 11a/11b devices running MacDot11e at MAC layer
            dprocess = TRUE;
        }
    }
    ...
    ERROR_Assert(keyItr->second.winStarts == hdr->seqNo,
                 "winStarts != hdr->seqNo");
    ERROR_Assert(ac.m_frameInfo == NULL,"AC has packet");
    ...
    ac.m_frameInfo = tempFrameInfo;
    ac.m_frameToSend = tempFrameInfo->msg;
    ac.m_priority = keyItr->first.second;
    ac.m_currentNextHopAddress = keyItr->first.first;
    ERROR_Assert(MAC_IsIdenticalMac802Address(
                 &ac.m_currentNextHopAddress,
...
static
UInt16 getMaxAmsduLength(Int32 lenExp)
{
    UInt16 maxAmsduLen = 0;
    if (lenExp == 0)
    {
        maxAmsduLen = 3895 - 56;
    }
...
/************** Interoperability feature *********START*******************/
            if (!stationItem
                || htInfo->neighbourMode == k_Dot11a
                || htInfo->neighbourMode == k_Dot11b)
            {
                // Station information not present or AP is communicating
                // to 11a or 11b device.
                return FALSE;
            }

            minMcsIndex = MacDot11GetMinMcs(dot11,
                htInfo->neighbourMode, htInfo->maxMcsIdx);

            if (htInfo->neighbourMode == k_Dot11ac)
            {
                assert(dot11->isVHTEnable);
                int maxMpduExp
                    = stationItem->data->
                       vhtInfo.staVhtCapabilityElement.
                       m_capabilitiesInfo.m_maxMpduLen;
                 maxAmsduSize = getMaxAmsduLength(maxMpduExp);
            }
            else
            {
                assert(htInfo->neighbourMode == k_Dot11n);
            ...
            if (dot11->stationType == DOT11_STA_IBSS)
            {
                ...
                if (stationInfo->probeStatus == IBSS_PROBE_COMPLETED)
                {
                    if (dot11->isVHTEnable)
                    {
                        int maxMpduExp
                            = stationInfo->vhtInfo
                            .staVhtCapabilityElement.
                            m_capabilitiesInfo.m_maxMpduLen;

                        maxAmsduSize = getMaxAmsduLength(maxMpduExp);
                        int tempMcs = (int)stationInfo->vhtInfo
                            .staVhtCapabilityElement.m_mcsNssSet;
                        UInt8 maxMcs = MacDot11acGetMaximumMcs(node,
                                                               dot11,
                                                               tempMcs);
                        minMcsIndex = maxMcs - 9;
                    }
                    else
                    {
                        int tempMcs =
...
/************** Interoperability feature *********START*******************/
                if (!dot11->associatedAP
                    ...
                    || htInfo->neighbourMode == k_Dot11b)
                {
                    // AP information not present or STA is communicating
                    // to 11a or 11b AP.
                    return FALSE;
                }
...
/************** Interoperability feature *********END*******************/
        ...
        if (dot11->isVHTEnable)
        {
            if (htInfo->neighbourMode == k_Dot11ac)
            {
                ERROR_Assert(minMcsIndex < 71 && minMcsIndex % 10 == 0,
                    "MCS index is not minimum as per given antenna elements");
            }
            else
            {
                assert(htInfo->neighbourMode != k_Dot11a
                    && htInfo->neighbourMode != k_Dot11b);
        ...
        else
        {
            assert(htInfo->neighbourMode != k_Dot11a
                    && htInfo->neighbourMode != k_Dot11b
                    && htInfo->neighbourMode != k_Dot11ac);
            ERROR_Assert(minMcsIndex == 0
                || minMcsIndex == 8
                || minMcsIndex == 16
                || minMcsIndex == 24,
            ...
            int acIndex = MacDot11ReturnAccessCategory(priority) ;
            while ((acIndex < 2
                    || duration < getAc(dot11, acIndex).m_TXOPLimit)
                                       ...
                                       BOOL updateOutputbuffer)
{
    ...
    switch(hdr->frameType)
    {
        ...
        case DOT11_QOS_DATA:
        {
            Mac802Address desAddr = hdr->destAddr;
...
/*** UAPSD *********************************START*******************/
            ...
            if (DEBUG_UAPSD)
            {
                printf("NodeId %d Time %" TYPES_64BITFMT "d Dropped QOS"
                    " Data\n",
                    node->nodeId, node->getNodeTime());
            }
            if (MacDot11IsAp(dot11) && dot11->isUapsdEnable)
            {
                ...
                MacDot11ApStationListGetItemWithGivenAddress(
                    ...
                    desAddr);
                if (stationItem) {
                    if (stationItem->data->uapsdInfo.uApsdEnabled)
                    {
                        assert(stationItem->data->
                            uapsdInfo.spInProgress);
                        stationItem->data->uapsdInfo.noOfPacketsSent++;

                        if (mc(dot11)->ac()->packetsForThisRA(
                            dot11, desAddr))
                        {
                            if (!stationItem->data->uapsdInfo.useTxop)
                            {
                                if (stationItem->data->uapsdInfo
                                    .noOfPacketsSent
                                        >= stationItem->data
                                    ->uapsdInfo.
                                    noOfPacketsToBeSentInServicePeriod)
                                {
                                    if (DEBUG_UAPSD)
                                    {
                                        printf("NodeId %d Time %" TYPES_64BITFMT "d"
                                            " AP SP->0 \n",
                                            node->nodeId,
                                            node->getNodeTime());
                                    }
                                    stationItem->data->uapsdInfo
                                        .spInProgress = false;
                                    stationItem->data->uapsdInfo
                                        .noOfPacketsSent = 0;
                                }
                        ...
                        else
                        {
                            if (DEBUG_UAPSD)
                            {
                                printf("NodeId %d Time %" TYPES_64BITFMT "d AP"
                                    " SP->0 \n",
                                    node->nodeId,
                                    node->getNodeTime());
                            }
            ...
            if (MacDot11IsBssStation(dot11)
                ...
                && dot11->isUapsdEnable
                && dot11->uapsdInfo.uApsdEnabledAp)
            {
                // Data packet dropped.
                dot11->uapsdInfo.uApsdTriggerSent = false;
            }
/*** UAPSD *********************************END*******************/
            break;
        }
/*** UAPSD *********************************START*******************/
        case DOT11_QOS_NULL:
        {
            ...
            if (MacDot11IsAp(dot11))
            {
                ERROR_Assert(!dot11->uapsdInfo.uApsdTriggerSent,
                    "Invalid State");
                DOT11_ApStationListItem* stationItem =
                MacDot11ApStationListGetItemWithGivenAddress(
                    node,
                ...
                if (stationItem)
                {
                    stationItem->data->uapsdInfo.spInProgress = false;
                }
            }
            break;
        }
                                     ...
                                     BOOL dropInternalCollision)
{
    ...
    {
        outputBuffer->tmpKey.first = getAc(dot11, acIndex).m_frameInfo->RA;
        outputBuffer->tmpKey.second = getAc(dot11, acIndex).m_priority;
    }
    else
    {
        ERROR_Assert(dot11->dot11TxFrameInfo->msg
            == getAc(dot11, acIndex).m_frameToSend,
    ...
    int acIndex)
{
    ...
    {
        ERROR_Assert(ac.m_isACHasPacket,
            "Ac is empty");
    ...
    UseShortPacketRetryCounter
        = (DOT11nMessage_ReturnPacketSize(ac.m_frameToSend)
            <= dot11->rtsThreshold);

    // If not greater than maximum retry count allowed, retransmit frame
    if (UseShortPacketRetryCounter)
    {
        ac.m_QSRC = ac.m_QSRC + 1;
        if (ac.m_QSRC < dot11->shortRetryLimit)
        {
    ...
    {
        ac.m_QLRC++;
        if (ac.m_QLRC < dot11->longRetryLimit)
        {
            RetryTheTransmission = TRUE;
        }
        else
    ...
    if (RetryTheTransmission)
    {
        ...
        getAc(dot11, dot11->currentACIndex).m_totalNoOfthisTypeFrameRetried++;
    ...
    else
    {
        ...
        {
            if (ac.m_frameInfo->baActive)
            {
                OutputBuffer *outputBuffer
                     = ac.m_outputBuffer;
                std::pair<Mac802Address,TosType> tmpKey;
                tmpKey.first = ac.m_frameInfo->RA;
                tmpKey.second = ac.m_priority;
                ...
                ERROR_Assert(keyItr
                    ...
                    "End of output buffer");
                ac.m_frameInfo->state = STATE_AWAITING_ACK;
                             ...
                             clocktype backoff)
{
    ...
    for (acCounter = 0;
        ...
        if (acCounter!=dot11->currentACIndex)
        {
            ...
            totalTime = (getAc(dot11, dot11->currentACIndex).m_AIFS
                - ac.m_AIFS) + backoff;
            if (totalTime>0 && (ac.m_frameToSend != NULL
                 || ac.m_isACHasPacket))
            {
                ac.m_BO = ac.m_BO - totalTime;
                if (ac.m_BO<0)
                {
                    ac.m_BO = 0;
                                  ...
                                  MacDataDot11* dot11)
{
    if (dot11->currentACIndex >= DOT11e_AC_BK)
    {
         MacDot11nPauseOtherAcsBo(node,
                                  dot11,
                                  getAc(dot11, dot11->currentACIndex).m_BO);
    ...
    MacDataDot11* dot11)
{
    ...
    else
    {
        ...
        if (MacDot11IsQoSEnabled(node, dot11) && dot11->currentACIndex
            >= DOT11e_AC_BK)
        {
            getAc(dot11, dot11->currentACIndex).m_QLRC = dot11->SLRC;
        }
    ...
    if (RetryTheTransmission)
    {
        ...
        if (DEBUG_UAPSD)
        {
            printf("NodeId %d Time %" TYPES_64BITFMT "d Retransmit \n",
                node->nodeId, node->getNodeTime());
        }
    ...
    else
    {
        ...
        if (dot11->dot11TxFrameInfo->isAMPDU)
        {
            ...
            MacDot11nHandleAmpduPacketDrop(node,
                                           ...
                                           FALSE);
            getAc(dot11, dot11->currentACIndex).m_totalNoOfthisTypeFrameDeQueued++;
        }
        else
        {
            if (dot11->dot11TxFrameInfo->baActive)
            {
                ...
                tmpKey.second = getAc(dot11, dot11->currentACIndex).m_priority;
                std::map<std::pair<Mac802Address,TosType>,
                    MapValue>::iterator keyItr;
                keyItr = outputBuffer->OutputBufferMap.find(tmpKey);
            ...
            else
            {
                if (dot11->dot11TxFrameInfo->isAmsdu)
                {
                    ...
                    MacDot11nHandleAmsduPacketDrop(node,
                                                    ...
                                                    dot11->currentMessage,
                                                    TRUE);
                    getAc(dot11, dot11->currentACIndex).
                        m_totalNoOfthisTypeFrameDeQueued++;
                }
                else
                {
                    dot11->pktsDroppedDcf++;
    ...
    Message* msg)
{
    switch (dot11->state)
    {
        ...
        case DOT11_S_WFNAV:
        {
            ...
            if (MacDot11StationPhyStatus(node, dot11) == PHY_TRX_OFF)
            {
                mc(dot11)->dc()->wakeUp(dot11,node);
            }
        ...
        case DOT11_S_WFDATA:
        {
            RfChainMode mode = k_Single_Rf_Chain;
            mc(dot11)->dc()->switchRfChains(dot11, node, mode);
            MacDot11StationSetState(node, dot11, DOT11_S_IDLE);
        ...
        MacDataDot11* dot11)
{
    ...
    AccessCategory& ac = getAc(dot11, currentACIndex);
    if (ac.m_frameToSend != NULL)
    {
        return FALSE;
    }
                                 ...
                                 DOT11_Frame* rxFrame)
{
    ...
    if (!process || keyItr->second.blockAckVrbls->state
            != BAA_STATE_DISABLED)
    {
        return;
    }
       ...
       DOT11n_DELBAFrame *delbaFrame =
        ...
        else
        {
                                    ...
                                    <= BAA_STATE_TRANSMITTING))
           {
                ...
                {
                    DOT11_FrameInfo *tempFrameInfo = *listItr;
                    if (getAc(dot11, acIndex).m_frameInfo
                        && getAc(dot11, acIndex).m_frameInfo == tempFrameInfo)
                    {
                         ...
                         int acIndex)
{
    ...
    if (initiator)
    {
        ...
        {
            mc(dot11)->interruptContention(dot11);
            MacDot11StationSetState(node, dot11, DOT11_S_IDLE);
        }
    }
    MacDot11nMgmtQueue_EnqueuePacket(node, dot11, frameInfo);
...
/*** UAPSD *********************************START*******************/
            ...
            && !dot11->ScanStarted)
        {
            if (DEBUG_UAPSD)
            {
                printf("NodeId %d Time %" TYPES_64BITFMT "d B->0\n",
                    node->nodeId, node->getNodeTime());
            }
...
/*** UAPSD *********************************START*******************/
            ...
            && !dot11->ScanStarted)
        {
            ...
            if (DEBUG_UAPSD)
            {
                printf("NodeId %d Time %" TYPES_64BITFMT "d U->0 T->0 \n",
                    node->nodeId, node->getNodeTime());
            }
            if (hdr->qoSControl.EOSP)
            {
                dot11->uapsdInfo.spInProgress = false;
                if (DEBUG_UAPSD)
                {
                    printf("NodeId %d Time %" TYPES_64BITFMT "d SP->0 \n",
                        node->nodeId, node->getNodeTime());
                }
                //MacDot11ApsdSleepIfNoData(node, dot11);
            }
            else
            {
                dot11->uapsdInfo.spInProgress = true;
                if (DEBUG_UAPSD)
                {
                    printf("NodeId %d Time %" TYPES_64BITFMT "d SP->1 \n",
                        node->nodeId, node->getNodeTime());
                }
            }
        }
        ...
        if (MacDot11IsAp(dot11))
        {
            ...
            if (stationItem)
            {
                ...
                {
                    // Implicit trigger. Start SP if possible.
                    if (mc(dot11)->ac()->packetsForThisRA(dot11, sourceNodeAddress))
                    {
                        if (!stationItem->data->uapsdInfo.spInProgress)
                        {
                            if (DEBUG_UAPSD)
                            {
                                printf("NodeId %d Time %" TYPES_64BITFMT "d Implicit"
                                    " SP->1 \n",
                                    node->nodeId, node->getNodeTime());
                            }
                            stationItem->data->uapsdInfo.spInProgress = true;
                            assert(stationItem->data->uapsdInfo.noOfPacketsSent == 0);
                        }
                    }
                    else
                    {
                        // No packets for the station. Send a QOS NULL
                        // towards the station indicating there is no data
                        if (DEBUG_UAPSD)
                        {
                            printf("NodeId %d Time %" TYPES_64BITFMT "d AP No Data."
                                " Sending QOS NULL \n",
                                node->nodeId, node->getNodeTime());
                        }
                        //assert(!stationItem->data->uapsdInfo.spInProgress);
                        //stationItem->data->uapsdInfo.spInProgress = false;
                        assert(stationItem->data->uapsdInfo.noOfPacketsSent == 0);
                        MacDot11StationTransmitTriggerFrame(node,
                                                dot11,
                                                sourceNodeAddress);
                    }
                }
...
//  ASSUMPTION:  None
//--------------------------------------------------------------------------
UInt32 MacDot11nCalculateMaxAmpduLength(int ampduLengthExponent)
{
    int pwrOfTwo = 13 + ampduLengthExponent;
    double maxAmpduLength = pow(2.0, pwrOfTwo) - 1;
    return (UInt32)maxAmpduLength;
}
         ...
         DOT11_ApStationListItem* stationItem = NULL;
                stationItem =
                    MacDot11ApStationListGetItemWithGivenAddress(
                                                ...
                                                dot11,
                                                nextHopAddress);
        ERROR_Assert(stationItem,"stationItem should be present");
        if (stationItem->data->uapsdInfo.uApsdEnabled)
        {
            return FALSE;
        }
...
/************** Interoperability feature *********START*******************/
        ...
        UInt8 ampduLengthExponent = 0;
        if (htInfo->neighbourMode == k_Dot11ac)
        {
            assert(dot11->isVHTEnable);
            ampduLengthExponent
                = stationItem->data->vhtInfo.
                    staVhtCapabilityElement.m_capabilitiesInfo.
                    m_maxAmpduLenExponent;
        }
        else
        {
            assert(htInfo->neighbourMode == k_Dot11n);
...
/************** Interoperability feature *********END*******************/
    ...
    if (tempIterator->second.numPackets >= MIN_MPDUS_IN_AMPDU)
    {
        ...
        while ((acIndex < 2 || duration < getAc(dot11, acIndex).m_TXOPLimit)
              ...
              && (*itr)->state == STATE_FRESH)
        {
            ...
            duration += PHY_GetTransmissionDuration(
             ...
             if ((acIndex < 2 || duration < getAc(dot11, acIndex).m_TXOPLimit)
                          ...
                          UInt8 numPackets)
{
    ...
    frameInfo->frameType = DOT11_QOS_DATA;
    getAc(dot11, acIndex).m_frameInfo = frameInfo;
    getAc(dot11, acIndex).m_frameToSend = frameInfo->msg;
    getAc(dot11, acIndex).m_priority = outputBuffer->roundRobinKeyItr->first.second;
    getAc(dot11, acIndex).m_currentNextHopAddress
        = outputBuffer->roundRobinKeyItr->first.first;
    getAc(dot11, acIndex).m_totalNoOfthisTypeFrameQueued++;
}
    ...
    switch(addbaFrame->action)
    {
        ...
        case DOT11n_DELBA:
        {
            ...
            if (delbaFrame->delbaParams.initiator)
            {
                ...
                UInt8 acIndex = (UInt8)MacDot11ReturnAccessCategory(priority);
                OutputBuffer *outputBuffer
                    = getAc(dot11, acIndex).m_outputBuffer;
                std::pair<Mac802Address,TosType> tmpKey;
                tmpKey.first = frameInfo->RA;
                tmpKey.second = priority;
                std::map<std::pair<Mac802Address,TosType>,MapValue>
...
/*** UAPSD *********************************START*******************/
                            ...
                            == DOT11_QOS_NULL)
                {
                    if (DEBUG_UAPSD)
                    {
                        printf("NodeId %d Time %" TYPES_64BITFMT "d Successfully sent QOS"
                            " NULL Data\n",
                            node->nodeId, node->getNodeTime());
                    }
                    if (MacDot11IsBssStation(dot11))
                    {
                        ERROR_Assert(dot11->isUapsdEnable,
                            "UAPSD should be enabled on the node");
                        ERROR_Assert(dot11->uapsdInfo.uApsdEnabledAp,
                            "AP should be UAPSD enabled");
...
/*** UAPSD *********************************START*******************/
                    ...
                    if (MacDot11IsBssStation(dot11)
                        ...
                        && dot11->uapsdInfo.uApsdEnabledAp)
                    {
                        // Data packet sent successfully. This acts as an
                        // implicit trigger.
                        dot11->uapsdInfo.uApsdTriggerSent = true;
                    }
/*** UAPSD *********************************END*******************/
                    UInt8 acIndex = (UInt8)MacDot11ReturnAccessCategory(priority);
                    OutputBuffer *outputBuffer
...
{
    DOT11_FrameHdr* hdr
        = reinterpret_cast<DOT11_FrameHdr*>MESSAGE_ReturnPacket(msg);
...
//--------------------------------------------------------------------------
//  NAME:        MacDot11nHandleNonQosData
//  PURPOSE:     Processes a non-qos packet received from Phy layer
//  PARAMETERS:  Node* node
//                  Pointer to node
//               MacDataDot11* dot11
//                  Pointer to Dot11 structure
...
//--------------------------------------------------------------------------
static
void MacDot11nHandleNonQosData(
    Node* node,
    MacDataDot11* dot11,
    Message* msg)
{
    bool isMyAddr = false;
    DOT11_FrameHdr* hdr = reinterpret_cast<DOT11_FrameHdr*>
    ...
    Message* msg)
{
...

    {
        ...
        {
            headerSize = rxVector.length - msgSize;
        }
        else
        {
...
/************** Interoperability feature *********START*******************/
                MacDot11InitNeighborHtInfo(
                    node, 
                    dot11,
                    hdr2->sourceAddr,
                    (STA_Capability_Mode)rxVector.phyType);
/************** Interoperability feature *********END*******************/
    ...
    else
    {
        ...
        switch (hdr->frameType)
        {
              ...
              case DOT11_DATA:
                  {
                      MacDot11nHandleNonQosData(node, dot11, msg);
                      break;
                  }
...
/*** UAPSD *********************************START*******************/
                case DOT11_QOS_NULL:
                {
                    ...
                    if (isMyframe)
                    {
                        ...
                        assert(dot11->isUapsdEnable);
                        MacDot11StationTransmitAck(node,
                            dot11, sourceNodeAddress);
                        if (MacDot11IsAp(dot11))
                        {
                            DOT11_ApStationListItem* stationItem =
                            MacDot11ApStationListGetItemWithGivenAddress(
                            ...
                            if (stationItem)
                            {
                                ...
                                if (!stationItem->data->uapsdInfo.spInProgress)
                                {
                                    ...
                                    stationItem->data->uapsdInfo
                                        .spInProgress = true;
                                    assert(stationItem->data->uapsdInfo
                                        .noOfPacketsSent == 0);
                                }
                            }
                        }
                        if (MacDot11IsBssStation(dot11))
                        {
                            // Station received QOS NULL from AP.
                            if (DEBUG_UAPSD)
                            {
                                printf("NodeId %d Time %" TYPES_64BITFMT "d QOS NULL got"
                                    " from AP. T->0 \n",
                                    node->nodeId, node->getNodeTime());
                            }
                            dot11->uapsdInfo.uApsdTriggerSent = false;
                   ...
                   MacDataDot11* dot11)
{
    ...
    clocktype inputBufferTimerExpireInterval = 0;
    IO_ReadTime(
        node,
        node->nodeId,
        dot11->myMacData->interfaceIndex,
    ...
    clocktype amsduBufferTimerExpireInterval = 0;
    IO_ReadTime(
        node,
        node->nodeId,
        dot11->myMacData->interfaceIndex,
    ...
    BOOL enableImmediateBAAgreement = FALSE;
    IO_ReadBool(
        node,
        node->nodeId,
        dot11->myMacData->interfaceIndex,
        nodeInput,
        "MAC-DOT11-ENABLE-DATA-BURSTING",
        &wasFound,
    ...
    if (wasFound)
    {
        if (enableImmediateBAAgreement == TRUE)
        {
            ...
            BOOL enableDelayedBAAgreement = FALSE;
            IO_ReadBool(
                node,
                node->nodeId,
                dot11->myMacData->interfaceIndex,
                nodeInput,
                "MAC-DOT11-ENABLE-DELAYED-BLOCK-ACK",
                &wasFound,
            ...
            clocktype blockAckPolicyTimeout = 0;
            IO_ReadTime(
                node,
                node->nodeId,
                dot11->myMacData->interfaceIndex,
                nodeInput,
                "MAC-DOT11-BLOCK-ACK-POLICY-TIMEOUT",
                &wasFound,
...
{
    int i = 0;
    for (i=0; i < mc(dot11)->ac()->getNumAc(); i++)
    {
        OutputBuffer *outputBuffer = getAc(dot11 ,i).m_outputBuffer;
        ...
        while (keyItr != outputBuffer->OutputBufferMap.end())
        {
         ...
         getAc(dot11, i).m_isACHasPacket = FALSE;
...
/*** UAPSD *********************************START*******************/
        if (MacDot11IsAp(dot11) && dot11->isUapsdEnable)
        {
            ...
            if (DEBUG_UAPSD)
            {
                printf("NodeId %d Time %" TYPES_64BITFMT "d Sending Broadcast \n",
                    node->nodeId, node->getNodeTime());
            }
        }
/*** UAPSD *********************************END*******************/
                ...
                if (MacDot11IsAp(dot11) && dot11->isUapsdEnable)
                {
                    ...
                    if (stationItem) {
                                ...
                                == DOT11_QOS_DATA)
                        {
                            ...
                            assert(stationItem->data->uapsdInfo.spInProgress);
                            if (mc(dot11)->ac()->packetsForThisRA(
                                dot11, dot11->dot11TxFrameInfo->RA) > 1)
                            {
                                if (stationItem->data->uapsdInfo.useTxop)
                                {
                                    if (DEBUG_UAPSD)
                                    ...
                                    hdr->qoSControl.EOSP = 0;
                                }
                                else
                                {
                                    if (stationItem->data->
                                            uapsdInfo.noOfPacketsSent + 1
                                        < stationItem->data->
                                    uapsdInfo.noOfPacketsToBeSentInServicePeriod)
                                    {
                                        if (DEBUG_UAPSD)
                                        {
                                            printf("NodeId %d Time %" TYPES_64BITFMT "d"
...
/*** UAPSD *********************************END*******************/
        ...
        UInt64 fragThreshold = 0;
        if (dot11->isVHTEnable)
        {
            fragThreshold = DOT11AC_FRAG_THRESH;
        }
        else
        {
        ...
        else
        {
                ...
                dot11->rtsThreshold)
            {
                ...
                if (useRifs)
                {
                   ...
                   mc(dot11)->updateStats("IFS", "", "RIFS");
                }
                else
                {
                   transmitDelay += dot11->sifs;
                   mc(dot11)->updateStats("IFS", "", "SIFS");
                }
            }
            else
            {
            ...
            if (DEBUG_AMPDU && dot11->dot11TxFrameInfo->isAMPDU)
            {
                printf("%" TYPES_64BITFMT "d Node %d Sending Ampdu"
                    ...
                    "d gi %s ChBandwidth %d \n",
                    node->getNodeTime(),
                    node->nodeId,
                    tempTxVector.mcs,
                    tempTxVector.length,
                    tempTxVector.gi ? "Gi_Short":"Gi_Long",
                    tempTxVector.chBwdth);
            }
            ...
            if (DEBUG_UAPSD)
            {
                ...
                MacDot11MacAddressToStr(dstAddress,
                    &(dot11->dot11TxFrameInfo->RA));
                if (dot11->dot11TxFrameInfo->frameType == DOT11_QOS_NULL)
                {
                    printf("NodeId %d Time %" TYPES_64BITFMT "d Sending Trigger to %s. \n",
                        node->nodeId, node->getNodeTime(), dstAddress);
                }
                else
                {
                    printf("NodeId %d Time %" TYPES_64BITFMT "d Sending QOS MPDU to %s. \n", 
                        node->nodeId, node->getNodeTime(), dstAddress);
                }
            }
            ...
            //{
#ifdef PRINT_DEBUG
                    clocktype dur = PHY_GetTransmissionDuration(
                            node, dot11->myMacData->phyNumber,
                            txVector);
                    printf("%" TYPES_64BITFMT "d Node %d Sending Mpdu of type %d"
                    " with mcs %d length %" TYPES_SIZEOFMFT 
                    "d gi %s ChBandwidth %d Format %d Mode %d Modulation %s Duration %lld\n",
                    node->getNodeTime(),
                    node->nodeId,
                    dot11->dot11TxFrameInfo->frameType,
                    tempTxVector.mcs,
                    ...
                    tempTxVector.chBwdth,
                    tempTxVector.format,
                    tempTxVector.phyType,
                    tempTxVector.nonHTMod? "DUP-OFDM":"OFDM",
                    dur);
#endif
            //}
...
/*** UAPSD *********************************START*******************/
/// \brief Implementation of MacDot11ApsdSleepIfNoData function
///
/// This function checks if an interface can change state to sleep mode and
/// activates the sleep mode if possible
///
/// \param node Pointer to node data structure
/// \param dot11 Pointer to Dot11 data structure
void MacDot11ApsdSleepIfNoData(
    Node* node,
    MacDataDot11* dot11)
{
    if ((dot11->apHasBroadcastDataToSend)
        || dot11->apHasUnicastDataToSend
        || getAc(dot11, 0).isAcHasPacket()
        || getAc(dot11, 1).isAcHasPacket()
        || getAc(dot11, 2).isAcHasPacket()
        || getAc(dot11, 3).isAcHasPacket()
        || dot11->uapsdInfo.spInProgress
        || dot11->uapsdInfo.uApsdTriggerSent)
    {
        // Don't go to sleep
        if (DEBUG_UAPSD)
        {
            printf("NodeId %d Time %" TYPES_64BITFMT "d Unable to sleep. B=%d U=%d\n",
                node->nodeId, node->getNodeTime(),
                dot11->apHasBroadcastDataToSend,
                dot11->apHasUnicastDataToSend);
        }
    }
    else
    {
            ...
            && !dot11->beaconsMissed)
        {
            if (MacDot11StartTimerForNextAwakePeriod(node, dot11))
            {
                if (DEBUG_UAPSD)
                {
                    printf("NodeId %d Time %" TYPES_64BITFMT "d Going to sleep\n",
                        node->nodeId, node->getNodeTime());
                }
            ...
            else
            {
                if (DEBUG_UAPSD)
                {
                    printf("NodeId %d Time %" TYPES_64BITFMT "d Unable to sleep due to"
                        " Next Awake Period\n",
                        node->nodeId, node->getNodeTime());
                }
            }
        ...
        else
        {
            if (DEBUG_UAPSD)
            {
                printf("NodeId %d Time %" TYPES_64BITFMT "d Unable to sleep\n",
                    node->nodeId, node->getNodeTime());
            }
        }
    }
...
/*** UAPSD *********************************START*******************/
        ...
        {
            dot11->uapsdInfo.uApsdTriggerSent = false;
            dot11->uapsdInfo.uApsdEnabledAp = false;
            dot11->uapsdInfo.spInProgress = false;
        }
/*** UAPSD *********************************END*******************/
...
/*** UAPSD *********************************START*******************/
    if (DEBUG_UAPSD)
    {
        printf("NodeId %d Time %" TYPES_64BITFMT "d CFOP B=%d U=%d T=%d\n",
            node->nodeId, node->getNodeTime(),
            dot11->apHasBroadcastDataToSend,
            dot11->apHasUnicastDataToSend,
            dot11->uapsdInfo.uApsdTriggerSent);
    ...
    if (MacDot11IsBssStation(dot11)&& MacDot11IsStationJoined(dot11)
        && dot11->isUapsdEnable && dot11->uapsdInfo.uApsdEnabledAp
        && !dot11->ScanStarted
        /*&& !dot11->uapsdInfo.uApsdTriggerSent*/)
    {
        if (!dot11->apHasBroadcastDataToSend
            ...
            && !dot11->uapsdInfo.uApsdTriggerSent)
        {
            ...
            if (DEBUG_UAPSD)
            {
                printf("NodeId %d Time %" TYPES_64BITFMT "d Preparing to send trigger\n",
                    node->nodeId, node->getNodeTime());
            }
            MacDot11StationTransmitTriggerFrame(node,
                                                dot11,
...
/*** UAPSD *********************************START*******************/
    if (MacDot11IsAp(dot11)
        && dot11->isUapsdEnable
        && !mc(dot11)->ac()->packetsForThisRA(dot11, ANY_MAC802))
    {
        // No more broadcast packets lined up to be sent
                              ...
                              BOOL reCalculating = FALSE)
{
    if (keyItr->second.numPackets > BLOCK_ACK_POLICY_THRESHOLD)
    {
                ...
                && (*listItr)->state == STATE_FRESH)
        {
            ...
            node, dot11->myMacData->phyNumber,
            tempTxVector);
            if (duration < getAc(dot11, acIndex).m_TXOPLimit)
            {
                numPkts++;
                listItr++;
            }
                                  ...
                                  MapValue>::iterator &keyItr)
{
    ...
    getAc(dot11, acIndex).m_frameInfo = (*listItr);
    getAc(dot11, acIndex).m_frameToSend = (*listItr)->msg;
    getAc(dot11, acIndex).m_priority = keyItr->first.second;
    getAc(dot11, acIndex).m_currentNextHopAddress = keyItr->first.first;
    getAc(dot11, acIndex).m_totalNoOfthisTypeFrameQueued++;
    getAc(dot11, acIndex).m_totalNoOfthisTypeFrameQueuedUnderBAA++;
}
...
/************** Interoperability feature *********START*******************/
            ...
            clocktype sifs = 0;
            DOT11_NeighborHtInfo* htInfo
                = MacDot11GetNeighHtInfo(
                dot11->htInfoList, dot11->dot11TxFrameInfo->RA);
            if (htInfo && htInfo->neighbourMode == k_Dot11b)
            {
                slotime = DOT11_802_11b_SLOT_TIME;
                sifs = DOT11_802_11b_SIFS;
            }
            else
            {
                slotime = dot11->slotTime;
                sifs = dot11->sifs;
            }
...
/************** Interoperability feature *********END*******************/
#ifdef PRINT_DEBUG
            printf("%" TYPES_64BITFMT "d Node %d Hold for ack timer type %d"
                    " with mcs %d length %" TYPES_SIZEOFMFT 
                    "d gi %s ChBandwidth %d Format %d Mode %d Modulation %s Duration %lld\n",
                    node->getNodeTime(),
                    node->nodeId,
                    ...
                    txVector.mcs,
                    txVector.length,
                    txVector.gi ? "Gi_Short":"Gi_Long",
                    txVector.chBwdth,
                    txVector.format,
                    txVector.phyType,
                    txVector.nonHTMod? "DUP-OFDM":"OFDM",
                    holdForAck);
#endif
        }
        else
                                           ...
                                           BOOL reCalculating)
{
        ...
        < keyItr->second.blockAckVrbls->numPktsToBeSentInCurrentSession)
    {
        ...
        duration += PHY_GetTransmissionDuration(node,
                                                dot11->myMacData->phyNumber,
                                                tempTxVector);
        while (duration < getAc(dot11, acIndex).m_TXOPLimit
                        ...
                        int acIndex)
{
    ...
    ERROR_Assert(getAc(dot11, acIndex).m_isACHasPacket,
