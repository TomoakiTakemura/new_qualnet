...
//---------------------------Power-Save-Mode-End-Updates-----------------//
    ...
    memset(aStation, 0, sizeof(DOT11_ApStation));
...
//---------------------------Power-Save-Mode-Updates---------------------//
    ...
    {
        queue = new Queue(node, "FIFO", dot11->unicastQueueSize);
...
/*** UAPSD *********************************START*******************/
                (dot11->isUapsdEnable && next->data->uapsdInfo.uApsdEnabled
                    && (mc(dot11)->ac()->packetsForThisRA(
                        dot11, next->data->macAddr))))
/*** UAPSD *********************************END*******************/
            {
                // The code is executed for legacy power save mode as well as
                // for UAPSD. In case of UAPSD, the following code is executed
                // when both AP and station is UAPSD enabled
                ...
                if (DEBUG_UAPSD)
                {
                    ...
                    {
                        printf("NodeId %d Time %" TYPES_64BITFMT "d DTIM Beacon."
                            " Setting U=1\n",
                            node->nodeId, node->getNodeTime());
                    }
                    else
                    {
                        printf("NodeId %d Time %" TYPES_64BITFMT "d Beacon."
                            " Setting U=1\n",
                            node->nodeId, node->getNodeTime());
                    }
                }
    ...
    if (!MacDot11ApStationListIsEmpty(node, dot11, ap->apStationList)) {
        ...
        do{
            ...
            if ((data->isSTAInPowerSaveMode == TRUE)
                ...
                getAc(dot11, dot11->currentACIndex).m_frameToSend = NULL;
                getAc(dot11, dot11->currentACIndex).m_BO = 0;
                getAc(dot11, dot11->currentACIndex).m_currentNextHopAddress = INVALID_802ADDRESS;
                break;
            }// end of if
            next = MacDot11ApStationListGetNextItem(node, dot11, next);
        } while (next != first);
    ...
    unsigned char* beacon)
{
   ...
   if (DEBUG_UAPSD)
   {
        printf("NodeId %d Time %" TYPES_64BITFMT "d Creating beacon. \n",
            node->nodeId, node->getNodeTime());
   }
...
/*** UAPSD *********************************END*******************/
            ...
            if (DEBUG_UAPSD)
            {
                printf("NodeId %d Time %" TYPES_64BITFMT "d Setting B=1\n",
                    node->nodeId, node->getNodeTime());
            }
        }
...
void MacDot11AddVHTOperation(Node* node,
            MacDataDot11* dot11,
            DOT11_Ie* element,
            DOT11_MacFrame* txFrame,
            int* txFrameSize)
{
    element->data = (unsigned char*)txFrame + *txFrameSize;
    element->length = 0;
    DOT11_IeHdr ieHdr(DOT11_VHT_OPERATION_ELEMENT_ID);
    Dot11_AppendToElement(element, ieHdr);

    VHT_OperationElement vhtOperationElement;
    memset(&vhtOperationElement, 0, sizeof(VHT_OperationElement));

    ChBandwidth chBwdth =
        (ChBandwidth)Phy802_11GetOperationChBwdth(
                            node->phyData[dot11->myMacData->phyNumber]);
    if (chBwdth <= CHBWDTH_40MHZ)
    {
        vhtOperationElement.m_operationInfo.m_chWidth = 0;
        ERROR_Assert(FALSE,
            "802.11ac supports channel bandwidth of 80 MHz (mandatory),"
            "and channel bandwidth of 160 MHz (optional)");
    }
    else if (chBwdth == CHBWDTH_80MHZ)
    {
        vhtOperationElement.m_operationInfo.m_chWidth = 1;
    }
    else if (chBwdth == CHBWDTH_160MHZ)
    {
        vhtOperationElement.m_operationInfo.m_chWidth = 2;
    }
    else
    {
        //80+80 is currently not supported
        ERROR_ReportError("80+80 is currently not supported");
    }

    vhtOperationElement.m_operationInfo.m_chCenterFreqSeg0
        = PHY802_11GetChIdxHighestBwdth(node->phyData[dot11->myMacData->phyNumber]);

    memcpy(element->data + element->length,
           &vhtOperationElement,
           sizeof(VHT_OperationElement));
    element->length += sizeof(VHT_OperationElement) ;
    Dot11_StoreIeLength(element);
    *txFrameSize += element->length;
...
void MacDot11AddVHTCapability(Node* node,
            MacDataDot11* dot11,
            DOT11_Ie* element,
            DOT11_MacFrame* txFrame,
            int* txFrameSize)
{
    element->data = (unsigned char*)txFrame + *txFrameSize;
    element->length = 0;
    DOT11_IeHdr ieHdr(DOT11N_VHT_CAPABILITIES_ELEMENT_ID);
    Dot11_AppendToElement(element, ieHdr);

    VHT_CapabilitiesElement vhtCapabilitiesElement;
    memset(&vhtCapabilitiesElement, 0, sizeof(VHT_CapabilitiesElement));
    vhtCapabilitiesElement.m_capabilitiesInfo.m_maxMpduLen
        = dot11->vhtMaxMpduSize;
    vhtCapabilitiesElement.m_capabilitiesInfo.m_maxAmpduLenExponent
        = dot11->vhtApmduLengthExponent;
    vhtCapabilitiesElement.m_capabilitiesInfo.m_vhtTxopPs
        = dot11->isVHTTxopPSEnable;
    ChBandwidth chBwdth =
    (ChBandwidth)Phy802_11GetOperationChBwdth(
                        node->phyData[dot11->myMacData->phyNumber]);
    vhtCapabilitiesElement.m_capabilitiesInfo.m_chWidth
         = (chBwdth == CHBWDTH_160MHZ? 1 : 0);
    vhtCapabilitiesElement.m_capabilitiesInfo.m_shortGi80Mhz
        = Phy802_11ShortGiCapable(node->phyData[dot11->myMacData->phyNumber]);
    vhtCapabilitiesElement.m_capabilitiesInfo.m_shortGi160Mhz
        = Phy802_11ShortGiCapable(node->phyData[dot11->myMacData->phyNumber]);
    vhtCapabilitiesElement.m_mcsNssSet
        = dot11->txVectorForHighestDataRate.mcs;

    memcpy(element->data + element->length,
           &vhtCapabilitiesElement,
           sizeof(VHT_CapabilitiesElement));
    element->length += sizeof(VHT_CapabilitiesElement) ;
    Dot11_StoreIeLength(element);
    *txFrameSize += element->length;
...
/************** Interoperability feature *********START*******************/        
        if (Phy802_11GetNumAtnaElems(
            node->phyData[dot11->myMacData->phyNumber]) <= 4)
        {
...
/************** Interoperability feature *********END*******************/
        htCapabilityElement.htCapabilitiesInfo.htGreenfield = FALSE;
        htCapabilityElement.htCapabilitiesInfo.shortGiFor20Mhz =
        Phy802_11ShortGiCapable(node->phyData[dot11->myMacData->phyNumber]);

        // 80MHz is mandatory for an VHT STA
...
/************** Interoperability feature *********END*******************/
    ...
    htCapabilityElement.htCapabilitiesInfo.smPowerSave = dot11->smpsMode;
...
/*** UAPSD *********************************START*******************/
        // Add APSD capability Info
        DOT11e_CapabilityInfo* capInfo
            = (DOT11e_CapabilityInfo*)(&beacon->capability);
        if (MacDot11IsQoSEnabled(node, dot11) && dot11->isUapsdEnable)
        {
            capInfo->APSD = 1;
        }
        else
...
/*** UAPSD *********************************END*******************/
        ...
        strcpy(beacon->ssid, dot11->stationMIB->dot11DesiredSSID);
       ...
       couldTransmit = TRUE;
        ...
        switch (dot11->state) {
            ...
            case DOT11_CONTENDING:
                ...
                if (mc(dot11)->ac()->CheckStateIfDifsOrBo(dot11) == 
                                                         k_State_BO)
                {
                    dot11->BO = 0;
                    MacDot11DTIMupdate(node, dot11);
                ...
                else if (mc(dot11)->ac()->CheckStateIfDifsOrBo(dot11) == 
                                                              k_State_DIFS)
                {
                    MacDot11StationSetState(node, dot11, DOT11_S_WFBEACON);
                    MacDot11DTIMupdate(node, dot11);
                    ...
                    if (MacDot11IsPC(dot11))
                    {
                        ...
                        {  // Send beacon using DIFS and Backoff
                            MacDot11BuiildInstantBeaconFrame(node, dot11);
                        }
                    }//if
                    // If AP then Always Send by DIFS and Backoff
...
/// \brief Function to check and enable protection mechanism at AP
/// during interoperability.
///
/// This API enables RTS/CTS mechanism when an AP detects its associated
/// stations have different capabilities.
///
/// \param node  Pointer to the node
/// \param dot11  Pointer to the MacData structure
///
/// \return N/A
void MacDot11UseProtectionMechanism(
    Node* node,
    MacDataDot11* dot11)
{
    DOT11_ApVars* ap = dot11->apVars;
    ...
    if (!MacDot11ApStationListIsEmpty(node, dot11, ap->apStationList))
    {
        first = MacDot11ApStationListGetFirst(node, dot11);
        DOT11_NeighborHtInfo* htInfo
            = MacDot11GetNeighHtInfo(
                dot11->htInfoList, first->data->macAddr);
        assert(htInfo);
        STA_Capability_Mode nm = htInfo->neighbourMode;
        next = MacDot11ApStationListGetNextItem(node, dot11, first);
        while (next != first)
        {
            htInfo
                = MacDot11GetNeighHtInfo(
                    dot11->htInfoList, next->data->macAddr);
            assert(htInfo);
            if (htInfo->neighbourMode != nm)
            {
                // Enable RTS/CTS mechanism if AP detects that it's associated
                // stations have different capabilities.
                dot11->rtsThreshold = 0;
                dot11->htP = k_NonHTMixedMode;
                break;
            }
            next = MacDot11ApStationListGetNextItem(node, dot11, next);
        }
