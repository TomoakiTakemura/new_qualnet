...
#ifdef PRINT_DEBUG
        printf("%" TYPES_64BITFMT "d Node %d Hold for cts timer"
...
#ifdef PRINT_DEBUG
    printf("%" TYPES_64BITFMT "d Node %d Hold for ack timer"
                " Datarate %d Duration %lld\n",
                 node->getNodeTime(),
                 node->nodeId,
                  PHY_GetTxDataRateType(node, dot11->myMacData->phyNumber),
    ...
    int acIndex)
{
    ...
    UseShortPacketRetryCounter =
        (MESSAGE_ReturnPacketSize(ac.m_frameToSend) <=
          dot11->rtsThreshold);
...
//--------------------Power-Save-Mode not supported in 802.11e-------------//
            ...
            if (ac.m_frameInfo != NULL)
            {
                if (ac.m_frameToSend == ac.m_frameInfo->msg)
                {
                    ac.m_frameInfo = NULL;
    ...
    int acIndex)
{
    ...
    {
/*** UAPSD *********************************START*******************/
        if ((ac.m_frameInfo->frameType >= DOT11_ASSOC_REQ
            && ac.m_frameInfo->frameType <= DOT11_ACTION ||
            ac.m_frameInfo->frameType == DOT11_PS_POLL ||
            ac.m_frameInfo->frameType == DOT11_QOS_NULL)&&
            (ac.m_frameToSend == ac.m_frameInfo->msg))
/*** UAPSD *********************************END*******************/
        {
            return TRUE;
        }
           ...
           if (dot11->isHTEnable)
          {
           ...
           if ((packetSize > dot11->rtsThreshold)&&
                ...
                MacDot11StationTransmitRTSFrame(node, dot11);
                  ...
                  if (dot11->isHTEnable)
                 {
                     MacDot11nSendMpdu(node, dot11, FALSE, TRUE);
                 }
                 else
                 {
                MacDot11StationTransmitDataFrame(node, dot11);
    ...
    MacDataDot11* dot11)
{
    ...
    switch (dot11->state) {
        ...
        case DOT11_CONTENDING:
            ...
            else if ((mc(dot11)->ac()->CheckStateIfDifsOrBo(dot11) == k_State_DIFS))
            {
                MacDot11StationSetState(node, dot11, DOT11_S_WFIBSSJITTER);
                clocktype delay = (clocktype)
                    (RANDOM_nrand(dot11->seed) % DOT11_BEACON_JITTER);
...
static
void Dot11_ParseVHTCapabilitiesElement(
    Node* node,
    DOT11_Ie* element,
    VHT_CapabilitiesElement* vhtCapabilityElement)
{
    ...
    Dot11_ReadFromElement(element, &ieOffset, &ieHdr,length);
    ERROR_Assert(
        ieHdr.id == DOT11N_VHT_CAPABILITIES_ELEMENT_ID,
        "Dot11_ParseCfsetIdElement: "
        "IE ID does not match.\n");
    element->length = ieHdr.length + DOT11_IE_HDR_LENGTH;

    ERROR_Assert(
        ieHdr.length == sizeof(VHT_CapabilitiesElement),
        "Dot11_ParseCfsetIdElement: "
        "CFParameterSet length is not equal.\n");
    memset(vhtCapabilityElement, 0, sizeof(VHT_CapabilitiesElement));
    memcpy(vhtCapabilityElement, element->data + ieOffset, ieHdr.length);
    ieOffset += ieHdr.length;
    ...
    int sizeofPacket)
{
    ...
    while (ieHdr.id != DOT11N_VHT_CAPABILITIES_ELEMENT_ID)
    {
    ...
    if (ieHdr.id == DOT11N_VHT_CAPABILITIES_ELEMENT_ID)
    {
        Dot11s_AssignToElement(&element, rxFrame, &offset, rxFrameSize);
        Dot11_ParseVHTCapabilitiesElement(node, &element, vhtCapabilityElement);
        isFound = TRUE;
    }
       return isFound;
}
...
static
void Dot11_ParseVHTOperationElement(
    Node* node,
    DOT11_Ie* element,
    VHT_OperationElement* vhtOperationElement)
{
    ...
    Dot11_ReadFromElement(element, &ieOffset, &ieHdr,length);
    ERROR_Assert(
        ieHdr.id == DOT11_VHT_OPERATION_ELEMENT_ID,
        "Dot11_ParseCfsetIdElement: "
        "IE ID does not match.\n");
    element->length = ieHdr.length + DOT11_IE_HDR_LENGTH;

    ERROR_Assert(
        ieHdr.length == sizeof(VHT_OperationElement),
        "Dot11_ParseCfsetIdElement: "
        "CFParameterSet length is not equal.\n");
    memset(vhtOperationElement, 0, sizeof(VHT_OperationElement));
    memcpy(vhtOperationElement, element->data + ieOffset, ieHdr.length);
    ieOffset += ieHdr.length;
    ...
    int sizeofPacket)
{
    ...
    // The first expected IE is cfSet
    while (ieHdr.id != DOT11_VHT_OPERATION_ELEMENT_ID)
    {
        offset += ieHdr.length + 2;
        if (offset >= rxFrameSize)
        {
    ...
    if (ieHdr.id == DOT11_VHT_OPERATION_ELEMENT_ID)
    {
        Dot11s_AssignToElement(&element, rxFrame, &offset, rxFrameSize);
        Dot11_ParseVHTOperationElement(node, &element, vhtOperationElement);
        isFound = TRUE;
    }
       return isFound;
}
...
/*** UAPSD *********************************START*******************/
/// \brief Implementation of Dot11_ParseEdcaParameterSet function
///
/// This function parses the edcaParameterSet
///
/// \param node Pointer to node data structure
/// \param element Pointer to information element
/// \param edcaParameterSet Pointer to the edcaParameterSet
    ...
    DOT11e_EDCAParameterSet* edcaParameterSet)
{
    ...
    Dot11_ReadFromElement(element, &ieOffset, &ieHdr,length);
    ERROR_Assert(
        ieHdr.id == DOT11_EDCA_PARAMETER_SET_ID,
        "Dot11_ParseCfsetIdElement: "
        "IE ID does not match.\n");
    element->length = ieHdr.length + DOT11_IE_HDR_LENGTH;
    ERROR_Assert(
        ieHdr.length == sizeof(DOT11e_EDCAParameterSet),
        "Dot11_ParseCfsetIdElement: "
        "CFParameterSet length is not equal.\n");
    memset(edcaParameterSet, 0, sizeof(DOT11e_EDCAParameterSet));
    memcpy(edcaParameterSet, element->data + ieOffset, ieHdr.length);
    ieOffset += ieHdr.length;
    ERROR_Assert(ieOffset == element->length,
        "Dot11_ParseCfsetIdElement: all fields not parsed.\n") ;
}

/// \brief Implementation of Dot11_GetEdcaParameterSet function
///
/// This function searches the EDCA parameter set in the received
/// message and returns its location.
///
/// \param node Pointer to node data structure
/// \param dot11 Pointer to Dot11 data structure
/// \param msg Received message
/// \param edcaParameterSet Location which points to the paramater set
/// \param sizeofPacket Size of the message
    ...
    int sizeofPacket)
{
    ...
    // The first expected IE is cfSet
    while (ieHdr.id != DOT11_EDCA_PARAMETER_SET_ID)
    {
        offset += ieHdr.length + 2;
        if (offset >= rxFrameSize)
        {
    ...
    if (ieHdr.id == DOT11_EDCA_PARAMETER_SET_ID)
    {
        Dot11s_AssignToElement(&element, rxFrame, &offset, rxFrameSize);
        Dot11_ParseEdcaParameterSet(node, &element, edcaParameterSet);
        isFound = TRUE;
    }
    return isFound;
}
...
//---------------------------Power-Save-Mode-End-Updates-----------------//
    ...
    if (dot11->isHTEnable)
    {
        ...
        if (isPresent)
        {
            if (operElem.htP)
            {
                // Enable use of RTS/CTS mechanism
    ...
    BOOL backoff)
{
    ...
    {
        MacDot11ManagementStartJoin(node,
                                    dot11);
