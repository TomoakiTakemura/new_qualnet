...
#define DOT11a_MAX_MCS_SINGLE_SS 7
#define DOT11b_MAX_MCS_SINGLE_SS 3
#define DOT11n_MAX_MCS_SINGLE_SS 7
#define DOT11ac_MAX_MCS_SINGLE_SS 9
...
/*** UAPSD *********************************START*******************/
#define DEBUG_UAPSD 0
#define MAX_802_11_PRIORITY 7
/*** UAPSD *********************************END*******************/
...
///** Protection mode
// *  The enum types for different HT protection modes
// */
...
/*** UAPSD *********************************START*******************/
struct Dot11_UApsdInfo
{
    Mac802Address       address;
    bool                isAp;
    bool                uApsdEnabled;
    Int32               noOfPacketsToBeSentInServicePeriod;
    Int32               noOfPacketsSent;
    clocktype           servicePeriodEndTime;
    bool                spInProgress;
    bool                useTxop;
};
/*** UAPSD *********************************END*******************/
...
struct DOT11_NeighborHtInfo
{
    ...
    DOT11_NeighborHtInfo(const Mac802Address& addr,
                         ...
                         STA_Capability_Mode  neighbourStaMode = k_Dot11n)
        : address(addr), isAp(ap), maxMcsIdx(maxMcs),
          stbcCapable(stbc), shortGiCapable(shortGi),
          isFortyMhzIntolerant(FortyMhzIntolerant),
          isOneSixtyMhzIntolerent(OneSixtyMhzIntolerent), neighbourMode(neighbourStaMode)
    {
        curMcsIdx = -1;
    }
};
...
/*********HT END****************************************************/
    ...
    VHTInfo vhtInfo;
    STA_Capability_Mode capabilityMode;
/*** UAPSD *********************************START*******************/
    Dot11_UApsdInfo uapsdInfo;
/*** UAPSD *********************************END*******************/
} DOT11_ApStation;
...
{
    bool uApsdEnabledAp;
    bool uApsdTriggerSent;
    bool spInProgress;
    int maxServicePeriodLength;
    int numTriggerSent;
    int numTriggerReceived;
};
/*** UAPSD *********************************END*******************/
...
//---------------------------Power-Save-Mode-End-Updates-----------------//
    ...
    BOOL isVHTTxopPSEnable;
    char smpsMode;
    void* macController;
    STA_Capability_Mode myPhyType;

    // duration of the guard interval (GI) portion of the sync interval
    clocktype guardIntervalDuration;

    // duration of the CCH interval portion of the sync
    // interval (this includes 1 GI)
    clocktype cchIntervalDuration;

    // duration of the SCH interval portion of the sync
    // interval (this includes 1 GI)
    clocktype schIntervalDuration;

    // whether the radio is capable of listening to both
    // the SCH and the CCH simultaneously
    BOOL dualBand;

    clocktype guardIntervalStartTime;
    clocktype guardIntervalEndTime;
};
...
MAC_PHY_TxRxVector MacDot11acSetTxVector(
    Node* node,
    MacDataDot11* dot11,
    unsigned int pktLength,
    DOT11_MacFrameType frameType,
...
/// \brief Function to get count of antenna elements
///
/// This API returns the count of antenna elements based on mcs for a 11n
/// enabled node
///
/// \param maxMcs  mcs value
///
/// \return Count of antenna elements
int MacDot11nGetAtnaElmts (int maxMcs);


/// \brief Function to get count of antenna elements
///
/// This API returns the count of antenna elements based on mcs for a 11ac
/// enabled node
///
/// \param maxMcs  mcs value
///
...
/// \brief Function to adjust maximum mcs value
///
/// This API adjusts maximum mcs value based on neighbor's mode
///
/// \param node  Pointer to the node
/// \param dot11  Pointer to the MacData structure
/// \param nm  Neighbor mode
/// \param maxMcs  Maximum mcs value
///
/// \return Adjusted mcs value
int MacDot11GetNeighborMaxMcs(
    Node* node,
    MacDataDot11* dot11,
    STA_Capability_Mode nm,
    int maxMcs);

/// \brief Function to get minumum mcs value
///
/// This API gets the minumum mcs value based on neighbor's mode
///
/// \param dot11  Pointer to the MacData structure
/// \param nm  Neighbor mode
/// \param maxMcs  Maximum mcs value
///
/// \return Minimum mcs value
int MacDot11GetMinMcs(
    MacDataDot11* dot11,
    STA_Capability_Mode nm,
    int maxMcs);

/// \brief Function to process a frame
///
/// This API processes a received frame
///
/// \param node  Pointer to the node
/// \param dot11  Pointer to the MacData structure
...
/// \brief Function to initialize neighbor's Information
///
/// This API initializes neighbor's information
///
/// \param node  Pointer to the node
/// \param dot11  Pointer to the MacData structure
/// \param neighborAddr  Neighbor address
/// \param nm  Neighbor mode
///
/// \return N/A
