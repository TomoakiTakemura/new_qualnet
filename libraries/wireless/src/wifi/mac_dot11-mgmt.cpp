...
/*** UAPSD *********************************START*******************/
/// \brief Implementation of MacDot11AddEDCAParametersetToAssocReqorReassocReq
///        function.
///
/// This function add EDCA parameter set to (re)association request packets
///
/// \param node Pointer to the node
/// \param dot11 Pointer to Dot11 data structure
/// \param element Pointer to information element
/// \param txFrame Pointer to (re)association request frame
/// \param txFrameSize Pointer to frame size
static
void MacDot11AddEDCAParametersetToAssocReqorReassocReq(Node* node,
            MacDataDot11* dot11,
            DOT11_Ie* element,
            ...
            int* txFrameSize)
{
    ...
    {
        dot11->edcaParamSet.qosInfo.EDCFParameterSetUpdateCount = 15;

        switch (dot11->uapsdInfo.maxServicePeriodLength)
        {
            case 0:
            {
                dot11->edcaParamSet.qosInfo.QueueRequest = 0;
                dot11->edcaParamSet.qosInfo.TXOPRequest = 0;
                break;
            }
            case 1:
            {
                dot11->edcaParamSet.qosInfo.QueueRequest = 1;
                dot11->edcaParamSet.qosInfo.TXOPRequest = 0;
                break;
            }
            case 2:
            {
                dot11->edcaParamSet.qosInfo.QueueRequest = 0;
                dot11->edcaParamSet.qosInfo.TXOPRequest = 1;
                break;
            }
            case 3:
            {
...
/*** UAPSD *********************************START*******************/
        if (dot11->isUapsdEnable)
        {
            MacDot11AddEDCAParametersetToAssocReqorReassocReq(
                node, dot11, element,
...
/*** UAPSD *********************************START*******************/
    if (dot11->isUapsdEnable)
    {
        assocFrame->capInfo.APSD = 1;
    }
    else
    {
        assocFrame->capInfo.APSD = 0;
    }
/*** UAPSD *********************************END*******************/
    if (dot11->stationCheckTimerStarted == FALSE)
    {
...
/*** UAPSD *********************************START*******************/
            if (MacDot11IsQoSEnabled(node, dot11))
            {
                ...
                bool isPresent = Dot11_GetEdcaParameterSet(node,
                    dot11, msg, &edcaParameterSet,
                    sizeof(DOT11e_AssocReqFrame));
                if (isPresent)
                {
                    MacDot11UpdNeighborUApsdInfo(node,
                        dot11, reqFrame->hdr.sourceAddr, edcaParameterSet);
                }
            }
/*** UAPSD *********************************END*******************/
            ...
            if (dot11->isHTEnable && isHTSTA)
            {
                ...
                // Update the data rate entry
                 ...
                 neighborEntry->txVector.phyType = 
                        (UInt8)htCapabilityElement.htCapabilitiesInfo.rxStbc;

                 DOT11_NeighborHtInfo* neighborHtInfo = 
                     MacDot11GetNeighHtInfo(dot11->htInfoList, reqFrame->hdr.sourceAddr);
            ...
            if (dot11->isHTEnable)
            {
                DOT11_NeighborHtInfo* htInfo
                    = MacDot11GetNeighHtInfo(
                        dot11->htInfoList, reqFrame->hdr.sourceAddr);
                assert(htInfo);

                htInfo->maxMcsIdx =
                    MacDot11GetNeighborMaxMcs(node,
                        dot11,
                        htInfo->neighbourMode,
                        htInfo->maxMcsIdx);

                htInfo->curMcsIdx = MacDot11GetMinMcs(
                    dot11, htInfo->neighbourMode, htInfo->maxMcsIdx);

                // Validation checks
                if (htInfo->neighbourMode == k_Dot11a)
                {
                    assert(htInfo->maxMcsIdx == 7);
                }
                else if (htInfo->neighbourMode == k_Dot11b)
                {
                    assert(htInfo->maxMcsIdx == 3);
                }

                MacDot11UseProtectionMechanism(node, dot11);
            }
/************** Interoperability feature *********END*******************/
...
/*** UAPSD *********************************START*******************/
        if (dot11->isUapsdEnable)
        {
            reassocFrame->capInfo.APSD = 1;
        }
        else
        {
            reassocFrame->capInfo.APSD = 0;
        }
/*** UAPSD *********************************END*******************/
        if (dot11->stationCheckTimerStarted == FALSE)
        {
...
/************** Interoperability feature *********START*******************/
            if (dot11->isHTEnable)
            {
                ...
                if (htInfo->neighbourMode == k_Dot11a)
                {
                    assert(htInfo->maxMcsIdx == DOT11a_MAX_MCS_SINGLE_SS);
                }
                else if (htInfo->neighbourMode == k_Dot11b)
                {
                    assert(htInfo->maxMcsIdx == DOT11b_MAX_MCS_SINGLE_SS);
                }
                else
                {
...
/*** UAPSD *********************************START*******************/
                if (capInfo->APSD)
                {
                    apInfo->uApsdEnabledAp = TRUE;
                    dot11->uapsdInfo.uApsdEnabledAp = TRUE;
                }
                else
                {
                    apInfo->uApsdEnabledAp = FALSE;
...
/************** Interoperability feature *********START*******************/
                ...
                // 11ac received either 11a/b/n packet.
                if (isPresent)
                {
...
/************** Interoperability feature *********START*******************/
        if (dot11->isHTEnable)
        {
            DOT11_NeighborHtInfo* htInfo
                = MacDot11GetNeighHtInfo(
                    dot11->htInfoList, beacon->hdr.sourceAddr);
            assert(htInfo);
    ...
    DOT11_Frame* rxFrame)
{
    ...
    {  
            ...
            (respFrame->transSeqNo == 2) )
        {
            ...
            {
                MacDot11StationTransmitAck(
                    node,dot11,respFrame->hdr.sourceAddr);
            }
    ...
    if (MacDot11IsQoSEnabled(node, dot11) && isQSTA)
    {
      ...
      //  }
        ...
        if (dot11->isHTEnable)
        {
            ...
            if (stationItem)
            {
                // Currently rts/cts mechanism is been used in dynamic
                // spatial multiplexing power save mode (smps) to indicate
                // the sta to use multiple receive chain mode
                if (dot11->smpsMode == k_Dynamic &&
                    stationItem->data->staHtCapabilityElement.
                        htCapabilitiesInfo.smPowerSave == k_Dynamic)
                {
                    if (dot11->rtsThreshold > 0)
                    {
                        dot11->rtsThreshold = 0;
                    }
                }
                if (dot11->smpsMode == k_Static &&
                    stationItem->data->staHtCapabilityElement.
                        htCapabilitiesInfo.smPowerSave == k_Static)
                {
                    DOT11_NeighborHtInfo* htInfo = MacDot11GetNeighHtInfo(
                                                dot11->htInfoList,
                                                rxARqFrame->hdr.sourceAddr);
                    if (htInfo)
                    {
                        STA_Capability_Mode nMode = htInfo->neighbourMode;
                        if ( nMode == k_Dot11ac && dot11->isVHTEnable)
                        {
                            htInfo->maxMcsIdx = 9;
                            htInfo->curMcsIdx = 0;
                        }
...
/*** UAPSD *********************************START*******************/
            ...
            memcpy(&dot11->associatedAP->apCapInfo, capInfo,
                sizeof(DOT11e_CapabilityInfo));
            dot11->uapsdInfo.uApsdEnabledAp
                = dot11->associatedAP->apCapInfo.APSD;
/*** UAPSD *********************************END*******************/
            ...
            if (dot11->isHTEnable)
            {
                ...
                BOOL offset = FALSE;
                DOT11_HT_OperationElement operElem;
                BOOL isPresent = Dot11_GetHTOperation(
                                    node, dot11, rxFrame, &operElem,
...
/************** Interoperability feature *********END*******************/
                    ...
                    PHY802_11AllignSpectralBand(node->phyData[dot11->
                            myMacData->phyNumber],
                        operElem.secondaryChOffset,
                        (ChBandwidth)vhtCapabilityElement.
                            m_capabilitiesInfo.m_chWidth,
                        vhtOperationElement.m_operationInfo.
                            m_chCenterFreqSeg0);
#endif
                }
                else
                ...
                {
                    // Update the data rate entry!
                    ERROR_Assert(dot11->associatedAP
                        != NULL,"not associated");
                    DOT11_DataRateEntry* neighborEntry
                    ...
                    neighborEntry->txVector.mcs
                        = dot11->associatedAP->mcsIndex;
                    neighborEntry->txVector.format
                        = htCapabilityElement.
                                htCapabilitiesInfo.htGreenfield?
                                (Mode)MODE_HT_GF: (Mode)MODE_NON_HT;
                    neighborEntry->txVector.chBwdth
                        = htCapabilityElement.htCapabilitiesInfo
                            .fortyMhzIntolerent?
                            (ChBandwidth)CHBWDTH_20MHZ
                                : (ChBandwidth)CHBWDTH_40MHZ;
                    neighborEntry->txVector.phyType =
                            (UInt8)htCapabilityElement
                                .htCapabilitiesInfo.rxStbc;
...
/************** Interoperability feature *********START*******************/
                if (dot11->isHTEnable)
                {
                    DOT11_NeighborHtInfo* htInfo
                        = MacDot11GetNeighHtInfo(
                            dot11->htInfoList, respFrame->hdr.sourceAddr);
                    assert(htInfo);
...
/*** UAPSD *********************************START*******************/
            if (dot11->isUapsdEnable)
            {
                DOT11e_CapabilityInfo *capInfo =
                    (DOT11e_CapabilityInfo *)(&dot11erspFrame->capInfo);
                memcpy(&dot11->associatedAP->apCapInfo,
                    capInfo, sizeof(DOT11e_CapabilityInfo));
                dot11->uapsdInfo.uApsdEnabledAp
                    = dot11->associatedAP->apCapInfo.APSD;
/*** UAPSD *********************************END*******************/
            }
    ...
    MacDataDot11* dot11)
{
    ...
    MacDot11ManagementSetState(node,
                               dot11,
    ...
    BOOL printStatistics)
{
    ...
    if (!MacDot11IsAp(dot11))
    {
        ...
        if (wasFound)
        {
            ...
            else if (strcmp(retString, "DISABLED") != 0)
            {
    ...
    {
        char errString[MAX_STRING_LENGTH];
        sprintf(errString,"Node %d InterfaceIndex %d "
            " Parameter MAC-DOT11-STA-CHANNEL is deprecated."
            " Channels are now configured using spectral bands."
            " Please refer to Wi-Fi User Guide for more information",
...
/*** UAPSD *********************************START*******************/
/// \brief Implementation of MacDot11StationTransmitTriggerFrame function
///
/// This API creates and sends trigger frame (QOS NULL frame)
///
/// \param node  Pointer to the node
/// \param dot11  Pointer to dot11 data structure
...
///
/// \return N/A
void MacDot11StationTransmitTriggerFrame(
    Node* node,
    MacDataDot11* dot11,
    Mac802Address destAddr)
{
    Message* pktToPhy = NULL;
    DOT11e_FrameHdr* newHdr = NULL;
    pktToPhy = MESSAGE_Alloc(node, 0, 0, 0);
    MESSAGE_PacketAlloc(node,
                        pktToPhy,
                        sizeof(DOT11e_FrameHdr),
                        TRACE_DOT11);
    newHdr = (DOT11e_FrameHdr*)MESSAGE_ReturnPacket(pktToPhy);
    memset(newHdr, 0, sizeof(DOT11e_FrameHdr));
    newHdr->frameType = DOT11_QOS_NULL;
    ...
    memset(newframeInfo, 0, sizeof(DOT11_FrameInfo));
    newframeInfo->msg = pktToPhy;
    newframeInfo->frameType = DOT11_QOS_NULL;
    newframeInfo->RA = newHdr->destAddr;
    newframeInfo->TA = dot11->selfAddr;
    newframeInfo->DA = newHdr->destAddr;
    newframeInfo->SA = dot11->selfAddr;
