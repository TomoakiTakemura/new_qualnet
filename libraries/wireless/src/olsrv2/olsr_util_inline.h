 ...
 */
#ifndef _OLSR_UTIL_INLINE_
#define _OLSR_UTIL_INLINE_
            ...
            const union olsr_ip_addr* y)
{
    ...
    {
    if(memcmp(&x->v6.s6_addr_8, &y->v6.s6_addr_8, sizeof(in6_addr)) == 0)
    {
        return OLSR_TRUE;
    }
    else
              ...
              const union olsr_ip_addr* x)
{
    ...
    for(i=0; i<olsr->iface_num; i++)
    {
        if(memcmp(&x->v6.s6_addr_8, &olsr->iface_addr[i].v6.s6_addr_8, sizeof(in6_addr)) == 0)
        {
        return OLSR_TRUE;
        }
    }
