...
// Suppressing this define will cause the boost::geometry code to use 2D
#define BG_USE_3D

typedef int BgKey; // Contains index into m_buildings or m_foliage
typedef bg::model::point<double, 2, bg::cs::cartesian> BgPoint2D;
typedef bg::model::point<double, 3, bg::cs::cartesian> BgPoint3D;
// Note: using float instead of double did not make any significant 
// difference in performance, although it did clutter up the code
// with a lot of static_cast<float>() calls.
#ifdef BG_USE_3D
typedef BgPoint3D BgPoint;
#else
typedef BgPoint2D BgPoint;
#endif
typedef bg::model::box<BgPoint> BgBox;
typedef bg::model::linestring<BgPoint> BgLinestring;

// This is what gets put into the rtree: the bounding box coordinates and 
// the m_buildings/m_foliage index
typedef std::pair<BgBox, BgKey> RtreeValue;

// Simplify the type name for the rtree
const int MaxRtreeElements = 16;
typedef bgi::rtree<RtreeValue, bgi::rstar<MaxRtreeElements> > BgiRtree;

// The query result is a vector of RtreeValue pairs.
typedef std::vector<RtreeValue> BgiResult;

#include "UrbanCache.h"

struct TerrainPathData;
typedef boost::shared_ptr<TerrainPathData> TerrainPathDataPointer;
...
private:
    ...
    TerrainPathDataPointer m_pathData;
...
public:
    ...
    void setPathData(TerrainPathDataPointer& p) {m_pathData = p;}
    void print();
};
...
public:
    ...
    // LTE-34
    volatile bool m_rtreesInitialized;  // used for lazy init of trees
    BgiRtree m_rtreeBuildings; 
    // Foliage looks just like buildings. That is why in some places the term 
    // "obstruction" appears which can be either a building or foliage.
    BgiRtree m_rtreeFoliage;
    // LTE-34-end
    ...
    QualNetUrbanTerrainData(TerrainData* td) : UrbanTerrainData(td) {
        ...
        m_rtreesInitialized = false; // LTE-34
    }
    ~QualNetUrbanTerrainData();
    void createRtrees(); // LTE-34
    ...
    UrbanPathPropertiesPointer getUrbanPathProperties(const  Coordinates* c1,
            const  Coordinates* c2,
            double pathWidth,
...
private:
    ...
    TerrainPathDataPointer getFeaturesOnPath(
        BgLinestring& path,
        bool includeFoliage);
    ...
    /// the obstruction. If so, it outputs the intersection points and the
    /// FaceIndexes for two intersected faces.
    /// \param[in] coordinateSystemType indicates LATLONALT or other
    /// \param[in] source   coordinates of the line segment starting point
    /// \param[in] dest     coordinates of the line segment end point
    /// \param[in] CheckInsideOut   when true, checks for the source and dest being
    ///  inside or outside of the obstruction. Set this true for line of sight.
    /// \param[in] obstruction  points to the building or foliage object
    /// \param[out] intersection1   the point where the line intersects face 1
    /// \param[out] intersection2   the point where the line intersects face 2
    /// \param[out] face1   index of the first intersected face
    /// \param[out] face2   index of the second intersected face
    ...
    /* OBE -- replaced by boost::geometery intersection queries.
