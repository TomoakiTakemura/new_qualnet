...
// This routine returns the data structure associated with
// the PHY layer for the provided <node, phyNum> pair.
//
// \param        node  A pointer to the local node data structure
// \param        phyNum  The integer index of the interface associated with
//    this physical layer process.
//
// \return A pointer to the physical layer data for this
// <node,phyNum> pair.
...
//    this physical layer process.
//
// \return A pointer to the specific data structure
...
// This routine sets the data structure to be associated with
// the satellite RSV PHY process for the provided <node, phyNum>
// pair.
//
// \param        node  A pointer to the local node data structure
// \param        phyNum  The integer index of the interface associated with
//    this physical layer process.
// \param        state  A pointer to the data structure
...
// This routine reports additional status to the MAC
// process including a copy of the packet from the
// MAC layer above if available.
//
// \param        node  A pointer to the local node data structure
// \param        phyIndex  The integer index of the interface associated
//    with this physical layer process.
// \param        status  The new status of the physical
//    interface.
// \param        receiveDuration  The length of time that the
//    frame is expected to continue to be received until the end of
//    frame signal is event is processed.
// \param        potentialIncomingPacket  A pointer to a message
...
// This routine reports general status to the MAC
...
// This routine sets the status mode of the local physical
// layer process.
//
// \param        node  A pointer to the local node data structure
// \param        phyIndex  The integer index of the interface associated
//    with this physical layer process.
// \param        newmode  The new status of the physical
//    interface.
//
// \return This routine does not return a value.
...
// This routine returns the current status mode of the local
...
//    with this physical layer process.
//
// \return The current mode of the physical layer process.
...
// This routine initializes the static parameters of the
...
// The number of bits in an byte of data

#define BITS_PER_BYTE (8)
// This routine initializes the configurable parameters of the
...
// \param        phyIndex  The integer index of the interface associated
//    with this physical layer process.
// \param        nodeInput  A pointer to the simulation
...
// This function implements model logic to set the current
...
//
// \param        phyData  A pointer to the generic PHY state
//    data for the current interface/node.
// \param        dateRateType  An enumerated set of available data
...
// This routine resets the run-time statistic counters to
// a known (zero) state.
//
// \param        node  A pointer to the local node data structure
// \param        phyIndex  The integer index of the interface associated
...
// This routine resets the physical layer state machine to
// a known (IDLE) state.
//
// \param        node  A pointer to the local node data structure
// \param        phyIndex  The integer index of the interface associated
...
// This routine simulates the acquisition of a target signal that
// was suitably strong to be detected by the local receiver.
//
// \param        node  A pointer to the local node data structure
// \param        phyIndex  The integer index of the interface associated
//    with this physical layer process.
// \param        msg  A pointer to a data structure representing a
//    frame undergoing reception.
// \param        rxPowermW  The current reception power of the frame
// \param        rxEndTime  The expected time at which the frame
...
// This routine simulates the deacquisition of a target signal
...
// This routine returns whether or not the current signal
...
// This routine returns the total number of bits required for
// a physical layer transmission not including guard time and
// preamble.  Essentially it converts the user data to coded data
// length (also in bits).
//
// \param        bytes  The number of user data bytes to encode
// \param        mt  The current modulation profile
//    of the transmitter
// \param        bc  The current block (inner)
//    code type of the transmitter
// \param        cc  The current
//    convolutional (outer) code type of the transmitter
// \param        useShortenLastCodeword  Whether or not to use RS
//    code shortening techniques
// \param        virtualOverheadBytesPtr  The number of bytes of overhead
//    to add to an existing packet to account for coding overhead
//
// \return The coded frame size of the packet
...
// This routine implements a function to translate from
...
// \param        phyNum  The integer index of the interface associated with
//    this physical layer process
// \param        packetsize  The size of the packet (in bytes) that is
//    associated with upper level data.
//
// \return The integer number of nanoseconds required to
...
// This routine implements a function to add two sources of
// All White Gaussian Noise (AWGN) cast as Signal to Noise
// Ratios (SNR).
//
// \param        oldval  The original SNR value
// \param        newval  The addend for the additional noise
//    source
//
// \return The aggregate SNR value for the two levels.  It must
...
// This routine implements a function to calculate the
// overall SNR of a packet across an entire satellite
// subnetwork.  This may either be the one-hop value or the
// two-hop value.
//
// \param        hdr  The packet header of the frame
//    that has been received.
// \param        rxSnr  The SNR of the frame as it was recieved on this
...
// This routine implements a gated adder.  If the header value
...
//
// \param        valid  Whether or not the existing valud of SNR is
//    valid or not.
// \param        oldval  The existing value of the SNR usually taken from
//    the received physical layer header
// \param        newval  The current value of the SNR as measured by the
...
// This routine returns the intermodulation distortion ratio of
// the physical layer channel.
//
// \param        state  A pointer to the physical layer
//    state machine state data for the current state machine
//
// \return The equivalent SNR for intermodulation noise as a
// power ratio.
...
// This routine updates the physical layer header upon reception.
// This routine is normally only called by the satellite
// node.
//
// \param        state  A pointer to the physical layer
//    state machine state data for the current state machine
// \param        hdr  A pointer to the header of the
...
// This routine calculates the Eb/No of the current packet
...
// should be discarded.
//
// \param        node : Node* : state  A pointer to the
...
//    machine
//
// \return This function returns TRUE if the packet should be
...
// This function initializes the Satellite RSV PHY process.  It is
...
// \param        nodeInput  A pointer to the deserialized data
...
// This function finalizes the Satellite RSV PHY process.  It is
...
// This function returns the current transceiver status for the
...
// \param        phyIndex  The logical index of the current interface
//
// \return This function returns the instantaneous sample
...
// This function implements the model logic to be executed when
...
// This function implements the algorithm to transmit a signal
...
//    physical layer state machine state data for the current state
//    machine
// \param        phyNum  The logical index of the current interface
// \param        msg  A pointer to a data structure representing the
//    frame to be sent on the wireless satellite channel.
// \param        useMacLayerSpecifiedDelay  An indicator to use the
//    standard MAC-specified delay value
// \param        delayUntilAirborne  The number of nanoseconds until
...
// This function implements the algorithm to respond to a
...
//    machine
// \param        phyIndex  The logical index of the current interface
// \param        channelIndex  The logical index of the channel upon
//    which the channel is arriving.
// \param        propRxInfo  A pointer to a data structure
...
// This function implements the model logic to respond to
...
// \param        msg  A pointer to a data structure representing the
//    frame to be sent on the wireless satellite channel.
// \param        channelIndex  The logical channel index the channel
...
// This function calculates the current transmission rate
...
//    data for the current interface/node.
//
// \return The current transmission rate expressed as bits/sec
...
// This function calculates the current signaling rate
...
//    data for the current interface/node.
//
// \return The current reception rate expressed as bits/sec
...
// This function returns the current data rate type for
...
//    data for the current interface/node.
//
// \return This function returns an enumerated data rate type.
...
// This function executes logic that sets the transmission rate
...
// This function sets the transmission rate of the local
...
// This function calculates the maximum transmission rate
...
// This function calculates the maximum transmission rate for
...
// This function calculates the amount of signaling
// time required to send a set number of bits through
// the channel. This function is called by other functions
// outside of this file.
//
// \param         size  The number of bits to be sent on the channel.
// \param         dataRate  The available bit rate on this channel
//
// \return An integer number of nanoseconds required to
...
// This function sets the internal state of the physical
// layer process to a new value of the power available to
// the local transmitter.  This
// function is called by other functions outside of this file.
//
// \param         phyData  A pointer to a data structure containing
//    generic physical layer process data
// \param         newTxPower_mW  A non-negative value for the power
...
// This function calculates the internal power available to
...
// \param         phyData  A pointer to a data structure containing
//    generic physical layer process data
// \param         txPower_mW  A pointer to a memory region that is
...
// This function calculates the angle of arrival of the last frame
// received by this transceiver process.
// This function is called by other functions outside of this
// file.
//
// \param         node  A pointer to a memory region that contains the
//    generic node state information for the currently executing
//    event.
// \param         phyNum  The logical index of the physical layer
//    process within the node context.
//
// \return The return value is the angle of arrival calculated
...
// This function invokes the process logic to reset the
// receive state machine during transmission.
// This function is called by other functions outside of this
// file.
//
// \param         node  A pointer to the data structure containing
//    information about the currently executing node context.
// \param         phyIndex  The logical index of the interface of the
//    currently executing node.
// \param         terminateOnlyOnRecieveError  A flag to indicate
...
// \param         receiveError  A pointer to a suitably sized and
//    appropriately aligned memory region to hold a copy of the
//    current reception status of the active frame.
// \param         endSignalTime  A pointer to a memory region that
...
// This function invokes the process logic to begin the
...
// This function invokes the process logic to lock the
...
// This function invokes the process logic to unlock the
// transceiver antenna in its current physical configuration.
...
// This function invokes the process logic to calculate
...
// \param         phyNum  The logical index of the interface of the
//    currently executing node.
// \param         azimuth  The azimuth angle at which the calculation
//    is to occur.
//
// \return A binary response that indicates whether or not the
...
// This function invokes the process logic to set the
...
// \param         phyNum  The logical index of the interface of the
//    currently executing node.
// \param         azimuth  The new azimuth angle for the
//    transceiver.
//
...
// This function stes the default value to the parameters of
// rxTxChannelInfoPtr.
//
// \param        node : Node* : state  A pointer to the
//    physical layer state machine state data for the current state
//    machine
// \param        phyNum  The logical index of the current interface
// \param        rxTxChannelInfoPtr  instance of
...
// This function adds a new TxRxChannelInfoPair to the linked list
...
//    machine
// \param        phyIndex  The logical index of the current interface
// \param        rxLinkChannelId  upLink channel index for satellite.and
//    downlink channel index for ground station
// \param        txLinkChannelId  downlink channel index for satellite.and
...
// This function returns RxLinkChannelInfo for channel index
// channelId.
//
// \param        node : Node* : state  A pointer to the
//    physical layer state machine state data for the current state
//    machine
// \param        phyIndex  The logical index of the current interface
// \param        channelId  channel index of required channel info.
//
...
// This function returns TxLinkChannelInfo for channel index
// channelId.
//
// \param        node : Node* : state  A pointer to the
//    physical layer state machine state data for the current state
...
// This function returns ChannelInfo for channel index channelId.
//
// \param        node : Node* : state  A pointer to the
//    physical layer state machine state data for the current state
//    machine.
...
// \param        channelId  channel index of required channel info.
//
// \return This function returns channelInfo for
...
// This function returns number of rx-tx pair in the linked list.
//
// \param        node : Node* : state  A pointer to the
//    physical layer state machine state data for the current state
//    machine
// \param        phyIndex  The logical index of the current interface
//
// \return This function returns number of rx-tx pair in
// the linked list.
...
// This function returns TxRxChannelInfoPair by index
...
//    machine
// \param        phyIndex  The logical index of the current interface
// \param        index  index of rx-tx channel pair in the linked list.
//
// \return This function returns
...
//    unavailable to a physically realizable implementation of a
//    receiver.
// \param        sigPowermW  the interference signal power in mW
//
// \return This function does not return a value.
