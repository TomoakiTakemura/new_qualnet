    ...
    double*            lowest)
{
    for (int lon = 0; lon < record->numRows; lon++) {
        for (int lat = 0; lat < record->numColumns; lat++) {
            *highest = MAX(*highest, (double)record->elevationData[lon][lat]);
            *lowest  = MIN(*lowest, (double)record->elevationData[lon][lat]);
        }
    }
}
...
{
    ERROR_Assert(index >= 0 && index < (int)m_numFiles, "Invalid index");
