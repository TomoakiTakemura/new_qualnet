...
// Process a frame if the local interface is presently
// configured to be in promiscuous mode.
//
// \param        node  A pointer to the local node data structure.
// \param        state  The local data structure
//    for the interface MAC state machine.
// \param        prevHop  The address of the previous hop
//    of the packet.
// \param        dstAddr  The address of the ultimate
...
// Return the MAC (generic) machine data for the provided
// <node, interfaceIndex> pair.
//
// \param        node  A pointer to the local node data structure.
// \param        macNum  The index of the interface desired.
//
// \return A pointer to the generic state machine data.
...
// Return the detailed state machine data for the provided
// <node, interfaceIndex> pair.
//
// \param        node  A pointer to the local node data structure.
// \param        macNum  The index of the interface desired.
...
// Queries the state data to see if a timer is currently set
// in the state machine.
//
// \param        msg  A pointer to a timer message
//
// \return If the pointer is non-NULL then the routine will
...
// Set the MAC-protocol-specific data pointer
//
// \param        node  The local node data structure
// \param        macNum  The integer indentifier of the local
//    interface
...
// Returns the transmission time for a packet.
//
// \param        node  A pointer to the local node data structure
// \param        interfaceIndex  The integer index of the local
//    interface.
// \param        msg  A pointer to the message that will be sent
//    and requires a transmission time.
//
// \return The integer amount of time, in fundamental
                                                           ...
                                                           Message *msg)
{
    ...
    Int64 dataRate = PHY_GetTxDataRate(node,
...
// This routine removes a packet from the network output queue
...
//    interface.
//
// \return The first available packet from the network layer
...
// This routine processes a frame that has been received on the
...
// \param        interfaceIndex  The integer index of the local
//    interface.
// \param        interfaceAddress  An IPv4 address representing
    ...
    NodeInput const *nodeInput)
{
    ...
    if (state->role == MacSatelliteBentpipeRoleGroundStation)
    {
        ...
        {
            ERROR_ReportErrorArgs("Node %d: MAC-SATELLITE-USE-UPLINK-CHANNEL "
        ...
        {
            ERROR_ReportErrorArgs("Node %d: MAC-SATELLITE-USE-DOWNLINK-CHANNEL "
                                  "parameter is not specified.",
                                  node->nodeId);
        }
        ...
        {
            ERROR_ReportErrorArgs("Node %u: MAC-SATELLITE-USE-UPLINK-CHANNEL "
                                  "and MAC-SATELLITE-USE-DOWNLINK-CHANNEL "
                                  "can not be the same.",
                                  node->nodeId);
        }
    ...
    else        //satellite
    {
        ...
        if (channelCount == 0)
        {
            ...
            {
                ERROR_ReportErrorArgs("Node %d: MAC-SATELLITE-UPLINK-CHANNEL "
                                      "parameter is not specified.",
                                      node->nodeId);
            }
            ...
            {
                ERROR_ReportErrorArgs("Node %d: MAC-SATELLITE-DOWNLINK-CHANNEL "
                                      "parameter is not specified.",
                                      node->nodeId);
            }
            ...
            if (upLinkChannelId == downLinkChannelId)
            {
                ERROR_ReportErrorArgs("Node %u: MAC-SATELLITE-UPLINK-CHANNEL "
                                      "and MAC-SATELLITE-DOWNLINK-CHANNEL "
                                      "can not be the same.",
                                      node->nodeId);
            }
