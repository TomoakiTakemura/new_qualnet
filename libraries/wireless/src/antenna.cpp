...
// Allocate new Omnidirectional struct.
//
//
// \return pointer to
...
// Fill the discontinuity between angles in radiation
// pattern file with linear interpolation.
//
// \param pattern_dB  array for storing antenna gains.
// \param startAngle  starting angle of discontinuity.
// \param endAngle  end angle of discontinuity.
// \param preGainValue  previously available gain value.
// \param nextGainValue  next available gain value.
//
...
float* ANTENNA_ReturnHeightPtr(Node* node, int phyIndex)
{
    ...
    switch (phyData->antennaData->antennaModelType)
    {
        ...
        default:
        {
            static float defaultHeight = ANTENNA_DEFAULT_HEIGHT;
