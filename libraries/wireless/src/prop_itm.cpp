    ...
    double m_dblSurfRef)
{
    ...
    int i,jai,jbi;
    // declaring + 2 with MAX_NUM_ELEVATION_SAMPLES to accomadate
    // the calculation for the last two points in the terrain profile
    // where m_Nelev is equal to MAX_NUM_ELEVATION_SAMPLES
    double dblelev[MAX_NUM_ELEVATION_SAMPLES + 2];
    ...
    for (i = 0; i < m_Nelev; i++) {
        ...
        ERROR_Assert(i < (sizeof(dblelev)/sizeof(dblelev[0])),
                     "Out-of-bound access in ITM pathloss calculation");
    }
