...
// Retrive AtmData ptr for particular interface of a node.
//
// \param node  The node
...
// Get Atm address Retrive AtmData ptr for
...
// Get Interface for a particular Atm address.
//
// \param node  The node
// \param linkAddressString  Get interface for this address string.
// \param interfaceIndex  Interface Index for atm address
// \param interfaceAddress  Interface Address for atm address
//    RETURN     : void : NULL
static
...
// Add a queue with specific queue index into particular
// interface.
//
// \param node  The node
...
// Initializes ATM interface queue parameters during startup.
//
// \param node  The node
...
// Interface initialization function for the ATM Layer2.
//
// \param node  pointer to the first node.
// \param interfaceIndex  interface Index used.
// \param nodeInput  Pointer to NodeInput
// \param nodeType  first node type
// \param linkNodeType  other node type
// \param otherNode :  node pointer of opposite interface
//    of the atm link.
// \param otherNodeInterfaceIndex  interface of the other node
                                   ...
                                   NodeAddress otherNodeId)
{
    ...
    ERROR_Assert(node->numAtmInterfaces <= MAX_NUM_INTERFACES, "too many ATM interfaces");
    ERROR_Assert(interfaceIndex >= 0 &&  interfaceIndex < MAX_NUM_INTERFACES, "interfaceIndex out of range");

    if (node->atmLayer2Data == NULL) {
        node->atmLayer2Data = (AtmLayer2Data**)
            MEM_malloc(sizeof(AtmLayer2Data*) * MAX_NUM_INTERFACES);
            ERROR_Assert(node->atmLayer2Data != NULL, "Insufficient memory for ATM l2 data");
            memset(node->atmLayer2Data,
                0,
    ...
    ERROR_Assert(atmLayer2Data != NULL, "Insufficient memory for ATM l2 element\n");
...
// Parse the link line from the configuration
// file for ATM Link.
//
// \param partitionData  partition data
// \param nodeInput  Pointer to NodeInput
// \param linkLine  current line of the node input
//
static
...
// queue has packet to send.
//
// \param  node  The Node Pointer
// \param  interfaceIndex  at this interface.
// \param  atmScheduler  atm scheduler at this interface.
//    RETURN     : void : NULL
static
...
// Add UNI header into the cell
//
// \param node  The Node Pointer
// \param msg  pointer to message.
// \param aal5ToAtml2Info  pointer to Aal5ToAtml2Info
//    RETURN     : void : NULL
static
...
// Demultiplexing cell from frame
//
// \param node  The Node Pointer
// \param interfaceIndex  Interface Index
// \param msg  Packet Pointer
//    RETURN     : void : NULL
static
...
// Insert cell into a particular queue.
//
// \param node  pointer to node.
// \param msg  message ptr that insert into queue.
// \param queueIndex  queue with that queueIndex
// \param interfaceIndex  at the interface.
// \param atmSchd  pointer to AtmScheduler.
//    RETURN     : void : NULL
static
...
// Send cell into upper layer.
//
// \param node  pointer to node.
// \param cellVci  VCI value.
// \param cellVpi  VPI value.
// \param interfaceIndex  at the interface.
// \param atmNniPart  pointer to AtmNniPart.
// \param msg  message ptr that insert into queue.
//
static
...
// Process Atm layer2 specific event.
//
// \param node  pointer to node.
// \param msg  message ptr that insert into queue.
//
...
// Check congestion is occur into this link or interface.
//
// \param   msg  pointer to Message.
                            // RETURN     : void : NULL
...
//
// \param node  pointer to node.
// \param incomingIntf  for which flow this BW specified.
// \param outgoingIntf  in which interface
// \param isAdd  add this BW or reduce this BW.
// \param bwInfo  pointer to AtmBWInfo.
//
// \return Return TRUE if action executed properly,
// otherwise FALSE.
...
// Get the Address of opposite direction of a ATM-LINK
//
// \param   node  pointer to Node.
// \param   atmInfIndex  interface used.
