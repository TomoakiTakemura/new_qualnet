...
// Cbr initialization  for tracing
//
...
{
	if (KANDEB){
		printf("%" TYPES_64BITFMT "d : Node %d: cbr.cpp in the #InitTrace#\n",node->getNodeTime(),node->nodeId);
	}
...
{
	if (KANDEB){
		printf("%" TYPES_64BITFMT "d : Node %d: cbr.cpp in the #PrintTraceXML#\n",node->getNodeTime(),node->nodeId);
	}
    char buf[MAX_STRING_LENGTH];
...
{
	if (KANDEB){
		printf("%" TYPES_64BITFMT "d : Node %d: cbr.cpp in the #Client#\n",node->getNodeTime(),node->nodeId);
	}
    char buf[MAX_STRING_LENGTH];
...
#if defined(ADVANCED_WIRELESS_LIB) || defined(UMTS_LIB)
...
{
	if (KANDEB){
		printf("%" TYPES_64BITFMT "d : Node %d: cbr.cpp in the #ReInit#\n",node->getNodeTime(),node->nodeId);
	}
...
{
	if (KANDEB){
		printf("%" TYPES_64BITFMT "d : Node %d: cbr.cpp in the #ClientInit#\n",node->getNodeTime(),node->nodeId);
	}
    char error[MAX_STRING_LENGTH];
    AppTimer *timer;
    AppDataCbrClient *clientPtr;
...
void AppCbrClientPrintStats(Node *node, AppDataCbrClient *clientPtr) {
	if (KANDEB){
		printf("%" TYPES_64BITFMT "d : Node %d: cbr.cpp in the #ClientPrintStats#\n",node->getNodeTime(),node->nodeId);
	}
    char addrStr[MAX_STRING_LENGTH];
...
{
	if (KANDEB){
		printf("%" TYPES_64BITFMT "d : Node %d: cbr.cpp in the #ClientFinalize#\n",node->getNodeTime(),node->nodeId);
	}
...
{
	if (KANDEB){
		printf("%" TYPES_64BITFMT "d : Node %d: cbr.cpp in the #ClinetGetCbrClient#\n",node->getNodeTime(),node->nodeId);
	}
    AppInfo *appList = node->appData.appPtr;
    AppDataCbrClient *cbrClient;
...
{
	if (KANDEB){
		printf("%" TYPES_64BITFMT "d : Node %d: cbr.cpp in the #ClientNewCbrClient#\n",node->getNodeTime(),node->nodeId);
	}
    AppDataCbrClient *cbrClient;
...
{
	if (KANDEB){
		printf("%" TYPES_64BITFMT "d : Node %d: cbr.cpp in the #ClientScheduleNextPkt#\n",node->getNodeTime(),node->nodeId);
	}
    AppTimer *timer;
    Message *timerMsg;
...
{
	if (KANDEB){
		printf("%" TYPES_64BITFMT "d : Node %d: cbr.cpp in the #Server#\n",node->getNodeTime(),node->nodeId);
	}

#ifdef DEBUG
		//printf("information to get message \n---------------------\n");
            ...
            if (db != NULL)
            {
                ...
                if (node->appData.appStats)
                {
                    ...
                    { // Only need this info once
                        ...
                        if (appParamInfo != NULL)
                        {
                            serverPtr->sessionId = appParamInfo->getSessionId();
                    ...
                    if (seqStatus == SequenceNumber::SEQ_NEW)
                    {
                        ...
                        {
                            serverPtr->hopCount += ipParamInfo->m_HopCount.get();
                        }
                    }
                }
            }
...
{
	if (KANDEB){
		printf("%" TYPES_64BITFMT "d : Node %d: cbr.cpp in the #ServerInit#\n",node->getNodeTime(),node->nodeId);
	}
...
AppCbrServerPrintStats(Node *node, AppDataCbrServer *serverPtr) {
	...
	if (KANDEB){
		printf("%" TYPES_64BITFMT "d : Node %d: cbr.cpp in the #ServerPrintStats#\n",node->getNodeTime(),node->nodeId);
	}
    char addrStr[MAX_STRING_LENGTH];
    char sessionStatusStr[MAX_STRING_LENGTH];
    char buf[MAX_STRING_LENGTH];
...
#ifdef LTE_LIB_LOG
#ifdef LTE_LIB_VALIDATION_LOG
    Float64 throughput = serverPtr->stats->GetThroughput().GetValue(node->getNodeTime());
...
{
	if (KANDEB){
		printf("%" TYPES_64BITFMT "d : Node %d: cbr.cpp in the #ServerFinalize#\n",node->getNodeTime(),node->nodeId);
	}
...
{
	if (KANDEB){
		printf("%" TYPES_64BITFMT "d : Node %d: cbr.cpp in the #ServerGetCbrServer#\n",node->getNodeTime(),node->nodeId);
	}
    AppInfo *appList = node->appData.appPtr;
...
{
	if (KANDEB){
		printf("%" TYPES_64BITFMT "d : Node %d: cbr.cpp in the #ServerNewCbrServer#\n",node->getNodeTime(),node->nodeId);
	}
    AppDataCbrServer *cbrServer;
...
{
	if (KANDEB){
		printf("%" TYPES_64BITFMT "d : Node %d: cbr.cpp in the #Reset#\n",node->getNodeTime(),node->nodeId);
	}
...
{
	if (KANDEB){
		printf("%" TYPES_64BITFMT "d : Node %d: cbr.cpp in the #ClientAddAddressInfomation#\n",node->getNodeTime(),node->nodeId);
	}
...
{
	if (KANDEB){
		printf("%" TYPES_64BITFMT "d : Node %d: cbr.cpp in the #ClientGetSessionAddressState#\n",node->getNodeTime(),node->nodeId);
	}
...
{
	if (KANDEB){
		printf("%" TYPES_64BITFMT "d : Node %d: cbr.cpp in the #UrlSessionStartCallback#\n",node->getNodeTime(),node->nodeId);
	}
