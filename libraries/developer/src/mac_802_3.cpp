...
// Dequeues packet from queue and process
...
//--------------------------------------------------------------------------
// Processes on successfully received Frame. Forwards the
...
// Handling events in post transmission
//  PARAMETERS::
// +node : Node* : Pointer to node
// +interfaceIndex : int : Interface Index
// +msg : Message* : Pointer to message
...
//--------------------------------------------------------------------------
// Models behavior of MAC layer for full duplex with
...
// Initialization function for the IEEE 802.3 full duplex
    ...
    BOOL forLink)
{
    ...
    {
        mac802_3->isFullDuplex = FALSE;
        // setting destination nodeId for a node in point to point link.
        if (numNodesInSubnet == 2)
        {
            ...
            {
                ERROR_ReportErrorArgs("Node %u is not the part of Link(%u -- %u)",
                    node->nodeId, nodeList[0].nodeId, nodeList[1].nodeId);
            }
        }
    }
            ...
            (isLinkFullDuplex && !strcmp(linkMacString, "MAC802.3")))
    {
        ...
        if (numNodesInSubnet != 2)
        {
            ERROR_ReportErrorArgs("Node:%u, Interface index: %u\n"
