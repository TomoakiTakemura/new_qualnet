...
static BOOL RipIsInADifferentMajorNetwork(NodeAddress nodeAddress1,
                                          NodeAddress rcvdSubnetMask,
                                          NodeAddress nodeAddress2,
                                          NodeAddress configuredMask);
                            ...
                            NetworkIpGetInterfaceSubnetMask(node, interfaceIndex)))
                    {
                        ...
                        for (i = 0; i < routeIndex; i++)
                        {
                            if (!RipIsInADifferentMajorNetwork(
                                    ...
                                    dataPtr->routeTable[i].subnetMask)
...
static void RipProcessResponse(Node* node,Message* msg)
{
    ...
    for (i = 0; i < numRtes; i++)
    {
        ...
        if (rtePtr->nextHop
            && RipGetRoute(node, rtePtr->nextHop)
            && (!RipIsInADifferentMajorNetwork(
                rtePtr->nextHop,
                rtePtr->subnetMask,
                NetworkIpGetInterfaceAddress(node, incomingInterfaceIndex),
                NetworkIpGetInterfaceSubnetMask(node, incomingInterfaceIndex)))
        ...
        if (!routePtr)
        {
            ...
            nwAddress = rtePtr->ipAddress & rtePtr->subnetMask;
            if (nwAddress == rtePtr->ipAddress)
            {
                ...
                for (j = 0; j < dataPtr->numRoutes; j++)
                {
                    ...
                    if (!RipIsInADifferentMajorNetwork(
                            ...
                            dataPtr->routeTable[j].subnetMask))
                    {
    ...
    NodeAddress configuredMask)
{
    ...
    if (rcvdsSubnetMask)
    {
        mask = rcvdsSubnetMask;
    }
    else
    {
        mask = RipReturnMaskFromIPAddress(nodeAddress1);
    }

    networkAddress1 = nodeAddress1 & mask;
    ...
    {
        mask = RipReturnMaskFromIPAddress(nodeAddress2);
    }

    networkAddress2 = nodeAddress2 & mask;
...
//add your code here:
    RipRoute* routePtr = NULL;
    unsigned int cost =  *(unsigned int*) routeCost;
