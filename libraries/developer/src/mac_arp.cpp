...
// Prints ARP Request | Reply Packet
//
// \param arpPacket  The header of ARP
//
static
...
// If thisinterface support ARP protocol, it return the
// ArpData structure of this interface otherwise NULL
//
// \param node  Pointer to node.
//
// \return ArpData structure pointer.
static
...
// This function check the suppoted protocol addr type
...
// This function creates an ARP packet
//
// \param node  Pointer to node.
// \param arpData  arp main Data structure of this interface.
// \param opCode  Type of operation i.e. Request or Reply.
// \param destAddr  request the Ethernet address for
//    this Ip address.
// \param dstMacAddr  Device address
...
// \param arpData  arp main Data structure of this interface.
// \param interfaceIndex  in which interface this request shuld be
//    broadcast.
//
static
void ArpSendRequest(
...
// Overloading function used to add entry static and dynamic
// entry
//
// \param node  the node where the table has
// \param ipAddress  The interface address
// \param intfIndex  Interface of the node
// \param protocolType  Whether IP
// \param macAddr  mac address
// \param timeOut  Current time
// \param entryType  Static entry or dynamic entry
//
static
...
// This function updates an entry in the ArpTranslationTable
...
//
// \param node  Pointer to node.
// \param arpData  ARP information
// \param protoType  Type of the network
// \param protocolAddress  The host
// \param etherAddr  Hardware address
...
// An ARP Reply packet is received, update the ARP cache table
//
// \param node  Pointer to node.
// \param msg  which will be process.
// \param interfaceIndex  incoming interface.
//
...
// \param msg  which will be process.
// \param interfaceIndex  incoming interface.
// \param srcHWAddress  source hardware address
//
...
// An ARP packet is received by Address Resolution module
// which
...
// This function buffer a data packet into ARP buffer
//
// \param node  Pointer to node.
// \param  data  Pointer to Arp Buffer,
//    to store details of the packet
// \param interfaceIndex  Interface of the node
// \param incomingInterface  Interface at which packet was arrived
// \param nextHop  nextHop Ip address.
...
// \param node  Pointer to node.
// \param arpData  Pointer to ArpData
...
// \param protoType  Protocol Type
//    + priority : TosType  :
// \param protocolAddress  Ethernet address surch for this Ip
//    address.
...
// This function may be call when interface is fault.
// Remove the all entries from the ArpTranslationTable.
//
// \param node  Pointer to the Node
// \param arpData  Arp related information
//
static
...
//
// \param node  Pointer to node.
// \param arpData  Pointer to ArpData for the particular interface.
//
static
void ArpDbFlush(
         Node* node,
...
// Static ARP cache entry inserted into the main ARP table
//
// \param node  Pointer to node.
// \param arpStaticTableInput  It is the input to the static
//    arp-table entry.
//
static
...
// \param newMacAddr  Current Interface Mac Address
//
         ...
         const NodeInput* nodeInput)
{
    ...
    for (i = 0; i < node->numberInterfaces; i++)
    {
        ...
        if (arpData->interfaceInfo[i].isEnable)
        {
            ...
            arpData->interfaceInfo[i].requestQueue = new Queue(node,
                                                               "FIFO",
                                                               ARP_QUEUE_SIZE);
    ...
    if (ArpIsEnable(node))
    {
       ...
       if (retVal)
       {
        ...
        // not going to use ARP for this node, give back memory.
        nwData->arModule = 0;
        addrResolnMod->arpData = 0;
        // free in reverse order to malloc
        // might help avoid heap fragmentation
        // probably makes no difference.
        MEM_free(addrResolnMod);
        MEM_free(arpData);
    }
...
// \param dstMacAddr  The node who has send the ARP-request
//    + interfaceIndex: int :
//
// \return Pointer to a Message structure
...
// Determine what kind of packet trace we need to worry about.
//
// \param Node  node doing the packet trace.
// \param nodeInput  structure containing
