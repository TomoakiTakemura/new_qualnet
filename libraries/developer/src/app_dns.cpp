...
{
    size_t urlLength = destString->length();
                        ...
                        BOOL end)
{
    ...
    APP_AddPayload(
        ...
        msg,
        packetToSend,
        (int)(dnsData->server->rrList->size() * sizeof(struct DnsRrRecord)
            + sizeof(struct DnsZoneTransferPacket)
            + sizeof(zoneFreshCount)));
...
void AppDnsSendNotifyToSlaves(Node* node)
{
    ...
    size_t size = zoneData->data->size();
    ...
    switch (msg->eventType)
    {
        case MSG_APP_FromTransport:
        {
            ...
            if (msgType == DNS_UPDATE)
            {
                    ...
                    slaveZoneRefreshCount))
                {
                    ...
                    clientPtr = AppDnsZoneClientNewDnsZoneClient(
                        ...
                        1,// itemsToSend
                        (Int32)(dnsData->server->rrList->size() *
                            sizeof(struct DnsRrRecord)) +
            ...
            else if (msgType == DNS_NOTIFY)
            {
                ...
                else
                {
                    ...
                    size_t size = zoneData->retryQueue->size();
        ...
        case MSG_APP_DNS_SERVER_INIT:
        {
            ...
            if (role == NameServer)
            {
                ...
                size_t size = dnsData->zoneData->data->size();
                item = dnsData->zoneData->data->begin();
                if (size != 0)
                {
                    tempData = (data*)*item;
                ...
                if (zoneMasterfound)
                {
                    ...
                    for (size_t rrCount = 0; rrCount < size; rrCount++)
                    {
                        memcpy(tempPtr, *rrRecord,
                               sizeof(struct DnsRrRecord));
                        tempPtr += sizeof(struct DnsRrRecord);
                    ...
                    MESSAGE_PacketAlloc(node,
                            eventMsg,
                            (int)(sizeof(enum DnsMessageType) +
                                  sizeof(zoneFreshCount)+
                                  sizeof(struct DnsZoneTransferPacket)+
                                  sizeof(Address) +
                                  sizeof(newServerLabel) +
                         ...
                         char* domain)
{
    ...
    for (Int32 levelCount = level; levelCount >= 0; levelCount--)
    {
            ...
            lineCount1++)
        {
            ...
            size_t strLen = strlen(tempLabel);
