...
static void SuperApplicationScheduleNextReply_TCP(Node* node,SuperApplicationData* serverPtr, clocktype delay) {
    ...
    if (replyPacketTimer->numRepToSend != 0)
    {
    ...
    if (db != NULL)
    {
        if (!node->partitionData->statsDb->statsAppEvents->recordFragment)
        {
            if (completePacketRvd || data->totFrag == 1)
            {
                APP_ReportStatsDbReceiveEvent(
                    ...
                    data->totalMessageSize,
                    (int)clientPtr->stats.numRepPacketsRcvd,
                    msgStatus);
            }
        }
        else
        {
            APP_ReportStatsDbReceiveEvent(
                ...
                MESSAGE_ReturnPacketSize(msg),
                (int)clientPtr->stats.numRepFragsRcvd,
                msgStatus);
        }
        ...
        {
               clientPtr->stats.hopCount += ipParamInfo->m_HopCount.get();
        }
    }
#endif
    ...
    while (totalBytesRead > 0 && (completePacketRvd == TRUE))
    {
        ...
        if (totalBytesRead >= header->totalPacketBytes)
        {
            ...
            if (db != NULL)
            {
                ...
                if (!appParamInfo->m_PktCreationTime.isNULL())
                {
                    packetDelay = node->getNodeTime() - appParamInfo->m_PktCreationTime.get();
                }
            }
#endif
...
// Make sure whether the below code section is really required for ADDON_DB
            ...
            if (db != NULL)
            {
                ...
                if (!node->partitionData->statsDb->statsAppEvents->recordFragment)
                {
                    ...
                    int NumreqMsgRcvd =
                        (int) serverPtr->stats.numReqPacketsRcvd;
                ...
                else
                {
                    ...
                    HandleStatsDBAppReceiveEventsInsertion(
                        ...
                        header->totalFragmentBytes,
                        (int)serverPtr->stats.numReqFragsRcvd);
                }
                ...
                {
                    serverPtr->stats.hopCount += ipParamInfo->m_HopCount.get();
                }
...
// SuperApplication initialization  for tracing
//
// \param node  Pointer to node
