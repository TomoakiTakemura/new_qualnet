...
void LinkLayer(Node *node, int interfaceIndex, Message *msg) {
    ...
    switch (msg->eventType) {
        ...
        case MSG_MAC_TimerExpired: {
            int nodeId = node ? node->nodeId : -1;
            ERROR_ReportWarningArgs("Timer event to node:%d interface:%d protocol:%d",
                                      nodeId, interfaceIndex, msg->getProtocol());
            break;
        }

        default: {
            int nodeId = node ? node->nodeId : -1;
            ERROR_ReportErrorArgs("invalid event type (%d) in LinkLayer node:%d", msg->eventType, nodeId);
        }
    }
}
