...
Ipv6InterfaceInitAdvt(Node* node, int interfaceIndex)
{
    ...
    if (interfaceInfo == NULL ||
        interfaceInfo->autoConfigParam.isDadEnable)
    {
        return;
    }
...
// Sets the router function.
//
// \param node  Pointer to node
// \param routerFunctionPtr  router
//    : function pointer.
...
// Use to send packet if IPv6 next hop address is known.
//
// \param node  Pointer to node
...
// Set timer to join and leave multicast groups.
//
// \param node  Pointer to node.
// \param mcastAddr  multicast group to join or leave.
// \param delay  delay.
//
...
// Give the routing protocol the messages that come from
...
// Return first IPv6 enabled interface.
//
//    +node           : Node* : Pointer to Node.
//
