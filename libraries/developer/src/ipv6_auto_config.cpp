                ...
                Int32 interfaceIndex)
{
    ...
    while (listItem != ipv6->autoConfigParameters.eligiblePrefixList->end())
    {
        ...
        ndopt_pi->nd_opt_pi_prefix_len =
                    (unsigned char) (prefixInfo->prefixLen);

        ndopt_pi->nd_opt_pi_flags_reserved = prefixInfo->LAReserved;

        ndopt_pi->nd_opt_pi_valid_time =
                    (unsigned int) (prefixInfo->validLifeTime / SECOND);

        ndopt_pi->nd_opt_pi_preferred_time =
                    (unsigned int) (prefixInfo->prefLifeTime / SECOND);
        ...
        Int32 interfaceId)
{
    ...
    if (IS_LINKLADDR6(interfaceInfo->globalAddr))
    {
        ...
        if (returnAddr != NULL)
        {
            ...
            IPv6AutoConfigInitiateDAD(node, interfaceId);
                                ...
                                Message* msg)
{
    ...
    if (!CMP_ADDR6(interfaceInfo->globalAddr, tgtaddr))
    {
        UInt8 counter = interfaceInfo->linkLocalAddr.s6_addr8[14] + 1;

        // invalidate current Global Address
        MAPPING_InvalidateGlobalAddressForInterface(
                node,
                interfaceId,
                &interfaceInfo->globalAddr,
        ...
        memcpy(&interfaceInfo->globalAddr.s6_addr8[14],
               &interfaceInfo->linkLocalAddr.s6_addr8[14],
               sizeof(UInt8));
                            ...
                            Int32 interfaceIndex)
{
    ...
    if (interfaceInfo->autoConfigParam.isDebugPrintAutoconfDad)
    {
        ...
        IO_ConvertIpAddressToString(
                                 &interfaceInfo->globalAddr,
                                 addressStr);
        printf("Tentative global address: %s\n", addressStr);
        printf("Neighbor Solicitation for DAD Sent\n");
    }
                                   ...
                                   Message* msg)
{
    ...
    bool isSameLinkLocalAddr =
        !CMP_ADDR6(nsp->nd_ns_target, interfaceInfo->globalAddr);
                            ...
                            Message* msg)
{
    ...
    if (interfaceInfo->autoConfigParam.addressState == TENTATIVE)
    {
        if (IS_LINKLADDR6(interfaceInfo->globalAddr))
        {
            if (interfaceInfo->autoConfigParam.isDebugPrintAutoconfDad)
            {
                ...
                printf("DAD Completed Successfully, "
                          "Configured Link Local Address is unique\n");
                printf("Node Id: %d\n", node->nodeId);
                IO_ConvertIpAddressToString(
                                     &interfaceInfo->linkLocalAddr,
                                     addressStr);
            ...
            if (returnAddr != NULL)
            {
                ...
                IPv6AutoConfigInitiateDAD(node, *interfaceIndx);
        ...
        if (interfaceInfo->autoConfigParam.prefLifetime < CLOCKTYPE_MAX)
        {
            in6_addr prefix;
            Ipv6GetPrefix(&interfaceInfo->globalAddr,
                  &prefix,
                            ...
                            Int32 interfaceIndex)
{
    ...
    if (ipv6->autoConfigParameters.eligiblePrefixList)
    {
        ...
        sprintf(addressString, "%.2x%.2x:%.2x%.2x::",
