    ...
    switch (msg->eventType)
    {
        case MSG_APP_FromTransOpenResult:
        {
            ...
            else
            {
                ...
                // check whether the earlier conn. is active or not
                ...
                (clientPtr->state == XMIT_SECONDARY_REQUEST))
            {
                ...
                if (!clientPtr->sessionIsClosed)
                {
                    Message *msg = APP_TcpCreateMessage(
                        node, clientPtr->connectionId, TRACE_HTTP);
                    APP_AddPayload(node, msg, payload, sendSize);
        ...
        case MSG_APP_FromTransDataReceived:
        {
            ...
            {
                clientPtr->lastReceiveTime = node->getNodeTime();
            }
...
#endif /* DEBUG */
                Int32 serverIndex = httpClient->serverIndex;
                AppHttpClientProcessDoneThinking(node, httpClient);
                // Dynamic address change
                // check if server index changed
                // dns
                // send DNS request if serverURL is present at server index
                if (httpClient->serverIndex != serverIndex)
                {
                    // check if current server address is URL
                    if (httpClient->serverUrls.size() != 0 &&
                        httpClient->serverUrls[httpClient->serverIndex].
                                                            size() != 0)
                    {
                        // set the dns info in client pointer if server
                        // url is present
                        ...
                        if (node->appData.appTrafficSender->
                                        ifUrlLocalHost(urlString) == FALSE)
                        {
                            // need to buffer the TCP open request untill
                            // URL is resolved and send the DNS request
                            node->appData.appTrafficSender->
                                    appTcpOpenConnection(
                                            node,
                                            APP_HTTP_CLIENT,
                                            httpClient->localAddr,
                                            node->appData.nextPortNum++,
                                            httpClient->remoteAddr,
                                            (UInt16) APP_HTTP_SERVER,
                                            httpClient->uniqueId,
                                            APP_DEFAULT_TOS,
                                            ANY_INTERFACE,
                                            urlString,
                                            PROCESS_IMMEDIATELY,
                                            httpClient->serverNodeIds
                                            [httpClient->serverIndex],
                                            httpClient->clientInterfaceIndex,
                                            httpClient->serverInterfaceIndex
                                            [httpClient->serverIndex]);
                        }
                    }
                }
                                 ...
                                 AppDataHttpClient *clientPtr)
{
    ...
    else
    {
        // Dynamic Address Change
        Address serverAddr;
        memset(&serverAddr, 0, sizeof(Address));
        Int32 serverIndex = AppHttpClientSelectNewServer(clientPtr);

        clientPtr->documentsOnCurrentServer =
        AppHttpClientConsecutiveDocumentRetrievals(clientPtr);

#ifdef DEBUG
        printf("    Request %d documents on server index %ld\n",
               clientPtr->documentsOnCurrentServer, serverIndex);
#endif /* DEBUG */

        if (serverIndex != clientPtr->serverIndex)
        {
            clocktype sum;
            ...
            clientPtr->serverIndex = serverIndex;
            ...
            if (!clientPtr->sessionIsClosed)
            {
                ...
                node->appData.appTrafficSender->appTcpCloseConnection(
                                                ...
                                                clientPtr->connectionId);
                clientPtr->state = IDLE;
                // Check if for this server a server url is specified
                if (clientPtr->serverUrls.size() != 0 &&
                   clientPtr->serverUrls[clientPtr->serverIndex].size() != 0)
                {
                    // Try to resolve URL from Traffic Sender
                    Address destAddress =
                    node->appData.appTrafficSender->lookupUrlInAddressCache(
                            clientPtr->serverUrls[clientPtr->serverIndex]);
                    if (destAddress.networkType != NETWORK_INVALID)
                    {
                        memcpy(&serverAddr, &destAddress, sizeof(Address));
                    }
                }
                ...
                if (serverAddr.networkType != NETWORK_INVALID)
                {
                    ...
                    node->appData.appTrafficSender->appTcpOpenConnection(
                            ...
                            clientPtr->serverUrls[clientPtr->serverIndex],
                            PROCESS_IMMEDIATELY,
                            clientPtr->serverNodeIds[clientPtr->serverIndex],
                            clientPtr->clientInterfaceIndex,
                            clientPtr->serverInterfaceIndex[clientPtr->serverIndex]);
                }
            }
                                   ...
                                   AppDataHttpClient *clientPtr)
{
             ...
             (clientPtr->state == WAIT_SECONDARY_RESPONSE))
    {
        if (clientPtr->lastReceiveTime <
            (node->getNodeTime() - clientPtr->threshhold))
        {
            clocktype sum;
                                ...
                                AppDataHttpClient *clientPtr)
{
    ...
    else
    {
        ...
        currentPageRequestTime =
            (node->getNodeTime() - clientPtr->primaryRequestSendTime);
    ...
    Int32 primaryRequestLength)
{
    ...
    clientPtr->primaryRequestSendTime = node->getNodeTime();
    ...
    std::vector<std::string> serverUrls)
{
    ...
    // Dynamic address
    clientPtr->serverIndex = AppHttpClientSelectNewServer(clientPtr);
    clientPtr->serverUrls.clear();
    clientPtr->serverNodeIds = NULL;
    clientPtr->serverInterfaceIndex = NULL;


    clientPtr->remoteAddr = clientPtr->servers[clientPtr->serverIndex];
    serverAddr = clientPtr->servers[clientPtr->serverIndex];
    ...
    {
        // dns
        clientPtr->serverUrls = serverUrls;
        url = clientPtr->serverUrls[clientPtr->serverIndex];
    }

    // Update the HTTP clientPtr with address information
    AppHttpAddAddressInformation(node, clientPtr);
...
AppHttpClientPrintStats(Node *node, AppDataHttpClient *clientPtr)
{
    ...
    if (clientPtr->appStats &&
        !clientPtr->appStats->IsSessionFinished(STAT_Unicast))
    {
        clientPtr->appStats->SessionFinish(node);
    }
                            ...
                            TransportToAppOpenResult *openResult)
{
    ...
    httpClient->sessionStart = node->getNodeTime();
    httpClient->sessionFinish = node->getNodeTime();
        ...
        case MSG_APP_FromTransOpenResult:
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    // Dynamic Address Change
                    AppHttpServerList* httpServer = NULL;
                    Int32 interfaceIndex =
                        MAPPING_GetInterfaceIndexFromInterfaceAddress(
                                            node, openResult->localAddr);
                    httpServer =
                    ...
                    if (!httpServer)
                    {
                        ERROR_ReportErrorArgs(
                                "Expected HTTP SERVER is not found\n");
                    }
                    if (httpServer->httpServerDataList == NULL)
                    {
                        httpServer->httpServerDataList =
                                        new std::list<AppDataHttpServer*>;
                    }
                    httpServer->httpServerDataList->push_back(serverPtr);
...
AppHttpServerInit(Node *node, Address serverAddr)
{
    for (Int32 i = 0; i < node->numberInterfaces; i++)
    {
        ...
        httpServer = AppHttpServerGetServerForThisInterfaceIndex(node, i);
        // register new if not already resgistered
        if (httpServer == NULL)
        {
            httpServer = (AppHttpServerList*)
                                MEM_malloc(sizeof(AppHttpServerList));
            httpServer->interfaceIndex = i;
            httpServer->httpServerDataList = NULL;
        ...
        if (ip->addressChangedHandlerList == NULL)
        {
            NetworkIpAddAddressChangedHandlerFunction(node,
                        &HttpServerHandleChangeAddressEvent);
        }
        else
        {
            ...
            while (listItem != NULL)
            {
                addressChangedHandler = (IpAddressChangedHandlerFunctionType)
                                                              listItem->data;
                if (!addressChangedHandler)
                {
                    ERROR_ReportErrorArgs("In "
                        "HttpServerInit and invalid "
                        "handler function");
                }
                if (addressChangedHandler ==
                    &HttpServerHandleChangeAddressEvent)
...
AppHttpServerFinalize(Node *node, AppInfo* appInfo)
{
    ...
    // Dynamic Address Change
    AppDataHttpServer* serverPtr = NULL;
    AppHttpServerList* httpServer = (AppHttpServerList*)appInfo->appDetail;
    if (node->appData.appStats == TRUE)
    {
        if (httpServer->httpServerDataList != NULL)
        {
            std::list<AppDataHttpServer*>::iterator it =
                                httpServer->httpServerDataList->begin();
            while (it != httpServer->httpServerDataList->end())
            {
                serverPtr = (AppDataHttpServer*)(*it);
                if (serverPtr->sessionIsClosed == FALSE)
                {
                    serverPtr->sessionFinish = node->getNodeTime();
                }
    ...
    if (httpServer->httpServerDataList != NULL)
    {
        delete(httpServer->httpServerDataList);
        httpServer->httpServerDataList = NULL;
    }
}
                           ...
                           TransportToAppOpenResult *openResult)
{
    ...
    httpServer->uniqueId = node->appData.uniqueId++;
    ...
    //Dynamic Address Change
    //APP_RegisterNewApp(node, APP_HTTP_SERVER, httpServer);
    //Application has already been registered at the Server Init time
...
{
    AppDataHttpClient* clientPtr = NULL;

    clientPtr = AppHttpClientGetHttpClientFromUniqueId(
                                            node,
                                            uniqueId);
    if (clientPtr == NULL)
    {
    ...
    if (serverAddr->networkType == NETWORK_IPV4)
    {
        ...
        {
            ERROR_ReportErrorArgs("HTTP Client: Node %d cannot find IPv6 "
                "http client for ipv4 dest\n",
                node->nodeId);
        }
    }
    else if (serverAddr->networkType == NETWORK_IPV6)
    {
        ...
        {
            ERROR_ReportErrorArgs("HTTP Client: Node %d cannot find IPv4 "
                "http client for ipv6 dest\n",
                node->nodeId);
        }
    }
    bool urlFound = FALSE;
    // Add the server address
    for (Int32 i=0; i < clientPtr->num_servers; i++)
    {
        if (clientPtr->serverUrls[i].size() != 0)
        {
            if (!clientPtr->serverUrls[i].compare(serverUrl))
            {
                clientPtr->servers[i] = *serverAddr;
                urlFound = TRUE;
                break;
            }
        }
    ...
    memcpy (&clientPtr->remoteAddr,
            &clientPtr->servers[clientPtr->serverIndex],
            sizeof(Address));
                            ...
                            AppDataHttpClient* clientPtr)
{
    ...
    if (clientPtr->serverInterfaceIndex == NULL)
    {
        clientPtr->serverInterfaceIndex =
                (Int16*)MEM_malloc(sizeof(Int16) * clientPtr->num_servers);
        for (Int32 i = 0; i < clientPtr->num_servers; i++)
        {
            clientPtr->serverInterfaceIndex[i] = ANY_INTERFACE;
        }
    }
    if (clientPtr->serverNodeIds == NULL)
    {
        clientPtr->serverNodeIds =
            (NodeAddress*)MEM_malloc(
                            sizeof(NodeAddress) * clientPtr->num_servers);
        for (Int32 i = 0; i < clientPtr->num_servers; i++)
        {
            clientPtr->serverNodeIds[i] = ANY_NODEID;
        }
    }

    // Find out server interface indexes
    for (Int32 i = 0; i < clientPtr->num_servers; i++)
    {
        NodeId serverNodeId = MAPPING_GetNodeIdFromInterfaceAddress(
                                                node,
                                                clientPtr->servers[i]);
        if (serverNodeId != INVALID_MAPPING)
        {
            clientPtr->serverNodeIds[i] = serverNodeId;
            clientPtr->serverInterfaceIndex[i] =
        ...
        if (serverNodeId == INVALID_MAPPING)
        {
            if (NetworkIpCheckIfAddressLoopBack(node, clientPtr->servers[i]))
            {
                clientPtr->serverNodeIds[i] = node->nodeId;
                clientPtr->serverInterfaceIndex[i] = DEFAULT_INTERFACE;
            }
        }
    }
...
{
    AppInfo* appList = node->appData.appPtr;
    AppHttpServerList* httpServerForThisInterfaceIndex = NULL;

    for (; appList != NULL; appList = appList->appNext)
    {
        if (appList->appType == APP_HTTP_SERVER)
        {
            httpServerForThisInterfaceIndex =
                (AppHttpServerList*) appList->appDetail;

            if (httpServerForThisInterfaceIndex->interfaceIndex ==
                                                            interfaceIndex)
            {
                return httpServerForThisInterfaceIndex;
            }
        }
    }
    return NULL;
...
{
    AppDataHttpServer* httpServerData;
    AppHttpServerList* httpServer =
        AppHttpServerGetServerForThisInterfaceIndex(
                                                node,
                                                interfaceIndex);
    ...
    subnetMask = 0;//to suppress the warning
    Address newAddress;
    MAPPING_GetInterfaceAddrForNodeIdAndIntfId(
                                node,
                                node->nodeId,
    ...
    if (httpServer != NULL)
    {
        ...
        else
        {
            ...
            while (it != httpServer->httpServerDataList->end())
            {
                httpServerData = (AppDataHttpServer*)(*it);
                node->appData.appTrafficSender->appTcpCloseConnection(
                                            node,
                                            httpServerData->connectionId);
                node->appData.appTrafficSender->appTcpListen(
                                    node,
                                    APP_HTTP_SERVER,
                                    newAddress,
...
// A new function to get the http server data pointer with the
// given connectionId from the new stucture HttpServer (used
// to store http server data and the corresponding interface index)
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
AppDataHttpServer*
AppHttpServerGetHttpServer(Node* node, Int32 connId)
{
    AppInfo* appList = node->appData.appPtr;
    AppHttpServerList* httpServer = NULL;
    AppDataHttpServer* httpServerData = NULL;

    for (; appList != NULL; appList = appList->appNext)
    {
        if (appList->appType == APP_HTTP_SERVER)
        {
            httpServer = (AppHttpServerList*) appList->appDetail;
            if (httpServer->httpServerDataList != NULL)
            {
                // traverse all the http server data nodes inside this
                // particular HttpServer stucture
                std::list<AppDataHttpServer*>::iterator it =
                                httpServer->httpServerDataList->begin();
                while (it != httpServer->httpServerDataList->end())
                {
                    ...
                    if (httpServerData->connectionId == connId)
                    {
...
AppHttpClientGetHttpClientFromUniqueId(Node *node, Int32 uniqueId)
{
    ...
    for (; appList != NULL; appList = appList->appNext)
    {
        if (appList->appType == APP_HTTP_CLIENT)
        {
            ...
            if (httpClient->uniqueId == uniqueId)
            {
                return httpClient;
            }
        }
