...
// \param msg  Pointer to packet to print headers from
//
...
// End for Dymo
    ...
    {
        ip->isOpenSSLEnabled = TRUE;
    }
    else
    {
        ERROR_ReportError("OPENSSL-REAL-CRYPTO-ENABLED expects YES or NO");
    }
#endif //CYBER_CORE
    IO_ReadTime(
        node->nodeId,
    ...
    for (i = 0; i < node->numberInterfaces; i++)
    {
        ip->interfaceInfo[i]->isDhcpEnabled = FALSE;
        ip->interfaceInfo[i]->listOfSecondaryDNSServer = NULL;
        ip->interfaceInfo[i]->addressState = PREFERRED;
        ip->interfaceInfo[i]->primaryDnsServer.interfaceAddr.ipv4 =
                                                                ANY_ADDRESS;
        ip->interfaceInfo[i]->primaryDnsServer.networkType = NETWORK_IPV4;
        ip->interfaceInfo[i]->subnetMask.interfaceAddr.ipv4 =
                                                        ANY_ADDRESS;
        ip->interfaceInfo[i]->subnetMask.networkType = NETWORK_IPV4;
        ip->interfaceInfo[i]->ipNumPktDropDueToInvalidAddressState = 0;
...
// Routs the packet using lookup table.
//
// \param node  pointer to node.
// \param destAddr  Destination Address
// \param outIntf  Outgoing interface.
...
// Process packet after receiving from adaptation layer
//
// \param node  Pointer to node.
...
// Tries to route and send the packet using
...
BOOL isISAKMPExchangePacket(Message* msg)
{
    unsigned short srcPort = 0;
    unsigned short destPort = 0;
    GetSrcAndDestPort(msg, &srcPort, &destPort);

    if (srcPort == ISAKMP_PORT && destPort == ISAKMP_PORT)
    {
        return TRUE;
    }
    else
    ...
    UInt8 ttl)
{
        ...
        !TunnelIsVirtualInterface(node, outgoingInterface))
    {
        ...
        if (sourceAddress.interfaceAddr.ipv4 != 0 &&
            sourceAddress.interfaceAddr.ipv4 != ANY_INTERFACE)
        {
            interfaceIndex = MAPPING_GetInterfaceIndexFromInterfaceAddress(
                                    node,
                                    sourceAddress);
        }
        // if it is not DHCP packet
        if (!NetworkIpIsDhcpPacket(msg))
        {
            // if there is no interface with this address
            // if source address is not valid or interface state is not valid
            if (interfaceIndex == ANY_INTERFACE ||
                !NetworkIpIsValidSourceAddressState(node,
                    sourceAddress.interfaceAddr.ipv4, interfaceIndex) ||
                destinationAddress.interfaceAddr.ipv4 == 0)
            {
                // free the packet as address state is not valid
                MESSAGE_Free(node, msg);
                    ...
                    sourceAddress.interfaceAddr.ipv4, interfaceIndex))
                {
                    ip->interfaceInfo[interfaceIndex]->
                        ipNumPktDropDueToInvalidAddressState++;
                }
                else if (outgoingInterface >= 0)
                {
                ...
                {
                    ip->interfaceInfo[DEFAULT_INTERFACE]->
                        ipNumPktDropDueToInvalidAddressState++;
                }
                return;
            }
    ...
    bool isIPv4Node = node->networkData.networkProtocol == IPV4_ONLY;
    bool isIPv6Node = node->networkData.networkProtocol == IPV6_ONLY;

    bool isDualIP = node->networkData.networkProtocol == DUAL_IP;

    bool isGsmLayer3 = node->networkData.networkProtocol == GSM_LAYER3;
    bool isUnivCell = node->networkData.networkProtocol == CELLULAR;

    bool srcIsIPv4 = sourceAddress.networkType == NETWORK_IPV4;
    bool dstIsIPv4 = destinationAddress.networkType == NETWORK_IPV4;
    bool isIPv4 = srcIsIPv4 && dstIsIPv4;

    bool srcIsIPv6 = sourceAddress.networkType == NETWORK_IPV6;
    bool dstIsIPv6 = destinationAddress.networkType == NETWORK_IPV6;
    bool isIPv6 = srcIsIPv6 && dstIsIPv6;

    bool srcIsATM = sourceAddress.networkType == NETWORK_ATM;
    bool dstIsATM = destinationAddress.networkType == NETWORK_ATM;
    bool isATM = srcIsATM && dstIsATM;

    bool isPermissibleIPv4(isIPv4Node || isDualIP);
    bool isPermissibleIPv6(isIPv6Node || isDualIP);
    bool isPermissibleATM(isIPv4Node);

#ifdef CELLULAR_LIB
    isPermissibleIPv4 |= isGsmLayer3;
#endif

#ifdef WIRELESS_LIB
    isPermissibleIPv4 |= isUnivCell;
#endif

    if (isPermissibleIPv4 && isIPv4)
    {
        NetworkIpSendRawMessage(
            node,
            msg,
    ...
    else if (isPermissibleIPv6 && isIPv6)
    {
        Ipv6SendRawMessage(
                node,
                msg,
    ...
    else if (isPermissibleATM && isATM)
    {
        NodeAddress srcAddr;
        NodeAddress dstAddr;
    ...
    unsigned ttl)
{
    ...
    // We should do all the routing in RoutePacketAndSendToMac().
        ...
        CellularLayer3IsUserDevices(node, interfaceIndex))
    {
        ...
        bool multiHomed = node->numberInterfaces > 1;
        int outgoingInterface(-1);
        NodeAddress nextHop(-1);
        BOOL routeType(FALSE);
        bool useCellular(true);
        char routeIndicator = 'M';
        ...
        if (!useCellular)
        {
            MacData* macData = (MacData*)node->macData[outgoingInterface];
            useCellular = true;
            routeIndicator = 'M';
        }

#if defined(DEBUG) && DEBUG > 1
        printf("Node[%d]: SendRawMessage UE routing packet from 0x%x to 0x%x\n",
           node->nodeId, ipHeader->ip_src, ipHeader->ip_dst);

        printf("Node[%d]: UE[%c] routing from incomingInterface %d routing via %c\n",
           node->nodeId, multiHomed ? 'M' : 'U', interfaceIndex,
           routeIndicator);
#endif

        if (useCellular)
        {
            CellularLayer3HandlePacketFromUpperOrOutside(node, msg, interfaceIndex, NETWORK_IPV4);
            return;
        }
        else
        {
            NetworkIpSendPacketOnInterface(node, msg, interfaceIndex,
                                           outgoingInterface, nextHop);
            return;
        }
    }
    ...
    {
        // if the pacekt is from transport layer and the destination is for special group
        // 224.0.0.x range of multicast is local to a datalink, just like a broadcast.
        // For internal routing protocols like EIGRP, OSPF, RIPv2 etc... they use the 224.0.0.x range
        // so that any neighboring routers on the same datalink (broadcast domain) will be able to send
        // and receive updates via the 224.0.0.x multicast address.
        // These multicasts are not forwarded past the originating broadcast domain, just like broadcasts.
        // There are many multicast ranges, specifically the only one you need to
        // know about right now is 224.0.0.0 - 224.0.0.255, which is link local.
        if (NetworkIpIsMulticastAddress(node, destinationAddress) &&
            destinationAddress <= IP_MAX_RESERVED_MULTICAST_ADDRESS)
        {
            // for special group, directly send to MAC without routes lookup
            NetworkIpSendPacketOnInterface(node,
                                  msg,
                                  CPU_INTERFACE,
                                  interfaceIndex,
    ...
    NodeAddress nextHop)
{
    ...
    // Dynamic address
    // Do not let any network originated packet to go to interface if
    // interface address state is not valid. Only DHCP packets should be
    // allowed. Ipv6 interface packets are exempted as DHCP supports
    // Ipv4 only
    if (outgoingInterface >= 0 &&
        ip->interfaceInfo[outgoingInterface]->interfaceType != NETWORK_IPV6
        && !NetworkIpIsDhcpPacket(msg))
    {
        if (!NetworkIpIsValidSourceAddressState(node,
                ipHeader->ip_src, outgoingInterface) ||
            ipHeader->ip_dst == 0)
        {
            MESSAGE_Free(node, msg);
            ip->interfaceInfo[outgoingInterface]->
...
//BROADCAST_IAHEP_START
            ...
            NetworkIpNeedsToForwardAppBroadcast(node, msg, ipHeader->ip_dst))
        {
            ...
            ret = ip->broadcastAppMapping->insert(pair<Int64,clocktype> (
                                                    tempId, node->getNodeTime()));

            if (!ret.second)
            {
                ret.first->second = node->getNodeTime();
            }
        }
//BROADCAST_IAHEP_END
...
// end: SOPS state update
    ...
    //{
    //    msg->pktNetworkSendTime = node->getNodeTime();
    //}
        ...
        if (fragHead)
        {
         ...
         // Below is the same code that used for non-fragment message above
                ...
                if (outgoingInterface != CPU_INTERFACE
                    ...
                    && ip->isIPsecEnabled == TRUE
                    && ipHeader->ip_p != IPPROTO_ISAKMP
                    && !isISAKMPExchangePacket(fragHead->msg))
                {
                    if (IPsecHandleOutboundPacket(node,
                                                  fragHead->msg,
                                                  incomingInterface,
...
// end: SOPS state update
        ...
        else
        {
            GUI_Receive(node->nodeId,
                ...
                //{
                //fragHead->msg->pktNetworkSendTime = node->getNodeTime();
                //}
    ...
    int incomingInterface)
{
    ...
    // Fix missing application hop by hop flow for some hops
    if (GUI_IsAppHopByHopFlowEnabled())
    {
        GUI_AppHopByHopFlow(node, msg, previousHopAddress);
    }
...
// begin: SOPS state update
//
//        This logic is very similar to the code block that follows it
//        which handles updates for the legacy QualNet GUI, with the
//        following exceptions:
//        - ROSPF is not expected as emulation packets, i.e., not considered
//        - Unicast and Multicast application traffic are both considered
//        - We make sure the source ID for application traffic corresponds
//          to the source IP address, not the previous hop ID
//
#ifdef EXATA
    if (msg->isEmulationPacket)
    {
        // Hop-by-hop traffic (from previous hop)
        //
        NodeId previousHopNodeId =
        ...
        // broadcast traffic is ignored
        //
        BOOL isValidDestination = FALSE;
        NodeAddress destAddress = ipHeader->ip_dst;

        if (destAddress != ANY_DEST) // not a network-wide broadcast address
        {
            // Check if this is a multicast packet and, if so,
            // if we are registered for the respective multicast group
            //
            if (NetworkIpIsMulticastAddress(node, destAddress) &&
                NetworkIpIsPartOfMulticastGroup(node, destAddress))
            {
                isValidDestination = TRUE;
            }
            else // not a multicast packet
            {
                // Check if the destination address matches
                // one of our interface addresses (i.e., unicast)
                //
...
//GuiEnd
    ...
    if (IsMyPacket(node, ipHeader->ip_dst, ipHeader->ip_src) ||
    ...
    const NodeInput *nodeInput)
{
    ...
    queue = new Queue(node,
                      ...
                      enableQueueStat
#ifdef ADDON_DB
                      ,false, 0, "Network CPU", true
#else
                      ,false, node->getNodeTime()
#endif
                      );
    if (enableQueueStat)
    {
    ...
    int posInQueue = DEQUEUE_NEXT_PACKET)
{
    ...
    if ((*scheduler).retrieve(ALL_PRIORITIES,
                          posInQueue,
                          msg,
                          &queuePriority, DEQUEUE_PACKET, node->getNodeTime()))
    {
    ...
    NetworkRoutingProtocolType type)
{
    switch (type)
    {
        ...
        case ROUTING_PROTOCOL_OSPFv2_TYPE1_EXTERNAL:
        {
            return ROUTING_ADMIN_DISTANCE_OSPFv2_TYPE1_EXTERNAL;
        }
        case ROUTING_PROTOCOL_OSPFv2_TYPE2_EXTERNAL:
        {
            return ROUTING_ADMIN_DISTANCE_OSPFv2_TYPE2_EXTERNAL;
        }
...
//              probably eliminate this function.
//-----------------------------------------------------------------------------
...
NetworkIpRunTimeStat(Node* node)
{
    ...
    if (node->guiOption)
    {
        ...
        {
            // Following dynamic statistics are supported only for
            // non Ipv6 nodes only.

            GUI_SendUnsignedData(node->nodeId,
                                 ip->stats.ipInReceivesId,
                                 ip->stats.ipInReceives,
                                 now + getSimStartTime(node));
            GUI_SendUnsignedData(node->nodeId,
                                 ip->stats.ipInHdrErrorsId,
                                 ip->stats.ipInHdrErrors,
                                 now + getSimStartTime(node));
        }
    }
                                      ...
                                      NodeAddress groupAddress)
{
    ...
    ListInsert(node, ip->multicastGroupList, node->getNodeTime(), entry);
...
BOOL NetworkIpIsPartOfMulticastGroup(Node *node,
                                     NodeAddress groupAddress,
                                     NodeAddress srcAddress)
{
    ...
    while (item)
    {
        ...
        // Found it!
        // If IGMPv2, them perform the legacy check else if it
        // is IGMPv3, then do additional checks.
        // Check against both grpaddress and srcaddr.
        if (igmp)
        {
            for (i = 0; i < node->numberInterfaces; i++)
            {
                IgmpNodeType nodeType = igmp->igmpInterfaceInfo[i]->igmpNodeType;
                if ((igmp->igmpInterfaceInfo[i]->igmpVersion == IGMP_V2) ||
                    ((groupAddress >= IANA_START_RANGE)
                    && (groupAddress <= IANA_END_RANGE)))
                {
                    if (entry->groupAddress == groupAddress)
                    {
                        return TRUE;
                ...
                {
                    igmpHost = igmp->igmpInterfaceInfo[i]->igmpHost;
                    grpPtr = IgmpCheckHostGroupList(
                        node, groupAddress, igmpHost->groupListHeadPointer);
                    if (grpPtr)
                    {
                        if (grpPtr->filterMode == INCLUDE)
                        {
                            for (itr = grpPtr->srcList.begin();
                                 itr < grpPtr->srcList.end();
                                 ++itr)
                            {
                                if (srcAddress == *itr)
                                {
                                    if (((nodeType == IGMP_PROXY) &&
                                       (entry->groupAddress == groupAddress))
                                       || (nodeType != IGMP_PROXY))
                                    {
                                        return TRUE;
                        ...
                        else // If Exclude NULL, then return true;
                        {
                            if (grpPtr->srcList.empty())
                            {
                                if (((nodeType == IGMP_PROXY) &&
                                    (entry->groupAddress == groupAddress))
                                    || (nodeType != IGMP_PROXY))
                            ...
                            {
                                // If non-empty exclude list, then
                                // it is my pkt only for those sources that
                                // are not in srclist
                                for (itr = grpPtr->srcList.begin();
                                     itr < grpPtr->srcList.end();
                                     ++itr)
...
//            filterMode - filter mode of the interface
//            sourceList - list of sources from where multicast traffic
//            is to be allowed or to be stopped.
// RETURN     None.
void NetworkIpJoinMulticastGroup(Node* node,
                                 Int32 interfaceId,
                                 NodeAddress mcastAddr,
...
#endif // ADDON_MAODV
        {
                if (igmp->igmpInterfaceInfo[interfaceId])
                {
                          ...
                          vector<NodeAddress> sourceList)
{
    ...
    else
    {
...
#endif // ADDON_MAODV
        {
            ...
            for (int intfId = 0; intfId < numIntf; intfId++)
            {
                if (igmp->igmpInterfaceInfo[intfId])
                {
                    // Send group joining request on every interface
              ...
              int interfaceIndex, NodeAddress previousHopAddress)
{
            ...
            || IpHeaderGetIpMoreFrag(ipHeader->ipFragment))
    {
        ...
        {
#ifdef CYBER_LIB
            // added to update the fragment received stats in DOS
            AppDOSAttackerInfo* info = NULL;
            info = (AppDOSAttackerInfo*) MESSAGE_ReturnInfo(msg, INFO_TYPE_DOSAttackerInfo);
            if (info == NULL)
            {
                // do nothing
                return;
                        ...
                        NodeAddress previousHopAddress)
{
    ...
    //interface
    BOOL isStaticRoute = rt->numStaticRoutes;
    ...
    if (node->networkData.networkProtocol == CELLULAR)
    {
         ...
         if ((CellularLayer3IsPacketGateway(node) &&
             CellularLayer3IsPacketForMyPlmn(node, msg, incomingInterface, NETWORK_IPV4)) ||
             CellularLayer3IsUserDevices(node, CPU_INTERFACE))
        {
            NetworkDataIp *ip = (NetworkDataIp*) node->networkData.networkVar;
            NetworkDataIcmp* icmp = (NetworkDataIcmp*) ip->icmpStruct;
            IpHeaderType* ipHeader = (IpHeaderType*)msg->packet;
...
#if defined(DEBUG) && DEBUG > 1
            printf("Node[%d]: RoutePacketAndSendToMac: UE routing packet from 0x%x to 0x%x\n",
                    node->nodeId, ipHeader->ip_src, ipHeader->ip_dst);

            printf("Node[%d]: UE[%c] routing from incomingInterface %d routing via %c\n",
                   node->nodeId, multiHomed ? 'M' : 'U', incomingInterface,
                   routeIndicator);
#endif

            if (useCellular)
            {
                CellularLayer3HandlePacketFromUpperOrOutside(node, msg, incomingInterface, NETWORK_IPV4);
                return;
            }
            else
            {
                NetworkIpSendPacketOnInterface(node, msg, incomingInterface,
                                               outgoingInterface, nextHop);
                return;
            }
        }
...
BOOL //inline
IsMyPacket(Node *node, NodeAddress destAddress, NodeAddress srcAddress)
{
    int i;
    ...
    if (NetworkIpIsMulticastAddress(node, destAddress))
    {
        if (NetworkIpIsPartOfMulticastGroup(node, destAddress, srcAddress))
        {
            return TRUE;
        }
...
NetworkIpInitStats(Node* node, NetworkIpStatsType* stats)
{
    ...
    if (node->guiOption)
    {
        if (node->networkData.networkProtocol != IPV6_ONLY)
        {
            ...
            stats->ipInReceivesId = GUI_DefineMetric(
                "IP: Number of Packets Received",
                node->nodeId,
                GUI_NETWORK_LAYER,
                0,
                GUI_UNSIGNED_TYPE,
            ...
            stats->ipInHdrErrorsId = GUI_DefineMetric(
                "IP: Number of Packets Received with Errors in IP Header",
                node->nodeId,
                GUI_NETWORK_LAYER,
                0,
                GUI_UNSIGNED_TYPE,
...
static clocktype GetLeastTime(
    ...
    clocktype time4)
{
    clocktype leastTime = node->getNodeTime();
...
NetworkIpPrintStats(Node *node)
{
    ...
    for (Int32 i = 0; i < node->numberInterfaces; i++)
    {
        ...
        if (ip->interfaceInfo[i]->isDhcpEnabled)
        {
            sprintf(buf, "Number of packets dropped due to invalid "
                    "address state of source or destination = %u",
                    ip->interfaceInfo[i]->
                        ipNumPktDropDueToInvalidAddressState);
            IO_PrintStat(node, "Network", "IP", ANY_DEST, i , buf);
        }
    }
    ...
    BOOL fragmentForMpls)
{
        ...
        off < (packetLen - hlen); off += fragmentedLen)
    {
        ...
        if (remaining_packetSize > (int)curPayloadLen)
        {
...
//DERIUS
    ...
    while (fp)
    {
        ...
        if (node->getNodeTime() >= fp->fragHoldTime)
        {
    ...
    else
    {
        ...
        fp->fragHoldTime = node->getNodeTime() + ip->ipFragHoldTime;
        fp->next = NULL;
    }
    ...
    const NodeInput* nodeInput)
{
    ...
    for (i = 0; i < node->numberInterfaces; i++)
    {
            ...
            || NetworkIpGetInterfaceType(node, i) == NETWORK_DUAL)
        {
            ...
            if (retVal)
            {
                ip->isSSMEnable = isSSMEnable;
                retVal = FALSE;
                bufVal[0] = '\0';
                if (ip->isSSMEnable)
                {
                    // Read SSM Group Range if provided
                    IO_ReadString(node->nodeId,
                                  NetworkIpGetInterfaceAddress(node, i),
                                  node->partitionData->nodeInput,
                                  "SSM-FIRST-GROUP-ADDRESS",
                                  &retVal,
                                  bufVal);
                    if (retVal)
                    {
                        Int32 numSubnetBits;
                        BOOL isNodeId;
                        IO_ParseNodeIdHostOrNetworkAddress(bufVal,
                                                        &ip->SSMStartGrpAddr,
                                                        &numSubnetBits,
                                                        &isNodeId);
                        retVal = FALSE;
                        ...
                        if (retVal)
                    {
                        IO_ParseNodeIdHostOrNetworkAddress(bufVal,
                                                         ...
                                                         &numSubnetBits,
                                                         &isNodeId);
                        ERROR_Assert(ip->SSMStartGrpAddr < ip->SSMLastGrpAddr,
                                    "Invalid Last Group Address");
                    }
                    else
                    {
                        ERROR_ReportError("Last Group Address Not Provided");
                    }
                    ...
                    else
                    {
                        IO_ConvertStringToNodeAddress(
                                                SSM_DEFAULT_FIRST_GRP_ADDR,
                                                &ip->SSMStartGrpAddr);
                        IO_ConvertStringToNodeAddress(
                                                 SSM_DEFAULT_LAST_GRP_ADDR,
                                                 &ip->SSMLastGrpAddr);
                    }
                }
            }
    ...
    std::string *protocolString)
{
    switch (type)
    {
        ...
        case ROUTING_PROTOCOL_OSPFv2_TYPE1_EXTERNAL:
            *protocolString = "OSPFv2_TYPE1_EXTERNAL";
            break;

        case ROUTING_PROTOCOL_OSPFv2_TYPE2_EXTERNAL:
            *protocolString = "OSPFv2_TYPE2_EXTERNAL";
            break;
    ...
    Message* msg)
{
    ...
    {
        if ((node->getNodeTime() - it->second) >= ip->broadcastForwardingTimeout)
          {
            tempIt= it;
            it++;
    ...
    NodeAddress nextHop)
{
    ...
    *timing = node->getNodeTime();
    ...
    if (node->networkData.networkProtocol != IPV6_ONLY)
    {
        NetworkIpAddPacketSentToMacDataPoints(node, msg, outgoingInterface);
    }
                          ...
                          NetworkType networkType)
{
    ...
    switch(messageType)
    {
        case INIT_DHCP:
        {
            ...
            oldAddress.networkType = networkType;

            interfaceInfo->ipAddress = 0;
            interfaceInfo->addressState = INVALID;
            interfaceInfo->isDhcpEnabled = TRUE;

            interfaceInfo->numHostBits = 0;

            for (Int32 i = 0; i < loopbackFwdTable->size; i++)
            {
                if (loopbackFwdTable->row[i].destAddress ==
                                            oldAddress.interfaceAddr.ipv4)
                {
                    loopbackFwdTable->row[i].destAddress =
                                        interfaceInfo->ipAddress ;
                }
            }

            MAPPING_InvalidateIpv4AddressForInterface(node,
                                        interfaceIndex,
                                        &oldAddress);

            IpSendNotificationOfAddressChange(node,
                                              interfaceIndex,
                                              &oldAddress,
                                              subnetMask,
                                              networkType);
            ...
            Address address ;
            address.interfaceAddr.ipv4 = 0;
            address.networkType = networkType;
        ...
        case INFORM_DHCP:
        {
            ...
            interfaceInfo->addressState = PREFERRED;
            interfaceInfo->isDhcpEnabled = TRUE;

            interfaceInfo->numHostBits =
               AddressMap_ConvertSubnetMaskToNumHostBits(subnetMask);
        ...
        case LEASE_EXPIRY:
        {
            ...
            address.interfaceAddr.ipv4 = 0;
            address.networkType = networkType;
            // when lease expires, the loopback table is also
            // updated such that old loopback address (i.e old ip-address)
            // is invalidated . This will get updated when a
            // new address is acquired again
            NetworkForwardingTable* loopbackFwdTable =
                                      &(ip->loopbackFwdTable);
            for (Int32 i = 0; i < loopbackFwdTable->size; i++)
            {
        ...
        case RECEIVE_ADDRESS:
        {
            ...
            interfaceInfo->ipAddress = address->interfaceAddr.ipv4;
            interfaceInfo->addressState = PREFERRED;
            interfaceInfo->isDhcpEnabled = TRUE;
            ...
            // add a default route to gateway  if wired network
        ...
        default:
        {
            ERROR_ReportWarning("In NetworkIpProcessDHCP: Unknown Message"
                                " from DHCP");
            break;
        }
    }
                                       ...
                                       NetworkType networkType)
{
    ...
    if (ip->addressChangedHandlerList == NULL)
    {
        return;
    }

    ListItem* listItem = ip->addressChangedHandlerList->first;

    IpAddressChangedHandlerFunctionType addressChangedHandler;

    while (listItem != NULL)
    {
        addressChangedHandler = (IpAddressChangedHandlerFunctionType)
                                                              listItem->data;

        ERROR_Assert(addressChangedHandler, "In "
                        "IpSendNotificationOfAddressChange and invalid "
                        "handler function");

        (addressChangedHandler)(node,
                                interfaceIndex,
                                address,
                                subnetMask,
                                networkType);
            ...
            IpAddressChangedHandlerFunctionType addressChangeFunctionPointer)
{
    ...
    if (ip->addressChangedHandlerList == NULL)
    {
        ListInit(node, &ip->addressChangedHandlerList);
    }

    ListInsert(node,
               ip->addressChangedHandlerList,
               node->getNodeTime(),
               (void*)addressChangeFunctionPointer);
}
...
{
    char* pktPtr = NULL;
    if (msg->headerProtocols[msg->numberOfHeaders - 1] == TRACE_IP)
    {
        // this is a packet with IP header
        IpHeaderType* ipHdr = (IpHeaderType*) MESSAGE_ReturnPacket(msg);
        if (ipHdr && ipHdr->ip_p == IPPROTO_UDP)
        {
            pktPtr = (char*)ipHdr + IpHeaderSize(ipHdr);
        }
    }
    else if (msg->headerProtocols[msg->numberOfHeaders - 1] == TRACE_UDP)
    {
        // this is an UDP packet and IP header not added yet
        pktPtr = MESSAGE_ReturnPacket(msg);
    }

    if (pktPtr)
    {
        TransportUdpHeader* udpHeader  = (TransportUdpHeader*)pktPtr;
        if (udpHeader->sourcePort == APP_DHCP_SERVER ||
            udpHeader->sourcePort == APP_DHCP_CLIENT ||
            udpHeader->sourcePort == APP_DHCP_RELAY)
        {
            return TRUE;
        }
    }
                                        ...
                                        Int32 interfaceIndex)
{
    ...
    {
        NetworkDataIp* ip = (NetworkDataIp*) node->networkData.networkVar;
        if (ip->interfaceInfo[interfaceIndex]->addressState == INVALID)
        {
            return FALSE;
        }
    }
...
bool isValidIpAddress(const char addressString[])
{
    ...
    else
    {
        while (index != string::npos)
        {
            ...
            {
                tempNumber = -1;
                istringstream (address.substr(previousIndex)) >> tempNumber;
                if (tempNumber < 0 || tempNumber > 255)
                {
                    return false;
                }
