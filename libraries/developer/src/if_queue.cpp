...
// Returns the positon of a packet
//
...
// Calculates the queue delay
//
// \param packetArrayIndex  The position of the packet Info field
...
// Helps in Printing Final Queue Statistics
//
...
// \param invokingProtocol  Protocol invoking finalize
//
...
// This function return the current queue behavior
//
//
...
// \param interfaceIndex  used to generate random seed
// \param qNumber  used to generate random seed
    ...
    )
{
    ...
    {
#ifdef ADDON_DB
        if (isFromNetworkLayer && node->partitionData->statsDb != NULL)
        {
...
// This function will free the memory in the queue
//
//
Queue::~Queue()
{
...
// This function runs the generic and then the
// algorithm-specific queue initialization routine.
//
// \param node  node
// \param queue  Pointer of pointer to Queue class
// \param queueTypeString[]  Queue Type as specified in config file
// \param queueSize  Queue Size
// \param interfaceIndex  used for setting random seed
// \param queueNumber  used for setting random seed
// \param infoFieldSize  Default value 0;
// \param enableQueueStat  Default value false
// \param showQueueInGui  To show this Queue in GUI
// \param currentTime  Current simulation time.
// \param configInfo  Pointer to a structure that contains
//    configuration parameters for a Queue Type
//    This is not required for FIFO. Is it is
...
{
    bool enableQueueStatBoolean = enableQueueStat?true:false;
    bool showQueueInGuiBoolean = showQueueInGui?true:false;

#ifdef ADDON_DB
    bool isFromNetworkLayerBoolean = isFromNetworkLayer?true:false;
#endif
    ...
    if (!strcmp(queueTypeString, "FIFO"))
    {
        *queue = new Queue(node,
                             ...
                             qNumber,
                             infoFieldSize,
                             enableQueueStatBoolean,
                             showQueueInGuiBoolean
#ifdef ADDON_DB
                             , 0, queuePosition, isFromNetworkLayerBoolean
#else
                             ,currentTime
#endif
                             , maxPktAge
    ...
    else if (!strcmp(queueTypeString, "RED"))
    {
        *queue = new RedQueue(node,
                             ...
                             showQueueInGuiBoolean,
                             currentTime,
                             (RedParameters*)configInfo
#ifdef ADDON_DB
                             , queuePosition, isFromNetworkLayerBoolean
#endif
                             , maxPktAge
                             );
    }
...
// This function prototype returns the number/index of the Queue
//
//
// \return Integer
