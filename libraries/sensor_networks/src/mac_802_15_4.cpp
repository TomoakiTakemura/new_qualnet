...
// Convert Variable Hardware address to ZigBee MACADDR
//
// \param node  Pointer to Node structure
// \param macHWAddr  Pointer to hardware address structure
// \param macAddr  Pointer to ZigBee MAC address structure
static
...
// Convert ZigBee Mac addtess to Variable Hardware address
//
// \param node  Pointer to Node structure
// \param macHWAddr  Pointer to hardware address structure
// \param macAddr  Pointer to mac 802 address structure
static
...
// To initiate Channel clear access request
//
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #SetTimer#\n",node->getNodeTime(),node->nodeId);
}
...
// Add MHR to a message
//
// \param node  Node receiving call
// \param interfaceIndex  Interface index
// \param msg  Message pointer
// \param frmType  Frame type (beacon/data/ack/MAC cmd)
// \param superSpec  Superframe specification
// \param GTSFields  GTS fields - not used currently
// \param PendAddrFields  List of devices for whom data
//    is pending
// \param CmdType  Command type for MAC cmd frame
//
static
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #ConstructPayload#\n",node->getNodeTime(),node->nodeId);
}
...
// \param msg  Message pointer
// \param FrmType  Frame type (beacon/data/ack/MAC cmd)
// \param dstAddrMode  Addressing mode (short/long)
// \param dstPANId  Destination PAN Id
// \param dstAddr  Destination address
// \param srcAddrMode  Source Address mode
// \param srcPANId  Source PAN Id
// \param srcAddr  Source address
// \param secuEnable  Whether security is enabled
// \param pending  Whether data pending
// \param ackreq  Whether Ack requested
// \param FCS  checksum - usually not calculated
//
static
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #AddCommandHandler#\n",node->getNodeTime(),node->nodeId);
}
...
// Construct acknowledgement for a message
//
// \param node  Node receiving call
// \param interfaceIndex  Interface index
// \param msg  Message pointer
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #ConstructureACK#\n",node->getNodeTime(),node->nodeId);
}
...
// Returns start of contention access period
//
// \param node  Node receiving call
// \param interfaceIndex  Interface index
//    + small          : BOOL      :
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #GetCAP#\n",node->getNodeTime(),node->nodeId);
}
    ...
    {
if(DEBUG){
printf("tCAP : To parent\n");
printf("t_CAP1 : %ld\nt_CAP2 : %ld\nnow : %ld\n",t_CAP1,t_CAP2,now);
}
        if (t_CAP2 < now || mac->mpib.macBeaconOrder == 15)
        {
			//printf("return CAP2(1) : %d, %d\n",t_CAP2 < now, mac->mpib.macBeaconOrder == 15);
            return t_CAP2;
        }
        else
        {
    ...
    {
if(DEBUG){
printf("GetCAP : To child and other destinations\n");
}
...
// Returns the start of contention access period
//
// \param node  Node receiving call
// \param interfaceIndex  Interface index
//    + type           : Int32       :
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #GetCAPbyType#\n",node->getNodeTime(),node->nodeId);
}
...
// Check if there is any pending packet in Network queue
//
// \param node  Node receiving call
// \param interfaceIndex  Interface index
//
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #CheckForOutgoingPacket#\n",node->getNodeTime(),node->nodeId);
}
...
// Get the respective GTS and superframe specifications of a 
// FFD/RFD etc
//
// \param node  Node receiving call
// \param interfaceIndex  Interface index
// \param gtsSpec  Pointer to Gts specification
//    + sfSpec  : M802_15_4SuperframeSpec**: Pointer to superframe 
//    specifications
// \param referenceBeaconTime  Last beacon sent/receive time
//
static
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #GetSpecks#\n",node->getNodeTime(),node->nodeId);
}
...
// Check if data packet can be transmitted during GTS
//
// \param node  Node receiving call
// \param interfaceIndex  Interface index
// \param mac  Pointer to MAC802.15.4 structure
// \param i  Position of the GTS descriptor
// \param msg  Message that needs to be send
//
// \return BOOL
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #CanProceedInGts#\n",node->getNodeTime(),node->nodeId);
}
...
// \param csmacaRes  Whether to resume CSMA-CA
//
static
void Mac802_15_4TaskFailed(Node* node,
                           ...
                           BOOL csmacaRes)
{if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #TaskFaild#\n",node->getNodeTime(),node->nodeId);
}
...
// Generates GTS characteristics
//
// \param gtsRequestType  Gts request type
// \param numslots  Number of slots
// \param gtsDirection  Gts direction
//
// \return UInt8
static
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #TaskSuccess#\n",node->getNodeTime(),node->nodeId);
}
...
// \param node  Node receiving call
// \param interfaceIndex  Interface index
// \param pktType  Type of packet for which CSMA has to be
//    started
//
static
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #CsmacaBegin#\n",node->getNodeTime(),node->nodeId);
}
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #Reset_TRX#\n",node->getNodeTime(),node->nodeId);
}
...
// Parses GTS characteristics
//
// \param gtsReqChrcts  GTS chracteristics
// \param gtsReqData  Pointer to the GTS request datastructure
//
static 
...
// Removes a GTS descriptor saved at a device
//
// \param node  Node which sends the message
// \param mac  802.15.4 data structure
// \param j  The position of GTS descriptot in the array
...
// \param frFunc  Function name from which dispatch() has
//    been called
//    + req_state      : PLMEsetTrxState   :
// \param mStatus  MAC layer status
//
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #Dispatch#\n",node->getNodeTime(),node->nodeId);
}
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #CsmacaResume#\n",node->getNodeTime(),node->nodeId);
}
...
// Callback received from CSMA-CA
//
// \param node  Node receiving call
// \param interfaceIndex  Interface index
// \param status  PHY status
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #CsmacaCallBack#\n",node->getNodeTime(),node->nodeId);
}
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #ToParent#\n",node->getNodeTime(),node->nodeId);
}
...
// Locate slot boundry for beacon enabled PAN after given time
//
// \param node  Node receiving call
// \param interfaceIndex  Interface index
// \param parent  If from parent
// \param wtime  Time after which slot boundry is to be
//    located
//
// \return clocktype
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #LocateBoundary#\n",node->getNodeTime(),node->nodeId);
}
...
// Find out if MAC layer can proceed with CSMA for a given pkt
// in current CAP
//
// \param node  Node receiving call
// \param interfaceIndex  Interface index
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #CanProceedWOcsmaca#\n",node->getNodeTime(),node->nodeId);
}
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #CalculateBcnRxTimer#\n",node->getNodeTime(),node->nodeId);
}
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #CalculateBcnRxTimer2#\n",node->getNodeTime(),node->nodeId);
}
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #TransmitCmdData#\n",node->getNodeTime(),node->nodeId);
}
...
// Handles Association response wait timer event
//
// \param node  Node receiving call
// \param interfaceIndex  Interface index
//
...
// Handles receive enable timer event
//
// \param node  Node receiving call
// \param interfaceIndex  Interface index
//
...
// Removes a GTS descriptor saved at PanCoordinator
//
// \param node  Node which sends the message
// \param mac  802.15.4 data structure
// \param i  The position of GTS descriptot in the array
//
// NOTES        ::None
static
...
// Updated the GTS descriptor in a beacon
//
// \param node  Node which sends the message
// \param mac  802.15.4 data structure
//
...
// Schedules GTS timer at PanCoordinator
//
// \param node  Node which sends the message
// \param interfaceIndex  interface index
// \param mac  802.15.4 data structure
...
// Handles Beacon transmit timer event
//
// \param node  Node receiving call
// \param interfaceIndex  Interface index
//
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #BeaconTxHandler#\n",node->getNodeTime(),node->nodeId);
}
...
// Handles beacon receive timer event
//
// \param node  Node receiving call
// \param interfaceIndex  Interface index
//
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #BeaconRxHandler#\n",node->getNodeTime(),node->nodeId);
}
        ...
        if (mac->taskP.mlme_sync_request_tracking)
        {
            ...
            else
            {
                ...
                if (DEBUG_TIMER)
                {
					...
					//takemura comment
					//set timer for next beacon Rx timer
...
// Handles beacon search timer event
//
// \param node  Node receiving call
// \param interfaceIndex  Interface index
//
...
// Handles tranmit BcnCmd Data event
//
// \param node  Node receiving call
// \param interfaceIndex  Interface index
//
...
// Handles inter frame space timer
//
// \param node  Node receiving call
// \param interfaceIndex  Interface index
//
...
void Mac802_15_4IFSHandler(Node* node, Int32 interfaceIndex)
{
    ...
    if (mac->rxCmd)
    {
        ...
        else if (wph->MSDU_CmdType == 0x04) // Data request
        {
			...
			//Mac802_15_4CanPeoceedWOcscaca : 
			//現在のCAP内でpktをCSMAで送信できるかどうかを判断
...
// Handles back off bound timer event
//
// \param node  Node receiving call
// \param interfaceIndex  Interface index
//
...
// Handles orphan response timeout timer event
//
// \param node  Node receiving call
// \param interfaceIndex  Interface index
//
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #PD_DATA_confirm#\n",node->getNodeTime(),node->nodeId);
}
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #PLME_CCA_confirm#\n",node->getNodeTime(),node->nodeId);
}
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #PLME_ED_confirm#\n",node->getNodeTime(),node->nodeId);
}
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #PLME_GET_confirm#\n",node->getNodeTime(),node->nodeId);
}
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #PLME_SET_TRX_STATE#\n",node->getNodeTime(),node->nodeId);
}
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #PLME_SET_confirm#\n",node->getNodeTime(),node->nodeId);
}
...
// \param frUpper  From upper layer
// \param status  PHY status
// \param mStatus  MAC status
//
static
void Mac802_15_4mcps_data_request(Node* node,
                                 Int32 interfaceIndex,
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #mcps_data_request#\n",node->getNodeTime(),node->nodeId);
}
...
// \param frUpper  Whether from upper layer
// \param status  Phy status
//
static
void Mac802_15_4mlme_scan_request(Node* node,
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #mlme_scan_request#\n",node->getNodeTime(),node->nodeId);
}
...
// Allows the PAN coordinator to initiate a new PAN
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #mlme_start_request#\n",node->getNodeTime(),node->nodeId);
}
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #mlme_associate_request#\n",node->getNodeTime(),node->nodeId);
}
...
// Initiate a response from SSCS sublayer
//
// \param node  Node receiving call
// \param interfaceIndex  Interface index
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #mlme_associate_response#\n",node->getNodeTime(),node->nodeId);
}
...
// Request for disassociation from a PAN
//
// \param node  Node receiving call
// \param interfaceIndex  Interface index
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #mlme_disassociate_request#\n",node->getNodeTime(),node->nodeId);
}
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #mlme_sync_request#\n",node->getNodeTime(),node->nodeId);
}
...
#ifdef CHANGE_BI
						if(mac->LastMyBeacon.isVariableBeacon){
							//this beacon is variable beacon
							//check whether next bcn is normal or variable
							...
							if(!mac->numVarBcnHandler == numVarB){
								//next beacon is variable beacon
	                        	mac->bcnRxT =
    	                          	Mac802_15_4SetTimer(
                                   	node,
                                   	mac,
                                   	M802_15_4VARBEACONRXTIMER,
                                   	MyApiCalculateVarBcnRxTimer2(node,
                                                            interfaceIndex),
                                                            NULL);
								mac->VarBcnRxT = mac->bcnRxT;
							}
							else{
								//next beacon is normal beacon
								mac->bcnRxT = mac->NorBcnRxT;
							}
						}
						else{
							//this beacon is normal beacon
							//set timer for next normal beacon
							//set timer for variable beacon if it will be used
                	        mac->bcnRxT =
                    	          Mac802_15_4SetTimer(
                        	           	node,
                            	       	mac,
							...
							MQTTClientData *mqttcli;
							mqttcli = MacClientGetClientApp(node);
							if(mqttcli->func == FN_SEND_CONNECT 
									&& mac->LastMyBeacon.VariableBO != mac->macBeaconOrder2){
								//use variable beacon
								mac->VarBcnRxT =
									Mac802_15_4SetTimer(
											node,
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #mlme_orphan_response#\n",node->getNodeTime(),node->nodeId);
}
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #mlme_poll_request#\n",node->getNodeTime(),node->nodeId);
}
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #mlme_reset_request#\n",node->getNodeTime(),node->nodeId);
}
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #mlme_rx_enable_request#\n",node->getNodeTime(),node->nodeId);
}
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #MCPS_DATA_indication#\n",node->getNodeTime(),node->nodeId);
}
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #MLME_DISASSOCIATE_indication#\n",node->getNodeTime(),node->nodeId);
}
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #MLME_SET_request#\n",node->getNodeTime(),node->nodeId);
}
...
// Updates GTS chracteristics obtained in a beacon 
// from panccoord
// PARAMETERS   
...
// Updates GTS trigger precedence
//
// \param node  Pointer to node
// \param interfaceIndex  Interface index on MAC
// \param bcnMsg  beacon message
//
static
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #RecvBeacon#\n",node->getNodeTime(),node->nodeId);
}
...
if (DEBUG){
		printf("\n#RecvBeacon#\nnum DataPkt queue is %d\nnum DataPkt dequeue is %d\n",mac->stats.numDataPktsQueuedForCap,mac->stats.numDataPktsDeQueuedForCap);
	}
	...
	//takemura
	//ここでビーコンの内容をNodeのMac層に反映している？
	if(DEBUG){
		printf("Node %d : Mac802_15_4RecvBeacon() UpdateMACDate from beacon\n",node->nodeId);
	}
    ...
    mac->panDes2.SuperframeSpec = bcnFrm->MSDU_SuperSpec;//BO,SOとかをBeaconから取得
    ...
    mac->numLostBeacons = 0;
    mac->macBeaconOrder2 = mac->sfSpec2.BO;// takemura kore juuyou kamo?
    mac->macSuperframeOrder2 = mac->sfSpec2.SO;
    ...
    mac->sfSpec2.superSpec = bcnFrm->MSDU_SuperSpec;// takemura  get bcnData and update superSpec
    Mac802_15_4SuperFrameParse(&mac->sfSpec2); // takemura  Update mac->sfSpec2->BO,BI,SO... by mac->sfSpec2.superSpec
    mac->gtsSpec3.fields = bcnFrm->MSDU_GTSFields;

	//takemura for CHANGE_BI
#ifdef CHANGE_BI
	MyApiBeaconData *BData;
	BData = (MyApiBeaconData *)MESSAGE_ReturnInfo(msg, INFO_TYPE_MYAPI_BEACON);
	mac->LastMyBeacon = *BData;
	if(BData->isVariableBeacon)
		mac->VarBcnRxTime = mac->macBcnRxTime;
	else{
		mac->NorBcnRxTime = mac->macBcnRxTime;
		mac->numVarBcnHandler = 0;
	}
#endif
...
// To process an incoming Ack message
//
// \param node  Pointer to node
// \param interfaceIndex  Interface index on MAC
// \param msg  Incoming message
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #RecvAck#\n",node->getNodeTime(),node->nodeId);
}
...
// \param node  Pointer to node
// \param mac  Pointer to MAC802.15.4 structure
// \param startSlot  GTS start slot
// \param numGtsSlots  GTS length
// \param gtsDirection  GTS direction
// \param wph  Pointer to the MAC header
//
// NOTES        ::None
static
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #CreateDescriptorForBeacon#\n",node->getNodeTime(),node->nodeId);
}
...
// \param interfaceIndex  interface index
// \param mac  Pointer to MAC802.15.4 structure
// \param wph  Pointer to the MAC header in the data packet
// \param payld  GTS chracteristics
//
// NOTES        ::None
static
...
// To process an incoming MAC command
//
// \param node  Pointer to node
// \param interfaceIndex  Interface index on MAC
// \param msg  Incoming message
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #RecvCommand#\n",node->getNodeTime(),node->nodeId);
}
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #RecvData#\n",node->getNodeTime(),node->nodeId);
}
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #QueuePackets#\n",node->getNodeTime(),node->nodeId);
}
...
if (DEBUG){
	printf("send in CAP\n");
				...
				//takemura
				if (DEBUG){
					printf("numDataQueueForCap : %d",mac->stats.numDataPktsQueuedForCap);
				}
                            }
                            else
                            {
           ...
           {
	if (DEBUG){
		printf("Use CAP for data transfer\n num QueuePkts is %d\nnum DeQueuePkts is %d\n",mac->stats.numDataPktsQueuedForCap,mac->stats.numDataPktsDeQueuedForCap);
	}
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #SendPacketInCap#\n",node->getNodeTime(),node->nodeId);
}
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #NetworkLayerHasPacketToSend#\n",node->getNodeTime(),node->nodeId);
}
   ...
   if ((mac->mpib.macBeaconOrder != 15) ||(mac->macBeaconOrder2 != 15))
	...
	if (KANDEB){
		//printf("mcps_data_request : %u\nmlme_poll_request : %u\nbackoffStatus : %u\nmcps_broadcast_request : %u\n",
		//	mac->taskP.mcps_data_request_STEP,
		//	mac->taskP.mlme_poll_request_STEP,
		//	mac->backoffStatus,
		//	mac->taskP.mcps_broadcast_request_STEP);
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #ReceivePacketFromPhy#\n",node->getNodeTime(),node->nodeId);
}
...
#ifdef CHANGE_BI
    	if(!mac->isPANCoor){
			if (MacClientIsUsedBeacon(node, msg, mac) == false){
				//clients stats isnt active, and this beacon is variable beacon.
				//this message is not useful
				if(DEBUG){
					printf("this Beacon is variable Beacon.\n---not used---\n");
				}
				MESSAGE_Free(node, msg);
				return;
			}
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #ReceivePhyStatusChangeNotification#\n",node->getNodeTime(),node->nodeId);
}
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #PrintStatus#\n",node->getNodeTime(),node->nodeId);
}
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #BroadcastHandler#\n",node->getNodeTime(),node->nodeId);
}
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #Layer#\n",node->getNodeTime(),node->nodeId);
}
        ...
        case MSG_MAC_TimerExpired:
        {
            ...
            switch (timerInfo->timerType)
            {
                ...
                case M802_15_4GTSDEALLOCATETIMER:
				 ...
				 //takemura
#ifdef CHANGE_BI
				case M802_15_4VARBEACONRXTIMER:
				{
                    mac802_15_4->bcnRxT = NULL;
					mac802_15_4->VarBcnRxT = NULL;
					MyApiVarBeaconRxHandler(node, interfaceIndex);
					break;
				}
#endif
                default:
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #Init#\n",node->getNodeTime(),node->nodeId);
}
    ...
    mac802_15_4->broadCastQueue = new Queue(node,
                                            "FIFO",
    ...
    mac802_15_4->capQueue = new Queue(node,
                                      "FIFO",
                                      M802_15_4BROADCASTQUEUESIZE);
...
{
if (KANDEB){
	printf("%" TYPES_64BITFMT "d : Node %d: mac.cpp in the #Finalize#\n",node->getNodeTime(),node->nodeId);
}
