...
const char stpGroupAddress[6] = {0x01, (char)0x80, (char)0xc2, 0x00, 0x00, 0x00};
...
// Send frame on a port supports 802.3
//
    ...
    const NodeInput* nodeInput)
{
        ...
        SWITCH_UNLIMITED_BACKPLANE_THROUGHPUT)
    {
            ...
            if (node->partitionData->statsDb != NULL)
            {
                if (node->partitionData->statsDb->statsAggregateTable->createQueueAggregateTable)
                {
                    ERROR_ReportError(
                        ...
                        " SWITCH-QUEUE-STATISTICS to be set to YES\n");
                }
        ...
        queue = new Queue(node, "FIFO", queueSize, 0, 0, 0, enableQueueStat);
