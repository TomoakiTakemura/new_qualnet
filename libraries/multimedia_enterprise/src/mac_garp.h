...
/// Enumerated values of an applicant's states.
typedef enum
{
...
/// Enumerated values of an applicant's management states.
typedef enum
{
    GARP_GID_Normal,
    GARP_GID_NoProtocol
...
/// Enumerated values of a registrar's states.
typedef enum
{
    GARP_GID_In,
    GARP_GID_Leave,
...
/// Enumerated values of a registrar's management states.
typedef enum
{
...
/// Elements to describe a GID's state.
typedef struct
{
...
//
/// Enumerated values to all possible events of GID.
typedef enum
{
    GARP_GID_NULL,                  // 0
...
/// Enumerated values for GARP self timers.
typedef enum
{
    GARP_JoinTimer,
    GARP_HoldTimer,
...
/// Elements of a GARP self timers.
...
/// Initialize the main Garp data structure.
///
/// \param node  Pointer to node.
/// \param garp  Pointer to GARP data.
void Garp_Init(Node* node,
               Garp* garp);

/// Create a new instance of GID for the given port.
///
/// \param node  Pointer to node.
/// \param garp  Pointer to GARP data.
/// \param interfaceIndex  Interface index.
/// \param portNumber  Assigned number to the port.
///
/// \return TRUE, if instance can be created.
/// FALSE, otherwise.
...
/// Destroy instance of GID for the given port.
///
/// \param node  Pointer to node.
/// \param garp  Pointer to GARP data.
/// \param portNumber  Assigned number to the port.
...
/// Find the GID instance for the given port.
///
/// \param firstGid  Pointer to GID list.
/// \param portNumber  Assigned number to the port.
/// \param gid  Pointer to collect GID instance.
///
...
/// Find a machine not used by any Port.
///
/// \param garp  Pointer to GARP data.
/// \param fromIndex  Index to start.
/// \param foundIndex  Pointer to collect index of machine.
///
/// \return TRUE, if port found,
/// FALSE, otherwire.
...
/// Change attribute's management state on given port.
///
/// \param node  Pointer to node.
/// \param portGid  Pointer to GID data.
/// \param index  Index of a GID mechine.
/// \param directive  Event to process on this machine.
///
...
/// Process state machines of an attribute, indicated by
/// gidIndex, with the received attribute event.
///
/// \param node  Pointer to node.
/// \param portGid  Pointer to GID data.
/// \param gidIndex  Index of GID machine.
/// \param msg  Event to process on this machine.
///
...
/// Process leave all event for all attributes present
/// at port gid.
///
/// \param portGid  Pointer to GID data.
///
...
/// Process after expiry of Leave timer.
///
/// \param node  Pointer to node.
/// \param portGid  Pointer to GID data.
/// \param portNumber  Assign number to the port.
///
...
/// Process after expiry of Join timer.
///
/// \param node  Pointer to node.
/// \param garp  Pointer to GID data.
/// \param portNumber  Assign number to the port.
///
...
/// Process after expiry of Leave All timer.
///
/// \param node  Pointer to node.
/// \param garp  Pointer to GID data.
/// \param portNumber  Assign number to the port.
...
/// Process after expiry of Hold timer.
///
/// \param node  Pointer to node.
/// \param garp  Pointer to GID data.
/// \param portNumber  Assigned number to the port.
...
/// Check to see either if a leave all event is
/// required to be sent or scan the GID machines
/// for messsages that require transmission.
///
/// \param node  Pointer to node.
/// \param portGid  Pointer to GID at this port.
/// \param gidIndex  Pointer to collect index of a machine.
///
/// \return GarpGidEvent.
...
/// Initiate necessary actions when all messages cannot be
...
/// Report given GID machine state.
///
/// \param machine  Pointer to particular machine.
/// \param state  Pointer to collect state of the machine.
///
void GarpGidTt_States(
    GarpGidMachine* machine,
    GarpGidStates* state);

/// Create a new instance of GIP.
///
/// \param max_attributes  Maximum attributes to be handled.
/// \param gip  Pointer to collect created GIP.
///
...
/// Destroy the instance of GIP.
///
/// \param gip  Pointer to GIP.
///
void GarpGip_DestroyGip(
    void* gip);

/// Connect the port into GIP propagation ring.
///
/// \param node  Pointer to node.
/// \param garp  Pointer to GARP data.
/// \param portNumber  Assigned number to the port.
...
/// Disconnect the port from GIP propagation ring.
///
/// \param node  Pointer to node.
/// \param garp  Pointer to GARP data.
/// \param portNumber  Assigned number to the port.
...
/// Process a GARP application frame received from MAC.
///
/// \param node  Pointer to node.
/// \param sw  Pointer to switch.
/// \param msg  Pointer to received message.
/// \param sourceAddr  Source address of the frame.
/// \param destAddr  destination address of the frame.
/// \param incomingInterfaceIndex  Incoming interface of switch.
/// \param vlanId  VLAN ID carried by the frame.
/// \param priority  Prioryty of this frame.
///
...
/// Decode received event for an attribute.
///
/// \param event  Received event.
/// \param decodedEvent  Decoded event.
///
...
///
/// \param node  Pointer to node.
/// \param interfaceIndex  Interface index to process the message.
