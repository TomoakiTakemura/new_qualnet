...
// Allocate memory for circular buffer in Circular Buff Vector
//
// \param sip  Pointer to the SipData instance
// \param connId  Connection Id for which Cir. Buf to be
//    alloted.
//
// \return NIL
...
// To get the cicular buffer corresponding to a connection ID
// note: there is one circular buffer for each conn.ID.
//
// \param sip  Pointer to the SipData instance
// \param connId  Connection ID of the TCP  connection
//
// \return Pointer to the desired circular Buffer
...
// Delete the circular buffer with conn. ID from recd message
//
// \param sip  Pointer to the SipData instance
// \param connId  ConnectionId of the cir.buffer to be deleted
//
// \return NIL
...
// Delete the entire circular buffer vector for this SipData
//
// \param sip  Pointer to the SipData instance
//
// \return NIL
...
// To write the contents of incoming packet from transport
...
// check if the passed buffer contains a complete message
//
// \param inputBuf  Pointer to the input buffer
// \param pktSize  size of the above buffer
//
// \return Actual message length, zero if incomplete
...
// Check if the Sip node is working as Client
//
// \param node  Pointer to the node to be checked
//
// \return True if the node is a client
...
// Check if the Sip node is working as server
//
// \param node  Pointer to the node to be checked
//
...
// Initializes the SIP-related structures of the sip node
//
...
// Reads the list of proxy nodes, and initializes each
...
// Handles the Connection delay timer event
//
...
RtpSession* RtpGetSessionPtrByDestAddr(RtpData* rtp,
                                       ...
                                       BOOL isRtpProcessed);
// Handles successful TCP connection establishment event
// at the client(UA) side.
//
// \param node  Pointer to the node initiating TCP connection
// \param sip  Pointer to SipData
// \param openResult  Pointer to
...
// Handles Invite requests at UA node
//
// \param node  Pointer to the node receiving the request
// \param sip  Pointer to SipData
// \param sipMsg  Pointer to SipMsg
//
...
// Handles Invite requests at Proxy node
//
// \param node  Pointer to the node receiving the request
// \param sip  Pointer to SipData
// \param sipMsg  Pointer to SipMsg
...
//
// \param node  Pointer to the node receiving the request
// \param sipMsg  Pointer to SipMsg carrying the request
//
...
// Handles Trying Response
//
// \param node  Pointer to node receiving the response
// \param sipMsg  Pointer to SipMsg carrying the response
//
...
// Handles Ringing Response
//
// \param node  Pointer to the node receiving the response
// \param sipMsg  Pointer to SipMsg carrying the response
//
...
// Handles OK response(Proxy node)
//
// \param node  Pointer to the node receiving the response
// \param sip  Pointer to SipData
// \param sipMsg  Pointer to SipMsg carrying the response
...
// Handles OK response(UA node) for Proxy routed call
//
// \param node  Pointer to the node receiving the response
// \param sip  Pointer to SipData
// \param sipMsg  Pointer to SipMsg carrying the response
...
// Handles Request packet receipt
//
// \param node  Pointer to the node receiving the packet
// \param sipMsg  Pointer to SipMsg carrying the packet
//
...
// Handles Response packet receipt
//
// \param node  Pointer to the node receiving the packet
// \param sipMsg  Pointer to SipMsg carrying the packet
//
...
// Handles TCP connection close event
//
// \param node  Pointer to the node closing the connection
// \param sipMsg  Pointer to SipMsg carrying the event
//
...
//
// \param node  Pointer to the node at which call times out
// \param sipMsg  Pointer to SipMsg carrying the event
//
...
void SipInitiateConnection(Node* node, void* voip)
{
    ...
    else if (sip->SipGetCallModel() == SipProxyRouted)
    {
         if (sip->SipGetTransProtocolType() == TCP)
         {
            ...
            std::string proxyUrl = sip->SipGetProxyUrl(); 
            // As proxyUrl is being passed by reference
            // Need to store in temp varible
            node->appData.appTrafficSender->appTcpOpenConnection(
                                                    node,
                                                    APP_SIP,
                                                    localAddr,
