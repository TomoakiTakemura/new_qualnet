...
void RoutingPimHandleProtocolEvent(Node* node, Message* msg)
{
...
#ifdef DEBUG
    {
        ...
        case MSG_ROUTING_PimSmKeepAliveTimeOut:
        {
            ...
            if (treeInfoBaseRowPtr != NULL)
            {
                        ...
                        treeInfoBaseRowPtr->keepAliveExpiryTimer)
                {
                    ...
                    {
                        Message* msdpMsg  = MESSAGE_Alloc(node,
                                             APP_LAYER,
                                             APP_MSDP,
                                             MSG_APP_MSDP_ExpireSGState);

                        // info = RPAddr + srcAddr + grpAddr
                        MESSAGE_InfoAlloc(node,
                                          msdpMsg,
                                          3 * sizeof(NodeAddress));
                        char* info = MESSAGE_ReturnInfo(msdpMsg);
                        NodeAddress RPAddr = RoutingPimSmFindRPForThisGroup(
                                                    node,
                                                    keepAliveTimer->grpAddr);
                        ...
                        memcpy(info + sizeof(NodeAddress),
                               &(keepAliveTimer->srcAddr),
                               sizeof(NodeAddress));
                        memcpy(info + 2 * sizeof(NodeAddress),
                               &(keepAliveTimer->grpAddr),
                               sizeof(NodeAddress));

                        MESSAGE_Send(node, msdpMsg, 0);
                    }
                }
                else
                {
        ...
        case MSG_ROUTING_PimSmNewSourceDiscoveredByMsdp:
        {
            RoutingPimSmHandleSAInfoSentByMsdp(node, msg);
            break;
        }
...
//  avoid potential errors when PIM is not active on this interface.
    ...
    switch (RoutingPimCommonHeaderGetType(commonHeader->rpChType))
    {
        ...
        case ROUTING_PIM_CANDIDATE_RP:
        {
            ...
            if (pimSmData)
            {
                // Ignore the C-RP packet if it is received on a
                // border interface
                if (!thisInterface->bsrData.isBorderRouter)
                {
#ifdef ADDON_DB
        ...
        case ROUTING_PIM_BOOTSTRAP:
        {
            ...
            // and QualNet was only configured for PIM-DM, a seg fault
            ...
            if (pimSmData)
            {
                // Ignore the BSM packet if it is received on a
                // border interface
                if (!thisInterface->bsrData.isBorderRouter)
                {
                                      ...
                                      int interfaceId)
{
    ...
    for (i = 0; i < (unsigned int)rt->size; i++)
    {
