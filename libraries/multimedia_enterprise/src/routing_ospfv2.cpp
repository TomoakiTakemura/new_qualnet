...
// Change this to #include "ospf_interface.h" when legacy IPNE is removed
#include "../interfaces/auto-ipne/src/ospf_interface.h"
#endif
...
void Ospfv2PrintNeighborList(Node* node)
{
    ...
    NeighborInfoMapItr neighMapItr;
    ...
    for (i = 0; i < node->numberInterfaces; i++)
    {
        ...
        neighMapItr = ospf->iface[i].neighborMap->begin();

        // Print out each of our neighbors in our neighbor map.
        while (neighMapItr != ospf->iface[i].neighborMap->end())
        {
            tempNeighborInfo = neighMapItr->second;
...
// Print LSA body trace information in XML format
//
// \param node  Pointer to node, doing the packet trace
// \param lsaHdr  Pointer to Ospfv2LinkStateHeader
//
    ...
    int interfaceIndex)
{
    Ospfv2Data* ospf = (Ospfv2Data*) NetworkIpGetRoutingProtocol(
                     node, ROUTING_PROTOCOL_OSPFv2);
    Ospfv2Interface* thisIntf = &ospf->iface[interfaceIndex];
    ...
    commonHdr->authenticationType = thisIntf->AuType;
    commonHdr->authenticationData[0] = 0;
    Ospfv2Stats* ifaceStats = &(thisIntf->interfaceStats);

#ifdef CYBER_CORE
    if (commonHdr->authenticationType == OSPFv2_AUTH_TYPE_NULL)
    {
        commonHdr->authenticationData[0] = 0;
    }
    else if (commonHdr->authenticationType == OSPFv2_AUTH_TYPE_SIMPLE)
    {
        strncpy(commonHdr->authenticationData,
                thisIntf->authenticationKey,
                OSPFv2_AUTH_SIMPLE_MAX_KEY_SIZE);
        ifaceStats->numPktSentWithAuth++;
    }
    else if (commonHdr->authenticationType == OSPFv2_AUTH_TYPE_MD5)
    {
        UInt8* authDataPos = (UInt8*) commonHdr->authenticationData;

        memset(authDataPos, 0, OSPFv2_AUTH_RESERVED_LENGTH);

        authDataPos += OSPFv2_AUTH_RESERVED_LENGTH;
        memset(authDataPos, thisIntf->keyId, OSPFv2_AUTH_KEY_ID_LENGTH);

        authDataPos += OSPFv2_AUTH_KEY_ID_LENGTH;
        *authDataPos = MD5_DIGEST_LENGTH;

        authDataPos += OSPFv2_AUTH_DATA_LENGTH_SIZE;
        *((UInt32*)authDataPos) = ospf->cryptoSequenceNum;

        ospf->cryptoSequenceNum++;
    ...
    NodeAddress neighborAddr)
{
    ...
    //        continue;
    //    }
    //    neighMapItr = ospf->iface[i].neighborMap->find(neighborAddr);

        neighMapItr =
            ospf->iface[interfaceIndex].neighborMap->find(neighborAddr);

        if (neighMapItr != ospf->iface[interfaceIndex].neighborMap->end())
        {
            return neighMapItr->second;
        }
  //  }
    ...
    NodeAddress neighborAddr)
{
    ...
    for (i = 0; i < node->numberInterfaces; i++)
    {
        ...
        // Find the neighbor in my neighbor info map.
        neighMapItr = ospf->iface[i].neighborMap->find(neighborAddr);
        if (neighMapItr != ospf->iface[i].neighborMap->end())
        {
            return i;
        }
    }
    ...
    NodeAddress neighborAddr)
{
    ...
    // Search for the neighbor in my neighbor map.
    neighMapItr =
        ospf->iface[interfaceIndex].neighborMap->find(neighborAddr);

    ERROR_Assert(
             neighMapItr != ospf->iface[interfaceIndex].neighborMap->end(),
             "Neighbor not found in the Neighbor map");
    ...
    // Then remove neighbor from neighbor map.
    free(neighMapItr->second);
    ospf->iface[interfaceIndex].neighborMap->erase(neighMapItr);
    ospf->neighborCount--;
    return;
}
    ...
    int interfaceId)
{
    ...
    neighMapItr = ospf->iface[interfaceId].neighborMap->begin();

    // Search for the neighbor in my neighbor map
    while (neighMapItr != ospf->iface[interfaceId].neighborMap->end())
    {
        nbrInfo = neighMapItr->second;
        ERROR_Assert(nbrInfo, "Neighbor not found in the Neighbor map");
...
BOOL Ospfv2RouterFullyAdjacentWithDR(Node* node, int interfaceId)
{
    ...
    while (neighMapItr != ospf->iface[interfaceId].neighborMap->end())
    {
        neighborInfo = neighMapItr->second;
    ...
    int interfaceIndex)
{
    ...
    Ospfv2Neighbor* neighborInfo = NULL;
    NeighborInfoMapItr neighMapItr;
    neighMapItr = ospf->iface[interfaceIndex].neighborMap->begin();

    while (neighMapItr != ospf->iface[interfaceIndex].neighborMap->end())
    {
        neighborInfo = neighMapItr->second;
    ...
    unsigned int areaId)
{
                                                           ...
                                                           interfaceIndex++)
    {
        ...
        while (neighMapItr !=
               ospf->iface[interfaceIndex].neighborMap->end())
        {
            tempNeighborInfo = neighMapItr->second;
            ERROR_Assert(tempNeighborInfo,
                "Neighbor not found in the Neighbor map");
    ...
    int interfaceIndex)
{
    ...
    neighMapItr = neighborMap->begin();

    while (neighMapItr != neighborMap->end())
    {
        nbrInfo = neighMapItr->second;
    ...
    int interfaceIndex)
{
    ...
    Ospfv2ListDREligibleRouters(
        node,
        ospf->iface[interfaceIndex].neighborMap,
        eligibleRoutersList,
        interfaceIndex);
        ...
        (oldBDR != ospf->iface[interfaceIndex].backupDesignatedRouter))
    {
        ...
        for (neighMapItr = ospf->iface[interfaceIndex].neighborMap->begin();
             neighMapItr != ospf->iface[interfaceIndex].neighborMap->end();
             neighMapItr++)
        {
            nbrInfo = neighMapItr->second;
    ...
    int interfaceId)
{
    ...
    NeighborInfoMapItr neighMapItr;
    neighborCount = thisIntf->neighborMap->size();
    ...
    *nbrList = (NodeAddress*) MEM_malloc(sizeof(NodeAddress)
    ...
    neighMapItr = thisIntf->neighborMap->begin();
    // Get a list of all my neighbors from this interface.
    while (neighMapItr != thisIntf->neighborMap->end())
    {
        tmpNbInfo = neighMapItr->second;
    ...
    int interfaceIndex)
{
       ...
       && (thisArea->isNSSAEnabled == TRUE)))
   {
    ...
    neighborFound = FALSE;
        ...
        {
            neighMapItr = thisIntf->neighborMap->find(sourceAddr);
            if (neighMapItr != thisIntf->neighborMap->end())
            {
                // The neighbor exists in our neighbor map, so no need to
                // add this neighbor to our neighbor map.
                tempNeighborInfo = neighMapItr->second;
                neighborFound = TRUE;
        ...
        else
        {
            ...
            while (neighMapItr != thisIntf->neighborMap->end())
            {
                tempNeighborInfo = (Ospfv2Neighbor*) neighMapItr->second;
    ...
    {
        // Neighbor does not exist in my neighbor map,
        // so add the neighbor into my neighbor map
        ...
        thisIntf->neighborMap->insert(
                                     std::pair<NodeAddress, Ospfv2Neighbor*>
                                     (sourceAddr, tempNeighborInfo));
        ...
        if (OSPFv2_DEBUG_HELLO)
        {
            ...
            printf("    adding neighbor %s to neighbor map\n",
                    sourceAddrStr);
        }
    }
#ifdef ADDON_NGCNMS
    ...
    Int32 i = interfaceIndex;
            ...
            (ospf->iface[i].ospfIfDemand == TRUE))
        {
            ...
            if (numNeighbor)
            {
                Ospfv2Neighbor* tmpNbInfo = NULL;
                Ospfv2Interface* thisIntf = NULL;
                NeighborInfoMapItr itTempMap;

                thisIntf = &ospf->iface[interfaceIndex];
                itTempMap = thisIntf->neighborMap->begin();
                tmpNbInfo = itTempMap->second;
...
#ifdef CYBER_CORE
        if (ospf->iface[i].AuType == OSPFv2_AUTH_TYPE_MD5)
        {
            MESSAGE_ExpandPacket(node, helloMsg, MD5_DIGEST_LENGTH);
        }
#endif // CYBER_CORE
        ...
        memset(helloPacket, 0, helloMsg->packetSize);
...
#ifdef CYBER_CORE
        if (ospf->iface[interfaceIndex].AuType == OSPFv2_AUTH_TYPE_MD5)
        {
            Ospfv2AppendMessageDigest(node,
                                      helloMsg,
                                      &helloPacket->commonHeader,
                                      interfaceIndex);
        }
    ...
    int interfaceIndex)
{
    ...
    if (nbrInfo->state == S_ExStart)
    {
        ...
        if (ospf->iface[interfaceIndex].AuType == OSPFv2_AUTH_TYPE_MD5)
        {
            MESSAGE_ExpandPacket(node, dbDscrpMsg, MD5_DIGEST_LENGTH);
        }
#endif // CYBER_CORE
    ...
    BOOL retx)
{
    ...
    if (ospf->iface[interfaceIndex].AuType == OSPFv2_AUTH_TYPE_MD5)
    {
        MESSAGE_ExpandPacket(node, reqMsg, MD5_DIGEST_LENGTH);
    }
#endif // CYBER_CORE
    ...
    memset(reqPkt, 0, reqMsg->packetSize);
    ...
    int interfaceId)
{
    ...
    while (listItem)
    {
        ...
        tempData = Ospfv2CopyLSA((char*)listItem->data);
        LSHeader = (Ospfv2LinkStateHeader*)tempData;
        ...
        LSHeader->linkStateAge += ospf->iface[interfaceId].infTransDelay/SECOND;
...
void Ospfv2SendLSUpdate(Node* node, NodeAddress nbrAddr, int interfaceId)
{
    ...
    while (!Ospfv2ListIsEmpty(thisInterface->updateLSAList))
    {
       ...
       // Incrememnt LS age transmission delay (in seconds).
       LSHeader->linkStateAge += thisInterface->infTransDelay/SECOND;
    ...
    int LSACount)
{
...
#ifdef CYBER_CORE
    if (ospf->iface[interfaceId].AuType == OSPFv2_AUTH_TYPE_MD5)
    {
        MESSAGE_ExpandPacket(node, updateMsg, MD5_DIGEST_LENGTH);
    }
#endif //CYBER_CORE
    ...
    memset(updatePkt, 0, updateMsg->packetSize);
    ...
    int interfaceIndex)
{
    ...
    if (ospf->iface[interfaceIndex].AuType == OSPFv2_AUTH_TYPE_MD5)
    {
        MESSAGE_ExpandPacket(node, ackMsg, MD5_DIGEST_LENGTH);
    }
#endif //CYBER_CORE
...
// To read the host routes from ospfv2 configuration file.
//
// \param node  Pointer to node running OSPFv2 which may have a host as
//    its neighbor
// \param ospfConfigFile  Pointer to OSPF configuration file that
...
#ifdef CYBER_CORE
        else if (!strcmp(parameterName, "OSPFv2-AUTHENTICATION-TYPE"))
        {
            char key[MAX_STRING_LENGTH];
            char keyId[MAX_STRING_LENGTH];
            Int8* position = strstr(aStr, valStr);
            position += strlen(valStr);
            position++;

            Int32 count = sscanf(position, "%s %s", key, keyId);

            if (count == 2)
            {
                if (iface->authenticationKey != NULL)
                {
                    ERROR_ReportWarningArgs(
                            "Node %d intf %d found duplicate "
                            "paramter for authentication key\n, "
                            "continue using first authentication key",
                            node->nodeId, interfaceIndex);

                    continue;
                }

                if (strcmp(valStr, "SIMPLE") == 0)
                {
                    iface->AuType = OSPFv2_AUTH_TYPE_SIMPLE;

                    if (strcmp(key, "KEY") == 0)
                    {
                        iface->authenticationKey =
                            (char*) MEM_malloc(
                                OSPFv2_AUTH_SIMPLE_MAX_KEY_SIZE);
                        memset(iface->authenticationKey,
                               0,
                               OSPFv2_AUTH_SIMPLE_MAX_KEY_SIZE);

                        if (strlen(keyId) >= OSPFv2_AUTH_SIMPLE_MAX_KEY_SIZE)
                        {
                            if (strlen(keyId) > OSPFv2_AUTH_SIMPLE_MAX_KEY_SIZE)
                            {
                                ERROR_ReportWarningArgs(
                                            "Authetication key has execeded "
                                            "maximum allowed key size, hence "
                                            "considering only first %d bytes",
                                            OSPFv2_AUTH_SIMPLE_MAX_KEY_SIZE);
                            }

                            strncpy(iface->authenticationKey,
                                    keyId,
                                    OSPFv2_AUTH_SIMPLE_MAX_KEY_SIZE);
                        }
                        else
                    ...
                    {
                        ERROR_ReportError(
                            "KEY field is not found in "
                            "\"OSPFv2-AUTHENTICATION-TYPE\" parameter.");
                    }
                }
                else if (strcmp(valStr, "MD5") == 0)
                {
                    iface->AuType = OSPFv2_AUTH_TYPE_MD5;

                    if (strcmp(key, "KEY-ID") == 0)
                    {
                        // 10 refers to base i.e. converting string into
                        // decimal number
                        long id = strtol(keyId, NULL, 10);
                        // keyId should be between 1 and 255
                        if (id > 0 && id < 256)
                        {
                            iface->keyId = (UInt8)id;
                        }
                        else
                        {
                            iface->keyId = 1;
                            ERROR_ReportWarningArgs(
                                    "Node %d intf %d, value of keyId should "
                                    "be between 1 and 255. Setting "
                                    "default Key-Id to 1.",
                                    node->nodeId, interfaceIndex);
                        }

                        position = strstr(aStr, key);
                        position += strlen(key);
                        // skip white space
                        ...
                        {
                            ERROR_ReportError(
                                "Parameter \"OSPFv2-AUTHENTICATION-TYPE\""
                                " is incorrectly configured");
                        }

                        if (strcmp(parameterName, "KEY") == 0)
                        {
                            iface->authenticationKey =
                                (char*)MEM_malloc(
                                    MD5_DIGEST_LENGTH);
                            ...
                            if (strlen(valStr) >= MD5_DIGEST_LENGTH)
                            {
                                if (strlen(valStr) > MD5_DIGEST_LENGTH)
                                {
                                    ERROR_ReportWarningArgs(
                    ...
                    {
                        ERROR_ReportError(
                                "KEY-ID field is not found in "
                                "\"OSPFv2-AUTHENTICATION-TYPE\" parameter.");
                    }
                }
                else
                {
                    ERROR_ReportWarning("Authentication type should be "
                                        "\"SIMPLE\" or \"MD5\", considering "
                                        "NULL authentication by default");
                }
            }
            else
            {
                ERROR_ReportError("Parameter \"OSPFv2-AUTHENTICATION-TYPE\""
                                  " is incorrectly configured");
            }
        }
#endif // CYBER_CORE
    ...
    BOOL rospf)
{
    ...
    for (i = 0; i < node->numberInterfaces; i++)
    {
        ...
        ospf->iface[i].neighborMap = new NeighborInfoMap;
...
// To add a single host information to router LSA
//
// \param node  Pointer to node running OSPFv2 which has a host as its
//    neighbor
// \param cost  The cost of the link between the host and the
//    corresponding router node
// \param hostAddress  IP address of the host node
// \param interfaceId  Node's interface index on which OSPF is configured
// \param linkInfo  Pointer to the list of links for the node's
//    router LSA
// \param ospfConfigFile  Pointer to OSPF configuration file that
//    contains information about hosts connected to OSPF routers
//
    ...
    BOOL flood)
{
    ...
    size_t hostListSize = 0;
    for (i = 0; i < (int)ospf->hostRoutes.size();i++)
    {
    ...
    listSize = listSize + (int)hostListSize;
    ...
    for (i = 0; i < node->numberInterfaces; i++)
    {
        ...
        else
        {
            ...
            // Get all my neighbors information from my neighbor map.
            while (neighMapItr != ospf->iface[i].neighborMap->end())
            {
                tempNeighborInfo = neighMapItr->second;
                ERROR_Assert(tempNeighborInfo,
    ...
    BOOL flush)
{
    ...
    int numNbr = ospf->iface[interfaceIndex].neighborMap->size();
    ...
    while (neighMapItr != ospf->iface[interfaceIndex].neighborMap->end())
    {
        ...
        ERROR_Assert(tempNeighborInfo,
            "Neighbor not present into the Neighbor map\n");
...
#ifdef IPNE_INTERFACE
    // Calculate LSA checksum
    OSPFPacketHandler::CalculateLinkStateChecksum(node, asExtLSAHeader);
#endif
    ...
    switch (eventType)
    {
        ...
        case E_InterfaceDown:
        {
            ...
            while (neighMapItr !=
                   ospf->iface[interfaceIndex].neighborMap->end())
            {
                Ospfv2Neighbor* nbrInfo = neighMapItr->second;
...
void Ospfv2FindASExternalPath(Node* node)
{
    ...
    while (listItem)
    {
        ...
        // Below calculation of route for OSPFv2_DESTINATION_NETWORK is
        // done by Wei Liu to fix bugzilla issue#6454 in jne_v40_dev
        // branch (Revision:70124).
        rowPtr = Ospfv2GetValidRoute(
                    node, asExternalRt, OSPFv2_DESTINATION_NETWORK);
        // Need to recalculate valid route for ASBR when rowPtr->nextHop
        // comes out to be zero in a case when ASBR and path calculating
        // router are in same network and forwardingAdd is 0.0.0.0
        if (!rowPtr || (!forwardingAdd && !rowPtr->nextHop))
        {
            rowPtr = Ospfv2GetValidRoute(
    ...
    int interfaceIndex)
{
    ...
    // Get OSPF header
    ospfHeader = (Ospfv2CommonHeader*) MESSAGE_ReturnPacket(msg);
    Ospfv2Stats* ifaceStats = &ospf->iface[interfaceIndex].interfaceStats;

#ifdef CYBER_CORE
    if (ospfHeader->authenticationType == thisIntf->AuType)
    {
        if (ospfHeader->authenticationType == OSPFv2_AUTH_TYPE_NULL)
        {
            // Do nothing.
        }
        else if (ospfHeader->authenticationType == OSPFv2_AUTH_TYPE_SIMPLE)
        {
            if (strncmp(ospfHeader->authenticationData,
                        thisIntf->authenticationKey,
                        OSPFv2_AUTH_SIMPLE_MAX_KEY_SIZE))
            {
                // OSPF packet is discarded because of mismatch in
                // authentication data
                ifaceStats->numPktDropDueAuthFail++;

                MESSAGE_Free(node, msg);
                return;
            }
        }
        else if (ospfHeader->authenticationType == OSPFv2_AUTH_TYPE_MD5)
        {
            Int8 received_keyId = *((Int8*) ospfHeader->authenticationData +
                            OSPFv2_AUTH_RESERVED_LENGTH);

            UInt8 received_md_length = *((UInt8*) ospfHeader->authenticationData
                                            + OSPFv2_AUTH_RESERVED_LENGTH
                                            + OSPFv2_AUTH_KEY_ID_LENGTH);
            UInt32 received_seq = *((UInt32*) (ospfHeader->authenticationData
                                        + OSPFv2_AUTH_RESERVED_LENGTH
                                        + OSPFv2_AUTH_KEY_ID_LENGTH
                                        + OSPFv2_AUTH_DATA_LENGTH_SIZE));

            Ospfv2Neighbor* tempNeighborInfo = NULL;

            tempNeighborInfo = Ospfv2GetNeighborInfoByIPAddress(
                                                    node,
                                                    interfaceIndex,
                                                    ospfHeader->routerID);

            if (received_seq > 0 &&
                tempNeighborInfo!= NULL &&
                received_seq < tempNeighborInfo->cryptoSequenceNum)
            {
                // As per RFC 2328 section Appendix D.5.3, received sequence
                // number should be greater than recorded sequence number.
                ifaceStats->numPktDropDueMismatchAuthSeqNo++;

                MESSAGE_Free(node, msg);
                return;
            }

            if (received_keyId != thisIntf->keyId)
            {
                ifaceStats->numPktDropDueMismatchAuthKeyId++;
            ...
            UInt8* temp_packet = (UInt8*)
                    MEM_malloc(ospfHeader->packetLength +
                                    MD5_DIGEST_LENGTH);
            UInt8* received_md = (UInt8*) MEM_malloc(received_md_length);

            memset(received_md, 0, received_md_length);
            memcpy(received_md,
                   msg->packet + ospfHeader->packetLength,
                   received_md_length);

            memset(temp_packet,
                   0,
                   ospfHeader->packetLength + MD5_DIGEST_LENGTH);
            memcpy(temp_packet,
                   MESSAGE_ReturnPacket(msg),
                   ospfHeader->packetLength);

            // Swapping from Host order to Network order. This is done for
            // message digest generation.
            //Ospfv2SwapBytes(temp_packet);
            OSPFPacketHandler::OspfSwapBytes(temp_packet, false);

            // Appending authentication key to ospf packet
            memcpy(temp_packet + ospfHeader->packetLength,
                   thisIntf->authenticationKey,
                   MD5_DIGEST_LENGTH);

            if (CRYPTO_VerifyIcvValue(temp_packet,
                                      ospfHeader->packetLength +
                                            MD5_DIGEST_LENGTH,
                                      (UInt8*) thisIntf->authenticationKey,
                                      (Int32) MD5_DIGEST_LENGTH,
                                      received_md,
                                      received_md_length,
                                      "MD5"))
            {
                if (tempNeighborInfo != NULL)
                {
                    tempNeighborInfo->cryptoSequenceNum = received_seq;
                }

                MEM_free(temp_packet);
                MEM_free(received_md);
            }
            else
            {
                // OSPF packet is discarded because of mismatch in
    ...
    {
        ERROR_ReportWarningArgs("Node %d intf %d dropping OSPF packet because of"
                                " mismatch in authentication type\n",
                                node->nodeId, interfaceIndex);
        ifaceStats->numPktDropDueMismatchAuthType++;
        MESSAGE_Free(node, msg);
        return;
    }
#endif // CYBER_CORE
...
void Ospfv2HandleRoutingProtocolEvent(Node* node, Message* msg)
{
    ...
    switch (msg->eventType)
    {
        ...
        case MSG_ROUTING_OspfFloodTimer:
        {
            ...
            if (!ospf->iface[interfaceId].floodTimerSet) break;

            Ospfv2SendLSUpdate(node, ANY_DEST, interfaceId);
        ...
        case MSG_ROUTING_OspfDelayedAckTimer:
        {
            ...
            if (!ospf->iface[interfaceId].delayedAckTimer) break;
...
void Ospfv2Finalize(Node* node)
{
    ...
    if (ospf->collectStat == TRUE)
    {
        ...
        for (i = 0; i < node->numberInterfaces; i++)
        {
            if ((NetworkIpGetUnicastRoutingProtocolType(node, i)
                == ROUTING_PROTOCOL_OSPFv2)
                && ospf->iface[i].AuType != OSPFv2_AUTH_TYPE_NULL)
            {
                char authType[MAX_STRING_LENGTH];
                Ospfv2Stats* ifaceStats = &ospf->iface[i].interfaceStats;

                strcpy(authType,
                    (ospf->iface[i].AuType ==
                        OSPFv2_AUTH_TYPE_SIMPLE) ? "SIMPLE" : "MD5");

                sprintf(buf,
                        "Number of Packets Sent With %s Authentication = %d",
                        authType, ifaceStats->numPktSentWithAuth);
                IO_PrintStat(
                    node,
                    "Network",
                    "OSPFv2",
                ...
                sprintf(buf,
                        "Number of Packets Dropped Because of Mismatch in "
                        "%s Authentication Type = %d",
                        authType, ifaceStats->numPktDropDueMismatchAuthType);
                IO_PrintStat(
                    node,
                    "Network",
                    "OSPFv2",
                ...
                sprintf(buf,
                        "Number of Packets Dropped Because of Mismatch in "
                        "%s Authentication Seq No. = %d",
                        authType, ifaceStats->numPktDropDueMismatchAuthSeqNo);
                IO_PrintStat(
                    node,
                    "Network",
                    "OSPFv2",
                ...
                sprintf(buf,
                        "Number of Packets Dropped Because of Mismatch in "
                        "%s Authentication KeyIds = %d",
                        authType, ifaceStats->numPktDropDueMismatchAuthKeyId);
                IO_PrintStat(
                    node,
                    "Network",
                    "OSPFv2",
                ...
                sprintf(buf,
                        "Number of Packets Dropped Because of %s Authentication"
                        " Failure = %d",
                        authType, ifaceStats->numPktDropDueAuthFail);
                IO_PrintStat(
                    node,
                    "Network",
                    "OSPFv2",
    ...
    for (int i = 0; i < node->numberInterfaces; i++)
    {
        ...
        // Free items from map and frees map
        delete ospf->iface[i].neighborMap;
    }
}
...
{
    for (Node* traverseNode = partition->firstNode; traverseNode; traverseNode = traverseNode->nextNodeData)
    {
        if (traverseNode->partitionId != partition->partitionId) continue;

        Ospfv2Data* ospf = (Ospfv2Data*) NetworkIpGetRoutingProtocol(
                                    traverseNode, ROUTING_PROTOCOL_OSPFv2);
        if (ospf == NULL) continue;
...
{
    for (Node* traverseNode = node; traverseNode; traverseNode = traverseNode->nextNodeData)
    {
        if (traverseNode->partitionId != node->partitionData->partitionId) continue;
        Ospfv2Data* ospf = (Ospfv2Data*)NetworkIpGetRoutingProtocol(traverseNode, ROUTING_PROTOCOL_OSPFv2);
        if (ospf == NULL) continue;

        // We have to insert the Neighbor State in the database.
        for (int i = 0; i < traverseNode->numberInterfaces; i++)
        {
            if (ospf->iface[i].type == OSPFv2_NON_OSPF_INTERFACE) continue;
             ...
             for (NeighborInfoMapItr neighMapItr = ospf->iface[i].neighborMap->begin();
                 neighMapItr != ospf->iface[i].neighborMap->end();
                 ++neighMapItr)
            {
                ...
                STATSDB_HandleOspfNeighborStateTableInsert(node, stats);
            }
        }
    }
}
...
void HandleStatsDBOspfRouterLsaTableInsertion(Node* node)
{
    for (Node* traverseNode = node; traverseNode; traverseNode = traverseNode->nextNodeData)
    {
        ...
        for (Ospfv2ListItem* item = ospf->area->first; item; item = item->next)
        {
            Ospfv2Area* thisArea = (Ospfv2Area*)item->data;
            for (Ospfv2ListItem* areaItem = thisArea->routerLSAList->first; areaItem; areaItem = areaItem->next)
            {
                StatsDbOspfLsa routerLsaStats;
                Ospfv2LinkStateHeader* LSHeader = (Ospfv2LinkStateHeader*)areaItem->data;
                if (LSHeader->linkStateType != OSPFv2_ROUTER) continue;
                ...
                Ospfv2RouterLSA* routerLSA = (Ospfv2RouterLSA*)areaItem->data;
                Ospfv2LinkInfo* linkList = (Ospfv2LinkInfo*)(routerLSA + 1);

                for (int i = 0; i < routerLSA->numLinks; i++)
                {
                    ...
                    routerLsaStats.linkType = Ospfv2GetLinkTypeString(linkList->type);

                    STATSDB_HandleOspfRouterLsaTableInsert(traverseNode, routerLsaStats);

                    // Place the linkList pointer properly
                    int numTos = linkList->numTOS;
...
void HandleStatsDBOspfNetworkLsaTableInsertion(Node* node)
{
    for (Node* traverseNode = node; traverseNode; traverseNode = traverseNode->nextNodeData)
    {
        ...
        {
            Ospfv2Area* thisArea = (Ospfv2Area*) item->data;
            for (Ospfv2ListItem* areaItem = thisArea->networkLSAList->first; areaItem; areaItem = areaItem->next)
            {
                StatsDbOspfLsa lsaStats;

                Ospfv2NetworkLSA* networkLSA = (Ospfv2NetworkLSA*)areaItem->data;
                Ospfv2LinkStateHeader* LSHeader = (Ospfv2LinkStateHeader*) areaItem->data;
                if (LSHeader->linkStateType != OSPFv2_NETWORK) continue;

                char tempAddr[MAX_STRING_LENGTH];
                IO_ConvertIpAddressToString(LSHeader->advertisingRouter, tempAddr);
                ...
                int numLink = (networkLSA->LSHeader.length - sizeof(Ospfv2NetworkLSA) - 4)/(sizeof(NodeAddress));
                NodeAddress* rtrAddr = ((NodeAddress*)(networkLSA + 1)) + 1;
                for (int i = 0; i < numLink; i++)
                {
                    ...
                    STATSDB_HandleOspfNetworkLsaTableInsert(traverseNode, lsaStats);
                }
            }
        }
    }
...
void HandleStatsDBOspfSummaryLsaTableInsertion(Node* node)
{
    for (Node* traverseNode = node; traverseNode; traverseNode = traverseNode->nextNodeData)
    {
        ...
        {
            Ospfv2Area* thisArea = (Ospfv2Area*)item->data;
            for (Ospfv2ListItem* areaItem = thisArea->routerSummaryLSAList->first; areaItem; areaItem = areaItem->next)
            {
                StatsDbOspfLsa lsaStats;

                Ospfv2LinkStateHeader* LSHeader = (Ospfv2LinkStateHeader*)areaItem->data;
                if (LSHeader->linkStateType != OSPFv2_ROUTER_SUMMARY) continue;
                ...
                STATSDB_HandleOspfSummaryLsaTableInsert(traverseNode, lsaStats);
            }

            for (Ospfv2ListItem* areaItem = thisArea->networkSummaryLSAList->first; areaItem; areaItem = areaItem->next)
            {
                StatsDbOspfLsa lsaStats;

                Ospfv2LinkStateHeader* LSHeader = (Ospfv2LinkStateHeader*)areaItem->data;
                if (LSHeader->linkStateType != OSPFv2_NETWORK_SUMMARY) continue;
...
void HandleStatsDBOspfExternalLsaTableInsertion(Node* node)
{
    for (Node* traverseNode = node; traverseNode; traverseNode = traverseNode->nextNodeData)
    {
        ...
        for (Ospfv2ListItem* item = ospf->asExternalLSAList->first; item; item = item->next)
        {
            StatsDbOspfLsa lsaStats;

            Ospfv2LinkStateHeader* LSHeader = (Ospfv2LinkStateHeader*)item->data;
            if (LSHeader->linkStateType != OSPFv2_ROUTER_AS_EXTERNAL) continue;
            ...
            Ospfv2ExternalLinkInfo* linkInfo = (Ospfv2ExternalLinkInfo*)(LSHeader + 1);
            IO_ConvertIpAddressToString(LSHeader->linkStateID, netStr);
            ...
            lsaStats.linkStateAge = LSHeader->linkStateAge;
            lsaStats.cost = Ospfv2ExternalLinkGetMetric(linkInfo->ospfMetric);

            STATSDB_HandleOspfExternalLsaTableInsert(traverseNode, lsaStats);

        }
        // Inserting OSPFv2_ROUTER_NSSA_EXTERNAL into ExternalLSA table
        for (Ospfv2ListItem* item = ospf->nssaExternalLSAList->first; item; item = item->next)
        {
            StatsDbOspfLsa lsaStats;

            Ospfv2LinkStateHeader* LSHeader = (Ospfv2LinkStateHeader*)item->data;
            if (LSHeader->linkStateType != OSPFv2_ROUTER_NSSA_EXTERNAL) continue;
...
const std::string& Ospfv2GetLinkTypeString(unsigned char type)
{
    static const std::string sPointToPoint("OSPFv2_POINT_TO_POINT");
    static const std::string sTransit("OSPFv2_TRANSIT");
    static const std::string sStub("OSPFv2_STUB");
    static const std::string sVirtual("OSPFv2_VIRTUAL");
    static const std::string sUnknown("UNKNOWN");

    switch (type)
    {
        case OSPFv2_POINT_TO_POINT:
            return sPointToPoint;
        case OSPFv2_TRANSIT:
            return sTransit;
        case OSPFv2_STUB:
            return sStub;
...
void HandleStatsDBOspfInterfaceStateTableInsertion(Node* node)
{
    for (Node* traverseNode = node; traverseNode; traverseNode = traverseNode->nextNodeData)
    {
        ...
        for (int i = 0; i < traverseNode->numberInterfaces; i++)
        {
            ...
            char netStr[MAX_ADDRESS_STRING_LENGTH];
            IO_ConvertIpAddressToString(NetworkIpGetInterfaceAddress(traverseNode, i), netStr);
            ...
            STATSDB_HandleOspfInterfaceStateTableInsert(node, stats);
        }
    }
}
...
void HandleStatsDBOspfSummaryTableInsertion(Node* node)
{
    ...
    for (Node* traverseNode = node; traverseNode; traverseNode = traverseNode->nextNodeData)
    {
        ...
        for (int i = 0; i < traverseNode->numberInterfaces; i++)
        {
            Ospfv2Interface* thisOspfIntf = &ospf->iface[i];
            if (thisOspfIntf == NULL || thisOspfIntf->type == OSPFv2_NON_OSPF_INTERFACE) continue;

            StatsDbOspfStats stats;

            char ipAddr[MAX_ADDRESS_STRING_LENGTH];
            IO_ConvertIpAddressToString(NetworkIpGetInterfaceAddress(traverseNode, i), ipAddr);
            stats.ipAddr = ipAddr;

            stats.numLSUpdateSent = thisOspfIntf->interfaceStats.numLSUpdateSent;
            stats.numLSUpdateBytesSent = thisOspfIntf->interfaceStats.numLSUpdateBytesSent;
            stats.numLSUpdateRecv = thisOspfIntf->interfaceStats.numLSUpdateRecv;
            stats.numLSUpdateBytesRecv = thisOspfIntf->interfaceStats.numLSUpdateBytesRecv;
            stats.numAckSent = thisOspfIntf->interfaceStats.numAckSent;
            stats.numAckBytesSent = thisOspfIntf->interfaceStats.numAckBytesSent;
            stats.numAckRecv = thisOspfIntf->interfaceStats.numAckRecv;
            stats.numAckBytesRecv = thisOspfIntf->interfaceStats.numAckBytesRecv;
            stats.numDDPktSent = thisOspfIntf->interfaceStats.numDDPktSent;
            stats.numDDPktBytesSent = thisOspfIntf->interfaceStats.numDDPktBytesSent;
            stats.numDDPktRecv = thisOspfIntf->interfaceStats.numDDPktRecv;
            stats.numDDPktRxmt = thisOspfIntf->interfaceStats.numDDPktRxmt;
            stats.numDDPktBytesRxmt = thisOspfIntf->interfaceStats.numDDPktBytesRxmt;
            stats.numDDPktBytesRecv = thisOspfIntf->interfaceStats.numDDPktBytesRecv;
            stats.numLSReqSent = thisOspfIntf->interfaceStats.numLSReqSent;
            stats.numLSReqBytesSent = thisOspfIntf->interfaceStats.numLSReqBytesSent;
            stats.numLSReqRecv = thisOspfIntf->interfaceStats.numLSReqRecv;
            stats.numLSReqRxmt = thisOspfIntf->interfaceStats.numLSReqRxmt;
            stats.numLSReqBytesRecv = thisOspfIntf->interfaceStats.numLSReqBytesRecv;

            // Now calculate the offered load and insert in the table
            Int64 bytesSent = stats.numLSUpdateBytesSent +
...
static
void HandleStatsDBOspfAggregateStatsCalculation(Node* node, StatsDbOspfStats* stats)
{
    double period = (double)node->getNodeTime() / SECOND;
...
void HandleStatsDBMospfSummaryTableInsertion(Node* node)
{
    // Check if the Table exists.
    StatsDb* db = node->partitionData->statsDb;
    if (!db || !db->statsOspfTable || !db->statsOspfTable->createMospfSummaryTable) return;

    for (Node* traverseNode = node; traverseNode; traverseNode = traverseNode->nextNodeData)
    {
        if (traverseNode->partitionId != node->partitionData->partitionId) continue;
        MospfData* mospf = (MospfData*)NetworkIpGetMulticastRoutingProtocol(traverseNode, MULTICAST_PROTOCOL_MOSPF);
        if (mospf == NULL) continue;

        StatsDBMospfSummary stats;

        stats.m_NumGroupLSAGenerated = mospf->mospfSummaryStats.m_NumGroupLSAGenerated;
        stats.m_NumGroupLSAFlushed = mospf->mospfSummaryStats.m_NumGroupLSAFlushed;
        stats.m_NumGroupLSAReceived = mospf->mospfSummaryStats.m_NumGroupLSAReceived;
        stats.m_NumRouterLSA_WCMRSent = mospf->mospfSummaryStats.m_NumRouterLSA_WCMRSent;
        stats.m_NumRouterLSA_WCMRReceived = mospf->mospfSummaryStats.m_NumRouterLSA_WCMRReceived;
        stats.m_NumRouterLSA_VLEPSent = mospf->mospfSummaryStats.m_NumRouterLSA_VLEPSent;
        stats.m_NumRouterLSA_VLEPReceived = mospf->mospfSummaryStats.m_NumRouterLSA_VLEPReceived;
        stats.m_NumRouterLSA_ASBRSent = mospf->mospfSummaryStats.m_NumRouterLSA_ASBRSent;
        stats.m_NumRouterLSA_ASBRReceived = mospf->mospfSummaryStats.m_NumRouterLSA_ASBRReceived;
        stats.m_NumRouterLSA_ABRSent = mospf->mospfSummaryStats.m_NumRouterLSA_ABRSent;
        stats.m_NumRouterLSA_ABRReceived = mospf->mospfSummaryStats.m_NumRouterLSA_ABRReceived;
...
void HandleStatsDBMospfMulticastNetSummaryTableInsertion(Node* node)
{
    // Check if the Table exists.
    StatsDb* db = node->partitionData->statsDb;
    if (!db || !db->statsSummaryTable || !db->statsSummaryTable->createMulticastNetSummaryTable) return;
    ...
    for (Node* traverseNode = node; traverseNode; traverseNode = traverseNode->nextNodeData)
    {
        ...
        stats.m_NumDataSent = mospf->mospfMulticastNetSummaryStats.m_NumDataSent;
        stats.m_NumDataRecvd = mospf->mospfMulticastNetSummaryStats.m_NumDataRecvd;
        stats.m_NumDataForwarded = mospf->mospfMulticastNetSummaryStats.m_NumDataForwarded;
        stats.m_NumDataDiscarded = mospf->mospfMulticastNetSummaryStats.m_NumDataDiscarded;
                               ...
                               int interfaceIndex)
{
    ...
    UInt8* temp_packet =
            (UInt8*) MEM_malloc(commonHdr->packetLength +
                MD5_DIGEST_LENGTH);
    UInt8* md = NULL;
    UInt32 md_length = 0;

    memset(temp_packet, 0, commonHdr->packetLength + MD5_DIGEST_LENGTH);
    memcpy(temp_packet,
           MESSAGE_ReturnPacket(msg),
           commonHdr->packetLength);
    ...
    // Appending authentication key to ospf packet
    memcpy(temp_packet + commonHdr->packetLength,
           thisIntf->authenticationKey,
           MD5_DIGEST_LENGTH);

    md = CRYPTO_GetIcvValue(temp_packet,
                           (Int32) (commonHdr->packetLength +
                                        MD5_DIGEST_LENGTH),
                           (UInt8*) thisIntf->authenticationKey,
                           (Int32) MD5_DIGEST_LENGTH,
                           &md_length,
    ...
    ERROR_Assert(md_length <= MD5_DIGEST_LENGTH,
                 "Generated message digest length should be less than "
                 "or equal to MD5_DIGEST_LENGTH");

    Int8* packet_pointer = MESSAGE_ReturnPacket(msg) + commonHdr->packetLength;
    memcpy(packet_pointer, md, md_length);
    MEM_free(temp_packet);
    MEM_free(md);
}
#endif // CYBER_CORE
