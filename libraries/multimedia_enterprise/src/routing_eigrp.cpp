        ...
        *found = TRUE;
               ...
               (i < eigrpConfigFile->numLines))
        {
            ...
            else if (strcmp(token, "EIGRP-HOLD-TIME") == 0)
            {
                ...
                int numValueRead = sscanf(currentLine, "%*s%d%s",
                                          &interfaceIndex, buf);
            ...
            else if (strcmp(token, "EIGRP-POISON-REVERSE") == 0)
            {
                ...
                {
                    ERROR_ReportError("Parameter \"EIGRP-POISON-REVERSE\""
                        " must be \"YES\" or \"NO \"");
                    abort();
                }
            }
    ...
    int interfaceIndex)
{
    ...
    newVector->load = MAX((unsigned char)1, vectorMetric->load);
    ...
    int interfaceIndex)
{
    ...
    if (eigrp != NULL)
    {
        ...
        // Make directly connected network (which was learned from .eigrp file)
        // through this interface unreachable.
        ListItem* tempItem = NULL;
        tempItem = eigrp->topologyTable->first;
        while (tempItem && tempItem->data)
        {
            int outgoingInterface = ((EigrpTopologyTableEntry*)
                (tempItem->data))->outgoingInterface;
            NodeAddress nextHop = ((EigrpTopologyTableEntry*)
                (tempItem->data))->metric.nextHop;
            if (outgoingInterface == interfaceIndex && nextHop == 0)
            {
                EigrpTopologyTableEntry* topologyEntry =
                    (EigrpTopologyTableEntry*) tempItem->data;
                // Make the entry unreachable in the topology table.
                topologyEntry->metric.delay =
                    EIGRP_DESTINATION_INACCESSABLE;
                return;
