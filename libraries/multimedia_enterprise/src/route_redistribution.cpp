        ...
        if (info->distributorProtocolType == routingProtocolType)
        {
            ...
            if (proceed == TRUE)
            {
                           ...
                           ROUTING_PROTOCOL_DEFAULT))
                {
                    ...
                    if (flag == FALSE)
                    {
                        ERROR_ReportErrorArgs("Either one or both routing"
                                " protocols specified in the router-config"
                                " file for route redistribution on"
                                " Node: %u are not running at the node.\n",
                                node->nodeId);
                    }
                }
