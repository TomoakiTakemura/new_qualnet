...
/*
*  FUNCTION    : RoutingPimSMCalculateChksum()
*  PURPOSE     : Calculates checksum
*  PARAMETERS  : addr - pointer to the newly constructed packet
                 len - length of the packet in bytes
*  RETURN VALUE: int.
*/
int
RoutingPimSMCalculateChksum(unsigned short* addr, int len)
{
    int sum;
    sum = 0;
...
/*
*  FUNCTION    : RoutingPimSMSendMsdpOriginateSAMessageEvent()
*  PURPOSE     : Creates MSG_APP_MSDP_SendSAMessage event
*  RETURN VALUE: NULL
 */

void
RoutingPimSMSendMsdpOriginateSAMessageEvent(Node* node,
                                            NodeAddress RPAddr,
                                            NodeAddress srcAddr,
                                            NodeAddress groupAddr,
                                            Message* msg = NULL)
{
    /* If this node is MSDP speaker then */
    /*create MSG_APP_MSDP_SendSAMessage event */
    char* info;
    RoutingPimSourceGroupList msdpSGInfo;
    RoutingPimSmTreeInfoBaseRow   * treeInfoBaseRowPtr = NULL;
    treeInfoBaseRowPtr = RoutingPimSmSearchTreeInfoBaseForThisGroup
    ...
    if (node->isMsdpEnabled == TRUE)
    {
        ...
        if (msdpData->saCache)
        {
            ...
            if (saCache->size() > 0)
            {
                MsdpSACacheMap :: iterator saCacheMapIt = saCache->find(RPAddr);
                if (saCacheMapIt != saCache->end())
                {
                    MsdpSGEntryPair sgEntryPair(srcAddr, groupAddr);
                    MsdpCacheSAItem* cacheSAItem = saCacheMapIt->second;
        ...
        {
            char* temp = NULL;
            temp = (char*) MESSAGE_ReturnPacket(msg);
            IpHeaderType* ipHeader = (IpHeaderType*) (temp);
            int innerhLen  = (int) IpHeaderSize(ipHeader);
            unsigned char ipProto = ipHeader->ip_p;

            // Swapping bytes of IP header
            EXTERNAL_ntoh(temp, sizeof(UInt32));
            temp = temp + sizeof(UInt32);
            EXTERNAL_ntoh(temp, sizeof(UInt16));
            temp = temp + sizeof(UInt16);
            EXTERNAL_ntoh(temp, sizeof(UInt16));
            temp = temp + sizeof(UInt32);
            ...
            /* Compute the chksum for the inner ip packet that carries the CBR pkt*/

            //struct libnet_ipv4_hdr* ipHdr = (struct libnet_ipv4_hdr*)ipHeader;
            ipHeader->ip_sum = 0;
            unsigned int inner_sum = 0;
            inner_sum = RoutingPimSMCalculateChksum(
                                            (unsigned short*)ipHeader,
                                            innerhLen);
            ipHeader->ip_sum =
                (unsigned short)MSDP_LIBNET_CKSUM_CARRY(inner_sum);
            // Set layer, protocol, event type
            MESSAGE_SetLayer(msg, APP_LAYER, APP_MSDP);
            MESSAGE_SetEvent(msg, MSG_APP_MSDP_SendSAMessage);
        }
        else
        {
        ...
        msdpSGInfo.srcAddr = srcAddr;
        msdpSGInfo.groupAddr = groupAddr;
        memcpy(info, &(msdpSGInfo), sizeof(RoutingPimSourceGroupList));
        memcpy(info + sizeof(RoutingPimSourceGroupList),
               &RPAddr,
               sizeof(NodeAddress));
             ...
             )
        {
            ...
            if (IAmDR && IAmRP)
            {
                ...
                if (treeInfoBaseRowPtr->isTunnelPresent)
                {
                    ...
                    if (treeInfoBaseRowPtr->isFirstRegisterPacket == true)
                    {
                        RoutingPimSMSendMsdpOriginateSAMessageEvent(
                                            node,
                                            RPAddr,
                                            srcAddr,
                                            grpAddr,
                               ...
                               LinkedList* rpAccessList)
{
    ...
    else
    {
        ...
        while (tmpBuf)
        {
            ...
            qualifierLen = (int)(tmpStr1 - tmpBuf);
            ...
            qualifierLen = (int)(tmpStr2 - tmpStr1);
                                 ...
                                 const NodeInput* nodeInput)
{
    ...
    {
        // Check if this a border router that separates the PIM-SM domain
        IO_ReadBool(
                node->nodeId,
                NetworkIpGetInterfaceAddress(node, interfaceIndex),
                nodeInput,
                "PIM-SM-BORDER-ROUTER",
                &retVal,
                &thisInterface->bsrData.isBorderRouter);
    }
}
                                    ...
                                    RoutingPimSmMulticastTreeState treeState)
{
    ...
    insertSucceeded = rowPtrMap->insert(std::make_pair(rowKey,
                                                  treeInfoBaseRowPtr));
                                   ...
                                   NodeAddress srcAddr, int incomingIntf)
{
    ...
    if (IamRP)
    {
        ...
        clocktype delay;
        RoutingPimSmTreeInfoBaseRow   * treeInfoBaseRowPtr = NULL;
        char* encapsulatedData = NULL;
        UInt32 enDataLength = 0;
        ...
        Message* newMsg = NULL;
        if (!PimSmRegisterPacketGetNullReg(registerPkt->rpSmRegPkt))
        {
...
/*
 * NAME:        RoutingPimSmHandleSAInfoSentByMsdp.
 * PURPOSE:     Processes the SA message information sent MSDP.
 * PARAMETERS:  node - pointer to the node which received the event.
 *              msg - message received
 * RETURN:      NONE.
 */
void
RoutingPimSmHandleSAInfoSentByMsdp(Node* node, Message* msg)
{
    RoutingPimSmTreeInfoBaseRow     * treeInfoBaseRowPtr;
    BOOL isPktRouted = FALSE;
    MsdpSGEntry* sgEntry = (MsdpSGEntry*)MESSAGE_ReturnInfo(msg);
    PimData* pim = (PimData*)
          NetworkIpGetMulticastRoutingProtocol(node, MULTICAST_PROTOCOL_PIM);
    PimDataSm* pimDataSm = NULL;
    int incomingInterface = *(MESSAGE_ReturnInfo(msg) + sizeof(MsdpSGEntry));
    if (pim->modeType == ROUTING_PIM_MODE_SPARSE)
    {
        pimDataSm = (PimDataSm*)pim->pimModePtr;
    }
    ...
    if (MESSAGE_ReturnPacketSize(msg) > 0)
    {
        char* temp = (char*) MESSAGE_ReturnPacket(msg);
        IpHeaderType* ipHeader = (IpHeaderType*) (temp);
        int innerhLen  = (int) IpHeaderSize(ipHeader);
        unsigned char ipProto = ipHeader->ip_p;
        ...
        {
            ERROR_ReportError("Invalid Application type");
        }
    }
    ...
    if (DEBUG_JP)
    {
        ...
        ctoa(node->getNodeTime(), clockStr);
        printf("Node %u Get MSG_ROUTING_PimSmNewSourceDiscoveredByMsdp at %s \n",
                node->nodeId, clockStr);
        printf(" srcAddr: %u, group: %u\n",
            sgEntry->srcAddr, sgEntry->groupAddr);
    }
    ...
    if (treeInfoBaseRowPtr->keepAliveExpiryTimer == 0)
    {
        ...
        clocktype delay;
        /* set KeepaliveTimer(S, G) to Keepalive_Period */
        timerInfo.srcAddr = sgEntry->srcAddr;
        timerInfo.grpAddr = sgEntry->groupAddr;
        timerInfo.intfIndex = (unsigned int)incomingInterface;
        timerInfo.treeState = ROUTING_PIMSM_SG;
    ...
    if (treeInfoBaseRowPtr->SPTbit == FALSE
        && MESSAGE_ReturnPacketSize(msg) != 0)
    {
        NodeAddress prevHop = ANY_IP;
