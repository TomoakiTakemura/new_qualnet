...
/// \defgroup Package_STATS_PHY STATS_PHY

/// \file
/// \ingroup Package_STATS_PHY
/// This file describes Physical Layer stats
...
/// Any signal power or interference smaller than this value
/// will not be counted
#define STAT_PHY_THRESHOLD_MW 1.0e-14

/// Enlisted to differentiate various PHY types based on
...
/// Implements a key for a phy session.  A session
/// is defined as statistics between a (senderId, receiverId,
/// channelIndex) tuple
struct STAT_PhySessionKey
{
    NodeId m_senderId;
    NodeId m_receiverId;
    int m_channelIndex;
    ...
    STAT_PhySessionKey(NodeId p_senderId, NodeId p_receiverId,
      int p_channelIndex, int p_phyIndex)
    : m_senderId(p_senderId), m_receiverId(p_receiverId),
      m_channelIndex(p_channelIndex), m_phyIndex(p_phyIndex)
    { ; }
    ...
    bool operator<(const STAT_PhySessionKey& rhs) const
    {
        ...
        //
        // Because this is stored on a per-interface
        // basis, the rest of these will always be the
        // same, at least for 802.11.  Therefore I put them last
        //
...
/// Implements statistics for a (senderId, receiverId,
...
struct STAT_PhySessionStatistics
{
    ...
    : senderId(0), receiverId(0), channelIndex(0), phyIndex(0),
      utilization(0), totalInterference(0.0), totalPathLoss(0.0),
      totalDelay(0.0), totalSignalPower(0.0), numSignals(0),
      numErrorSignals(0), lastSignalStartTime(CLOCKTYPE_MAX)
    { ; }

    STAT_PhySessionStatistics(NodeId sender, NodeId receiver,
      int channel, int phyIndex)
    : senderId(sender), receiverId(receiver),
      channelIndex(channel), phyIndex(phyIndex),
      utilization(0), totalInterference(0.0), totalPathLoss(0.0),
      totalDelay(0.0), totalSignalPower(0.0), numSignals(0),
      numErrorSignals(0), lastSignalStartTime(CLOCKTYPE_MAX)
    { ; }
...
/// Implements a common physical layer statistics class.  See
...
public:
    ...
    typedef SessionData::iterator SessionIterator;
...
protected:
    ...
    // If session statistics do not exist for this session
    // then new ones are created.
    ...
    STAT_PhySessionStatistics& GetSession(
        ...
        int phyIndex)
    {
       STAT_PhySessionKey key(senderId, receiverId, channelIndex, phyIndex);
       SessionIterator it = m_Sessions.find(key);

       if (it == m_Sessions.end())
       {
           SessionInserterResult inserter
             = m_Sessions.insert(
               SessionPair(
                 key,
                 STAT_PhySessionStatistics(
                   senderId, receiverId, channelIndex, phyIndex
                 )
               )
             );
...
/// Implements global physical aggregated statistics.
