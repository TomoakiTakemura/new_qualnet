...
namespace UTIL 
{
    ...
    template <class T> class Interlock : public IpcLock
    {
        ...
        int get()
        {
            return UTIL_AtomicRead(&m_run_flag);
        }
...
private: int spin_busy()
    ...
    public:
        ...
        void finalize()
        {
            ...
            size_t remainingExecs = m_list.size();

            if (INTERLOCK_DEBUG)
            {
                printf("%" TYPES_SIZEOFMFT "d remaining items in work list after join.\n",
                       remainingExecs);
            }

            if (remainingExecs > 0)
            {
                m_execs += (int)remainingExecs;
                m_worker->run(m_list); 
            }
            ...
            if (INTERLOCK_DEBUG)
            {
                printf("Completed remaining work items (list_size=%" TYPES_SIZEOFMFT "d).\n",
                       remainingExecs);
            }
            ...
            if (INTERLOCK_DEBUG)
            {
                printf("Inserts: %d, Execs: %d: Remaining: %" TYPES_SIZEOFMFT "d\n",
                       m_inserts, 
                       m_execs,
                       remainingExecs);
            }
        ...
        static void* runout(void *data)
        {
            ...
            for (;;)
            {
                ...
                else
                {
                    ...
                    size_t work_list_size = workList.size();
