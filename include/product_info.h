...
/// \defgroup Package_Product_Information Product Information

/// \file
/// \ingroup Package_Product_Information
/// This file contains APIs for product information.
...
/// Defines product kernel which should be either QualNet
/// or EXATA now.
...
/// A class which provide general information about
...
public:
    ...
    /// For certain products, this is the underlying product's name,
    /// whereas GetProductName will return the derived product's name.
    static const char* GetKernelName();

    /// For certain products, this is the underlying product's version,
    /// whereas GetCurrentVersion will return the specific product's version.
    ...
    /// The current kernel version as a string.
    static const char* GetKernelVersionString();
...
#ifdef _WIN32
    // API       :: GetWindowsVersion
    // PURPOSE   :: This function returns the windows version on which EXata
    //              is running.
    // PARAMETERS:: None
    // RETURN    :: Float64 : Windows version
    static Float64 GetWindowsVersion();
#endif
};
