...
/// \defgroup Package_STATS STATS

/// \file
/// \ingroup Package_STATS
/// This file describes the implementation of the STATS API.
...
typedef short STAT_DestAddressType;
...
const std::string& STAT_AddrToString(STAT_DestAddressType type);
...
/// Return timing info for this message.  If no timing
...
///
/// \return The timing info for this message
STAT_Timing* STAT_GetTiming(Node* node, Message* message);

/// Statistic base class for STAT_Statistic and
...
public:
    ...
    void AddToHierarchy(D_Hierarchy* h, const std::string& path);
...
/// Base class for model statistics.  Inherited by model
/// classes.
...
/// Implements a statistic that sums all input data points.
...
/// Implements a statistic that computes the maximum of all
...
/// Implements a statistic that computes the minimum of all
...
/// Implements a statistic that averages all input data
...
/// Implements a statistic that calculates the session
...
/// Implements a statistic that calculates throughput or
/// offered load.
/// Value is 0 if no data points have been added.
...
/// Implements a statistic that calculates utilization
/// Value is 0 if no data points have been added.
/// AddDataPoint will call SessionStart if it has not been
/// called explicitly.  Throughput value is computed between
/// SessionStart time and SessionFinish time or the current
...
/// Implements a statistic that calculates smoothed average.
...
/// Implements a statistic that calculates weighted average by time.
/// Value is 0 if no data points have been added.
/// 
...
/// Base class for all aggregated statistics.  All stats must
...
/// Used for sending aggregated data across partitions.  This
...
/// Implements a statistic that aggregates the sum of all
/// statistics.
/// Value is 0 if no data points have been added.
...
class STAT_AggregatedSumVariant : public STAT_AggregatedStatistic
{
protected:
    Float64 old_Value;
public:
    STAT_AggregatedSumVariant()
    {
        old_Value = 0;
    }
    virtual void AddDataPoint(clocktype now, double val);
...
class STAT_AggregatedAverageVariant : public STAT_AggregatedStatistic
{
protected:
    Float64 old_Value;
public:
    STAT_AggregatedAverageVariant()
    {
        old_Value = 0;
    }
    virtual void AddDataPoint(clocktype now, double val);
...
/// Implements a statistic that aggregates the average of all
...
/// Implements a statistic that aggregates throughput.  
/// Value is 0 if no data points have been added.
...
/// Implements a statistic that divides other statisics.
