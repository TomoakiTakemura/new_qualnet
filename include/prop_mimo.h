...
struct MIMO_TGn_cluster {
  double weight;   
  double AoA;      // Angle of Arrival in degrees (or departure, the model is symetric Rx<->Tx)
  double AS;       // Angular Spread in degrees.
  //enum {Uniform, Laplacian} PAS_Model;
};


typedef Eigen::Matrix<std::complex<double>, Eigen::Dynamic, Eigen::Dynamic> MIMO_Matrix_t;

struct MIMO_Data {
  int    m_elementCount;
  double m_elementSpace;
  MIMO_Matrix_t m_correlation;
  MIMO_Matrix_t m_LOS;
  double m_LOS_weight;

  // a quick default constructor to zero the elementCount
  // allows a simple lazy initialization
  MIMO_Data() : m_elementCount(0), m_elementSpace(0.0),m_LOS_weight(0.0) {
    m_correlation.resize(1,1);
    m_LOS.resize(1,1);
  }
  MIMO_Data& operator=(const MIMO_Data& other) {
    m_elementCount = other.m_elementCount;
    m_elementSpace = other.m_elementSpace;
