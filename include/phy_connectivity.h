...
public:
    ...
    struct SatComNodeInfo
    {
        ...
        bool operator < (const SatComNodeInfo &info2) const
        {
    ...
    struct ChannelNodeInfo
    {
        ...
        //        
        ChannelNodeInfo(int n, int p, PhyModel m = PHY_NONE,
                        AntennaModelType t = ANTENNA_OMNIDIRECTIONAL,
                        float ah = 0.0, double tp = 0.0,
                        double loss = 0.0, double gain = 0.0) :
            nodeId(n), phyIndex(p), model(m), type(t), antennaHeight(ah),
            txPower_mW(tp), systemLossIndB(loss), gain_dBi(gain)
        {}

        bool operator < (const ChannelNodeInfo &info2) const
        {
    ...
    struct ListeningChannelNodeInfo
    {
        ...
        bool operator < (const ListeningChannelNodeInfo &info2) const
        {
            if (nodeId < info2.nodeId) 
            {
                return true;
    ...
    struct AdjacencyNodeValue
    {
        ...
        AdjacencyNodeValue(
            ...
            double p): 
                connected(conn), sType(st),
                rType(rt), pathloss_dB(p)  {
                    rxPower_mw = 0;
                    rxPower_mwWorst = 0;
                    reachable = FALSE;
