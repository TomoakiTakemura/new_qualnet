...
inline void UTIL_AtomicSetFloat(UTIL_AtomicInteger *v, float x) {
    void* vx = &x;  // cast through void* to avoid strict-aliasing issues
    int ix = *(int*)vx;
    UTIL_AtomicSet(v, ix);
}

inline float UTIL_AtomicReadFloat(UTIL_AtomicInteger *v) {
    int ix = UTIL_AtomicRead(v);
    void* vx = &ix;  // cast through void* to avoid strict-aliasing issues
    return *(float*)vx;
}

#ifndef _LP64
inline void UTIL_AtomicSetGP(UTIL_AtomicInteger *v, void *x) {
    int ix = (int)x;
    UTIL_AtomicSet(v,ix);
}

inline void *UTIL_AtomicReadGP(UTIL_AtomicInteger* v) {
    int ix = UTIL_AtomicRead(v);
    return (void*)ix;
}
#endif //!defined(_LP64)
