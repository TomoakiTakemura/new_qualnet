...
enum AppType
{
    ...
    APP_LOOKUP_SERVER,
    APP_LOOKUP_CLIENT,
    APP_MIMO,             // need random numbers for MIMO propagation model
...
struct AppIdentifier
{
    ...
    bool operator < (const AppIdentifier& rhs) const
    {
        ...
        if (sourceAddress < rhs.sourceAddress)
            {
                return true;
            }
        if (rhs.sourceAddress < sourceAddress)
            {
                return false;
            }
        return false;
...
//takemura
#define APP_TIMER_SEND_PNG     (5)  // for sending pingreq packet
#define APP_TIMER_RESEND_PKT   (6)  // for sending retransmit packet
#define APP_TIMER_SLEEP_TIMER  (7)  // for judge mqttclient is lost
#define APP_TIMER_KEEP_ALIVE   (8)
