...
public:
    ...
    virtual bool getDoStreetsCross(const Coordinates*,
                                   const Coordinates*) { return false; }
...
// Path properties may be cached. Smart pointers are used to manage
// when to delete the object path properties object.
typedef boost::shared_ptr<UrbanPathProperties> UrbanPathPropertiesPointer;
...
public:
    ...
    // averages.
    double getDistanceThroughBuilding(const int) const { return m_avgBuildingWidth; }
    double getDistanceThroughFoliage(const int) const  { return m_avgFoliageWidth; }
    double getFoliageDensity(const int) const          { return m_region->getAvgFoliageDensity(); }
    FoliatedState getFoliatedState(const int) const    { return m_region->getFoliatedState(); }
...
public:
    ...
    virtual void initialize(NodeInput*) {}
    virtual void finalize() {}

    virtual bool hasData() { return false; } // subclasses should return true.

    virtual double getElevationAt(const Coordinates*) { return 0.0; }
    ...
    virtual void getElevationBoundaries(int, Coordinates* sw, Coordinates* ne)
    {
        sw->common.c1 = -1.0;
        sw->common.c2 = -1.0;
        sw->common.c3 = -1.0;
    ...
    virtual void getHighestAndLowestElevation(const Coordinates*,
                                              const Coordinates*,
                                              double* highest,
                                              double* lowest) {
        *highest = 0.0;
...
public:
    ...
    virtual std::string* fileList(NodeInput*) {
        return new std::string("");
    }

    virtual void populateRegionData(
        TerrainRegion*,
        TerrainRegion::Mode = TerrainRegion::FULL) {}

    // point functions
    virtual bool isPositionIndoors(const Coordinates*) { return false; }
    virtual bool arePositionsInSameBuilding(const Coordinates*,
                                            const Coordinates*) { return false; }

    virtual double getElevationAt(const Coordinates*) { return 0.0; }

    virtual bool getEdgeOfBuilding(const  Coordinates*,
                                   const  Coordinates*,
    ...
    virtual UrbanPathPropertiesPointer getUrbanPathProperties(
        const  Coordinates*,
        const  Coordinates*,
        double,
        bool = false,
        PartitionData* = NULL) { return UrbanPathPropertiesPointer(); }
};
...
public:
    ...
    UrbanPathPropertiesPointer getUrbanPathProperties(PathSegment* ps,
                                                bool includeFoliage = false);
    ...
    std::string* terrainFileList(NodeInput* nodeInput, bool isUrbanIncluded = true);
