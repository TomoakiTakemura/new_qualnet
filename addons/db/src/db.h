...
#define STATSDB_DOUBLE_FORMAT "%.9f"
#define STATSDB_TIME_FORMAT "%.9f"
...
struct StatsDBStatusTable
{
    ...
    struct ltndaddr
    {
        bool operator () (const std::pair<Int32, NodeAddress> &s1,
...
struct StatsDBEventsTable
{
    ...
    BOOL createExternalEventsTable;
};
...
struct StatsDBExternalEventContent
{
    bool Use() {
    ...
    bool Use(int p) {
        if (!Use()) return false;
        if (all) return true;
        return (protocols.find(p) != protocols.end());
    }
    StatsDBExternalEventContent() : active(false), all(false) { }
...
struct StatsDBConnTable
{
    ...
    struct ltaddr
    {
        bool operator () (const std::pair<Address, Int32> &s1,
    ...
    struct ltndintpair
    {
        bool operator () (const std::pair<Int32, Int32> &s1,
...
struct TransSrcDstData
{
    ...
    BOOL isUpdated ;
    TransSrcDstData(const std::string &)
    {
        SenderPort = 0;
        ReceiverPort = 0;
        SegmentsSentToApp = 0;
...
void InitializeStatsDBExternalEventsTable(PartitionData* partitionData, NodeInput* nodeInput);
...
struct ltaddrpair
{
    bool operator () (const std::pair<Address, Address> &s1,
...
struct ltportpair
{
    bool operator () (const std::pair<short, short> &s1,
        std::pair<short, short>& s2) const
    {
...
/// Enumerates the type of statsdb protocol
typedef
enum
{
...
//----------------------------------------------------------
/// This structure holds the join/leave information
...
// PURPOSE : builds and executes CREATE TABLE staement
//   only does work in partition 0
// PARAMETERS
// + db : StatsDb* : Pointer to the database
...
// PURPOSE : formulates UPDATE of the form
//           UPDATE table SET column = column + value... WHERE qualifiers
// PARAMETERS
// + db : StatsDb* : Pointer to the database
...
// PURPOSE : formulates UPDATE of the form
//           UPDATE table SET column = value ... WHERE qualifiers
// PARAMETERS
// + db : StatsDb* : Pointer to the database
// + table : std::string : name of desired table to perform update on
// + columns : std::vector<std::string> : column names you are updating
...
//--------------------------------------------------------------------------
// FUNCTION:  AddUpdateQueryToBuffer
// PURPOSE : adds query to a stored buffer of queries to perform later,
//  an attempt to minimize number of lock acquistions
// PARAMETERS
// + db : StatsDb* : Pointer to the database
...
class StatsDBAppSummaryParam;   // Required forward definition!
