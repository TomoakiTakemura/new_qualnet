                       ...
                       char* prefix)
{
    ...
    if (partition->partitionId == 0)
    {
        ...
        if (wasFound)
        {
            ...
            else if (strcmp(buf, "MARIADB") == 0
                     || strcmp(buf, "MYSQL") == 0 /* backwards compatibility */)
            {
                db->engineType = UTIL::Database::dbMariaDB;
            }
            else
            {
                ERROR_ReportErrorArgs(
                    "Invalid Value (%s) for STATS-DB-ENGINE parameter\n",
                    buf);
            }
        }
        else
        ...
        {
            ERROR_ReportError
                    ("STATS-DB-ENGINE must be set to MARIADB when in running in distributed mode\n");
        }
        ...
        if (db->engineType == UTIL::Database::dbMariaDB)
        {
            ...
            IO_ReadString(
                ...
                ANY_ADDRESS,
                nodeInput,
                "STATS-DB-MARIADB-SOCKET-FILE-PATH",
                &wasFound,
                buf);
            ...
            {
                // Check STATS-DB-MYSQL-SOCKET-FILE-PATH for backwards
                // compatibility
                IO_ReadString(
                    ANY_NODEID,
                    ANY_ADDRESS,
                    nodeInput,
                    "STATS-DB-MYSQL-SOCKET-FILE-PATH",
                    &wasFound,
                    buf);
                if (wasFound)
                {
            ...
            IO_ReadString(
                          ...
                          ANY_ADDRESS,
                          nodeInput,
                          "STATS-DB-MARIADB-STORAGE-ENGINE",
                          &wasFound,
                          buf);
            if (!wasFound)
            {
                // Check STATS-DB-MYSQL-STORAGE-ENGINE for backwards
                // compatibility
                IO_ReadString(
                    ANY_NODEID,
                    ANY_ADDRESS,
                    nodeInput,
                    "STATS-DB-MYSQL-STORAGE-ENGINE",
                    &wasFound,
                    buf);
            }

            // if the value DEFAULT is passed in then set the storageEngine
            // to blank, so create table will not add anything, resulting
            // in the database default choice.  Any other value present
            // is passed to the database, According to
            // http://dev.mysql.com/doc/refman/5.1/en/create-table.html the
            // default should result but it currently throws an error.
            // If the value is absent we use MyISAM.
            if (wasFound)
            {
                IO_ConvertStringToUpperCase(buf);
                if (strcmp(buf, "DEFAULT") == 0)
                {
                    strcpy(db->storageEngine, "");
                }
                else
                {
                    strcpy(db->storageEngine, buf);
                }
            }
            else
            {
                strcpy(db->storageEngine, "MYISAM");
            }


            db->driver = new UTIL::Database::MariaDBNativeDriver(server,
                                                             username,
                                                             password,
                                                             std::string(database),
                                                             socketFilePath);
        ...
        else if (db->engineType == UTIL::Database::dbSqlite)
        {
            ...
            db->driver = new UTIL::Database::Sqlite3Driver(database);
        }
    ...
    if (wasFound)
    {
            ...
            else if (strcmp(buf, "CUSTOM") != 0)
            {
                ...
                ERROR_ReportWarningArgs("Invalid Value (%s) for STATS-DB-LEVEL parameter\n", buf);
            }
        }
        else
        {
...
struct STATSDB_RegressionRow
{
    ...
    STATSDB_RegressionRow(char** row, Int32 numCols, Int32 ignoreColumn, std::vector<bool> floatColumns)
    {
        for (i = 0; i < numCols; i++)
        {
            if (i != ignoreColumn)
            {
                if (floatColumns[i] && strlen(row[i])>0)
                {
                    char buf[20];
                     sprintf(buf, "%.5g", atof(row[i]));
                     m_cols.push_back(buf);
                }
                else
                {
    ...
    bool equals(const STATSDB_RegressionRow& rhs) const
    {
...
//----------------------------------
static bool STATSDB_CompareTables(
    ...
    const char* tableName)
{
    ...
    // an undocumented feature to control the number of row misses
    // that get printed during output.
    char* rowStr = getenv("DB_REGRESSION_ROWS");
    int dbRegressionRowsToPrint = 0;
    if (rowStr) dbRegressionRowsToPrint = atoi(rowStr);
    if (dbRegressionRowsToPrint == 0) dbRegressionRowsToPrint = 10;
    ...
    for (i = 1; i <= numRow; i++)
    {
        string colName(table[i * numCol + 1]);
        string colType(table[i * numCol + 2]);
        if (colType == "float") colType = "real";
        if (colType == "double") colType = "real";
        if (colType == "integer") colType = "int";
        if (colType == "bigint") colType = "int";
        if (colType == "TEXT") colType = "text";
        if (colType.substr(0, 7) == "varchar") colType = "varchar";
        if (colType.substr(0, 7) == "VARCHAR") colType = "varchar";

        if (colName == "RowId") ignoreCol = i - 1;
        floatColumns.push_back(colType == "real");
        expectedColumns.push_back(colName);
        expectedTypes.push_back(colType);
    }
    ...
    for (i = 1; i <= numRow; i++)
    {
        ...
        compareColumns.push_back(colName);
        compareTypes.push_back(colType);
    }

    // Now verify that columns match
    if (expectedColumns.size() != compareColumns.size())
    {
        printf("Different number of columns in %s (got:%ld expected:%ld)\n",
            tableName,
            compareColumns.size(),
    ...
    for (size_t j = 0; j < expectedColumns.size(); j++)
    {
        ...
        {
            printf("Different column name in %s (got:%s expected:%s)\n",
                tableName,
        ...
        {
            printf("Different type in %s.%s (got:%s expected:%s)\n",
                tableName,
                expectedColumns[j].c_str(),
    ...
    int errCount = 0;
    if (compareRows.size() != expectedRows.size())
    {
        printf("Table %s row count mismatch (got:%ld expected:%ld)\n",
            tableName,
            compareRows.size(),
            expectedRows.size());
        return false;
    ...
    while (itLeft != expectedRows.end() || itRight != compareRows.end())
    {
        if (itLeft == expectedRows.end())
        {
            if (errCount < dbRegressionRowsToPrint)
            {
            ...
            ++errCount;
            if (errCount == dbRegressionRowsToPrint) printf("Further errors suppressed\n");
            ++itRight;
        }
        else if (itRight == compareRows.end() || comp(*itLeft, *itRight))
        {
            if (errCount < dbRegressionRowsToPrint)
            {
        ...
        else
        {
            if (false)
            {
                printf("Match            %s", (**itRight).m_cols[0].c_str());
                for (size_t j = 1; j < (**itRight).m_cols.size(); j++)
                {
                    printf("|%s", (**itRight).m_cols[j].c_str());
                }
    ...
    if (errCount > 0)
    {
        printf("%d errors in table %s\n", errCount, tableName);
        return false;
    }
    printf("table %s matches\n", tableName);
    return true;
}
...
void STATSDB_Regression(char* prefix)
{
    ...
    printf("Comparing database file:%s with %s\n", expectedDb.fileName(), compareDb.fileName());
    ...
    for (it = expectedTables.begin(); it != expectedTables.end(); ++it)
    {
        if (!STATSDB_CompareTables(&expectedDb, &compareDb, it->c_str()))
        fail = true;
    }

    if (fail) exit(-1);
    // No error was found, exit with error code 0
    printf("databases have identical tables\n");
    exit(0);
}
                                     ...
                                     NodeInput* nodeInput)
{
    ...
    clocktype mcrInterval = db->statsAggregateTable->aggregateInterval;
    if (partition->maxSimClock % mcrInterval == partition->maxSimClock)
    {
        db->statsAggregateTable->mcrSize = 1;
    }
    else if (partition->maxSimClock % mcrInterval == 0)
    {
        db->statsAggregateTable->mcrSize = partition->maxSimClock / mcrInterval;
    }
    else
    {
        db->statsAggregateTable->mcrSize = (partition->maxSimClock / mcrInterval) + 1;
    }
                                  ...
                                  NodeInput* nodeInput)
{
    ...
    IO_ReadBool(ANY_NODEID, ANY_ADDRESS, nodeInput,
        "STATS-DB-EXTERNAL-EVENTS-TABLE", &wasFound,
        &db->statsEventsTable->createExternalEventsTable);
    if ((wasFound && db->statsEventsTable->createExternalEventsTable) ||
       (!wasFound && db->statsTable->createEventsTable))
    {
        db->statsEventsTable->createExternalEventsTable = TRUE;
        InitializeStatsDBExternalEventsTable(partition, nodeInput);
    }
       ...
       (!wasFound && db->statsTable->createEventsTable))
    {
        ...
        db->queueDbPtr->STATSDB_InitializeQueueEventTable(partition, nodeInput);
   }
                                        ...
                                        NodeInput* nodeInput)
{
    ...
    columns.push_back(
        string_pair("IntervalBasedUnicastMCR", "float"));
    columns.push_back(
        string_pair("IntervalBasedMulticastMCR", "float"));
    columns.push_back(
        string_pair("IntervalBasedAverageMCR", "float"));
    ...
    if (wasFound)
    {
        if (strcmp(buf, "YES") == 0)
        {
            db->statsAppAggregate->isIntervalBasedAvgDelay = TRUE;
            columns.push_back(
                string_pair("IntervalBasedAverageDelay", "float"));
        }
        else if (strcmp(buf, "NO") != 0)
        {
            // We have invalid values.
            ERROR_ReportWarning(
                "Invalid Value for"
                " STATS-DB-APPLICATION-AGGREGATE-INTERVAL-BASED-AVERAGE-DELAY parameter,"
                "using Default\n");
            db->statsAppAggregate->isIntervalBasedAvgDelay = TRUE;
            columns.push_back(
                string_pair("IntervalBasedAverageDelay", "float"));
        ...
        NodeInput* nodeInput)
{
    ...
    db->statsNetEvents->lengthOfNetworkEventsTbName =
        (Int32)strlen(db->statsNetEvents->networkEventsTbColsName);
}
...
static void getProtocolNames(std::map<std::string, std::string>& m) {
    struct localizer  {
        localizer(std::map<std::string, std::string>& m) : m_map(m) { }
        std::map<std::string, std::string>& m_map;
        void operator() (const char* name, int ivalue) {
            char buf[10];
            sprintf(buf, "%d", ivalue);
            m_map.insert(make_pair(name, buf));
        }
    };
    localizer l(m);
    l("IP", 0);
...
void InitializeStatsDBExternalEventsTable(PartitionData* partition, NodeInput* nodeInput)
{
    StatsDb* db = partition->statsDb;
    if (db == NULL || partition->partitionId > 0) return;

    db->statsExternalEvents->active = true;

    STATSDB_Table_External_Events table(db);

    table.AddColumn("RowId", "rowid");
    table.AddColumn("Timestamp", "real");
    table.AddColumn("NodeID", "integer");
    table.AddColumn("MessageID", "VARCHAR(64)");
    table.AddColumn("SourceAddress", "VARCHAR(64)");
    table.AddColumn("DestinationAddress", "VARCHAR(64)");
    table.AddColumn("EventType", "VARCHAR(64)");
    table.AddColumn("TOS", "integer");
    table.AddColumn("TotalLength", "integer");
    table.AddColumn("Identification", "integer");
    table.AddColumn("FragmentFlags", "integer");
    table.AddColumn("FragmentOffset", "integer");
    table.AddColumn("TTL", "integer");
    table.AddColumn("Protocol", "integer");
    table.AddColumn("Device", "VARCHAR(64)");
    table.AddColumn("DeviceIPAddress", "VARCHAR(64)");
    table.AddColumn("SrcMacAddress", "VARCHAR(64)");
    table.AddColumn("DstMacAddress", "VARCHAR(64)");

    std::map<std::string, std::string> protocolNames;
    getProtocolNames(protocolNames);
    table.ConfigNumbers(nodeInput, "STATS-DB-EXTERNAL-EVENTS-PROTOCOL", db->statsExternalEvents->protocols, "ICMP,TCP,UDP", &protocolNames);
    if (db->statsExternalEvents->Use(-1)) db->statsExternalEvents->all = true;
...
void InitializeStatsDBMulticastConnTable(PartitionData* partition, NodeInput* /*nodeInput*/)
{
    ...
    CreateTable(db, "MULTICAST_Connectivity", columns);
}
    ...
    NodeAddress externalNodeId)
{
    ...
    if (db == NULL)
    {
        ...
        StatsDBAppEventParam* appParamInfo = new(MESSAGE_AddInfo(
                node,
                msg,
                sizeof(StatsDBAppEventParam),
                INFO_TYPE_AppStatsDbContent))(StatsDBAppEventParam);

    ERROR_Assert(appParam != NULL, "Unable to add an info field!");
    appParam->m_NodeId = (externalNodeId == -1) ? node->nodeId : externalNodeId;
    appParamInfo->m_NodeId = appParam->m_NodeId;
    appParamInfo->m_SessionInitiator = appParam->m_SessionInitiator;
    appParamInfo->m_ReceiverId = appParam->m_ReceiverId;
    appParamInfo->SetReceiverAddr(appParam->m_TargetAddr.get());
    appParamInfo->SetSocketInterfaceMsgId(*appParam);
        if (appParam->m_MsgSize.isNULL())
                appParamInfo->SetMsgSize(msgSize);
        else
                appParamInfo->SetMsgSize(appParam->m_MsgSize.get());

    // TESTING. Pre-empt the info fields for TCP Only
    MESSAGE_AddInfo(
        node,
        msg,
    ...
    if (appParam->m_FragId.get() == 0)
    {
       appParam->SetMsgSeqNum(msg->sequenceNumber);
    }
    appParamInfo->m_FragId.set(appParam->m_FragId);
    ...
    appParamInfo->m_SessionId.set(appParam->m_SessionId);
    appParamInfo->m_Priority.set(appParam->m_Priority);
    appParamInfo->m_MsgFailureType.set(appParam->m_MsgFailureType);
    appParamInfo->m_Delay.set(appParam->m_Delay);
    appParamInfo->m_Jitter.set(appParam->m_Jitter);

    appParamInfo->SetPacketCreateTime(node->getNodeTime());
    ...
    if (!node->partitionData->statsDb->statsAppEvents->recordFragment &&
        (appParam->m_IsFragmentation || appParam->m_FragId.get() == 0))
    {
         appParamInfo->SetMessageId(mapParamInfo->msgId, false);
         StatsDBAppEventParam tempAppParam = *appParamInfo;
         tempAppParam.m_MsgSize = appParam->m_TotalMsgSize;
         if (externalNodeId == -1) STATSDB_HandleAppEventsTableUpdate(node, NULL, tempAppParam);
    }
    else if (node->partitionData->statsDb->statsAppEvents->recordFragment)
    {
        appParamInfo->SetMessageId(mapParamInfo->msgId, true);
        if (externalNodeId == -1) STATSDB_HandleAppEventsTableUpdate(node, NULL, *appParamInfo);
    }
    else
    {
         appParamInfo->SetMessageId(mapParamInfo->msgId, true);
    ...
    Int32 numReqMsgRcvd)
{
    ...
    appParam.SetReceiverAddr(appParamInfo->m_TargetAddr);
        appParam.SetMessageId(mapParamInfo->msgId, node->partitionData->statsDb->statsAppEvents->recordFragment);
    ...
    appParam.SetMsgSeqNum(msg->sequenceNumber);
    appParam.m_FragId.set(appParamInfo->m_FragId);
    appParam.m_SessionId.set(appParamInfo->m_SessionId);
    appParam.m_Priority.set(appParamInfo->m_Priority);
    appParam.m_MsgFailureType.set(appParamInfo->m_MsgFailureType);
    appParam.SetDelay(delay);
    if (numReqMsgRcvd > 1)
    {
        appParam.SetJitter(jitter);
    }
    appParam.SetSocketInterfaceMsgId(*appParamInfo);
    ...
    char* failureType)
{
    ...
    appParam.SetMessageId(mapParamInfo->msgId, true);
    appParam.m_MsgSize = size;
    strcpy(appParam.m_EventType, eventType);
    ...
    appParam.m_SessionInitiator = appParamInfo->m_SessionInitiator;
    appParam.m_ReceiverId = appParamInfo->m_ReceiverId;
    appParam.m_TargetAddr.set(appParamInfo->m_TargetAddr);
        appParam.SetMessageId(mapParamInfo->msgId, node->partitionData->statsDb->statsAppEvents->recordFragment);
    appParam.m_MsgSize = size;
    strcpy(appParam.m_EventType, eventType);
    ...
    Int32 ipHdrSize)
{
    ...
    {
        // Info does not exist. Insert one.
                ipParamInfo = new(MESSAGE_AddInfo(
                        node,
                        msg,
                        sizeof(StatsDBNetworkEventParam),
                        INFO_TYPE_NetStatsDbContent)) (StatsDBNetworkEventParam);
    }
    ERROR_Assert(ipParamInfo != NULL, "Unable to add an info field!");
    ipParamInfo->m_NodeId = node->nodeId;
    ...
    ipParamInfo->AddToHopCount(hopCount);
    ...
    if (ipParamInfo->m_PktType.get() == StatsDBNetworkEventParam::CONTROL &&
    ...
    if (!db->statsNetEvents->networkStatsDBIncoming &&
        ...
        ipParamInfo->m_PktType.get() != StatsDBNetworkEventParam::CONTROL)
    {
        return;
    }
...
void HandleStatsDBAppAggregateUpdate(Node* node)
{
    StatsDb* db = node->partitionData->statsDb;
    ...
    STATSDB_HandleAppAggregateTableUpdate(node);
}
...
void StatsDBSendAggregateTimerMessage(PartitionData* partition)
{
    ...
    if (db->statsAggregateTable->createAppAggregateTable)
    {
        ...
        aggregateTimer->eventTime = partition->getGlobalTime() +
            db->statsAggregateTable->aggregateInterval;
...
void StatsDBSendStatusTimerMessage(PartitionData* partition)
{
    ...
    if (db->statsStatusTable->createNodeStatusTable)
    {
        ...
        statusTimer->eventTime = partition->getGlobalTime() +
            db->statsStatusTable->statusInterval;
        PARTITION_SchedulePartitionEvent(partition,
            statusTimer,
    ...
    BOOL isDataPacket)
{
    ...
    if (outgoing)
    {
        if (!isForward)
        {
            ...
            {
                // First Packet
                stats->firstPacketSendTime = node->getNodeTime();
            }
            stats->lastPacketSendTime = node->getNodeTime();
        }
    ...
    else
    {
        ...
        clocktype delay = node->getNodeTime() - *timing;
        clocktype jitter = 0;
        ...
        for (interfaceIndex = 0; interfaceIndex < traverseNode->numberInterfaces; interfaceIndex++)
        {
            ...
            for (iter = oneHopData->begin(); iter != oneHopData->end(); iter++)
            {
                ...
                phyParam.m_Utilization =
                    iter->second.utilization / node->getNodeTime();
...
void StatsDBSendSummaryTimerMessage(PartitionData* partition)
{
    ...
    if (db->statsSummaryTable->createAppSummaryTable)
    {
        ...
        summaryTimer->eventTime = partition->getGlobalTime() +
            db->statsSummaryTable->summaryInterval;
...
void StatsDBSendConnTimerMessage(PartitionData* partition)
{
    ...
    if (db->statsConnTable->createAppConnTable)
    {
        ...
        connTimer->eventTime = partition->getGlobalTime() +
...
void StatsDBAddMessageAppMsgId(Node *node, Message *msg, StatsDBAppEventParam *appParam)
{
    ...
    {
        // Info does not exist. Insert one.
                mapParamInfo = new(MESSAGE_AddInfo(
                        node,
                        msg,
                        sizeof(StatsDBMappingParam),
                        INFO_TYPE_StatsDbMapping))(StatsDBMappingParam);
    }
    ERROR_Assert(mapParamInfo != NULL, "Unable to add an info field!");

    size_t ls = sprintf(mapParamInfo->msgId, "%s%dS%d",
        (appParam->m_NodeId == node->nodeId) ? "N" : "NX",
        node->nodeId, appParam->m_MsgSeqNum.get());

    if (appParam->m_fragEnabled)
    {
        ls += sprintf(mapParamInfo->msgId + ls, "AF%d", appParam->m_FragId.get());
    }
...
void StatsDBAddMessageMsgId(Node *node, Message *msg)
{
        ...
        StatsDBMappingParam *mapParamInfo = new(MESSAGE_AddInfo(
                node,
                msg,
                sizeof(StatsDBMappingParam),
                INFO_TYPE_StatsDbMapping))(StatsDBMappingParam);
    ...
    sprintf(mapParamInfo->msgId, "N%dS%d", node->nodeId, msg->sequenceNumber);
                                  ...
                                  NodeAddress prevHopAddr)
{
    ...
    {
                nextPrevHopInfo = new(MESSAGE_AddInfo(node,
                        msg,
                        sizeof(StatsDBMessageNextPrevHop),
                        INFO_TYPE_MessageNextPrevHop))(StatsDBMessageNextPrevHop);
                               ...
                               NodeAddress dstAddr)
{
    ...
    if (msgAddrInfo == NULL)
    {
                msgAddrInfo = new(MESSAGE_AddInfo(node,
                        msg,
                        sizeof(StatsDBMessageAddrInfo),
                        INFO_TYPE_MessageAddrInfo))(StatsDBMessageAddrInfo);
                             ...
                             Int32 msgSize)
{
          ...
          || eventType == "TCPReceiveFromUpper" || eventType == "TCPReceiveFromLower" )
    {
        ...
        sprintf(msgId, "%s", tcpMapParamInfo->msgId) ;
       ...
       NodeAddress groupAddr)
{
    ...
    else if (strcmp(eventType, "Leave") == 0)
    {
                               ...
                               NodeAddress groupAddr)
{
    ...
    {
        ERROR_ReportWarning("STATSDB_SendMulticastGroupInfo called with invalid message type");
        return;
    }

    for (i = 0; i < node->partitionData->getNumPartitions(); i++)
    {
    ...
    AppData::APPL_MULTICAST_NODE_MAP** nodeMap)
{
    ...
    Node* firstNode = node->partitionData->firstNode;

    // If map is empty, then nobody has joined the group,
    // so return.
    if (firstNode->appData.multicastMap == NULL)
    {
        return 0;
    }

    // Go through groupAddr row of the map
    it = firstNode->appData.multicastMap->find(groupAddr);
    if (it == firstNode->appData.multicastMap->end())
    {
        return 0;
    }
...
void STATSDB_ProcessEvent(PartitionData *partition, Message *msg)
{
    ...
    switch (msg->eventType)
    {
        case MSG_STATSDB_APP_InsertSummary:
        {
            ...
            msg->eventTime = partition->getGlobalTime() +
                db->statsSummaryTable->summaryInterval;
    ...
    if (db && db->statsEventsTable->createPhyEventsTable)
    {
        ...
        switch(thisPhy->phyModel)
        {
            case PHY_ABSTRACT:
            {
                ...
                phyParam.SetSignalPower_db(phy_abstract->txPower_dBm);
                break;
            }
            case PHY802_11a:
            case PHY802_11b:
            {
                ...
                phyParam.SetSignalPower_db(phy802_11->txPower_dBm);
                break;
            }
#ifdef LTE_LIB
            case PHY_LTE:
            {
                phyParam.SetSignalPower_db(PhyLteGetTxPower(node,
                                                         phyIndex,
                                                         packet));
                break;
            }
    ...
    float txPower_dBm)
{
    ...
    if (db && db->statsEventsTable->createPhyEventsTable)
    {
        ...
        switch (thisPhy->phyModel)
        {
            ...
            case PHY_LTE:
            {
                phyParam.SetSignalPower_db(txPower_dBm);
...
{
    if (db == NULL) return;
    if (db->partition->partitionId != 0) return;
    ...
    if (columns.size() > 0)
    {
        ...
        for (size_t i = 1; i < columns.size(); i++ )
        {
            if (columns[i].first.length() > 0)
            {
                std::string colType = columns[i].second;
                // for mysql float is single precision, real and double are identical
                // by default but a database parameter can change it. We'll assume default
                // for sqlite all significant digits are stored, for any data type
                // mapping float to real keeps all available digits in both cases.
                if (colType == "float") colType = "real";
                if (colType == "double") colType = "real";
                query += ("," + columns[i].first + " " + colType);
            }
        }

        query += ")";

        // override the database storage engine if one is set in the configuration
        // My MariaDB (or MySQL) installations default to INNODB which is slow buy
        // supports transactions.  For what we are doing here MyISAM is much faster.
        if (db->storageEngine[0])
        {
            query += " ENGINE ";
            query += db->storageEngine;
        }
        AddQueryToBufferStatsDb(db, query);
    }
    if (STATS_DEBUG)
    {
        ...
        printf("Time Taken to create table %s, %s\n",
           name.c_str(), temp);
    }
}
    ...
    const char* joinOperator = "AND")
{
    ...
    sources += " FROM ";
    sources += table;
    sources += GatherWhereClause(qualifiersColumns, qualifiersValues, joinOperator);
    return sources;
}
    ...
    const char* joinOperator)
{
        ...
        query += table;
        query += GatherWhereClause(qualifiersColumns, qualifiersColumns, joinOperator);
        AddQueryToBufferStatsDb(db, query);
}
    ...
    const char* joinOperator)
{
    ...
    return Select(db, GetSelectSQL(table.c_str(), columns, qualifiersColumns, qualifiersValues));
}
...
void AddUpdateQueryToBuffer(StatsDb* db,
                            const std::string& query)
{
    AddQueryToBufferStatsDb(db, query);
}
    ...
    std::vector<std::string>& insertList)
{
    ...
    {
        AddQueryToBufferStatsDb(db, insertList[i]);
    }
}
