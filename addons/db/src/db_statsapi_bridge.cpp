...
Int32 STAT_GlobalAppStatisticsBridge::numFields(PartitionData* partition)
{
    ...
    if (db->statsAppAggregate->isIntervalBasedAvgDelay)
    {
        requiredFieldsIndex[APP_AGGREGATE_TABLE_INTERVAL_BASED_AVG_DELAY]
                                                                    = TRUE;
        numOptionalFields++;
    }
...
// Function to compose SQL updation string

std::string
STAT_GlobalAppStatisticsBridge::composeGlobalAppStatisticsUpdateSQLString(
    Node* node,
    PartitionData* partition,
    Int64 intervalIndex,
    clocktype interval)
{
    std::vector<std::string> newValues;
    // Here 3 is for interval based cloumns "UnicastMCR",
    // "MulticastMCR" and "AverageMCR".
    newValues.reserve(3);
    std::vector<std::string> columns;
    columns.reserve(3);
    std::vector<std::string> qualifiersColumns;
    qualifiersColumns.reserve(1); // Here 1 is for column "Timestamp".
    std::vector<std::string> qualifiersValues;
    qualifiersValues.reserve(1);
    double timeVal = (double) interval / SECOND;

    for (Int32 i = APP_AGGREGATE_TABLE_INTERVAL_BASED_UNI_MCR;
         i <= APP_AGGREGATE_TABLE_INTERVAL_BASED_AVG_DELAY;
         i++)
    {
        if (tableConfigurations[i].configuration == ""
            || requiredFieldsIndex[i])
        {
            ...
            {
                newValues.push_back(STATSDB_DoubleToString(
                    valueForIndex(node, i, intervalIndex)));
            }
        }
    }
    qualifiersColumns.push_back(tableConfigurations[0].name);
    qualifiersValues.push_back(STATSDB_DoubleToString(timeVal));
    return GetUpdateValuesSQL("APPLICATION_Aggregate",
                              columns,
                              newValues,
                              qualifiersColumns,
                              qualifiersValues,
...
double STAT_GlobalAppStatisticsBridge::valueForIndex(Node* node,
                                                     ...
                                                     Int64 intervalIndex)
{
    double unicastIntervalBasedMCR = 0;
    double multicastIntervalBasedMCR = 0;
    double messageSent = 0;
    double messageReceived = 0;
    double totalMsgReceived = 0;
    double totalMsgSent = 0;
    ...
    switch (index)
    {
        case APP_AGGREGATE_TABLE_INTERVAL_BASED_UNI_MCR:
            messageSent = globalApp.GetIntervalBasedMessagesSent(
                                        STAT_Unicast,
                                        intervalIndex).GetValue(node->getNodeTime());
            messageReceived = globalApp.GetIntervalBasedMessagesReceived(
                                        STAT_Unicast,
                                        intervalIndex).GetValue(node->getNodeTime());
            if (messageSent > 0)
            {
                unicastIntervalBasedMCR = messageReceived / messageSent;
            }
            return unicastIntervalBasedMCR;

        case APP_AGGREGATE_TABLE_INTERVAL_BASED_MULTI_MCR:
            messageSent = globalApp.GetIntervalBasedAggregatedEffectiveMessagesSent(
                                        STAT_Multicast,
                                        intervalIndex).GetValue(node->getNodeTime());
            messageReceived = globalApp.GetIntervalBasedMessagesReceived(
                                        STAT_Multicast,
            ...
            if (messageSent > 0)
            {
                multicastIntervalBasedMCR = messageReceived / messageSent;
            }
            return multicastIntervalBasedMCR;

        case APP_AGGREGATE_TABLE_INTERVAL_BASED_AVG_MCR:
            totalMsgReceived =
                globalApp.GetIntervalBasedMessagesReceived(STAT_Unicast, intervalIndex).
                                        GetValue(node->getNodeTime())
                +
                globalApp.GetIntervalBasedMessagesReceived(STAT_Multicast, intervalIndex).
                                        GetValue(node->getNodeTime());

            totalMsgSent =
                globalApp.GetIntervalBasedMessagesSent(STAT_Unicast, intervalIndex).
                                        GetValue(node->getNodeTime())
                +
                globalApp.GetIntervalBasedAggregatedEffectiveMessagesSent
                       (STAT_Multicast, intervalIndex).GetValue(node->getNodeTime());

            if (totalMsgSent > 0)
            {
                return totalMsgReceived/totalMsgSent;
            }
            else
            {
                return 0;
        ...
        case APP_AGGREGATE_TABLE_INTERVAL_BASED_AVG_DELAY:
            totalUnicastDelay =
                globalApp.GetIntervalBasedAverageDelay(STAT_Unicast, intervalIndex).
                                           GetValue(node->getNodeTime());

            totalMulticastDelay =
                globalApp.GetIntervalBasedAverageDelay(STAT_Multicast, intervalIndex).
                                           GetValue(node->getNodeTime());
                                                     ...
                                                     Int32 index)
{
    ...
    double currTime = (double) node->getNodeTime()/ SECOND;
    ...
    switch (index)
    {
        ...
        case APP_AGGREGATE_TABLE_UNI_OFFERLOAD:
            return (globalApp.GetDataSent(STAT_Unicast).
                                GetValue(node->getNodeTime()) / currTime) * 8;
...
void STAT_GlobalAppStatisticsBridge::initializeNameTypeConfigurations()
{
    ...
    tableConfigurations[APP_AGGREGATE_TABLE_INTERVAL_BASED_UNI_MCR] =
                    StatsDB_NameTypeConfiguration(
                                           "IntervalBasedUnicastMCR",
                                           "float",
                                           "");
    requiredFieldsIndex[APP_AGGREGATE_TABLE_INTERVAL_BASED_UNI_MCR] = TRUE;

    tableConfigurations[APP_AGGREGATE_TABLE_INTERVAL_BASED_MULTI_MCR] =
                    StatsDB_NameTypeConfiguration(
                                           "IntervalBasedMulticastMCR",
                                           "float",
                                           "");
    requiredFieldsIndex[APP_AGGREGATE_TABLE_INTERVAL_BASED_MULTI_MCR] = TRUE;

    tableConfigurations[APP_AGGREGATE_TABLE_INTERVAL_BASED_AVG_MCR] =
                    StatsDB_NameTypeConfiguration(
                                           "IntervalBasedAverageMCR",
                                           "float",
                                           "");
    requiredFieldsIndex[APP_AGGREGATE_TABLE_INTERVAL_BASED_AVG_MCR] = TRUE;
    ...
    tableConfigurations[APP_AGGREGATE_TABLE_INTERVAL_BASED_AVG_DELAY] =
        StatsDB_NameTypeConfiguration("IntervalBasedAverageDelay",
             "float",
             "STATS-DB-APPLICATION-AGGREGATE-INTERVAL-BASED-AVERAGE-DELAY");
...
STAT_NetSummaryBridge::composeNetSummaryInsertSQLString(
                                        ...
                                        std::vector<std::string>* insertList)
{
    size_t countOfNetSummary;
    size_t netSummarySize = netSummary.size();
    Int32 i;
                                                           ...
                                                           Int32 index)
{
    ...
    switch (index)
    {
        ...
        case TRANS_AGGREGATE_TABLE_UNI_OFFERLOAD:
            return globalTransport->GetOfferedLoad(STAT_Unicast).GetValue(node->getNodeTime());
        case TRANS_AGGREGATE_TABLE_MULTI_OFFERLOAD:
            return globalTransport->GetOfferedLoad(STAT_Multicast).GetValue(node->getNodeTime());
        case TRANS_AGGREGATE_TABLE_BROAD_OFFERLOAD:
            return globalTransport->GetOfferedLoad(STAT_Broadcast).GetValue(node->getNodeTime());
        case TRANS_AGGREGATE_TABLE_UNI_THROUGHPUT:
            return globalTransport->GetThroughput(STAT_Unicast).GetValue(node->getNodeTime());
        case TRANS_AGGREGATE_TABLE_MULTI_THROUGHPUT:
            return globalTransport->GetThroughput(STAT_Multicast).GetValue(node->getNodeTime());
        case TRANS_AGGREGATE_TABLE_BROAD_THROUGHPUT:
            return globalTransport->GetThroughput(STAT_Broadcast).GetValue(node->getNodeTime());
        ...
        case TRANS_AGGREGATE_TABLE_UNI_AVG_DELAY:
            return globalTransport->GetAverageDelay(STAT_Unicast).GetValue(node->getNodeTime());
        case TRANS_AGGREGATE_TABLE_MULTI_AVG_DELAY:
            return globalTransport->GetAverageDelay(STAT_Multicast).GetValue(node->getNodeTime());
        case TRANS_AGGREGATE_TABLE_BROAD_AVG_DELAY:
            return globalTransport->GetAverageDelay(STAT_Broadcast).GetValue(node->getNodeTime());
        default:
            return 0;
    }
}
                                                                   ...
                                                                   Int32 index)
{
    switch (index)
    {
        case TRANS_AGGREGATE_TABLE_UNI_SEGMENTS_SENT:
            return (UInt64)(globalTransport->GetDataSegmentsSent(STAT_Unicast).GetValue(node->getNodeTime()) +
                globalTransport->GetControlSegmentsSent(STAT_Unicast).GetValue(node->getNodeTime()));
        case TRANS_AGGREGATE_TABLE_MULTI_SEGMENTS_SENT:
            return (UInt64)(globalTransport->GetDataSegmentsSent(STAT_Multicast).GetValue(node->getNodeTime()) +
                globalTransport->GetControlSegmentsSent(STAT_Multicast).GetValue(node->getNodeTime()));
        case TRANS_AGGREGATE_TABLE_BROAD_SEGMENTS_SENT:
            return (UInt64)(globalTransport->GetDataSegmentsSent(STAT_Broadcast).GetValue(node->getNodeTime()) +
                globalTransport->GetControlSegmentsSent(STAT_Broadcast).GetValue(node->getNodeTime()));
        case TRANS_AGGREGATE_TABLE_UNI_SEGMENTS_RECV:
            return (UInt64)(UInt64)(globalTransport->GetDataSegmentsReceived(STAT_Unicast).GetValue(node->getNodeTime()) +
                globalTransport->GetControlSegmentsReceived(STAT_Unicast).GetValue(node->getNodeTime()));
        case TRANS_AGGREGATE_TABLE_MULTI_SEGMENTS_RECV:
            return (UInt64)(globalTransport->GetDataSegmentsReceived(STAT_Multicast).GetValue(node->getNodeTime()) +
                globalTransport->GetControlSegmentsReceived(STAT_Multicast).GetValue(node->getNodeTime()));
        case TRANS_AGGREGATE_TABLE_BROAD_SEGMENTS_RECV:
            return (UInt64)(globalTransport->GetDataSegmentsReceived(STAT_Broadcast).GetValue(node->getNodeTime()) +
                globalTransport->GetControlSegmentsReceived(STAT_Broadcast).GetValue(node->getNodeTime()));
        case TRANS_AGGREGATE_TABLE_UNI_BYTES_SENT:
            return (UInt64)(globalTransport->GetDataBytesSent(STAT_Unicast).GetValue(node->getNodeTime()) +
                globalTransport->GetControlBytesSent(STAT_Unicast).GetValue(node->getNodeTime()) +
                globalTransport->GetOverheadBytesSent(STAT_Unicast).GetValue(node->getNodeTime()));
        case TRANS_AGGREGATE_TABLE_MULTI_BYTES_SENT:
            return (UInt64)(globalTransport->GetDataBytesSent(STAT_Multicast).GetValue(node->getNodeTime()) +
                globalTransport->GetControlBytesSent(STAT_Multicast).GetValue(node->getNodeTime()) +
                globalTransport->GetOverheadBytesSent(STAT_Multicast).GetValue(node->getNodeTime()));
        case TRANS_AGGREGATE_TABLE_BROAD_BYTES_SENT:
            return (UInt64)(globalTransport->GetDataBytesSent(STAT_Broadcast).GetValue(node->getNodeTime()) +
                globalTransport->GetControlBytesSent(STAT_Broadcast).GetValue(node->getNodeTime()) +
                globalTransport->GetOverheadBytesSent(STAT_Broadcast).GetValue(node->getNodeTime()));
        case TRANS_AGGREGATE_TABLE_UNI_BYTES_RECV:
            return (UInt64)(globalTransport->GetDataBytesReceived(STAT_Unicast).GetValue(node->getNodeTime()) +
                globalTransport->GetControlBytesReceived(STAT_Unicast).GetValue(node->getNodeTime()) +
                globalTransport->GetOverheadBytesReceived(STAT_Unicast).GetValue(node->getNodeTime()));
        case TRANS_AGGREGATE_TABLE_MULTI_BYTES_RECV:
            return (UInt64)(globalTransport->GetDataBytesReceived(STAT_Multicast).GetValue(node->getNodeTime()) +
                globalTransport->GetControlBytesReceived(STAT_Multicast).GetValue(node->getNodeTime()) +
                globalTransport->GetOverheadBytesReceived(STAT_Multicast).GetValue(node->getNodeTime()));
        case TRANS_AGGREGATE_TABLE_BROAD_BYTES_RECV:
            return (UInt64)(globalTransport->GetDataBytesReceived(STAT_Broadcast).GetValue(node->getNodeTime()) +
                globalTransport->GetControlBytesReceived(STAT_Broadcast).GetValue(node->getNodeTime()) +
                globalTransport->GetOverheadBytesReceived(STAT_Broadcast).GetValue(node->getNodeTime()));
        default:
            return 0;
    }
}
    ...
    std::vector<std::string>* insertList)
{
    ...
    size_t countOfTransportSummary;
    size_t transportSummarySize = transportSummary->size();
    Int32 i;
                                                                  ...
                                                                  Int32 index)
{
    switch (index)
    {
        case PHY_AGGREGATE_TABLE_SIGS_TRANS:
            return (UInt64) globalPhysical->GetSignalsTransmitted().GetValue(node->getNodeTime());
        case PHY_AGGREGATE_TABLE_SIGS_LOCKED:
            return (UInt64) globalPhysical->GetSignalsLocked().GetValue(node->getNodeTime());
        case PHY_AGGREGATE_TABLE_SIGS_RECV:
            return (UInt64) globalPhysical->GetSignalsToMac().GetValue(node->getNodeTime());
        case PHY_AGGREGATE_TABLE_SIGS_DROPPED:
            return (UInt64) globalPhysical->GetSignalsWithErrors().GetValue(node->getNodeTime());
        case PHY_AGGREGATE_TABLE_SIGS_DROPPED_INTERFERENCE:
            return (UInt64) globalPhysical->GetSignalsInterferenceErrors().GetValue(node->getNodeTime());
                                                          ...
                                                          Int32 index)
{
    ...
    switch (index)
    {
        ...
        case PHY_AGGREGATE_TABLE_UTILIZATION:
            return globalPhysical->GetAverageUtilization().GetValue(node->getNodeTime());
        case PHY_AGGREGATE_TABLE_AVG_INTERFERENCE:
            if (globalPhysical->GetAverageInterference_dBm().GetValue(node->getNodeTime()) == 0)
            {
                return -120;
            }
            else
            {
                return globalPhysical->GetAverageInterference_dBm().GetValue(node->getNodeTime());
            }
        case PHY_AGGREGATE_TABLE_PATHLOSS:
            return globalPhysical->GetAveragePathloss_dB().GetValue(node->getNodeTime());
        case PHY_AGGREGATE_TABLE_DELAY:
            return globalPhysical->GetAverageDelay().GetValue(node->getNodeTime());
        case PHY_AGGREGATE_TABLE_SIG_POWER:
            return globalPhysical->GetAverageSignalPower_dBm().GetValue(node->getNodeTime());
...
void STAT_PhySummaryBridge::composePhysicalSummaryInsertSQLString(
    ...
    std::vector<std::string>* insertList)
{
    size_t countOfPhysicalSummary;
    size_t phySummarySize = phySummary->size();
    Int32 i;
    ...
    STAT_PhySessionStatistics& summary)
{
    ...
    switch (index)
    {
        ...
        case PHY_SUMMARY_TABLE_UTILIZATION:
            return summary.utilization / node->getNodeTime();
        ...
        case PHY_SUMMARY_TABLE_PATHLOSS:
            return in_db(summary.totalPathLoss, summary.numSignals);
        case PHY_SUMMARY_TABLE_DELAY:
            return summary.totalDelay / summary.numSignals;
        case PHY_SUMMARY_TABLE_SIG_POWER:
            return in_db(summary.totalSignalPower, summary.numSignals);
        default:
            return 0;
    }
}
                                                             ...
                                                             Int32 index)
{
    ...
    switch (index)
    {
        case MAC_AGGREGATE_TABLE_UNI_DATA_FRAMES_SENT:
            return (UInt64) globalMac->GetDataFramesSent(STAT_Unicast).GetValue(node->getNodeTime());
        case MAC_AGGREGATE_TABLE_UNI_DATA_FRAMES_RCVD:
            return (UInt64) globalMac->GetDataFramesReceived(STAT_Unicast).GetValue(node->getNodeTime());
        case MAC_AGGREGATE_TABLE_UNI_CONTROL_FRAMES_SENT:
            return (UInt64) globalMac->GetControlFramesSent(STAT_Unicast).GetValue(node->getNodeTime());
        case MAC_AGGREGATE_TABLE_UNI_CONTROL_FRAMES_RCVD:
            return (UInt64) globalMac->GetControlFramesReceived(STAT_Unicast).GetValue(node->getNodeTime());
        case MAC_AGGREGATE_TABLE_UNI_DATA_BYTES_SENT:
            return (UInt64) globalMac->GetDataBytesSent(STAT_Unicast).GetValue(node->getNodeTime());
        case MAC_AGGREGATE_TABLE_UNI_DATA_BYTES_RCVD:
            return (UInt64) globalMac->GetDataBytesReceived(STAT_Unicast).GetValue(node->getNodeTime());
        case MAC_AGGREGATE_TABLE_UNI_CONTROL_BYTES_SENT:
            return (UInt64) globalMac->GetControlBytesSent(STAT_Unicast).GetValue(node->getNodeTime());
        case MAC_AGGREGATE_TABLE_UNI_CONTROL_BYTES_RCVD:
            return (UInt64) globalMac->GetControlBytesReceived(STAT_Unicast).GetValue(node->getNodeTime());
        case MAC_AGGREGATE_TABLE_BROAD_DATA_FRAMES_SENT:
            return (UInt64) globalMac->GetDataFramesSent(STAT_Broadcast).GetValue(node->getNodeTime());
        case MAC_AGGREGATE_TABLE_BROAD_DATA_FRAMES_RCVD:
            return (UInt64) globalMac->GetDataFramesReceived(STAT_Broadcast).GetValue(node->getNodeTime());
        case MAC_AGGREGATE_TABLE_BROAD_CONTROL_FRAMES_SENT:
            return (UInt64) globalMac->GetControlFramesSent(STAT_Broadcast).GetValue(node->getNodeTime());
        case MAC_AGGREGATE_TABLE_BROAD_CONTROL_FRAMES_RCVD:
            return (UInt64) globalMac->GetControlFramesReceived(STAT_Broadcast).GetValue(node->getNodeTime());
        case MAC_AGGREGATE_TABLE_BROAD_DATA_BYTES_SENT:
            return (UInt64) globalMac->GetDataBytesSent(STAT_Broadcast).GetValue(node->getNodeTime());
        case MAC_AGGREGATE_TABLE_BROAD_DATA_BYTES_RCVD:
            return (UInt64) globalMac->GetDataBytesReceived(STAT_Broadcast).GetValue(node->getNodeTime());
        case MAC_AGGREGATE_TABLE_BROAD_CONTROL_BYTES_SENT:
            return (UInt64) globalMac->GetControlBytesSent(STAT_Broadcast).GetValue(node->getNodeTime());
        case MAC_AGGREGATE_TABLE_BROAD_CONTROL_BYTES_RCVD:
            return (UInt64) globalMac->GetControlBytesReceived(STAT_Broadcast).GetValue(node->getNodeTime());
        case MAC_AGGREGATE_TABLE_MULTI_DATA_FRAMES_SENT:
            return (UInt64) globalMac->GetDataFramesSent(STAT_Multicast).GetValue(node->getNodeTime());
        case MAC_AGGREGATE_TABLE_MULTI_DATA_FRAMES_RCVD:
            return (UInt64) globalMac->GetDataFramesReceived(STAT_Multicast).GetValue(node->getNodeTime());
        case MAC_AGGREGATE_TABLE_MULTI_CONTROL_FRAMES_SENT:
            return (UInt64) globalMac->GetControlFramesSent(STAT_Multicast).GetValue(node->getNodeTime());
        case MAC_AGGREGATE_TABLE_MULTI_CONTROL_FRAMES_RCVD:
            return (UInt64) globalMac->GetControlFramesReceived(STAT_Multicast).GetValue(node->getNodeTime());
        case MAC_AGGREGATE_TABLE_MULTI_DATA_BYTES_SENT:
            return (UInt64) globalMac->GetDataBytesSent(STAT_Multicast).GetValue(node->getNodeTime());
        case MAC_AGGREGATE_TABLE_MULTI_DATA_BYTES_RCVD:
            return (UInt64) globalMac->GetDataBytesReceived(STAT_Multicast).GetValue(node->getNodeTime());
        case MAC_AGGREGATE_TABLE_MULTI_CONTROL_BYTES_SENT:
            return (UInt64) globalMac->GetControlBytesSent(STAT_Multicast).GetValue(node->getNodeTime());
        case MAC_AGGREGATE_TABLE_MULTI_CONTROL_BYTES_RCVD:
            return (UInt64) globalMac->GetControlBytesReceived(STAT_Multicast).GetValue(node->getNodeTime());
        default:
            return 0;
    }
}
                                                     ...
                                                     Int32 index)
{
    ...
    switch (index)
    {
        ...
        case MAC_AGGREGATE_TABLE_CARRIED_LOAD:
            return globalMac->GetCarriedLoad().GetValue(node->getNodeTime());
        case MAC_AGGREGATE_TABLE_UNI_AVG_QUEUING_DELAY:
            return globalMac->GetAverageQueuingDelay(STAT_Unicast).GetValue(node->getNodeTime());
        case MAC_AGGREGATE_TABLE_UNI_AVG_MED_ACCESS_DELAY:
            return globalMac->GetAverageMediumAccessDelay(STAT_Unicast).GetValue(node->getNodeTime());
        case MAC_AGGREGATE_TABLE_UNI_AVG_MED_DELAY:
            return globalMac->GetAverageMediumDelay(STAT_Unicast).GetValue(node->getNodeTime());
        ...
        case MAC_AGGREGATE_TABLE_BROAD_AVG_QUEUING_DELAY:
            return globalMac->GetAverageQueuingDelay(STAT_Broadcast).GetValue(node->getNodeTime());
        case MAC_AGGREGATE_TABLE_BROAD_AVG_MED_ACCESS_DELAY:
            return globalMac->GetAverageMediumAccessDelay(STAT_Broadcast).GetValue(node->getNodeTime());
        case MAC_AGGREGATE_TABLE_BROAD_AVG_MED_DELAY:
            return globalMac->GetAverageMediumDelay(STAT_Broadcast).GetValue(node->getNodeTime());
        ...
        case MAC_AGGREGATE_TABLE_MULTI_AVG_QUEUING_DELAY:
            return globalMac->GetAverageQueuingDelay(STAT_Multicast).GetValue(node->getNodeTime());
        case MAC_AGGREGATE_TABLE_MULTI_AVG_MED_ACCESS_DELAY:
            return globalMac->GetAverageMediumAccessDelay(STAT_Multicast).GetValue(node->getNodeTime());
        case MAC_AGGREGATE_TABLE_MULTI_AVG_MED_DELAY:
            return globalMac->GetAverageMediumDelay(STAT_Multicast).GetValue(node->getNodeTime());
...
void STAT_MacSummaryBridge::composeMacSummaryInsertSQLString(
    ...
    std::vector<std::string>* insertList)
{
    size_t countOfMacSummary;
    size_t macSummarySize = macSummary->size();
    Int32 i;
                                                               ...
                                                               Int32 index)
{
    switch (index)
    {
        case QUEUE_AGGREGATE_PACKETS_ENQUEUED:
            return (UInt64) globalQueue->GetPacketsEnqueued().GetValue(node->getNodeTime());
        case QUEUE_AGGREGATE_PACKETS_DEQUEUED:
            return (UInt64) globalQueue->GetPacketsDequeued().GetValue(node->getNodeTime());
        case QUEUE_AGGREGATE_PACKETS_DROPPED:
            return (UInt64) globalQueue->GetPacketsDropped().GetValue(node->getNodeTime());
        case QUEUE_AGGREGATE_PACKETS_DROPPED_FORCE:
            return (UInt64) globalQueue->GetPacketsDroppedForcefully().GetValue(node->getNodeTime());
        case QUEUE_AGGREGATE_BYTES_ENQUEUED:
            return (UInt64) globalQueue->GetBytesEnqueued().GetValue(node->getNodeTime());
        case QUEUE_AGGREGATE_BYTES_DEQUEUED:
            return (UInt64) globalQueue->GetBytesDequeued().GetValue(node->getNodeTime());
        case QUEUE_AGGREGATE_BYTES_DROPPED:
            return (UInt64) globalQueue->GetBytesDropped().GetValue(node->getNodeTime());
        case QUEUE_AGGREGATE_BYTES_DROPPED_FORCE:
            return (UInt64) globalQueue->GetBytesDroppedForcefully().GetValue(node->getNodeTime());
        default:
            return 0;
    }
}
...
void STAT_QueueSummaryBridge::composeQueueSummaryInsertSQLString(
    ...
    std::vector<std::string>* insertList)
{
    size_t countOfQueueSummary;
    size_t queueSummarySize = queueSummary->size();
    Int32 i;
...
STAT_MulticastAppSummaryBridge::composeMutlicastAppSummaryInsertSQLString(
                                        ...
                                        std::vector<std::string>* insertList)
{
    size_t countOfMutlicastSessionSummary;
    size_t mutlicastSessionSummarySize = sessionSummary->size();
    Int32 i;
