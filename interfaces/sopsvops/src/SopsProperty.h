...
/// Derived from Property to contain the specific methods
...
/// Contains a key string, that may be wildcarded, and the
...
/// The SOPS property manager extends the PropertyManager to
...
public:
    ...
    /// \brief Returns a SopsProperty* for the given key
    /// \parameter key  the key name of the property
    /// \return NULL if the property does not exist, else a pointer to it.
    SopsProperty* getProperty(const std::string& key);
...
/// Shorthand call to SopsPropertyManager::getInstance()->setProperty
inline
void SetSopsProperty(const std::string& key, const std::string& value)
{
    if (SopsVopsInterfaceEnabled())
    {
        SopsPropertyManager::getInstance()->
