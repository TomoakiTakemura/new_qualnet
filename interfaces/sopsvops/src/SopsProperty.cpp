...
SopsProperty* SopsPropertyManager::getProperty(const std::string& key)
{
    pthread_rwlock_rdlock(&m_rwlock_properties);
    PropertyManagerIterator it = m_properties.find(key);
    pthread_rwlock_unlock(&m_rwlock_properties);
...
{
#ifdef SOPS_DEBUG
    SopsLog("SopsProperty::setPartition(%p)\n", partition);
#endif
    this->m_partition = partition;
#ifdef USE_MPI
    // When using MPI, the RpcInterface has to know if it is 
    // the master process or not. It is the master process
    // if it is running in partition zero.
    ERROR_Assert(m_vops != NULL, "interface is NULL");
    bool isMaster = partition->partitionId == 0;
    m_vops->setIsMasterProcess(isMaster);
#endif
}
...
{
#ifdef SOPS_DEBUG
    SopsLog("SopsPropertyManager::initialize()\n");
#endif
    PropertyManager::initialize();
    m_vops = new SopsRpcInterface(SopsPort(), this);
#ifdef SOPS_DEBUG
    SopsLog("-   SopsRpcInterface port=%d\n", SopsPort());
#endif
}
...
int SopsPropertyManager::sendOneProperty(SopsProperty *prop)
{
    ...
    m_lastErrno = m_vops->getLastErrno();
