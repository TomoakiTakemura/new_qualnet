...
void CAgiInterfaceMappingImpl<T>::LoadInterface(long entityId, std::ifstream& mappingFile)
{
    ...
    StkInterface stkInterface = std::pair<std::string, std::string>(xmtrPath, rcvrPath);
    StkQualNetInterfaceInfo stkQualNetInterfaceInfo(stkInterface, rainOutagePercent);

    StkQualnetInterface stkQualnetInterface = std::pair<int, StkQualNetInterfaceInfo>(qualnetInterfaceId, stkQualNetInterfaceInfo);
    ...
    if (entityIt != m_entityIdMap.end())
    {
        ...
        else
        {
            ...
            StkQualnetInterface stkQualnetInterface =
                std::pair<int, StkQualNetInterfaceInfo>(qualnetInterfaceId, stkQualNetInterfaceInfo);
            entityIt->second.push_back(stkQualnetInterface);
        }

        m_interfaceSatMap[std::pair<long, int>(entityId, qualnetInterfaceId)] = true;
    }
    std::getline(mappingFile, line);
}
...
void CAgiInterfaceMappingImpl<T>::LoadEntity(std::ifstream& mappingFile)
{
    ...
    while (notDone)
    {
        ...
        else if (token == "ENTITYID")
        {
            ...
            m_entityIdMap.insert(std::pair<long, std::list<StkQualnetInterface> >(entityId, newList));
            m_entitySatMap.insert(std::pair<long, bool>(entityId, false));
        }
