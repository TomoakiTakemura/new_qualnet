...
#include ".\STK11\agi_interface_util_stk11_create.h"
...
void CAgiInterfaceUtil::Initialize(StkVersion stkVersion, const bool& agiVerParamFound)
{
    ...
    else if(stkVersion == eStk11)
    {
        m_pUtilImpl = CreateSTK11Util();
        if (!m_pUtilImpl->ConstructFactory())
        {
            ERROR_ReportError("QualNet Interface Initialization Error:  Configuration file parameter AGI-STK-VERSION has a value of 11 but the STK11 installation can not be found.");
        }
	}
    else
    {
...
void CAgiInterfaceUtil::Shutdown()
{
    m_pUtilImpl->Shutdown();
}
