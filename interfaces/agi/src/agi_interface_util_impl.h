...
protected:
    AgStkCommUtilLib::IAgStkCommUtilFactory* GetFactory() const { return m_pStkCommUtilFactory; }

private:
    CAgiInterfaceSmartPtr<ID> m_xmtrID;
    CAgiInterfaceSmartPtr<ID> m_rcvrID;
    CAgiInterfaceSmartPtr<TIME> m_currentTime;
    CAgiInterfaceSmartPtr<SIGNAL> m_signal;
    void ReportError(const std::string& errorMsg);

    bool m_initialized;
    CAgiInterfaceSmartPtr<AgStkCommUtilLib::IAgStkCommUtilFactory> m_pStkCommUtilFactory;
    CAgiInterfaceSmartPtr<MAPPING> m_pMappingObject;
};
                                                                                                ...
                                                                                                PropPathProfile* pPathProfile)
{
    ...
    if (pPathProfile)
    {
        ...
        m_xmtrID->SetInfo(req.xmtrId.first, xmtInterfaceIndex, req.channelIndex,
                          m_pMappingObject->IsSatellite(std::pair<long, int>(req.xmtrId.first, xmtInterfaceIndex)));
        m_rcvrID->SetInfo(req.rcvrId.first, req.rcvrId.second, req.channelIndex,
                          m_pMappingObject->IsSatellite(std::pair<long, int>(req.rcvrId.first, req.rcvrId.second)));
...
void CAgiInterfaceUtilImpl<ID, TIME, SIGNAL, MAPPING>::ComputeReceiveGain(const CAgiInterfaceUtil::ComputeRequest& req, double& gain_dB)
{
    ...
    m_xmtrID->SetInfo(req.xmtrId.first, req.xmtrId.second, req.channelIndex,
                      m_pMappingObject->IsSatellite(std::pair<long, int>(req.xmtrId.first, req.xmtrId.second)));
    m_rcvrID->SetInfo(req.rcvrId.first, req.rcvrId.second, req.channelIndex,
                      m_pMappingObject->IsSatellite(std::pair<long, int>(req.rcvrId.first, req.rcvrId.second)));
